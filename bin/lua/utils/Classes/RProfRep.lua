-- RProfRep.lua
local c_rprofrep = require("rprofrep_decoder_lua")
...
function RProfRep:new(rprofrep_files)
    ...
    for _, filename in ipairs(rprofrep_files) do
        ...
        local rprofrep_obj = c_rprofrep.new(filename)
...
function RProfRep:__for_each_unit(fn, callback)
    if self.__in_for_each_rank then
        local ctx = self.__current_rprofrep
        ctx[fn](ctx, callback)
    else
        self:for_each_rank(function(_)
            local ctx = self.__current_rprofrep
            ctx[fn](ctx, callback)
        end)
    end
...
function RProfRep:for_each_pid(callback, filter)
    self.__in_for_each_pid = true
    self:__for_each_unit("for_each_pid", function (pid, node)
        if not filter or filter[pid] then
...
function RProfRep:for_each_gpu(callback, filter)
    self.__in_for_each_gpu = true
    self:__for_each_unit("for_each_gpu", function (gpu_id, node)
        local gpu_node_id = self:get_gpu_id(gpu_id)
        if not filter or filter[gpu_node_id] then
            self.__current_node = node
            callback(gpu_node_id)
            self.__current_node = nil
        end
    end)
...
function RProfRep:get_source_location(event)
    ...
    local loc = self.reports_rprofrep[event:rank()]:get_location(event)
...
function RProfRep:is_domain_traced(id)
...
function RProfRep:get_reports_filename(rank)
    ...
    if not rank then
        local arr = {}
        for _, filename in pairs(self.reports_filename) do
...
function RProfRep:topology_get_profiled_gpu()
    local gpu_list = {}
    self:for_each_gpu(function (gpu_id)
        table.insert(gpu_list, gpu_id)
    end)
    return gpu_list
end


function RProfRep:topology_get_profiled_process()
    local pid_list = {}
    self:for_each_pid(function (pid)
