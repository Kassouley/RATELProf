...
#define COPY_INTERCEPT() { \
    ...
    CHECK_RATELPROF_CALL( \
        ratelprof_activity_pool_push_gpu_activity(copy_activity)); \
};
...
#define DISPATCH_INTERCEPT() { \
    ...
    CHECK_RATELPROF_CALL( \
        ratelprof_activity_pool_get_queue_from_signal(signal.handle, &queue)); \
    ...
    CHECK_RATELPROF_CALL( \
        ratelprof_activity_pool_push_gpu_activity(kernel_activity)); \
};
