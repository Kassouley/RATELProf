...
local env      = require ("profile.env")

local profile = {}

local function handle_option(options)
    if options["blacklist"] then    env.set_filter_env_var(options["blacklist"], 1) end
    if options["whitelist"] then    env.set_filter_env_var(options["whitelist"], 2) end
    if options["output"] then       env.set_env(settings._ENV.OUTPUT_FILE, options["output"]) end

    for _, trace_option in pairs(settings._TRACES) do
        if options["all-trace"] or options[trace_option.option] then env.set_env(trace_option.var, "1") end
    end 

    if options["hsa-trace-for-rprof"] then env.set_env(settings._ENV.DOMAIN_PROFILING, "1") end

    local plugin_path = options["plugin"]
    if plugin_path then
        if plugin_path == "stdout"        then plugin_path = settings._INSTALL_DIR..'/lib/libplugin_stdout.so'
        elseif plugin_path == "json"      then plugin_path = settings._INSTALL_DIR..'/lib/libplugin_json.so'
        elseif plugin_path == "json-dyn"  then plugin_path = settings._INSTALL_DIR..'/lib/libplugin_json_dyn.so' end
        if not lfs.file_exists(plugin_path) then
            print("Error: Plugin library '"..plugin_path.."' not found.")
            os.exit(1)
        end
        env.set_env(settings._ENV.PLUGIN_PATH, plugin_path) 
    end
end
...
function profile.process_profiling(positional_args, options_values)
    ...
    local preload_libs = {
        settings._INSTALL_DIR.."/lib/libratelprof.so",
        settings._INSTALL_DIR.."/lib/libratelprof_wrapper.so"
    }
    ...
    env.set_env("LD_PRELOAD", preload_lib)
    ...
    local bytes_written = utils.execute_command("cat "..output_file.." | wc -c")
