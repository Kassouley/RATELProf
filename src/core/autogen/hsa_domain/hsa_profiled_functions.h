/* # THIS FILE HAS BEEN AUTOMATICALLY GENERATED BY THE GILDA TOOL.
 * # DO NOT MODIFY UNLESS YOU KNOW WHAT YOU ARE DOING.
 * # ANY CHANGES MAY BE OVERWRITTEN BY SUBSEQUENT RUNS OF GILDA.
 */ 

#ifndef HSA_PROFILED_FUNCTIONS_H 
#define HSA_PROFILED_FUNCTIONS_H 

#include "hsa/hsa.h"
#include "hsa/hsa_ext_amd.h" 
#include <string.h>

#define HSA_STRING_SIZE_MAX 128

     
// HSA API Function Prototype
uint64_t i_hsa_queue_load_read_index_scacquire(const hsa_queue_t * queue);
hsa_status_t i_hsa_ext_image_clear(hsa_agent_t agent, hsa_ext_image_t image, const void * data, const hsa_ext_image_region_t * image_region);
uint64_t i_hsa_queue_cas_write_index_acquire(const hsa_queue_t * queue, uint64_t expected, uint64_t value);
hsa_status_t i_hsa_executable_get_symbol(hsa_executable_t executable, const char * module_name, const char * symbol_name, hsa_agent_t agent, int32_t call_convention, hsa_executable_symbol_t * symbol);
void i_hsa_signal_and_scacq_screl(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_executable_agent_global_variable_define(hsa_executable_t executable, hsa_agent_t agent, const char * variable_name, void * address);
hsa_status_t i_hsa_code_object_get_symbol(hsa_code_object_t code_object, const char * symbol_name, hsa_code_symbol_t * symbol);
hsa_status_t i_hsa_amd_signal_async_handler(hsa_signal_t signal, hsa_signal_condition_t cond, hsa_signal_value_t value, hsa_amd_signal_handler handler, void * arg);
void i_hsa_signal_xor_scacq_screl(hsa_signal_t signal, hsa_signal_value_t value);
void i_hsa_signal_and_relaxed(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_wavefront_get_info(hsa_wavefront_t wavefront, hsa_wavefront_info_t attribute, void * value);
hsa_status_t i_hsa_amd_memory_lock(void * host_ptr, size_t size, hsa_agent_t * agents, int num_agent, void ** agent_ptr);
void i_hsa_signal_or_screlease(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_executable_validate(hsa_executable_t executable, uint32_t * result);
hsa_status_t i_hsa_queue_destroy(hsa_queue_t * queue);
hsa_status_t i_hsa_isa_get_info(hsa_isa_t isa, hsa_isa_info_t attribute, uint32_t index, void * value);
hsa_status_t i_hsa_amd_memory_lock_to_pool(void * host_ptr, size_t size, hsa_agent_t * agents, int num_agent, hsa_amd_memory_pool_t pool, uint32_t flags, void ** agent_ptr);
hsa_status_t i_hsa_system_major_extension_supported(uint16_t extension, uint16_t version_major, uint16_t * version_minor, _Bool * result);
hsa_status_t i_hsa_amd_profiling_get_async_copy_time(hsa_signal_t signal, hsa_amd_profiling_async_copy_time_t * time);
void i_hsa_signal_or_relaxed(hsa_signal_t signal, hsa_signal_value_t value);
uint64_t i_hsa_queue_cas_write_index_scacquire(const hsa_queue_t * queue, uint64_t expected, uint64_t value);
hsa_status_t i_hsa_code_object_get_info(hsa_code_object_t code_object, hsa_code_object_info_t attribute, void * value);
void i_hsa_signal_and_screlease(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_amd_pointer_info(const void * ptr, hsa_amd_pointer_info_t * info, void *(*alloc)(size_t), uint32_t * num_agents_accessible, hsa_agent_t ** accessible);
hsa_status_t i_hsa_cache_get_info(hsa_cache_t cache, hsa_cache_info_t attribute, void * value);
hsa_status_t i_hsa_amd_svm_attributes_get(void * ptr, size_t size, hsa_amd_svm_attribute_pair_t * attribute_list, size_t attribute_count);
hsa_status_t i_hsa_signal_group_create(uint32_t num_signals, const hsa_signal_t * signals, uint32_t num_consumers, const hsa_agent_t * consumers, hsa_signal_group_t * signal_group);
hsa_status_t i_hsa_memory_deregister(void * ptr, size_t size);
hsa_status_t i_hsa_amd_ipc_memory_attach(const hsa_amd_ipc_memory_t * handle, size_t len, uint32_t num_agents, const hsa_agent_t * mapping_agents, void ** mapped_ptr);
hsa_status_t i_hsa_amd_queue_cu_get_mask(const hsa_queue_t * queue, uint32_t num_cu_mask_count, uint32_t * cu_mask);
uint64_t i_hsa_queue_add_write_index_scacq_screl(const hsa_queue_t * queue, uint64_t value);
hsa_status_t i_hsa_agent_major_extension_supported(uint16_t extension, hsa_agent_t agent, uint16_t version_major, uint16_t * version_minor, _Bool * result);
hsa_status_t i_hsa_ext_sampler_destroy(hsa_agent_t agent, hsa_ext_sampler_t sampler);
hsa_status_t i_hsa_queue_create(hsa_agent_t agent, uint32_t size, hsa_queue_type32_t type, void (*callback)(hsa_status_t, hsa_queue_t *, void *), void * data, uint32_t private_segment_size, uint32_t group_segment_size, hsa_queue_t ** queue);
hsa_status_t i_hsa_amd_vmem_get_alloc_properties_from_handle(hsa_amd_vmem_alloc_handle_t memory_handle, hsa_amd_memory_pool_t * pool, hsa_amd_memory_type_t * type);
hsa_status_t i_hsa_amd_memory_pool_allocate(hsa_amd_memory_pool_t memory_pool, size_t size, uint32_t flags, void ** ptr);
void i_hsa_signal_or_release(hsa_signal_t signal, hsa_signal_value_t value);
uint64_t i_hsa_queue_cas_write_index_acq_rel(const hsa_queue_t * queue, uint64_t expected, uint64_t value);
hsa_status_t i_hsa_code_object_serialize(hsa_code_object_t code_object, hsa_status_t (*alloc_callback)(size_t, hsa_callback_data_t, void **), hsa_callback_data_t callback_data, const char * options, void ** serialized_code_object, size_t * serialized_code_object_size);
void i_hsa_signal_xor_acquire(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_amd_profiling_convert_tick_to_system_domain(hsa_agent_t agent, uint64_t agent_tick, uint64_t * system_tick);
hsa_status_t i_hsa_executable_destroy(hsa_executable_t executable);
void i_hsa_signal_add_screlease(hsa_signal_t signal, hsa_signal_value_t value);
hsa_signal_value_t i_hsa_signal_load_acquire(hsa_signal_t signal);
hsa_status_t i_hsa_executable_create_alt(hsa_profile_t profile, hsa_default_float_rounding_mode_t default_float_rounding_mode, const char * options, hsa_executable_t * executable);
hsa_status_t i_hsa_amd_memory_async_copy_rect(const hsa_pitched_ptr_t * dst, const hsa_dim3_t * dst_offset, const hsa_pitched_ptr_t * src, const hsa_dim3_t * src_offset, const hsa_dim3_t * range, hsa_agent_t copy_agent, hsa_amd_copy_direction_t dir, uint32_t num_dep_signals, const hsa_signal_t * dep_signals, hsa_signal_t completion_signal);
hsa_signal_value_t i_hsa_signal_load_scacquire(hsa_signal_t signal);
hsa_status_t i_hsa_agent_iterate_caches(hsa_agent_t agent, hsa_status_t (*callback)(hsa_cache_t, void *), void * data);
hsa_signal_value_t i_hsa_signal_cas_acq_rel(hsa_signal_t signal, hsa_signal_value_t expected, hsa_signal_value_t value);
uint64_t i_hsa_queue_load_read_index_relaxed(const hsa_queue_t * queue);
hsa_status_t i_hsa_agent_iterate_isas(hsa_agent_t agent, hsa_status_t (*callback)(hsa_isa_t, void *), void * data);
void i_hsa_signal_xor_relaxed(hsa_signal_t signal, hsa_signal_value_t value);
void i_hsa_signal_subtract_acq_rel(hsa_signal_t signal, hsa_signal_value_t value);
uint64_t i_hsa_queue_load_read_index_acquire(const hsa_queue_t * queue);
hsa_status_t i_hsa_isa_get_info_alt(hsa_isa_t isa, hsa_isa_info_t attribute, void * value);
void i_hsa_signal_and_scacquire(hsa_signal_t signal, hsa_signal_value_t value);
uint64_t i_hsa_queue_add_write_index_release(const hsa_queue_t * queue, uint64_t value);
hsa_status_t i_hsa_system_get_info(hsa_system_info_t attribute, void * value);
hsa_status_t i_hsa_memory_register(void * ptr, size_t size);
hsa_status_t i_hsa_isa_get_round_method(hsa_isa_t isa, hsa_fp_type_t fp_type, hsa_flush_mode_t flush_mode, hsa_round_method_t * round_method);
hsa_status_t i_hsa_isa_iterate_wavefronts(hsa_isa_t isa, hsa_status_t (*callback)(hsa_wavefront_t, void *), void * data);
hsa_status_t i_hsa_amd_vmem_unmap(void * va, size_t size);
void i_hsa_signal_subtract_acquire(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_system_get_major_extension_table(uint16_t extension, uint16_t version_major, size_t table_length, void * table);
void i_hsa_signal_store_release(hsa_signal_t signal, hsa_signal_value_t value);
uint64_t i_hsa_queue_load_write_index_scacquire(const hsa_queue_t * queue);
hsa_status_t i_hsa_amd_spm_release(hsa_agent_t preferred_agent);
hsa_status_t i_hsa_code_symbol_get_info(hsa_code_symbol_t code_symbol, hsa_code_symbol_info_t attribute, void * value);
hsa_signal_value_t i_hsa_signal_wait_acquire(hsa_signal_t signal, hsa_signal_condition_t condition, hsa_signal_value_t compare_value, uint64_t timeout_hint, hsa_wait_state_t wait_state_hint);
hsa_signal_value_t i_hsa_signal_cas_release(hsa_signal_t signal, hsa_signal_value_t expected, hsa_signal_value_t value);
hsa_status_t i_hsa_amd_memory_pool_get_info(hsa_amd_memory_pool_t memory_pool, hsa_amd_memory_pool_info_t attribute, void * value);
hsa_status_t i_hsa_amd_pointer_info_set_userdata(const void * ptr, void * userdata);
hsa_status_t i_hsa_amd_vmem_get_access(void * va, hsa_access_permission_t * perms, hsa_agent_t agent_handle);
hsa_status_t i_hsa_iterate_agents(hsa_status_t (*callback)(hsa_agent_t, void *), void * data);
void i_hsa_queue_store_write_index_screlease(const hsa_queue_t * queue, uint64_t value);
hsa_status_t i_hsa_agent_iterate_regions(hsa_agent_t agent, hsa_status_t (*callback)(hsa_region_t, void *), void * data);
hsa_status_t i_hsa_executable_readonly_variable_define(hsa_executable_t executable, hsa_agent_t agent, const char * variable_name, void * address);
hsa_status_t i_hsa_executable_iterate_symbols(hsa_executable_t executable, hsa_status_t (*callback)(hsa_executable_t, hsa_executable_symbol_t, void *), void * data);
hsa_status_t i_hsa_amd_vmem_handle_release(hsa_amd_vmem_alloc_handle_t memory_handle);
hsa_status_t i_hsa_amd_memory_pool_can_migrate(hsa_amd_memory_pool_t src_memory_pool, hsa_amd_memory_pool_t dst_memory_pool, _Bool * result);
void i_hsa_queue_store_write_index_relaxed(const hsa_queue_t * queue, uint64_t value);
hsa_status_t i_hsa_amd_image_get_info_max_dim(hsa_agent_t agent, hsa_agent_info_t attribute, void * value);
hsa_status_t i_hsa_amd_spm_set_dest_buffer(hsa_agent_t preferred_agent, size_t size_in_bytes, uint32_t * timeout, uint32_t * size_copied, void * dest, _Bool * is_data_loss);
hsa_signal_value_t i_hsa_signal_wait_relaxed(hsa_signal_t signal, hsa_signal_condition_t condition, hsa_signal_value_t compare_value, uint64_t timeout_hint, hsa_wait_state_t wait_state_hint);
hsa_status_t i_hsa_amd_portable_export_dmabuf(const void * ptr, size_t size, int * dmabuf, uint64_t * offset);
void i_hsa_signal_store_screlease(hsa_signal_t signal, hsa_signal_value_t value);
void i_hsa_signal_or_acquire(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_code_object_destroy(hsa_code_object_t code_object);
void i_hsa_signal_subtract_screlease(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_region_get_info(hsa_region_t region, hsa_region_info_t attribute, void * value);
void i_hsa_signal_subtract_scacq_screl(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_amd_profiling_async_copy_enable(_Bool enable);
hsa_status_t i_hsa_soft_queue_create(hsa_region_t region, uint32_t size, hsa_queue_type32_t type, uint32_t features, hsa_signal_t doorbell_signal, hsa_queue_t ** queue);
hsa_status_t i_hsa_shut_down();
hsa_status_t i_hsa_code_object_iterate_symbols(hsa_code_object_t code_object, hsa_status_t (*callback)(hsa_code_object_t, hsa_code_symbol_t, void *), void * data);
hsa_status_t i_hsa_extension_get_name(uint16_t extension, const char ** name);
hsa_signal_value_t i_hsa_signal_exchange_relaxed(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_executable_get_symbol_by_name(hsa_executable_t executable, const char * symbol_name, const hsa_agent_t * agent, hsa_executable_symbol_t * symbol);
hsa_status_t i_hsa_ext_image_create_with_layout(hsa_agent_t agent, const hsa_ext_image_descriptor_t * image_descriptor, const void * image_data, hsa_access_permission_t access_permission, hsa_ext_image_data_layout_t image_data_layout, size_t image_data_row_pitch, size_t image_data_slice_pitch, hsa_ext_image_t * image);
uint64_t i_hsa_queue_load_write_index_relaxed(const hsa_queue_t * queue);
hsa_status_t i_hsa_ext_image_copy(hsa_agent_t agent, hsa_ext_image_t src_image, const hsa_dim3_t * src_offset, hsa_ext_image_t dst_image, const hsa_dim3_t * dst_offset, const hsa_dim3_t * range);
hsa_status_t i_hsa_code_object_reader_create_from_memory(const void * code_object, size_t size, hsa_code_object_reader_t * code_object_reader);
void i_hsa_signal_xor_release(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_amd_register_deallocation_callback(void * ptr, hsa_amd_deallocation_callback_t callback, void * user_data);
hsa_status_t i_hsa_signal_group_destroy(hsa_signal_group_t signal_group);
hsa_status_t i_hsa_amd_vmem_export_shareable_handle(int * dmabuf_fd, hsa_amd_vmem_alloc_handle_t handle, uint64_t flags);
uint64_t i_hsa_queue_add_write_index_acq_rel(const hsa_queue_t * queue, uint64_t value);
hsa_status_t i_hsa_signal_destroy(hsa_signal_t signal);
hsa_status_t i_hsa_agent_extension_supported(uint16_t extension, hsa_agent_t agent, uint16_t version_major, uint16_t version_minor, _Bool * result);
hsa_status_t i_hsa_amd_image_create(hsa_agent_t agent, const hsa_ext_image_descriptor_t * image_descriptor, const hsa_amd_image_descriptor_t * image_layout, const void * image_data, hsa_access_permission_t access_permission, hsa_ext_image_t * image);
hsa_status_t i_hsa_isa_compatible(hsa_isa_t code_object_isa, hsa_isa_t agent_isa, _Bool * result);
hsa_status_t i_hsa_amd_queue_set_priority(hsa_queue_t * queue, hsa_amd_queue_priority_t priority);
void i_hsa_signal_and_acq_rel(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_code_object_reader_destroy(hsa_code_object_reader_t code_object_reader);
uint64_t i_hsa_queue_cas_write_index_release(const hsa_queue_t * queue, uint64_t expected, uint64_t value);
hsa_status_t i_hsa_executable_load_code_object(hsa_executable_t executable, hsa_agent_t agent, hsa_code_object_t code_object, const char * options);
hsa_status_t i_hsa_amd_memory_async_copy(void * dst, hsa_agent_t dst_agent, const void * src, hsa_agent_t src_agent, size_t size, uint32_t num_dep_signals, const hsa_signal_t * dep_signals, hsa_signal_t completion_signal);
hsa_status_t i_hsa_amd_memory_unlock(void * host_ptr);
void i_hsa_signal_subtract_release(hsa_signal_t signal, hsa_signal_value_t value);
hsa_signal_value_t i_hsa_signal_exchange_acquire(hsa_signal_t signal, hsa_signal_value_t value);
void i_hsa_signal_store_relaxed(hsa_signal_t signal, hsa_signal_value_t value);
void i_hsa_signal_subtract_scacquire(hsa_signal_t signal, hsa_signal_value_t value);
void i_hsa_signal_and_release(hsa_signal_t signal, hsa_signal_value_t value);
hsa_signal_value_t i_hsa_signal_cas_acquire(hsa_signal_t signal, hsa_signal_value_t expected, hsa_signal_value_t value);
hsa_status_t i_hsa_isa_get_exception_policies(hsa_isa_t isa, hsa_profile_t profile, uint16_t * mask);
void i_hsa_signal_add_release(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_code_object_reader_create_from_file(hsa_file_t file, hsa_code_object_reader_t * code_object_reader);
hsa_status_t i_hsa_executable_symbol_get_info(hsa_executable_symbol_t executable_symbol, hsa_executable_symbol_info_t attribute, void * value);
hsa_status_t i_hsa_amd_ipc_memory_create(void * ptr, size_t len, hsa_amd_ipc_memory_t * handle);
void i_hsa_queue_store_write_index_release(const hsa_queue_t * queue, uint64_t value);
hsa_status_t i_hsa_amd_memory_fill(void * ptr, uint32_t value, size_t count);
void i_hsa_signal_add_acq_rel(hsa_signal_t signal, hsa_signal_value_t value);
hsa_signal_value_t i_hsa_signal_cas_scacq_screl(hsa_signal_t signal, hsa_signal_value_t expected, hsa_signal_value_t value);
uint64_t i_hsa_queue_cas_write_index_scacq_screl(const hsa_queue_t * queue, uint64_t expected, uint64_t value);
hsa_status_t i_hsa_amd_ipc_signal_create(hsa_signal_t signal, hsa_amd_ipc_signal_t * handle);
hsa_status_t i_hsa_amd_spm_acquire(hsa_agent_t preferred_agent);
hsa_status_t i_hsa_amd_coherency_get_type(hsa_agent_t agent, hsa_amd_coherency_type_t * type);
hsa_status_t i_hsa_system_get_extension_table(uint16_t extension, uint16_t version_major, uint16_t version_minor, void * table);
hsa_status_t i_hsa_amd_ipc_memory_detach(void * mapped_ptr);
hsa_status_t i_hsa_ext_image_get_capability_with_layout(hsa_agent_t agent, hsa_ext_image_geometry_t geometry, const hsa_ext_image_format_t * image_format, hsa_ext_image_data_layout_t image_data_layout, uint32_t * capability_mask);
hsa_status_t i_hsa_amd_vmem_address_free(void * va, size_t size);
hsa_status_t i_hsa_amd_svm_attributes_set(void * ptr, size_t size, hsa_amd_svm_attribute_pair_t * attribute_list, size_t attribute_count);
hsa_status_t i_hsa_ext_image_import(hsa_agent_t agent, const void * src_memory, size_t src_row_pitch, size_t src_slice_pitch, hsa_ext_image_t dst_image, const hsa_ext_image_region_t * image_region);
void i_hsa_signal_or_scacquire(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_amd_svm_prefetch_async(void * ptr, size_t size, hsa_agent_t agent, uint32_t num_dep_signals, const hsa_signal_t * dep_signals, hsa_signal_t completion_signal);
void i_hsa_signal_add_scacq_screl(hsa_signal_t signal, hsa_signal_value_t value);
void i_hsa_signal_silent_store_relaxed(hsa_signal_t signal, hsa_signal_value_t value);
void i_hsa_queue_store_read_index_relaxed(const hsa_queue_t * queue, uint64_t value);
hsa_status_t i_hsa_agent_get_exception_policies(hsa_agent_t agent, hsa_profile_t profile, uint16_t * mask);
void i_hsa_signal_add_scacquire(hsa_signal_t signal, hsa_signal_value_t value);
hsa_signal_value_t i_hsa_signal_exchange_release(hsa_signal_t signal, hsa_signal_value_t value);
void i_hsa_signal_add_relaxed(hsa_signal_t signal, hsa_signal_value_t value);
void i_hsa_signal_and_acquire(hsa_signal_t signal, hsa_signal_value_t value);
uint64_t i_hsa_queue_add_write_index_relaxed(const hsa_queue_t * queue, uint64_t value);
hsa_status_t i_hsa_amd_agent_memory_pool_get_info(hsa_agent_t agent, hsa_amd_memory_pool_t memory_pool, hsa_amd_agent_memory_pool_info_t attribute, void * value);
hsa_status_t i_hsa_amd_profiling_set_profiler_enabled(hsa_queue_t * queue, int enable);
hsa_signal_value_t i_hsa_signal_exchange_acq_rel(hsa_signal_t signal, hsa_signal_value_t value);
uint64_t i_hsa_queue_cas_write_index_screlease(const hsa_queue_t * queue, uint64_t expected, uint64_t value);
hsa_status_t i_hsa_amd_async_function(void (*callback)(void *), void * arg);
void i_hsa_queue_store_read_index_screlease(const hsa_queue_t * queue, uint64_t value);
hsa_status_t i_hsa_memory_allocate(hsa_region_t region, size_t size, void ** ptr);
hsa_status_t i_hsa_amd_signal_value_pointer(hsa_signal_t signal, volatile hsa_signal_value_t ** value_ptr);
uint32_t i_hsa_amd_signal_wait_any(uint32_t signal_count, hsa_signal_t * signals, hsa_signal_condition_t * conds, hsa_signal_value_t * values, uint64_t timeout_hint, hsa_wait_state_t wait_hint, hsa_signal_value_t * satisfying_value);
hsa_status_t i_hsa_amd_vmem_retain_alloc_handle(hsa_amd_vmem_alloc_handle_t * memory_handle, void * addr);
hsa_status_t i_hsa_amd_vmem_set_access(void * va, size_t size, const hsa_amd_memory_access_desc_t * desc, size_t desc_cnt);
hsa_status_t i_hsa_memory_assign_agent(void * ptr, hsa_agent_t agent, hsa_access_permission_t access);
hsa_status_t i_hsa_amd_vmem_address_reserve(void ** va, size_t size, uint64_t address, uint64_t flags);
hsa_status_t i_hsa_amd_memory_async_copy_on_engine(void * dst, hsa_agent_t dst_agent, const void * src, hsa_agent_t src_agent, size_t size, uint32_t num_dep_signals, const hsa_signal_t * dep_signals, hsa_signal_t completion_signal, hsa_amd_sdma_engine_id_t engine_id, _Bool force_copy_on_sdma);
void i_hsa_signal_xor_scacquire(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_amd_coherency_set_type(hsa_agent_t agent, hsa_amd_coherency_type_t type);
hsa_status_t i_hsa_queue_inactivate(hsa_queue_t * queue);
hsa_status_t i_hsa_amd_vmem_import_shareable_handle(int dmabuf_fd, hsa_amd_vmem_alloc_handle_t * handle);
hsa_signal_value_t i_hsa_signal_cas_relaxed(hsa_signal_t signal, hsa_signal_value_t expected, hsa_signal_value_t value);
uint64_t i_hsa_queue_add_write_index_scacquire(const hsa_queue_t * queue, uint64_t value);
hsa_signal_value_t i_hsa_signal_exchange_screlease(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_amd_vmem_handle_create(hsa_amd_memory_pool_t pool, size_t size, hsa_amd_memory_type_t type, uint64_t flags, hsa_amd_vmem_alloc_handle_t * memory_handle);
hsa_status_t i_hsa_agent_get_info(hsa_agent_t agent, hsa_agent_info_t attribute, void * value);
void i_hsa_signal_or_acq_rel(hsa_signal_t signal, hsa_signal_value_t value);
void i_hsa_signal_xor_acq_rel(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_system_extension_supported(uint16_t extension, uint16_t version_major, uint16_t version_minor, _Bool * result);
hsa_status_t i_hsa_signal_group_wait_any_relaxed(hsa_signal_group_t signal_group, const hsa_signal_condition_t * conditions, const hsa_signal_value_t * compare_values, hsa_wait_state_t wait_state_hint, hsa_signal_t * signal, hsa_signal_value_t * value);
void i_hsa_signal_or_scacq_screl(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_executable_iterate_agent_symbols(hsa_executable_t executable, hsa_agent_t agent, hsa_status_t (*callback)(hsa_executable_t, hsa_agent_t, hsa_executable_symbol_t, void *), void * data);
hsa_status_t i_hsa_amd_deregister_deallocation_callback(void * ptr, hsa_amd_deallocation_callback_t callback);
hsa_status_t i_hsa_signal_create(hsa_signal_value_t initial_value, uint32_t num_consumers, const hsa_agent_t * consumers, hsa_signal_t * signal);
hsa_status_t i_hsa_amd_memory_copy_engine_status(hsa_agent_t dst_agent, hsa_agent_t src_agent, uint32_t * engine_ids_mask);
hsa_status_t i_hsa_ext_image_get_capability(hsa_agent_t agent, hsa_ext_image_geometry_t geometry, const hsa_ext_image_format_t * image_format, uint32_t * capability_mask);
hsa_status_t i_hsa_ext_image_destroy(hsa_agent_t agent, hsa_ext_image_t image);
void i_hsa_queue_store_read_index_release(const hsa_queue_t * queue, uint64_t value);
hsa_signal_value_t i_hsa_signal_wait_scacquire(hsa_signal_t signal, hsa_signal_condition_t condition, hsa_signal_value_t compare_value, uint64_t timeout_hint, hsa_wait_state_t wait_state_hint);
hsa_status_t i_hsa_executable_get_info(hsa_executable_t executable, hsa_executable_info_t attribute, void * value);
void i_hsa_signal_silent_store_screlease(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_executable_validate_alt(hsa_executable_t executable, const char * options, uint32_t * result);
hsa_status_t i_hsa_executable_create(hsa_profile_t profile, hsa_executable_state_t executable_state, const char * options, hsa_executable_t * executable);
hsa_status_t i_hsa_executable_iterate_program_symbols(hsa_executable_t executable, hsa_status_t (*callback)(hsa_executable_t, hsa_executable_symbol_t, void *), void * data);
hsa_status_t i_hsa_memory_free(void * ptr);
hsa_status_t i_hsa_amd_memory_pool_free(void * ptr);
hsa_status_t i_hsa_amd_memory_migrate(const void * ptr, hsa_amd_memory_pool_t memory_pool, uint32_t flags);
hsa_status_t i_hsa_amd_interop_unmap_buffer(void * ptr);
hsa_status_t i_hsa_amd_signal_create(hsa_signal_value_t initial_value, uint32_t num_consumers, const hsa_agent_t * consumers, uint64_t attributes, hsa_signal_t * signal);
hsa_status_t i_hsa_executable_load_program_code_object(hsa_executable_t executable, hsa_code_object_reader_t code_object_reader, const char * options, hsa_loaded_code_object_t * loaded_code_object);
hsa_signal_value_t i_hsa_signal_load_relaxed(hsa_signal_t signal);
hsa_status_t i_hsa_signal_group_wait_any_scacquire(hsa_signal_group_t signal_group, const hsa_signal_condition_t * conditions, const hsa_signal_value_t * compare_values, hsa_wait_state_t wait_state_hint, hsa_signal_t * signal, hsa_signal_value_t * value);
uint64_t i_hsa_queue_cas_write_index_relaxed(const hsa_queue_t * queue, uint64_t expected, uint64_t value);
void i_hsa_signal_subtract_relaxed(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_executable_freeze(hsa_executable_t executable, const char * options);
uint64_t i_hsa_queue_load_write_index_acquire(const hsa_queue_t * queue);
hsa_status_t i_hsa_amd_vmem_map(void * va, size_t size, size_t in_offset, hsa_amd_vmem_alloc_handle_t memory_handle, uint64_t flags);
hsa_status_t i_hsa_amd_ipc_signal_attach(const hsa_amd_ipc_signal_t * handle, hsa_signal_t * signal);
hsa_status_t i_hsa_amd_register_system_event_handler(hsa_amd_system_event_callback_t callback, void * data);
hsa_status_t i_hsa_amd_queue_cu_set_mask(const hsa_queue_t * queue, uint32_t num_cu_mask_count, const uint32_t * cu_mask);
hsa_status_t i_hsa_ext_image_export(hsa_agent_t agent, hsa_ext_image_t src_image, void * dst_memory, size_t dst_row_pitch, size_t dst_slice_pitch, const hsa_ext_image_region_t * image_region);
hsa_status_t i_hsa_status_string(hsa_status_t status, const char ** status_string);
void i_hsa_signal_xor_screlease(hsa_signal_t signal, hsa_signal_value_t value);
uint64_t i_hsa_queue_add_write_index_acquire(const hsa_queue_t * queue, uint64_t value);
uint64_t i_hsa_queue_add_write_index_screlease(const hsa_queue_t * queue, uint64_t value);
hsa_status_t i_hsa_code_object_get_symbol_from_name(hsa_code_object_t code_object, const char * module_name, const char * symbol_name, hsa_code_symbol_t * symbol);
hsa_status_t i_hsa_isa_from_name(const char * name, hsa_isa_t * isa);
hsa_status_t i_hsa_amd_agent_iterate_memory_pools(hsa_agent_t agent, hsa_status_t (*callback)(hsa_amd_memory_pool_t, void *), void * data);
hsa_status_t i_hsa_amd_agent_set_async_scratch_limit(hsa_agent_t agent, size_t threshold);
hsa_status_t i_hsa_ext_image_create(hsa_agent_t agent, const hsa_ext_image_descriptor_t * image_descriptor, const void * image_data, hsa_access_permission_t access_permission, hsa_ext_image_t * image);
hsa_signal_value_t i_hsa_signal_exchange_scacq_screl(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_init();
hsa_status_t i_hsa_executable_load_agent_code_object(hsa_executable_t executable, hsa_agent_t agent, hsa_code_object_reader_t code_object_reader, const char * options, hsa_loaded_code_object_t * loaded_code_object);
hsa_signal_value_t i_hsa_signal_cas_screlease(hsa_signal_t signal, hsa_signal_value_t expected, hsa_signal_value_t value);
hsa_status_t i_hsa_amd_profiling_get_dispatch_time(hsa_agent_t agent, hsa_signal_t signal, hsa_amd_profiling_dispatch_time_t * time);
hsa_status_t i_hsa_amd_portable_close_dmabuf(int dmabuf);
hsa_status_t i_hsa_memory_copy(void * dst, const void * src, size_t size);
hsa_signal_value_t i_hsa_signal_exchange_scacquire(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_ext_image_data_get_info_with_layout(hsa_agent_t agent, const hsa_ext_image_descriptor_t * image_descriptor, hsa_access_permission_t access_permission, hsa_ext_image_data_layout_t image_data_layout, size_t image_data_row_pitch, size_t image_data_slice_pitch, hsa_ext_image_data_info_t * image_data_info);
hsa_status_t i_hsa_executable_global_variable_define(hsa_executable_t executable, const char * variable_name, void * address);
void i_hsa_signal_add_acquire(hsa_signal_t signal, hsa_signal_value_t value);
hsa_status_t i_hsa_ext_image_data_get_info(hsa_agent_t agent, const hsa_ext_image_descriptor_t * image_descriptor, hsa_access_permission_t access_permission, hsa_ext_image_data_info_t * image_data_info);
hsa_signal_value_t i_hsa_signal_cas_scacquire(hsa_signal_t signal, hsa_signal_value_t expected, hsa_signal_value_t value);
hsa_status_t i_hsa_amd_interop_map_buffer(uint32_t num_agents, hsa_agent_t * agents, int interop_handle, uint32_t flags, size_t * size, void ** ptr, size_t * metadata_size, const void ** metadata);
hsa_status_t i_hsa_ext_sampler_create(hsa_agent_t agent, const hsa_ext_sampler_descriptor_t * sampler_descriptor, hsa_ext_sampler_t * sampler);
hsa_status_t i_hsa_code_object_deserialize(void * serialized_code_object, size_t serialized_code_object_size, const char * options, hsa_code_object_t * code_object);
hsa_status_t i_hsa_amd_agents_allow_access(uint32_t num_agents, const hsa_agent_t * agents, const uint32_t * flags, const void * ptr); 

     
// HSA API ID enum
typedef enum {
	HSA_API_ID_hsa_queue_load_read_index_scacquire,
	HSA_API_ID_hsa_ext_image_clear,
	HSA_API_ID_hsa_queue_cas_write_index_acquire,
	HSA_API_ID_hsa_executable_get_symbol,
	HSA_API_ID_hsa_signal_and_scacq_screl,
	HSA_API_ID_hsa_executable_agent_global_variable_define,
	HSA_API_ID_hsa_code_object_get_symbol,
	HSA_API_ID_hsa_amd_signal_async_handler,
	HSA_API_ID_hsa_signal_xor_scacq_screl,
	HSA_API_ID_hsa_signal_and_relaxed,
	HSA_API_ID_hsa_wavefront_get_info,
	HSA_API_ID_hsa_amd_memory_lock,
	HSA_API_ID_hsa_signal_or_screlease,
	HSA_API_ID_hsa_executable_validate,
	HSA_API_ID_hsa_queue_destroy,
	HSA_API_ID_hsa_isa_get_info,
	HSA_API_ID_hsa_amd_memory_lock_to_pool,
	HSA_API_ID_hsa_system_major_extension_supported,
	HSA_API_ID_hsa_amd_profiling_get_async_copy_time,
	HSA_API_ID_hsa_signal_or_relaxed,
	HSA_API_ID_hsa_queue_cas_write_index_scacquire,
	HSA_API_ID_hsa_code_object_get_info,
	HSA_API_ID_hsa_signal_and_screlease,
	HSA_API_ID_hsa_amd_pointer_info,
	HSA_API_ID_hsa_cache_get_info,
	HSA_API_ID_hsa_amd_svm_attributes_get,
	HSA_API_ID_hsa_signal_group_create,
	HSA_API_ID_hsa_memory_deregister,
	HSA_API_ID_hsa_amd_ipc_memory_attach,
	HSA_API_ID_hsa_amd_queue_cu_get_mask,
	HSA_API_ID_hsa_queue_add_write_index_scacq_screl,
	HSA_API_ID_hsa_agent_major_extension_supported,
	HSA_API_ID_hsa_ext_sampler_destroy,
	HSA_API_ID_hsa_queue_create,
	HSA_API_ID_hsa_amd_vmem_get_alloc_properties_from_handle,
	HSA_API_ID_hsa_amd_memory_pool_allocate,
	HSA_API_ID_hsa_signal_or_release,
	HSA_API_ID_hsa_queue_cas_write_index_acq_rel,
	HSA_API_ID_hsa_code_object_serialize,
	HSA_API_ID_hsa_signal_xor_acquire,
	HSA_API_ID_hsa_amd_profiling_convert_tick_to_system_domain,
	HSA_API_ID_hsa_executable_destroy,
	HSA_API_ID_hsa_signal_add_screlease,
	HSA_API_ID_hsa_signal_load_acquire,
	HSA_API_ID_hsa_executable_create_alt,
	HSA_API_ID_hsa_amd_memory_async_copy_rect,
	HSA_API_ID_hsa_signal_load_scacquire,
	HSA_API_ID_hsa_agent_iterate_caches,
	HSA_API_ID_hsa_signal_cas_acq_rel,
	HSA_API_ID_hsa_queue_load_read_index_relaxed,
	HSA_API_ID_hsa_agent_iterate_isas,
	HSA_API_ID_hsa_signal_xor_relaxed,
	HSA_API_ID_hsa_signal_subtract_acq_rel,
	HSA_API_ID_hsa_queue_load_read_index_acquire,
	HSA_API_ID_hsa_isa_get_info_alt,
	HSA_API_ID_hsa_signal_and_scacquire,
	HSA_API_ID_hsa_queue_add_write_index_release,
	HSA_API_ID_hsa_system_get_info,
	HSA_API_ID_hsa_memory_register,
	HSA_API_ID_hsa_isa_get_round_method,
	HSA_API_ID_hsa_isa_iterate_wavefronts,
	HSA_API_ID_hsa_amd_vmem_unmap,
	HSA_API_ID_hsa_signal_subtract_acquire,
	HSA_API_ID_hsa_system_get_major_extension_table,
	HSA_API_ID_hsa_signal_store_release,
	HSA_API_ID_hsa_queue_load_write_index_scacquire,
	HSA_API_ID_hsa_amd_spm_release,
	HSA_API_ID_hsa_code_symbol_get_info,
	HSA_API_ID_hsa_signal_wait_acquire,
	HSA_API_ID_hsa_signal_cas_release,
	HSA_API_ID_hsa_amd_memory_pool_get_info,
	HSA_API_ID_hsa_amd_pointer_info_set_userdata,
	HSA_API_ID_hsa_amd_vmem_get_access,
	HSA_API_ID_hsa_iterate_agents,
	HSA_API_ID_hsa_queue_store_write_index_screlease,
	HSA_API_ID_hsa_agent_iterate_regions,
	HSA_API_ID_hsa_executable_readonly_variable_define,
	HSA_API_ID_hsa_executable_iterate_symbols,
	HSA_API_ID_hsa_amd_vmem_handle_release,
	HSA_API_ID_hsa_amd_memory_pool_can_migrate,
	HSA_API_ID_hsa_queue_store_write_index_relaxed,
	HSA_API_ID_hsa_amd_image_get_info_max_dim,
	HSA_API_ID_hsa_amd_spm_set_dest_buffer,
	HSA_API_ID_hsa_signal_wait_relaxed,
	HSA_API_ID_hsa_amd_portable_export_dmabuf,
	HSA_API_ID_hsa_signal_store_screlease,
	HSA_API_ID_hsa_signal_or_acquire,
	HSA_API_ID_hsa_code_object_destroy,
	HSA_API_ID_hsa_signal_subtract_screlease,
	HSA_API_ID_hsa_region_get_info,
	HSA_API_ID_hsa_signal_subtract_scacq_screl,
	HSA_API_ID_hsa_amd_profiling_async_copy_enable,
	HSA_API_ID_hsa_soft_queue_create,
	HSA_API_ID_hsa_shut_down,
	HSA_API_ID_hsa_code_object_iterate_symbols,
	HSA_API_ID_hsa_extension_get_name,
	HSA_API_ID_hsa_signal_exchange_relaxed,
	HSA_API_ID_hsa_executable_get_symbol_by_name,
	HSA_API_ID_hsa_ext_image_create_with_layout,
	HSA_API_ID_hsa_queue_load_write_index_relaxed,
	HSA_API_ID_hsa_ext_image_copy,
	HSA_API_ID_hsa_code_object_reader_create_from_memory,
	HSA_API_ID_hsa_signal_xor_release,
	HSA_API_ID_hsa_amd_register_deallocation_callback,
	HSA_API_ID_hsa_signal_group_destroy,
	HSA_API_ID_hsa_amd_vmem_export_shareable_handle,
	HSA_API_ID_hsa_queue_add_write_index_acq_rel,
	HSA_API_ID_hsa_signal_destroy,
	HSA_API_ID_hsa_agent_extension_supported,
	HSA_API_ID_hsa_amd_image_create,
	HSA_API_ID_hsa_isa_compatible,
	HSA_API_ID_hsa_amd_queue_set_priority,
	HSA_API_ID_hsa_signal_and_acq_rel,
	HSA_API_ID_hsa_code_object_reader_destroy,
	HSA_API_ID_hsa_queue_cas_write_index_release,
	HSA_API_ID_hsa_executable_load_code_object,
	HSA_API_ID_hsa_amd_memory_async_copy,
	HSA_API_ID_hsa_amd_memory_unlock,
	HSA_API_ID_hsa_signal_subtract_release,
	HSA_API_ID_hsa_signal_exchange_acquire,
	HSA_API_ID_hsa_signal_store_relaxed,
	HSA_API_ID_hsa_signal_subtract_scacquire,
	HSA_API_ID_hsa_signal_and_release,
	HSA_API_ID_hsa_signal_cas_acquire,
	HSA_API_ID_hsa_isa_get_exception_policies,
	HSA_API_ID_hsa_signal_add_release,
	HSA_API_ID_hsa_code_object_reader_create_from_file,
	HSA_API_ID_hsa_executable_symbol_get_info,
	HSA_API_ID_hsa_amd_ipc_memory_create,
	HSA_API_ID_hsa_queue_store_write_index_release,
	HSA_API_ID_hsa_amd_memory_fill,
	HSA_API_ID_hsa_signal_add_acq_rel,
	HSA_API_ID_hsa_signal_cas_scacq_screl,
	HSA_API_ID_hsa_queue_cas_write_index_scacq_screl,
	HSA_API_ID_hsa_amd_ipc_signal_create,
	HSA_API_ID_hsa_amd_spm_acquire,
	HSA_API_ID_hsa_amd_coherency_get_type,
	HSA_API_ID_hsa_system_get_extension_table,
	HSA_API_ID_hsa_amd_ipc_memory_detach,
	HSA_API_ID_hsa_ext_image_get_capability_with_layout,
	HSA_API_ID_hsa_amd_vmem_address_free,
	HSA_API_ID_hsa_amd_svm_attributes_set,
	HSA_API_ID_hsa_ext_image_import,
	HSA_API_ID_hsa_signal_or_scacquire,
	HSA_API_ID_hsa_amd_svm_prefetch_async,
	HSA_API_ID_hsa_signal_add_scacq_screl,
	HSA_API_ID_hsa_signal_silent_store_relaxed,
	HSA_API_ID_hsa_queue_store_read_index_relaxed,
	HSA_API_ID_hsa_agent_get_exception_policies,
	HSA_API_ID_hsa_signal_add_scacquire,
	HSA_API_ID_hsa_signal_exchange_release,
	HSA_API_ID_hsa_signal_add_relaxed,
	HSA_API_ID_hsa_signal_and_acquire,
	HSA_API_ID_hsa_queue_add_write_index_relaxed,
	HSA_API_ID_hsa_amd_agent_memory_pool_get_info,
	HSA_API_ID_hsa_amd_profiling_set_profiler_enabled,
	HSA_API_ID_hsa_signal_exchange_acq_rel,
	HSA_API_ID_hsa_queue_cas_write_index_screlease,
	HSA_API_ID_hsa_amd_async_function,
	HSA_API_ID_hsa_queue_store_read_index_screlease,
	HSA_API_ID_hsa_memory_allocate,
	HSA_API_ID_hsa_amd_signal_value_pointer,
	HSA_API_ID_hsa_amd_signal_wait_any,
	HSA_API_ID_hsa_amd_vmem_retain_alloc_handle,
	HSA_API_ID_hsa_amd_vmem_set_access,
	HSA_API_ID_hsa_memory_assign_agent,
	HSA_API_ID_hsa_amd_vmem_address_reserve,
	HSA_API_ID_hsa_amd_memory_async_copy_on_engine,
	HSA_API_ID_hsa_signal_xor_scacquire,
	HSA_API_ID_hsa_amd_coherency_set_type,
	HSA_API_ID_hsa_queue_inactivate,
	HSA_API_ID_hsa_amd_vmem_import_shareable_handle,
	HSA_API_ID_hsa_signal_cas_relaxed,
	HSA_API_ID_hsa_queue_add_write_index_scacquire,
	HSA_API_ID_hsa_signal_exchange_screlease,
	HSA_API_ID_hsa_amd_vmem_handle_create,
	HSA_API_ID_hsa_agent_get_info,
	HSA_API_ID_hsa_signal_or_acq_rel,
	HSA_API_ID_hsa_signal_xor_acq_rel,
	HSA_API_ID_hsa_system_extension_supported,
	HSA_API_ID_hsa_signal_group_wait_any_relaxed,
	HSA_API_ID_hsa_signal_or_scacq_screl,
	HSA_API_ID_hsa_executable_iterate_agent_symbols,
	HSA_API_ID_hsa_amd_deregister_deallocation_callback,
	HSA_API_ID_hsa_signal_create,
	HSA_API_ID_hsa_amd_memory_copy_engine_status,
	HSA_API_ID_hsa_ext_image_get_capability,
	HSA_API_ID_hsa_ext_image_destroy,
	HSA_API_ID_hsa_queue_store_read_index_release,
	HSA_API_ID_hsa_signal_wait_scacquire,
	HSA_API_ID_hsa_executable_get_info,
	HSA_API_ID_hsa_signal_silent_store_screlease,
	HSA_API_ID_hsa_executable_validate_alt,
	HSA_API_ID_hsa_executable_create,
	HSA_API_ID_hsa_executable_iterate_program_symbols,
	HSA_API_ID_hsa_memory_free,
	HSA_API_ID_hsa_amd_memory_pool_free,
	HSA_API_ID_hsa_amd_memory_migrate,
	HSA_API_ID_hsa_amd_interop_unmap_buffer,
	HSA_API_ID_hsa_amd_signal_create,
	HSA_API_ID_hsa_executable_load_program_code_object,
	HSA_API_ID_hsa_signal_load_relaxed,
	HSA_API_ID_hsa_signal_group_wait_any_scacquire,
	HSA_API_ID_hsa_queue_cas_write_index_relaxed,
	HSA_API_ID_hsa_signal_subtract_relaxed,
	HSA_API_ID_hsa_executable_freeze,
	HSA_API_ID_hsa_queue_load_write_index_acquire,
	HSA_API_ID_hsa_amd_vmem_map,
	HSA_API_ID_hsa_amd_ipc_signal_attach,
	HSA_API_ID_hsa_amd_register_system_event_handler,
	HSA_API_ID_hsa_amd_queue_cu_set_mask,
	HSA_API_ID_hsa_ext_image_export,
	HSA_API_ID_hsa_status_string,
	HSA_API_ID_hsa_signal_xor_screlease,
	HSA_API_ID_hsa_queue_add_write_index_acquire,
	HSA_API_ID_hsa_queue_add_write_index_screlease,
	HSA_API_ID_hsa_code_object_get_symbol_from_name,
	HSA_API_ID_hsa_isa_from_name,
	HSA_API_ID_hsa_amd_agent_iterate_memory_pools,
	HSA_API_ID_hsa_amd_agent_set_async_scratch_limit,
	HSA_API_ID_hsa_ext_image_create,
	HSA_API_ID_hsa_signal_exchange_scacq_screl,
	HSA_API_ID_hsa_init,
	HSA_API_ID_hsa_executable_load_agent_code_object,
	HSA_API_ID_hsa_signal_cas_screlease,
	HSA_API_ID_hsa_amd_profiling_get_dispatch_time,
	HSA_API_ID_hsa_amd_portable_close_dmabuf,
	HSA_API_ID_hsa_memory_copy,
	HSA_API_ID_hsa_signal_exchange_scacquire,
	HSA_API_ID_hsa_ext_image_data_get_info_with_layout,
	HSA_API_ID_hsa_executable_global_variable_define,
	HSA_API_ID_hsa_signal_add_acquire,
	HSA_API_ID_hsa_ext_image_data_get_info,
	HSA_API_ID_hsa_signal_cas_scacquire,
	HSA_API_ID_hsa_amd_interop_map_buffer,
	HSA_API_ID_hsa_ext_sampler_create,
	HSA_API_ID_hsa_code_object_deserialize,
	HSA_API_ID_hsa_amd_agents_allow_access, 
    HSA_API_ID_NB_FUNCTION,
    HSA_API_ID_UNKNOWN,
} hsa_api_id_t;
 

     
// Return HSA API function name for a given ID
static inline const char* get_hsa_funame_by_id(hsa_api_id_t id) 
{
    switch(id) {
		case HSA_API_ID_hsa_queue_load_read_index_scacquire : return "hsa_queue_load_read_index_scacquire";
		case HSA_API_ID_hsa_ext_image_clear : return "hsa_ext_image_clear";
		case HSA_API_ID_hsa_queue_cas_write_index_acquire : return "hsa_queue_cas_write_index_acquire";
		case HSA_API_ID_hsa_executable_get_symbol : return "hsa_executable_get_symbol";
		case HSA_API_ID_hsa_signal_and_scacq_screl : return "hsa_signal_and_scacq_screl";
		case HSA_API_ID_hsa_executable_agent_global_variable_define : return "hsa_executable_agent_global_variable_define";
		case HSA_API_ID_hsa_code_object_get_symbol : return "hsa_code_object_get_symbol";
		case HSA_API_ID_hsa_amd_signal_async_handler : return "hsa_amd_signal_async_handler";
		case HSA_API_ID_hsa_signal_xor_scacq_screl : return "hsa_signal_xor_scacq_screl";
		case HSA_API_ID_hsa_signal_and_relaxed : return "hsa_signal_and_relaxed";
		case HSA_API_ID_hsa_wavefront_get_info : return "hsa_wavefront_get_info";
		case HSA_API_ID_hsa_amd_memory_lock : return "hsa_amd_memory_lock";
		case HSA_API_ID_hsa_signal_or_screlease : return "hsa_signal_or_screlease";
		case HSA_API_ID_hsa_executable_validate : return "hsa_executable_validate";
		case HSA_API_ID_hsa_queue_destroy : return "hsa_queue_destroy";
		case HSA_API_ID_hsa_isa_get_info : return "hsa_isa_get_info";
		case HSA_API_ID_hsa_amd_memory_lock_to_pool : return "hsa_amd_memory_lock_to_pool";
		case HSA_API_ID_hsa_system_major_extension_supported : return "hsa_system_major_extension_supported";
		case HSA_API_ID_hsa_amd_profiling_get_async_copy_time : return "hsa_amd_profiling_get_async_copy_time";
		case HSA_API_ID_hsa_signal_or_relaxed : return "hsa_signal_or_relaxed";
		case HSA_API_ID_hsa_queue_cas_write_index_scacquire : return "hsa_queue_cas_write_index_scacquire";
		case HSA_API_ID_hsa_code_object_get_info : return "hsa_code_object_get_info";
		case HSA_API_ID_hsa_signal_and_screlease : return "hsa_signal_and_screlease";
		case HSA_API_ID_hsa_amd_pointer_info : return "hsa_amd_pointer_info";
		case HSA_API_ID_hsa_cache_get_info : return "hsa_cache_get_info";
		case HSA_API_ID_hsa_amd_svm_attributes_get : return "hsa_amd_svm_attributes_get";
		case HSA_API_ID_hsa_signal_group_create : return "hsa_signal_group_create";
		case HSA_API_ID_hsa_memory_deregister : return "hsa_memory_deregister";
		case HSA_API_ID_hsa_amd_ipc_memory_attach : return "hsa_amd_ipc_memory_attach";
		case HSA_API_ID_hsa_amd_queue_cu_get_mask : return "hsa_amd_queue_cu_get_mask";
		case HSA_API_ID_hsa_queue_add_write_index_scacq_screl : return "hsa_queue_add_write_index_scacq_screl";
		case HSA_API_ID_hsa_agent_major_extension_supported : return "hsa_agent_major_extension_supported";
		case HSA_API_ID_hsa_ext_sampler_destroy : return "hsa_ext_sampler_destroy";
		case HSA_API_ID_hsa_queue_create : return "hsa_queue_create";
		case HSA_API_ID_hsa_amd_vmem_get_alloc_properties_from_handle : return "hsa_amd_vmem_get_alloc_properties_from_handle";
		case HSA_API_ID_hsa_amd_memory_pool_allocate : return "hsa_amd_memory_pool_allocate";
		case HSA_API_ID_hsa_signal_or_release : return "hsa_signal_or_release";
		case HSA_API_ID_hsa_queue_cas_write_index_acq_rel : return "hsa_queue_cas_write_index_acq_rel";
		case HSA_API_ID_hsa_code_object_serialize : return "hsa_code_object_serialize";
		case HSA_API_ID_hsa_signal_xor_acquire : return "hsa_signal_xor_acquire";
		case HSA_API_ID_hsa_amd_profiling_convert_tick_to_system_domain : return "hsa_amd_profiling_convert_tick_to_system_domain";
		case HSA_API_ID_hsa_executable_destroy : return "hsa_executable_destroy";
		case HSA_API_ID_hsa_signal_add_screlease : return "hsa_signal_add_screlease";
		case HSA_API_ID_hsa_signal_load_acquire : return "hsa_signal_load_acquire";
		case HSA_API_ID_hsa_executable_create_alt : return "hsa_executable_create_alt";
		case HSA_API_ID_hsa_amd_memory_async_copy_rect : return "hsa_amd_memory_async_copy_rect";
		case HSA_API_ID_hsa_signal_load_scacquire : return "hsa_signal_load_scacquire";
		case HSA_API_ID_hsa_agent_iterate_caches : return "hsa_agent_iterate_caches";
		case HSA_API_ID_hsa_signal_cas_acq_rel : return "hsa_signal_cas_acq_rel";
		case HSA_API_ID_hsa_queue_load_read_index_relaxed : return "hsa_queue_load_read_index_relaxed";
		case HSA_API_ID_hsa_agent_iterate_isas : return "hsa_agent_iterate_isas";
		case HSA_API_ID_hsa_signal_xor_relaxed : return "hsa_signal_xor_relaxed";
		case HSA_API_ID_hsa_signal_subtract_acq_rel : return "hsa_signal_subtract_acq_rel";
		case HSA_API_ID_hsa_queue_load_read_index_acquire : return "hsa_queue_load_read_index_acquire";
		case HSA_API_ID_hsa_isa_get_info_alt : return "hsa_isa_get_info_alt";
		case HSA_API_ID_hsa_signal_and_scacquire : return "hsa_signal_and_scacquire";
		case HSA_API_ID_hsa_queue_add_write_index_release : return "hsa_queue_add_write_index_release";
		case HSA_API_ID_hsa_system_get_info : return "hsa_system_get_info";
		case HSA_API_ID_hsa_memory_register : return "hsa_memory_register";
		case HSA_API_ID_hsa_isa_get_round_method : return "hsa_isa_get_round_method";
		case HSA_API_ID_hsa_isa_iterate_wavefronts : return "hsa_isa_iterate_wavefronts";
		case HSA_API_ID_hsa_amd_vmem_unmap : return "hsa_amd_vmem_unmap";
		case HSA_API_ID_hsa_signal_subtract_acquire : return "hsa_signal_subtract_acquire";
		case HSA_API_ID_hsa_system_get_major_extension_table : return "hsa_system_get_major_extension_table";
		case HSA_API_ID_hsa_signal_store_release : return "hsa_signal_store_release";
		case HSA_API_ID_hsa_queue_load_write_index_scacquire : return "hsa_queue_load_write_index_scacquire";
		case HSA_API_ID_hsa_amd_spm_release : return "hsa_amd_spm_release";
		case HSA_API_ID_hsa_code_symbol_get_info : return "hsa_code_symbol_get_info";
		case HSA_API_ID_hsa_signal_wait_acquire : return "hsa_signal_wait_acquire";
		case HSA_API_ID_hsa_signal_cas_release : return "hsa_signal_cas_release";
		case HSA_API_ID_hsa_amd_memory_pool_get_info : return "hsa_amd_memory_pool_get_info";
		case HSA_API_ID_hsa_amd_pointer_info_set_userdata : return "hsa_amd_pointer_info_set_userdata";
		case HSA_API_ID_hsa_amd_vmem_get_access : return "hsa_amd_vmem_get_access";
		case HSA_API_ID_hsa_iterate_agents : return "hsa_iterate_agents";
		case HSA_API_ID_hsa_queue_store_write_index_screlease : return "hsa_queue_store_write_index_screlease";
		case HSA_API_ID_hsa_agent_iterate_regions : return "hsa_agent_iterate_regions";
		case HSA_API_ID_hsa_executable_readonly_variable_define : return "hsa_executable_readonly_variable_define";
		case HSA_API_ID_hsa_executable_iterate_symbols : return "hsa_executable_iterate_symbols";
		case HSA_API_ID_hsa_amd_vmem_handle_release : return "hsa_amd_vmem_handle_release";
		case HSA_API_ID_hsa_amd_memory_pool_can_migrate : return "hsa_amd_memory_pool_can_migrate";
		case HSA_API_ID_hsa_queue_store_write_index_relaxed : return "hsa_queue_store_write_index_relaxed";
		case HSA_API_ID_hsa_amd_image_get_info_max_dim : return "hsa_amd_image_get_info_max_dim";
		case HSA_API_ID_hsa_amd_spm_set_dest_buffer : return "hsa_amd_spm_set_dest_buffer";
		case HSA_API_ID_hsa_signal_wait_relaxed : return "hsa_signal_wait_relaxed";
		case HSA_API_ID_hsa_amd_portable_export_dmabuf : return "hsa_amd_portable_export_dmabuf";
		case HSA_API_ID_hsa_signal_store_screlease : return "hsa_signal_store_screlease";
		case HSA_API_ID_hsa_signal_or_acquire : return "hsa_signal_or_acquire";
		case HSA_API_ID_hsa_code_object_destroy : return "hsa_code_object_destroy";
		case HSA_API_ID_hsa_signal_subtract_screlease : return "hsa_signal_subtract_screlease";
		case HSA_API_ID_hsa_region_get_info : return "hsa_region_get_info";
		case HSA_API_ID_hsa_signal_subtract_scacq_screl : return "hsa_signal_subtract_scacq_screl";
		case HSA_API_ID_hsa_amd_profiling_async_copy_enable : return "hsa_amd_profiling_async_copy_enable";
		case HSA_API_ID_hsa_soft_queue_create : return "hsa_soft_queue_create";
		case HSA_API_ID_hsa_shut_down : return "hsa_shut_down";
		case HSA_API_ID_hsa_code_object_iterate_symbols : return "hsa_code_object_iterate_symbols";
		case HSA_API_ID_hsa_extension_get_name : return "hsa_extension_get_name";
		case HSA_API_ID_hsa_signal_exchange_relaxed : return "hsa_signal_exchange_relaxed";
		case HSA_API_ID_hsa_executable_get_symbol_by_name : return "hsa_executable_get_symbol_by_name";
		case HSA_API_ID_hsa_ext_image_create_with_layout : return "hsa_ext_image_create_with_layout";
		case HSA_API_ID_hsa_queue_load_write_index_relaxed : return "hsa_queue_load_write_index_relaxed";
		case HSA_API_ID_hsa_ext_image_copy : return "hsa_ext_image_copy";
		case HSA_API_ID_hsa_code_object_reader_create_from_memory : return "hsa_code_object_reader_create_from_memory";
		case HSA_API_ID_hsa_signal_xor_release : return "hsa_signal_xor_release";
		case HSA_API_ID_hsa_amd_register_deallocation_callback : return "hsa_amd_register_deallocation_callback";
		case HSA_API_ID_hsa_signal_group_destroy : return "hsa_signal_group_destroy";
		case HSA_API_ID_hsa_amd_vmem_export_shareable_handle : return "hsa_amd_vmem_export_shareable_handle";
		case HSA_API_ID_hsa_queue_add_write_index_acq_rel : return "hsa_queue_add_write_index_acq_rel";
		case HSA_API_ID_hsa_signal_destroy : return "hsa_signal_destroy";
		case HSA_API_ID_hsa_agent_extension_supported : return "hsa_agent_extension_supported";
		case HSA_API_ID_hsa_amd_image_create : return "hsa_amd_image_create";
		case HSA_API_ID_hsa_isa_compatible : return "hsa_isa_compatible";
		case HSA_API_ID_hsa_amd_queue_set_priority : return "hsa_amd_queue_set_priority";
		case HSA_API_ID_hsa_signal_and_acq_rel : return "hsa_signal_and_acq_rel";
		case HSA_API_ID_hsa_code_object_reader_destroy : return "hsa_code_object_reader_destroy";
		case HSA_API_ID_hsa_queue_cas_write_index_release : return "hsa_queue_cas_write_index_release";
		case HSA_API_ID_hsa_executable_load_code_object : return "hsa_executable_load_code_object";
		case HSA_API_ID_hsa_amd_memory_async_copy : return "hsa_amd_memory_async_copy";
		case HSA_API_ID_hsa_amd_memory_unlock : return "hsa_amd_memory_unlock";
		case HSA_API_ID_hsa_signal_subtract_release : return "hsa_signal_subtract_release";
		case HSA_API_ID_hsa_signal_exchange_acquire : return "hsa_signal_exchange_acquire";
		case HSA_API_ID_hsa_signal_store_relaxed : return "hsa_signal_store_relaxed";
		case HSA_API_ID_hsa_signal_subtract_scacquire : return "hsa_signal_subtract_scacquire";
		case HSA_API_ID_hsa_signal_and_release : return "hsa_signal_and_release";
		case HSA_API_ID_hsa_signal_cas_acquire : return "hsa_signal_cas_acquire";
		case HSA_API_ID_hsa_isa_get_exception_policies : return "hsa_isa_get_exception_policies";
		case HSA_API_ID_hsa_signal_add_release : return "hsa_signal_add_release";
		case HSA_API_ID_hsa_code_object_reader_create_from_file : return "hsa_code_object_reader_create_from_file";
		case HSA_API_ID_hsa_executable_symbol_get_info : return "hsa_executable_symbol_get_info";
		case HSA_API_ID_hsa_amd_ipc_memory_create : return "hsa_amd_ipc_memory_create";
		case HSA_API_ID_hsa_queue_store_write_index_release : return "hsa_queue_store_write_index_release";
		case HSA_API_ID_hsa_amd_memory_fill : return "hsa_amd_memory_fill";
		case HSA_API_ID_hsa_signal_add_acq_rel : return "hsa_signal_add_acq_rel";
		case HSA_API_ID_hsa_signal_cas_scacq_screl : return "hsa_signal_cas_scacq_screl";
		case HSA_API_ID_hsa_queue_cas_write_index_scacq_screl : return "hsa_queue_cas_write_index_scacq_screl";
		case HSA_API_ID_hsa_amd_ipc_signal_create : return "hsa_amd_ipc_signal_create";
		case HSA_API_ID_hsa_amd_spm_acquire : return "hsa_amd_spm_acquire";
		case HSA_API_ID_hsa_amd_coherency_get_type : return "hsa_amd_coherency_get_type";
		case HSA_API_ID_hsa_system_get_extension_table : return "hsa_system_get_extension_table";
		case HSA_API_ID_hsa_amd_ipc_memory_detach : return "hsa_amd_ipc_memory_detach";
		case HSA_API_ID_hsa_ext_image_get_capability_with_layout : return "hsa_ext_image_get_capability_with_layout";
		case HSA_API_ID_hsa_amd_vmem_address_free : return "hsa_amd_vmem_address_free";
		case HSA_API_ID_hsa_amd_svm_attributes_set : return "hsa_amd_svm_attributes_set";
		case HSA_API_ID_hsa_ext_image_import : return "hsa_ext_image_import";
		case HSA_API_ID_hsa_signal_or_scacquire : return "hsa_signal_or_scacquire";
		case HSA_API_ID_hsa_amd_svm_prefetch_async : return "hsa_amd_svm_prefetch_async";
		case HSA_API_ID_hsa_signal_add_scacq_screl : return "hsa_signal_add_scacq_screl";
		case HSA_API_ID_hsa_signal_silent_store_relaxed : return "hsa_signal_silent_store_relaxed";
		case HSA_API_ID_hsa_queue_store_read_index_relaxed : return "hsa_queue_store_read_index_relaxed";
		case HSA_API_ID_hsa_agent_get_exception_policies : return "hsa_agent_get_exception_policies";
		case HSA_API_ID_hsa_signal_add_scacquire : return "hsa_signal_add_scacquire";
		case HSA_API_ID_hsa_signal_exchange_release : return "hsa_signal_exchange_release";
		case HSA_API_ID_hsa_signal_add_relaxed : return "hsa_signal_add_relaxed";
		case HSA_API_ID_hsa_signal_and_acquire : return "hsa_signal_and_acquire";
		case HSA_API_ID_hsa_queue_add_write_index_relaxed : return "hsa_queue_add_write_index_relaxed";
		case HSA_API_ID_hsa_amd_agent_memory_pool_get_info : return "hsa_amd_agent_memory_pool_get_info";
		case HSA_API_ID_hsa_amd_profiling_set_profiler_enabled : return "hsa_amd_profiling_set_profiler_enabled";
		case HSA_API_ID_hsa_signal_exchange_acq_rel : return "hsa_signal_exchange_acq_rel";
		case HSA_API_ID_hsa_queue_cas_write_index_screlease : return "hsa_queue_cas_write_index_screlease";
		case HSA_API_ID_hsa_amd_async_function : return "hsa_amd_async_function";
		case HSA_API_ID_hsa_queue_store_read_index_screlease : return "hsa_queue_store_read_index_screlease";
		case HSA_API_ID_hsa_memory_allocate : return "hsa_memory_allocate";
		case HSA_API_ID_hsa_amd_signal_value_pointer : return "hsa_amd_signal_value_pointer";
		case HSA_API_ID_hsa_amd_signal_wait_any : return "hsa_amd_signal_wait_any";
		case HSA_API_ID_hsa_amd_vmem_retain_alloc_handle : return "hsa_amd_vmem_retain_alloc_handle";
		case HSA_API_ID_hsa_amd_vmem_set_access : return "hsa_amd_vmem_set_access";
		case HSA_API_ID_hsa_memory_assign_agent : return "hsa_memory_assign_agent";
		case HSA_API_ID_hsa_amd_vmem_address_reserve : return "hsa_amd_vmem_address_reserve";
		case HSA_API_ID_hsa_amd_memory_async_copy_on_engine : return "hsa_amd_memory_async_copy_on_engine";
		case HSA_API_ID_hsa_signal_xor_scacquire : return "hsa_signal_xor_scacquire";
		case HSA_API_ID_hsa_amd_coherency_set_type : return "hsa_amd_coherency_set_type";
		case HSA_API_ID_hsa_queue_inactivate : return "hsa_queue_inactivate";
		case HSA_API_ID_hsa_amd_vmem_import_shareable_handle : return "hsa_amd_vmem_import_shareable_handle";
		case HSA_API_ID_hsa_signal_cas_relaxed : return "hsa_signal_cas_relaxed";
		case HSA_API_ID_hsa_queue_add_write_index_scacquire : return "hsa_queue_add_write_index_scacquire";
		case HSA_API_ID_hsa_signal_exchange_screlease : return "hsa_signal_exchange_screlease";
		case HSA_API_ID_hsa_amd_vmem_handle_create : return "hsa_amd_vmem_handle_create";
		case HSA_API_ID_hsa_agent_get_info : return "hsa_agent_get_info";
		case HSA_API_ID_hsa_signal_or_acq_rel : return "hsa_signal_or_acq_rel";
		case HSA_API_ID_hsa_signal_xor_acq_rel : return "hsa_signal_xor_acq_rel";
		case HSA_API_ID_hsa_system_extension_supported : return "hsa_system_extension_supported";
		case HSA_API_ID_hsa_signal_group_wait_any_relaxed : return "hsa_signal_group_wait_any_relaxed";
		case HSA_API_ID_hsa_signal_or_scacq_screl : return "hsa_signal_or_scacq_screl";
		case HSA_API_ID_hsa_executable_iterate_agent_symbols : return "hsa_executable_iterate_agent_symbols";
		case HSA_API_ID_hsa_amd_deregister_deallocation_callback : return "hsa_amd_deregister_deallocation_callback";
		case HSA_API_ID_hsa_signal_create : return "hsa_signal_create";
		case HSA_API_ID_hsa_amd_memory_copy_engine_status : return "hsa_amd_memory_copy_engine_status";
		case HSA_API_ID_hsa_ext_image_get_capability : return "hsa_ext_image_get_capability";
		case HSA_API_ID_hsa_ext_image_destroy : return "hsa_ext_image_destroy";
		case HSA_API_ID_hsa_queue_store_read_index_release : return "hsa_queue_store_read_index_release";
		case HSA_API_ID_hsa_signal_wait_scacquire : return "hsa_signal_wait_scacquire";
		case HSA_API_ID_hsa_executable_get_info : return "hsa_executable_get_info";
		case HSA_API_ID_hsa_signal_silent_store_screlease : return "hsa_signal_silent_store_screlease";
		case HSA_API_ID_hsa_executable_validate_alt : return "hsa_executable_validate_alt";
		case HSA_API_ID_hsa_executable_create : return "hsa_executable_create";
		case HSA_API_ID_hsa_executable_iterate_program_symbols : return "hsa_executable_iterate_program_symbols";
		case HSA_API_ID_hsa_memory_free : return "hsa_memory_free";
		case HSA_API_ID_hsa_amd_memory_pool_free : return "hsa_amd_memory_pool_free";
		case HSA_API_ID_hsa_amd_memory_migrate : return "hsa_amd_memory_migrate";
		case HSA_API_ID_hsa_amd_interop_unmap_buffer : return "hsa_amd_interop_unmap_buffer";
		case HSA_API_ID_hsa_amd_signal_create : return "hsa_amd_signal_create";
		case HSA_API_ID_hsa_executable_load_program_code_object : return "hsa_executable_load_program_code_object";
		case HSA_API_ID_hsa_signal_load_relaxed : return "hsa_signal_load_relaxed";
		case HSA_API_ID_hsa_signal_group_wait_any_scacquire : return "hsa_signal_group_wait_any_scacquire";
		case HSA_API_ID_hsa_queue_cas_write_index_relaxed : return "hsa_queue_cas_write_index_relaxed";
		case HSA_API_ID_hsa_signal_subtract_relaxed : return "hsa_signal_subtract_relaxed";
		case HSA_API_ID_hsa_executable_freeze : return "hsa_executable_freeze";
		case HSA_API_ID_hsa_queue_load_write_index_acquire : return "hsa_queue_load_write_index_acquire";
		case HSA_API_ID_hsa_amd_vmem_map : return "hsa_amd_vmem_map";
		case HSA_API_ID_hsa_amd_ipc_signal_attach : return "hsa_amd_ipc_signal_attach";
		case HSA_API_ID_hsa_amd_register_system_event_handler : return "hsa_amd_register_system_event_handler";
		case HSA_API_ID_hsa_amd_queue_cu_set_mask : return "hsa_amd_queue_cu_set_mask";
		case HSA_API_ID_hsa_ext_image_export : return "hsa_ext_image_export";
		case HSA_API_ID_hsa_status_string : return "hsa_status_string";
		case HSA_API_ID_hsa_signal_xor_screlease : return "hsa_signal_xor_screlease";
		case HSA_API_ID_hsa_queue_add_write_index_acquire : return "hsa_queue_add_write_index_acquire";
		case HSA_API_ID_hsa_queue_add_write_index_screlease : return "hsa_queue_add_write_index_screlease";
		case HSA_API_ID_hsa_code_object_get_symbol_from_name : return "hsa_code_object_get_symbol_from_name";
		case HSA_API_ID_hsa_isa_from_name : return "hsa_isa_from_name";
		case HSA_API_ID_hsa_amd_agent_iterate_memory_pools : return "hsa_amd_agent_iterate_memory_pools";
		case HSA_API_ID_hsa_amd_agent_set_async_scratch_limit : return "hsa_amd_agent_set_async_scratch_limit";
		case HSA_API_ID_hsa_ext_image_create : return "hsa_ext_image_create";
		case HSA_API_ID_hsa_signal_exchange_scacq_screl : return "hsa_signal_exchange_scacq_screl";
		case HSA_API_ID_hsa_init : return "hsa_init";
		case HSA_API_ID_hsa_executable_load_agent_code_object : return "hsa_executable_load_agent_code_object";
		case HSA_API_ID_hsa_signal_cas_screlease : return "hsa_signal_cas_screlease";
		case HSA_API_ID_hsa_amd_profiling_get_dispatch_time : return "hsa_amd_profiling_get_dispatch_time";
		case HSA_API_ID_hsa_amd_portable_close_dmabuf : return "hsa_amd_portable_close_dmabuf";
		case HSA_API_ID_hsa_memory_copy : return "hsa_memory_copy";
		case HSA_API_ID_hsa_signal_exchange_scacquire : return "hsa_signal_exchange_scacquire";
		case HSA_API_ID_hsa_ext_image_data_get_info_with_layout : return "hsa_ext_image_data_get_info_with_layout";
		case HSA_API_ID_hsa_executable_global_variable_define : return "hsa_executable_global_variable_define";
		case HSA_API_ID_hsa_signal_add_acquire : return "hsa_signal_add_acquire";
		case HSA_API_ID_hsa_ext_image_data_get_info : return "hsa_ext_image_data_get_info";
		case HSA_API_ID_hsa_signal_cas_scacquire : return "hsa_signal_cas_scacquire";
		case HSA_API_ID_hsa_amd_interop_map_buffer : return "hsa_amd_interop_map_buffer";
		case HSA_API_ID_hsa_ext_sampler_create : return "hsa_ext_sampler_create";
		case HSA_API_ID_hsa_code_object_deserialize : return "hsa_code_object_deserialize";
		case HSA_API_ID_hsa_amd_agents_allow_access : return "hsa_amd_agents_allow_access"; 
        default : return NULL;
    }
    return NULL;
}
 

     
// Return HSA API function ID for a given name
static inline hsa_api_id_t get_hsa_funid_by_name(const char* name) 
{
    if (name == NULL) return HSA_API_ID_UNKNOWN;
	else if (strcmp(name, "hsa_queue_load_read_index_scacquire") == 0) return HSA_API_ID_hsa_queue_load_read_index_scacquire;
	else if (strcmp(name, "hsa_ext_image_clear") == 0) return HSA_API_ID_hsa_ext_image_clear;
	else if (strcmp(name, "hsa_queue_cas_write_index_acquire") == 0) return HSA_API_ID_hsa_queue_cas_write_index_acquire;
	else if (strcmp(name, "hsa_executable_get_symbol") == 0) return HSA_API_ID_hsa_executable_get_symbol;
	else if (strcmp(name, "hsa_signal_and_scacq_screl") == 0) return HSA_API_ID_hsa_signal_and_scacq_screl;
	else if (strcmp(name, "hsa_executable_agent_global_variable_define") == 0) return HSA_API_ID_hsa_executable_agent_global_variable_define;
	else if (strcmp(name, "hsa_code_object_get_symbol") == 0) return HSA_API_ID_hsa_code_object_get_symbol;
	else if (strcmp(name, "hsa_amd_signal_async_handler") == 0) return HSA_API_ID_hsa_amd_signal_async_handler;
	else if (strcmp(name, "hsa_signal_xor_scacq_screl") == 0) return HSA_API_ID_hsa_signal_xor_scacq_screl;
	else if (strcmp(name, "hsa_signal_and_relaxed") == 0) return HSA_API_ID_hsa_signal_and_relaxed;
	else if (strcmp(name, "hsa_wavefront_get_info") == 0) return HSA_API_ID_hsa_wavefront_get_info;
	else if (strcmp(name, "hsa_amd_memory_lock") == 0) return HSA_API_ID_hsa_amd_memory_lock;
	else if (strcmp(name, "hsa_signal_or_screlease") == 0) return HSA_API_ID_hsa_signal_or_screlease;
	else if (strcmp(name, "hsa_executable_validate") == 0) return HSA_API_ID_hsa_executable_validate;
	else if (strcmp(name, "hsa_queue_destroy") == 0) return HSA_API_ID_hsa_queue_destroy;
	else if (strcmp(name, "hsa_isa_get_info") == 0) return HSA_API_ID_hsa_isa_get_info;
	else if (strcmp(name, "hsa_amd_memory_lock_to_pool") == 0) return HSA_API_ID_hsa_amd_memory_lock_to_pool;
	else if (strcmp(name, "hsa_system_major_extension_supported") == 0) return HSA_API_ID_hsa_system_major_extension_supported;
	else if (strcmp(name, "hsa_amd_profiling_get_async_copy_time") == 0) return HSA_API_ID_hsa_amd_profiling_get_async_copy_time;
	else if (strcmp(name, "hsa_signal_or_relaxed") == 0) return HSA_API_ID_hsa_signal_or_relaxed;
	else if (strcmp(name, "hsa_queue_cas_write_index_scacquire") == 0) return HSA_API_ID_hsa_queue_cas_write_index_scacquire;
	else if (strcmp(name, "hsa_code_object_get_info") == 0) return HSA_API_ID_hsa_code_object_get_info;
	else if (strcmp(name, "hsa_signal_and_screlease") == 0) return HSA_API_ID_hsa_signal_and_screlease;
	else if (strcmp(name, "hsa_amd_pointer_info") == 0) return HSA_API_ID_hsa_amd_pointer_info;
	else if (strcmp(name, "hsa_cache_get_info") == 0) return HSA_API_ID_hsa_cache_get_info;
	else if (strcmp(name, "hsa_amd_svm_attributes_get") == 0) return HSA_API_ID_hsa_amd_svm_attributes_get;
	else if (strcmp(name, "hsa_signal_group_create") == 0) return HSA_API_ID_hsa_signal_group_create;
	else if (strcmp(name, "hsa_memory_deregister") == 0) return HSA_API_ID_hsa_memory_deregister;
	else if (strcmp(name, "hsa_amd_ipc_memory_attach") == 0) return HSA_API_ID_hsa_amd_ipc_memory_attach;
	else if (strcmp(name, "hsa_amd_queue_cu_get_mask") == 0) return HSA_API_ID_hsa_amd_queue_cu_get_mask;
	else if (strcmp(name, "hsa_queue_add_write_index_scacq_screl") == 0) return HSA_API_ID_hsa_queue_add_write_index_scacq_screl;
	else if (strcmp(name, "hsa_agent_major_extension_supported") == 0) return HSA_API_ID_hsa_agent_major_extension_supported;
	else if (strcmp(name, "hsa_ext_sampler_destroy") == 0) return HSA_API_ID_hsa_ext_sampler_destroy;
	else if (strcmp(name, "hsa_queue_create") == 0) return HSA_API_ID_hsa_queue_create;
	else if (strcmp(name, "hsa_amd_vmem_get_alloc_properties_from_handle") == 0) return HSA_API_ID_hsa_amd_vmem_get_alloc_properties_from_handle;
	else if (strcmp(name, "hsa_amd_memory_pool_allocate") == 0) return HSA_API_ID_hsa_amd_memory_pool_allocate;
	else if (strcmp(name, "hsa_signal_or_release") == 0) return HSA_API_ID_hsa_signal_or_release;
	else if (strcmp(name, "hsa_queue_cas_write_index_acq_rel") == 0) return HSA_API_ID_hsa_queue_cas_write_index_acq_rel;
	else if (strcmp(name, "hsa_code_object_serialize") == 0) return HSA_API_ID_hsa_code_object_serialize;
	else if (strcmp(name, "hsa_signal_xor_acquire") == 0) return HSA_API_ID_hsa_signal_xor_acquire;
	else if (strcmp(name, "hsa_amd_profiling_convert_tick_to_system_domain") == 0) return HSA_API_ID_hsa_amd_profiling_convert_tick_to_system_domain;
	else if (strcmp(name, "hsa_executable_destroy") == 0) return HSA_API_ID_hsa_executable_destroy;
	else if (strcmp(name, "hsa_signal_add_screlease") == 0) return HSA_API_ID_hsa_signal_add_screlease;
	else if (strcmp(name, "hsa_signal_load_acquire") == 0) return HSA_API_ID_hsa_signal_load_acquire;
	else if (strcmp(name, "hsa_executable_create_alt") == 0) return HSA_API_ID_hsa_executable_create_alt;
	else if (strcmp(name, "hsa_amd_memory_async_copy_rect") == 0) return HSA_API_ID_hsa_amd_memory_async_copy_rect;
	else if (strcmp(name, "hsa_signal_load_scacquire") == 0) return HSA_API_ID_hsa_signal_load_scacquire;
	else if (strcmp(name, "hsa_agent_iterate_caches") == 0) return HSA_API_ID_hsa_agent_iterate_caches;
	else if (strcmp(name, "hsa_signal_cas_acq_rel") == 0) return HSA_API_ID_hsa_signal_cas_acq_rel;
	else if (strcmp(name, "hsa_queue_load_read_index_relaxed") == 0) return HSA_API_ID_hsa_queue_load_read_index_relaxed;
	else if (strcmp(name, "hsa_agent_iterate_isas") == 0) return HSA_API_ID_hsa_agent_iterate_isas;
	else if (strcmp(name, "hsa_signal_xor_relaxed") == 0) return HSA_API_ID_hsa_signal_xor_relaxed;
	else if (strcmp(name, "hsa_signal_subtract_acq_rel") == 0) return HSA_API_ID_hsa_signal_subtract_acq_rel;
	else if (strcmp(name, "hsa_queue_load_read_index_acquire") == 0) return HSA_API_ID_hsa_queue_load_read_index_acquire;
	else if (strcmp(name, "hsa_isa_get_info_alt") == 0) return HSA_API_ID_hsa_isa_get_info_alt;
	else if (strcmp(name, "hsa_signal_and_scacquire") == 0) return HSA_API_ID_hsa_signal_and_scacquire;
	else if (strcmp(name, "hsa_queue_add_write_index_release") == 0) return HSA_API_ID_hsa_queue_add_write_index_release;
	else if (strcmp(name, "hsa_system_get_info") == 0) return HSA_API_ID_hsa_system_get_info;
	else if (strcmp(name, "hsa_memory_register") == 0) return HSA_API_ID_hsa_memory_register;
	else if (strcmp(name, "hsa_isa_get_round_method") == 0) return HSA_API_ID_hsa_isa_get_round_method;
	else if (strcmp(name, "hsa_isa_iterate_wavefronts") == 0) return HSA_API_ID_hsa_isa_iterate_wavefronts;
	else if (strcmp(name, "hsa_amd_vmem_unmap") == 0) return HSA_API_ID_hsa_amd_vmem_unmap;
	else if (strcmp(name, "hsa_signal_subtract_acquire") == 0) return HSA_API_ID_hsa_signal_subtract_acquire;
	else if (strcmp(name, "hsa_system_get_major_extension_table") == 0) return HSA_API_ID_hsa_system_get_major_extension_table;
	else if (strcmp(name, "hsa_signal_store_release") == 0) return HSA_API_ID_hsa_signal_store_release;
	else if (strcmp(name, "hsa_queue_load_write_index_scacquire") == 0) return HSA_API_ID_hsa_queue_load_write_index_scacquire;
	else if (strcmp(name, "hsa_amd_spm_release") == 0) return HSA_API_ID_hsa_amd_spm_release;
	else if (strcmp(name, "hsa_code_symbol_get_info") == 0) return HSA_API_ID_hsa_code_symbol_get_info;
	else if (strcmp(name, "hsa_signal_wait_acquire") == 0) return HSA_API_ID_hsa_signal_wait_acquire;
	else if (strcmp(name, "hsa_signal_cas_release") == 0) return HSA_API_ID_hsa_signal_cas_release;
	else if (strcmp(name, "hsa_amd_memory_pool_get_info") == 0) return HSA_API_ID_hsa_amd_memory_pool_get_info;
	else if (strcmp(name, "hsa_amd_pointer_info_set_userdata") == 0) return HSA_API_ID_hsa_amd_pointer_info_set_userdata;
	else if (strcmp(name, "hsa_amd_vmem_get_access") == 0) return HSA_API_ID_hsa_amd_vmem_get_access;
	else if (strcmp(name, "hsa_iterate_agents") == 0) return HSA_API_ID_hsa_iterate_agents;
	else if (strcmp(name, "hsa_queue_store_write_index_screlease") == 0) return HSA_API_ID_hsa_queue_store_write_index_screlease;
	else if (strcmp(name, "hsa_agent_iterate_regions") == 0) return HSA_API_ID_hsa_agent_iterate_regions;
	else if (strcmp(name, "hsa_executable_readonly_variable_define") == 0) return HSA_API_ID_hsa_executable_readonly_variable_define;
	else if (strcmp(name, "hsa_executable_iterate_symbols") == 0) return HSA_API_ID_hsa_executable_iterate_symbols;
	else if (strcmp(name, "hsa_amd_vmem_handle_release") == 0) return HSA_API_ID_hsa_amd_vmem_handle_release;
	else if (strcmp(name, "hsa_amd_memory_pool_can_migrate") == 0) return HSA_API_ID_hsa_amd_memory_pool_can_migrate;
	else if (strcmp(name, "hsa_queue_store_write_index_relaxed") == 0) return HSA_API_ID_hsa_queue_store_write_index_relaxed;
	else if (strcmp(name, "hsa_amd_image_get_info_max_dim") == 0) return HSA_API_ID_hsa_amd_image_get_info_max_dim;
	else if (strcmp(name, "hsa_amd_spm_set_dest_buffer") == 0) return HSA_API_ID_hsa_amd_spm_set_dest_buffer;
	else if (strcmp(name, "hsa_signal_wait_relaxed") == 0) return HSA_API_ID_hsa_signal_wait_relaxed;
	else if (strcmp(name, "hsa_amd_portable_export_dmabuf") == 0) return HSA_API_ID_hsa_amd_portable_export_dmabuf;
	else if (strcmp(name, "hsa_signal_store_screlease") == 0) return HSA_API_ID_hsa_signal_store_screlease;
	else if (strcmp(name, "hsa_signal_or_acquire") == 0) return HSA_API_ID_hsa_signal_or_acquire;
	else if (strcmp(name, "hsa_code_object_destroy") == 0) return HSA_API_ID_hsa_code_object_destroy;
	else if (strcmp(name, "hsa_signal_subtract_screlease") == 0) return HSA_API_ID_hsa_signal_subtract_screlease;
	else if (strcmp(name, "hsa_region_get_info") == 0) return HSA_API_ID_hsa_region_get_info;
	else if (strcmp(name, "hsa_signal_subtract_scacq_screl") == 0) return HSA_API_ID_hsa_signal_subtract_scacq_screl;
	else if (strcmp(name, "hsa_amd_profiling_async_copy_enable") == 0) return HSA_API_ID_hsa_amd_profiling_async_copy_enable;
	else if (strcmp(name, "hsa_soft_queue_create") == 0) return HSA_API_ID_hsa_soft_queue_create;
	else if (strcmp(name, "hsa_shut_down") == 0) return HSA_API_ID_hsa_shut_down;
	else if (strcmp(name, "hsa_code_object_iterate_symbols") == 0) return HSA_API_ID_hsa_code_object_iterate_symbols;
	else if (strcmp(name, "hsa_extension_get_name") == 0) return HSA_API_ID_hsa_extension_get_name;
	else if (strcmp(name, "hsa_signal_exchange_relaxed") == 0) return HSA_API_ID_hsa_signal_exchange_relaxed;
	else if (strcmp(name, "hsa_executable_get_symbol_by_name") == 0) return HSA_API_ID_hsa_executable_get_symbol_by_name;
	else if (strcmp(name, "hsa_ext_image_create_with_layout") == 0) return HSA_API_ID_hsa_ext_image_create_with_layout;
	else if (strcmp(name, "hsa_queue_load_write_index_relaxed") == 0) return HSA_API_ID_hsa_queue_load_write_index_relaxed;
	else if (strcmp(name, "hsa_ext_image_copy") == 0) return HSA_API_ID_hsa_ext_image_copy;
	else if (strcmp(name, "hsa_code_object_reader_create_from_memory") == 0) return HSA_API_ID_hsa_code_object_reader_create_from_memory;
	else if (strcmp(name, "hsa_signal_xor_release") == 0) return HSA_API_ID_hsa_signal_xor_release;
	else if (strcmp(name, "hsa_amd_register_deallocation_callback") == 0) return HSA_API_ID_hsa_amd_register_deallocation_callback;
	else if (strcmp(name, "hsa_signal_group_destroy") == 0) return HSA_API_ID_hsa_signal_group_destroy;
	else if (strcmp(name, "hsa_amd_vmem_export_shareable_handle") == 0) return HSA_API_ID_hsa_amd_vmem_export_shareable_handle;
	else if (strcmp(name, "hsa_queue_add_write_index_acq_rel") == 0) return HSA_API_ID_hsa_queue_add_write_index_acq_rel;
	else if (strcmp(name, "hsa_signal_destroy") == 0) return HSA_API_ID_hsa_signal_destroy;
	else if (strcmp(name, "hsa_agent_extension_supported") == 0) return HSA_API_ID_hsa_agent_extension_supported;
	else if (strcmp(name, "hsa_amd_image_create") == 0) return HSA_API_ID_hsa_amd_image_create;
	else if (strcmp(name, "hsa_isa_compatible") == 0) return HSA_API_ID_hsa_isa_compatible;
	else if (strcmp(name, "hsa_amd_queue_set_priority") == 0) return HSA_API_ID_hsa_amd_queue_set_priority;
	else if (strcmp(name, "hsa_signal_and_acq_rel") == 0) return HSA_API_ID_hsa_signal_and_acq_rel;
	else if (strcmp(name, "hsa_code_object_reader_destroy") == 0) return HSA_API_ID_hsa_code_object_reader_destroy;
	else if (strcmp(name, "hsa_queue_cas_write_index_release") == 0) return HSA_API_ID_hsa_queue_cas_write_index_release;
	else if (strcmp(name, "hsa_executable_load_code_object") == 0) return HSA_API_ID_hsa_executable_load_code_object;
	else if (strcmp(name, "hsa_amd_memory_async_copy") == 0) return HSA_API_ID_hsa_amd_memory_async_copy;
	else if (strcmp(name, "hsa_amd_memory_unlock") == 0) return HSA_API_ID_hsa_amd_memory_unlock;
	else if (strcmp(name, "hsa_signal_subtract_release") == 0) return HSA_API_ID_hsa_signal_subtract_release;
	else if (strcmp(name, "hsa_signal_exchange_acquire") == 0) return HSA_API_ID_hsa_signal_exchange_acquire;
	else if (strcmp(name, "hsa_signal_store_relaxed") == 0) return HSA_API_ID_hsa_signal_store_relaxed;
	else if (strcmp(name, "hsa_signal_subtract_scacquire") == 0) return HSA_API_ID_hsa_signal_subtract_scacquire;
	else if (strcmp(name, "hsa_signal_and_release") == 0) return HSA_API_ID_hsa_signal_and_release;
	else if (strcmp(name, "hsa_signal_cas_acquire") == 0) return HSA_API_ID_hsa_signal_cas_acquire;
	else if (strcmp(name, "hsa_isa_get_exception_policies") == 0) return HSA_API_ID_hsa_isa_get_exception_policies;
	else if (strcmp(name, "hsa_signal_add_release") == 0) return HSA_API_ID_hsa_signal_add_release;
	else if (strcmp(name, "hsa_code_object_reader_create_from_file") == 0) return HSA_API_ID_hsa_code_object_reader_create_from_file;
	else if (strcmp(name, "hsa_executable_symbol_get_info") == 0) return HSA_API_ID_hsa_executable_symbol_get_info;
	else if (strcmp(name, "hsa_amd_ipc_memory_create") == 0) return HSA_API_ID_hsa_amd_ipc_memory_create;
	else if (strcmp(name, "hsa_queue_store_write_index_release") == 0) return HSA_API_ID_hsa_queue_store_write_index_release;
	else if (strcmp(name, "hsa_amd_memory_fill") == 0) return HSA_API_ID_hsa_amd_memory_fill;
	else if (strcmp(name, "hsa_signal_add_acq_rel") == 0) return HSA_API_ID_hsa_signal_add_acq_rel;
	else if (strcmp(name, "hsa_signal_cas_scacq_screl") == 0) return HSA_API_ID_hsa_signal_cas_scacq_screl;
	else if (strcmp(name, "hsa_queue_cas_write_index_scacq_screl") == 0) return HSA_API_ID_hsa_queue_cas_write_index_scacq_screl;
	else if (strcmp(name, "hsa_amd_ipc_signal_create") == 0) return HSA_API_ID_hsa_amd_ipc_signal_create;
	else if (strcmp(name, "hsa_amd_spm_acquire") == 0) return HSA_API_ID_hsa_amd_spm_acquire;
	else if (strcmp(name, "hsa_amd_coherency_get_type") == 0) return HSA_API_ID_hsa_amd_coherency_get_type;
	else if (strcmp(name, "hsa_system_get_extension_table") == 0) return HSA_API_ID_hsa_system_get_extension_table;
	else if (strcmp(name, "hsa_amd_ipc_memory_detach") == 0) return HSA_API_ID_hsa_amd_ipc_memory_detach;
	else if (strcmp(name, "hsa_ext_image_get_capability_with_layout") == 0) return HSA_API_ID_hsa_ext_image_get_capability_with_layout;
	else if (strcmp(name, "hsa_amd_vmem_address_free") == 0) return HSA_API_ID_hsa_amd_vmem_address_free;
	else if (strcmp(name, "hsa_amd_svm_attributes_set") == 0) return HSA_API_ID_hsa_amd_svm_attributes_set;
	else if (strcmp(name, "hsa_ext_image_import") == 0) return HSA_API_ID_hsa_ext_image_import;
	else if (strcmp(name, "hsa_signal_or_scacquire") == 0) return HSA_API_ID_hsa_signal_or_scacquire;
	else if (strcmp(name, "hsa_amd_svm_prefetch_async") == 0) return HSA_API_ID_hsa_amd_svm_prefetch_async;
	else if (strcmp(name, "hsa_signal_add_scacq_screl") == 0) return HSA_API_ID_hsa_signal_add_scacq_screl;
	else if (strcmp(name, "hsa_signal_silent_store_relaxed") == 0) return HSA_API_ID_hsa_signal_silent_store_relaxed;
	else if (strcmp(name, "hsa_queue_store_read_index_relaxed") == 0) return HSA_API_ID_hsa_queue_store_read_index_relaxed;
	else if (strcmp(name, "hsa_agent_get_exception_policies") == 0) return HSA_API_ID_hsa_agent_get_exception_policies;
	else if (strcmp(name, "hsa_signal_add_scacquire") == 0) return HSA_API_ID_hsa_signal_add_scacquire;
	else if (strcmp(name, "hsa_signal_exchange_release") == 0) return HSA_API_ID_hsa_signal_exchange_release;
	else if (strcmp(name, "hsa_signal_add_relaxed") == 0) return HSA_API_ID_hsa_signal_add_relaxed;
	else if (strcmp(name, "hsa_signal_and_acquire") == 0) return HSA_API_ID_hsa_signal_and_acquire;
	else if (strcmp(name, "hsa_queue_add_write_index_relaxed") == 0) return HSA_API_ID_hsa_queue_add_write_index_relaxed;
	else if (strcmp(name, "hsa_amd_agent_memory_pool_get_info") == 0) return HSA_API_ID_hsa_amd_agent_memory_pool_get_info;
	else if (strcmp(name, "hsa_amd_profiling_set_profiler_enabled") == 0) return HSA_API_ID_hsa_amd_profiling_set_profiler_enabled;
	else if (strcmp(name, "hsa_signal_exchange_acq_rel") == 0) return HSA_API_ID_hsa_signal_exchange_acq_rel;
	else if (strcmp(name, "hsa_queue_cas_write_index_screlease") == 0) return HSA_API_ID_hsa_queue_cas_write_index_screlease;
	else if (strcmp(name, "hsa_amd_async_function") == 0) return HSA_API_ID_hsa_amd_async_function;
	else if (strcmp(name, "hsa_queue_store_read_index_screlease") == 0) return HSA_API_ID_hsa_queue_store_read_index_screlease;
	else if (strcmp(name, "hsa_memory_allocate") == 0) return HSA_API_ID_hsa_memory_allocate;
	else if (strcmp(name, "hsa_amd_signal_value_pointer") == 0) return HSA_API_ID_hsa_amd_signal_value_pointer;
	else if (strcmp(name, "hsa_amd_signal_wait_any") == 0) return HSA_API_ID_hsa_amd_signal_wait_any;
	else if (strcmp(name, "hsa_amd_vmem_retain_alloc_handle") == 0) return HSA_API_ID_hsa_amd_vmem_retain_alloc_handle;
	else if (strcmp(name, "hsa_amd_vmem_set_access") == 0) return HSA_API_ID_hsa_amd_vmem_set_access;
	else if (strcmp(name, "hsa_memory_assign_agent") == 0) return HSA_API_ID_hsa_memory_assign_agent;
	else if (strcmp(name, "hsa_amd_vmem_address_reserve") == 0) return HSA_API_ID_hsa_amd_vmem_address_reserve;
	else if (strcmp(name, "hsa_amd_memory_async_copy_on_engine") == 0) return HSA_API_ID_hsa_amd_memory_async_copy_on_engine;
	else if (strcmp(name, "hsa_signal_xor_scacquire") == 0) return HSA_API_ID_hsa_signal_xor_scacquire;
	else if (strcmp(name, "hsa_amd_coherency_set_type") == 0) return HSA_API_ID_hsa_amd_coherency_set_type;
	else if (strcmp(name, "hsa_queue_inactivate") == 0) return HSA_API_ID_hsa_queue_inactivate;
	else if (strcmp(name, "hsa_amd_vmem_import_shareable_handle") == 0) return HSA_API_ID_hsa_amd_vmem_import_shareable_handle;
	else if (strcmp(name, "hsa_signal_cas_relaxed") == 0) return HSA_API_ID_hsa_signal_cas_relaxed;
	else if (strcmp(name, "hsa_queue_add_write_index_scacquire") == 0) return HSA_API_ID_hsa_queue_add_write_index_scacquire;
	else if (strcmp(name, "hsa_signal_exchange_screlease") == 0) return HSA_API_ID_hsa_signal_exchange_screlease;
	else if (strcmp(name, "hsa_amd_vmem_handle_create") == 0) return HSA_API_ID_hsa_amd_vmem_handle_create;
	else if (strcmp(name, "hsa_agent_get_info") == 0) return HSA_API_ID_hsa_agent_get_info;
	else if (strcmp(name, "hsa_signal_or_acq_rel") == 0) return HSA_API_ID_hsa_signal_or_acq_rel;
	else if (strcmp(name, "hsa_signal_xor_acq_rel") == 0) return HSA_API_ID_hsa_signal_xor_acq_rel;
	else if (strcmp(name, "hsa_system_extension_supported") == 0) return HSA_API_ID_hsa_system_extension_supported;
	else if (strcmp(name, "hsa_signal_group_wait_any_relaxed") == 0) return HSA_API_ID_hsa_signal_group_wait_any_relaxed;
	else if (strcmp(name, "hsa_signal_or_scacq_screl") == 0) return HSA_API_ID_hsa_signal_or_scacq_screl;
	else if (strcmp(name, "hsa_executable_iterate_agent_symbols") == 0) return HSA_API_ID_hsa_executable_iterate_agent_symbols;
	else if (strcmp(name, "hsa_amd_deregister_deallocation_callback") == 0) return HSA_API_ID_hsa_amd_deregister_deallocation_callback;
	else if (strcmp(name, "hsa_signal_create") == 0) return HSA_API_ID_hsa_signal_create;
	else if (strcmp(name, "hsa_amd_memory_copy_engine_status") == 0) return HSA_API_ID_hsa_amd_memory_copy_engine_status;
	else if (strcmp(name, "hsa_ext_image_get_capability") == 0) return HSA_API_ID_hsa_ext_image_get_capability;
	else if (strcmp(name, "hsa_ext_image_destroy") == 0) return HSA_API_ID_hsa_ext_image_destroy;
	else if (strcmp(name, "hsa_queue_store_read_index_release") == 0) return HSA_API_ID_hsa_queue_store_read_index_release;
	else if (strcmp(name, "hsa_signal_wait_scacquire") == 0) return HSA_API_ID_hsa_signal_wait_scacquire;
	else if (strcmp(name, "hsa_executable_get_info") == 0) return HSA_API_ID_hsa_executable_get_info;
	else if (strcmp(name, "hsa_signal_silent_store_screlease") == 0) return HSA_API_ID_hsa_signal_silent_store_screlease;
	else if (strcmp(name, "hsa_executable_validate_alt") == 0) return HSA_API_ID_hsa_executable_validate_alt;
	else if (strcmp(name, "hsa_executable_create") == 0) return HSA_API_ID_hsa_executable_create;
	else if (strcmp(name, "hsa_executable_iterate_program_symbols") == 0) return HSA_API_ID_hsa_executable_iterate_program_symbols;
	else if (strcmp(name, "hsa_memory_free") == 0) return HSA_API_ID_hsa_memory_free;
	else if (strcmp(name, "hsa_amd_memory_pool_free") == 0) return HSA_API_ID_hsa_amd_memory_pool_free;
	else if (strcmp(name, "hsa_amd_memory_migrate") == 0) return HSA_API_ID_hsa_amd_memory_migrate;
	else if (strcmp(name, "hsa_amd_interop_unmap_buffer") == 0) return HSA_API_ID_hsa_amd_interop_unmap_buffer;
	else if (strcmp(name, "hsa_amd_signal_create") == 0) return HSA_API_ID_hsa_amd_signal_create;
	else if (strcmp(name, "hsa_executable_load_program_code_object") == 0) return HSA_API_ID_hsa_executable_load_program_code_object;
	else if (strcmp(name, "hsa_signal_load_relaxed") == 0) return HSA_API_ID_hsa_signal_load_relaxed;
	else if (strcmp(name, "hsa_signal_group_wait_any_scacquire") == 0) return HSA_API_ID_hsa_signal_group_wait_any_scacquire;
	else if (strcmp(name, "hsa_queue_cas_write_index_relaxed") == 0) return HSA_API_ID_hsa_queue_cas_write_index_relaxed;
	else if (strcmp(name, "hsa_signal_subtract_relaxed") == 0) return HSA_API_ID_hsa_signal_subtract_relaxed;
	else if (strcmp(name, "hsa_executable_freeze") == 0) return HSA_API_ID_hsa_executable_freeze;
	else if (strcmp(name, "hsa_queue_load_write_index_acquire") == 0) return HSA_API_ID_hsa_queue_load_write_index_acquire;
	else if (strcmp(name, "hsa_amd_vmem_map") == 0) return HSA_API_ID_hsa_amd_vmem_map;
	else if (strcmp(name, "hsa_amd_ipc_signal_attach") == 0) return HSA_API_ID_hsa_amd_ipc_signal_attach;
	else if (strcmp(name, "hsa_amd_register_system_event_handler") == 0) return HSA_API_ID_hsa_amd_register_system_event_handler;
	else if (strcmp(name, "hsa_amd_queue_cu_set_mask") == 0) return HSA_API_ID_hsa_amd_queue_cu_set_mask;
	else if (strcmp(name, "hsa_ext_image_export") == 0) return HSA_API_ID_hsa_ext_image_export;
	else if (strcmp(name, "hsa_status_string") == 0) return HSA_API_ID_hsa_status_string;
	else if (strcmp(name, "hsa_signal_xor_screlease") == 0) return HSA_API_ID_hsa_signal_xor_screlease;
	else if (strcmp(name, "hsa_queue_add_write_index_acquire") == 0) return HSA_API_ID_hsa_queue_add_write_index_acquire;
	else if (strcmp(name, "hsa_queue_add_write_index_screlease") == 0) return HSA_API_ID_hsa_queue_add_write_index_screlease;
	else if (strcmp(name, "hsa_code_object_get_symbol_from_name") == 0) return HSA_API_ID_hsa_code_object_get_symbol_from_name;
	else if (strcmp(name, "hsa_isa_from_name") == 0) return HSA_API_ID_hsa_isa_from_name;
	else if (strcmp(name, "hsa_amd_agent_iterate_memory_pools") == 0) return HSA_API_ID_hsa_amd_agent_iterate_memory_pools;
	else if (strcmp(name, "hsa_amd_agent_set_async_scratch_limit") == 0) return HSA_API_ID_hsa_amd_agent_set_async_scratch_limit;
	else if (strcmp(name, "hsa_ext_image_create") == 0) return HSA_API_ID_hsa_ext_image_create;
	else if (strcmp(name, "hsa_signal_exchange_scacq_screl") == 0) return HSA_API_ID_hsa_signal_exchange_scacq_screl;
	else if (strcmp(name, "hsa_init") == 0) return HSA_API_ID_hsa_init;
	else if (strcmp(name, "hsa_executable_load_agent_code_object") == 0) return HSA_API_ID_hsa_executable_load_agent_code_object;
	else if (strcmp(name, "hsa_signal_cas_screlease") == 0) return HSA_API_ID_hsa_signal_cas_screlease;
	else if (strcmp(name, "hsa_amd_profiling_get_dispatch_time") == 0) return HSA_API_ID_hsa_amd_profiling_get_dispatch_time;
	else if (strcmp(name, "hsa_amd_portable_close_dmabuf") == 0) return HSA_API_ID_hsa_amd_portable_close_dmabuf;
	else if (strcmp(name, "hsa_memory_copy") == 0) return HSA_API_ID_hsa_memory_copy;
	else if (strcmp(name, "hsa_signal_exchange_scacquire") == 0) return HSA_API_ID_hsa_signal_exchange_scacquire;
	else if (strcmp(name, "hsa_ext_image_data_get_info_with_layout") == 0) return HSA_API_ID_hsa_ext_image_data_get_info_with_layout;
	else if (strcmp(name, "hsa_executable_global_variable_define") == 0) return HSA_API_ID_hsa_executable_global_variable_define;
	else if (strcmp(name, "hsa_signal_add_acquire") == 0) return HSA_API_ID_hsa_signal_add_acquire;
	else if (strcmp(name, "hsa_ext_image_data_get_info") == 0) return HSA_API_ID_hsa_ext_image_data_get_info;
	else if (strcmp(name, "hsa_signal_cas_scacquire") == 0) return HSA_API_ID_hsa_signal_cas_scacquire;
	else if (strcmp(name, "hsa_amd_interop_map_buffer") == 0) return HSA_API_ID_hsa_amd_interop_map_buffer;
	else if (strcmp(name, "hsa_ext_sampler_create") == 0) return HSA_API_ID_hsa_ext_sampler_create;
	else if (strcmp(name, "hsa_code_object_deserialize") == 0) return HSA_API_ID_hsa_code_object_deserialize;
	else if (strcmp(name, "hsa_amd_agents_allow_access") == 0) return HSA_API_ID_hsa_amd_agents_allow_access; 
    return HSA_API_ID_UNKNOWN;
}
 

     
// Return HSA API function adress for a given ID
static inline void* get_hsa_funaddr_by_id(hsa_api_id_t id) 
{
    switch(id) {
		case HSA_API_ID_hsa_queue_load_read_index_scacquire : return i_hsa_queue_load_read_index_scacquire;
		case HSA_API_ID_hsa_ext_image_clear : return i_hsa_ext_image_clear;
		case HSA_API_ID_hsa_queue_cas_write_index_acquire : return i_hsa_queue_cas_write_index_acquire;
		case HSA_API_ID_hsa_executable_get_symbol : return i_hsa_executable_get_symbol;
		case HSA_API_ID_hsa_signal_and_scacq_screl : return i_hsa_signal_and_scacq_screl;
		case HSA_API_ID_hsa_executable_agent_global_variable_define : return i_hsa_executable_agent_global_variable_define;
		case HSA_API_ID_hsa_code_object_get_symbol : return i_hsa_code_object_get_symbol;
		case HSA_API_ID_hsa_amd_signal_async_handler : return i_hsa_amd_signal_async_handler;
		case HSA_API_ID_hsa_signal_xor_scacq_screl : return i_hsa_signal_xor_scacq_screl;
		case HSA_API_ID_hsa_signal_and_relaxed : return i_hsa_signal_and_relaxed;
		case HSA_API_ID_hsa_wavefront_get_info : return i_hsa_wavefront_get_info;
		case HSA_API_ID_hsa_amd_memory_lock : return i_hsa_amd_memory_lock;
		case HSA_API_ID_hsa_signal_or_screlease : return i_hsa_signal_or_screlease;
		case HSA_API_ID_hsa_executable_validate : return i_hsa_executable_validate;
		case HSA_API_ID_hsa_queue_destroy : return i_hsa_queue_destroy;
		case HSA_API_ID_hsa_isa_get_info : return i_hsa_isa_get_info;
		case HSA_API_ID_hsa_amd_memory_lock_to_pool : return i_hsa_amd_memory_lock_to_pool;
		case HSA_API_ID_hsa_system_major_extension_supported : return i_hsa_system_major_extension_supported;
		case HSA_API_ID_hsa_amd_profiling_get_async_copy_time : return i_hsa_amd_profiling_get_async_copy_time;
		case HSA_API_ID_hsa_signal_or_relaxed : return i_hsa_signal_or_relaxed;
		case HSA_API_ID_hsa_queue_cas_write_index_scacquire : return i_hsa_queue_cas_write_index_scacquire;
		case HSA_API_ID_hsa_code_object_get_info : return i_hsa_code_object_get_info;
		case HSA_API_ID_hsa_signal_and_screlease : return i_hsa_signal_and_screlease;
		case HSA_API_ID_hsa_amd_pointer_info : return i_hsa_amd_pointer_info;
		case HSA_API_ID_hsa_cache_get_info : return i_hsa_cache_get_info;
		case HSA_API_ID_hsa_amd_svm_attributes_get : return i_hsa_amd_svm_attributes_get;
		case HSA_API_ID_hsa_signal_group_create : return i_hsa_signal_group_create;
		case HSA_API_ID_hsa_memory_deregister : return i_hsa_memory_deregister;
		case HSA_API_ID_hsa_amd_ipc_memory_attach : return i_hsa_amd_ipc_memory_attach;
		case HSA_API_ID_hsa_amd_queue_cu_get_mask : return i_hsa_amd_queue_cu_get_mask;
		case HSA_API_ID_hsa_queue_add_write_index_scacq_screl : return i_hsa_queue_add_write_index_scacq_screl;
		case HSA_API_ID_hsa_agent_major_extension_supported : return i_hsa_agent_major_extension_supported;
		case HSA_API_ID_hsa_ext_sampler_destroy : return i_hsa_ext_sampler_destroy;
		case HSA_API_ID_hsa_queue_create : return i_hsa_queue_create;
		case HSA_API_ID_hsa_amd_vmem_get_alloc_properties_from_handle : return i_hsa_amd_vmem_get_alloc_properties_from_handle;
		case HSA_API_ID_hsa_amd_memory_pool_allocate : return i_hsa_amd_memory_pool_allocate;
		case HSA_API_ID_hsa_signal_or_release : return i_hsa_signal_or_release;
		case HSA_API_ID_hsa_queue_cas_write_index_acq_rel : return i_hsa_queue_cas_write_index_acq_rel;
		case HSA_API_ID_hsa_code_object_serialize : return i_hsa_code_object_serialize;
		case HSA_API_ID_hsa_signal_xor_acquire : return i_hsa_signal_xor_acquire;
		case HSA_API_ID_hsa_amd_profiling_convert_tick_to_system_domain : return i_hsa_amd_profiling_convert_tick_to_system_domain;
		case HSA_API_ID_hsa_executable_destroy : return i_hsa_executable_destroy;
		case HSA_API_ID_hsa_signal_add_screlease : return i_hsa_signal_add_screlease;
		case HSA_API_ID_hsa_signal_load_acquire : return i_hsa_signal_load_acquire;
		case HSA_API_ID_hsa_executable_create_alt : return i_hsa_executable_create_alt;
		case HSA_API_ID_hsa_amd_memory_async_copy_rect : return i_hsa_amd_memory_async_copy_rect;
		case HSA_API_ID_hsa_signal_load_scacquire : return i_hsa_signal_load_scacquire;
		case HSA_API_ID_hsa_agent_iterate_caches : return i_hsa_agent_iterate_caches;
		case HSA_API_ID_hsa_signal_cas_acq_rel : return i_hsa_signal_cas_acq_rel;
		case HSA_API_ID_hsa_queue_load_read_index_relaxed : return i_hsa_queue_load_read_index_relaxed;
		case HSA_API_ID_hsa_agent_iterate_isas : return i_hsa_agent_iterate_isas;
		case HSA_API_ID_hsa_signal_xor_relaxed : return i_hsa_signal_xor_relaxed;
		case HSA_API_ID_hsa_signal_subtract_acq_rel : return i_hsa_signal_subtract_acq_rel;
		case HSA_API_ID_hsa_queue_load_read_index_acquire : return i_hsa_queue_load_read_index_acquire;
		case HSA_API_ID_hsa_isa_get_info_alt : return i_hsa_isa_get_info_alt;
		case HSA_API_ID_hsa_signal_and_scacquire : return i_hsa_signal_and_scacquire;
		case HSA_API_ID_hsa_queue_add_write_index_release : return i_hsa_queue_add_write_index_release;
		case HSA_API_ID_hsa_system_get_info : return i_hsa_system_get_info;
		case HSA_API_ID_hsa_memory_register : return i_hsa_memory_register;
		case HSA_API_ID_hsa_isa_get_round_method : return i_hsa_isa_get_round_method;
		case HSA_API_ID_hsa_isa_iterate_wavefronts : return i_hsa_isa_iterate_wavefronts;
		case HSA_API_ID_hsa_amd_vmem_unmap : return i_hsa_amd_vmem_unmap;
		case HSA_API_ID_hsa_signal_subtract_acquire : return i_hsa_signal_subtract_acquire;
		case HSA_API_ID_hsa_system_get_major_extension_table : return i_hsa_system_get_major_extension_table;
		case HSA_API_ID_hsa_signal_store_release : return i_hsa_signal_store_release;
		case HSA_API_ID_hsa_queue_load_write_index_scacquire : return i_hsa_queue_load_write_index_scacquire;
		case HSA_API_ID_hsa_amd_spm_release : return i_hsa_amd_spm_release;
		case HSA_API_ID_hsa_code_symbol_get_info : return i_hsa_code_symbol_get_info;
		case HSA_API_ID_hsa_signal_wait_acquire : return i_hsa_signal_wait_acquire;
		case HSA_API_ID_hsa_signal_cas_release : return i_hsa_signal_cas_release;
		case HSA_API_ID_hsa_amd_memory_pool_get_info : return i_hsa_amd_memory_pool_get_info;
		case HSA_API_ID_hsa_amd_pointer_info_set_userdata : return i_hsa_amd_pointer_info_set_userdata;
		case HSA_API_ID_hsa_amd_vmem_get_access : return i_hsa_amd_vmem_get_access;
		case HSA_API_ID_hsa_iterate_agents : return i_hsa_iterate_agents;
		case HSA_API_ID_hsa_queue_store_write_index_screlease : return i_hsa_queue_store_write_index_screlease;
		case HSA_API_ID_hsa_agent_iterate_regions : return i_hsa_agent_iterate_regions;
		case HSA_API_ID_hsa_executable_readonly_variable_define : return i_hsa_executable_readonly_variable_define;
		case HSA_API_ID_hsa_executable_iterate_symbols : return i_hsa_executable_iterate_symbols;
		case HSA_API_ID_hsa_amd_vmem_handle_release : return i_hsa_amd_vmem_handle_release;
		case HSA_API_ID_hsa_amd_memory_pool_can_migrate : return i_hsa_amd_memory_pool_can_migrate;
		case HSA_API_ID_hsa_queue_store_write_index_relaxed : return i_hsa_queue_store_write_index_relaxed;
		case HSA_API_ID_hsa_amd_image_get_info_max_dim : return i_hsa_amd_image_get_info_max_dim;
		case HSA_API_ID_hsa_amd_spm_set_dest_buffer : return i_hsa_amd_spm_set_dest_buffer;
		case HSA_API_ID_hsa_signal_wait_relaxed : return i_hsa_signal_wait_relaxed;
		case HSA_API_ID_hsa_amd_portable_export_dmabuf : return i_hsa_amd_portable_export_dmabuf;
		case HSA_API_ID_hsa_signal_store_screlease : return i_hsa_signal_store_screlease;
		case HSA_API_ID_hsa_signal_or_acquire : return i_hsa_signal_or_acquire;
		case HSA_API_ID_hsa_code_object_destroy : return i_hsa_code_object_destroy;
		case HSA_API_ID_hsa_signal_subtract_screlease : return i_hsa_signal_subtract_screlease;
		case HSA_API_ID_hsa_region_get_info : return i_hsa_region_get_info;
		case HSA_API_ID_hsa_signal_subtract_scacq_screl : return i_hsa_signal_subtract_scacq_screl;
		case HSA_API_ID_hsa_amd_profiling_async_copy_enable : return i_hsa_amd_profiling_async_copy_enable;
		case HSA_API_ID_hsa_soft_queue_create : return i_hsa_soft_queue_create;
		case HSA_API_ID_hsa_shut_down : return i_hsa_shut_down;
		case HSA_API_ID_hsa_code_object_iterate_symbols : return i_hsa_code_object_iterate_symbols;
		case HSA_API_ID_hsa_extension_get_name : return i_hsa_extension_get_name;
		case HSA_API_ID_hsa_signal_exchange_relaxed : return i_hsa_signal_exchange_relaxed;
		case HSA_API_ID_hsa_executable_get_symbol_by_name : return i_hsa_executable_get_symbol_by_name;
		case HSA_API_ID_hsa_ext_image_create_with_layout : return i_hsa_ext_image_create_with_layout;
		case HSA_API_ID_hsa_queue_load_write_index_relaxed : return i_hsa_queue_load_write_index_relaxed;
		case HSA_API_ID_hsa_ext_image_copy : return i_hsa_ext_image_copy;
		case HSA_API_ID_hsa_code_object_reader_create_from_memory : return i_hsa_code_object_reader_create_from_memory;
		case HSA_API_ID_hsa_signal_xor_release : return i_hsa_signal_xor_release;
		case HSA_API_ID_hsa_amd_register_deallocation_callback : return i_hsa_amd_register_deallocation_callback;
		case HSA_API_ID_hsa_signal_group_destroy : return i_hsa_signal_group_destroy;
		case HSA_API_ID_hsa_amd_vmem_export_shareable_handle : return i_hsa_amd_vmem_export_shareable_handle;
		case HSA_API_ID_hsa_queue_add_write_index_acq_rel : return i_hsa_queue_add_write_index_acq_rel;
		case HSA_API_ID_hsa_signal_destroy : return i_hsa_signal_destroy;
		case HSA_API_ID_hsa_agent_extension_supported : return i_hsa_agent_extension_supported;
		case HSA_API_ID_hsa_amd_image_create : return i_hsa_amd_image_create;
		case HSA_API_ID_hsa_isa_compatible : return i_hsa_isa_compatible;
		case HSA_API_ID_hsa_amd_queue_set_priority : return i_hsa_amd_queue_set_priority;
		case HSA_API_ID_hsa_signal_and_acq_rel : return i_hsa_signal_and_acq_rel;
		case HSA_API_ID_hsa_code_object_reader_destroy : return i_hsa_code_object_reader_destroy;
		case HSA_API_ID_hsa_queue_cas_write_index_release : return i_hsa_queue_cas_write_index_release;
		case HSA_API_ID_hsa_executable_load_code_object : return i_hsa_executable_load_code_object;
		case HSA_API_ID_hsa_amd_memory_async_copy : return i_hsa_amd_memory_async_copy;
		case HSA_API_ID_hsa_amd_memory_unlock : return i_hsa_amd_memory_unlock;
		case HSA_API_ID_hsa_signal_subtract_release : return i_hsa_signal_subtract_release;
		case HSA_API_ID_hsa_signal_exchange_acquire : return i_hsa_signal_exchange_acquire;
		case HSA_API_ID_hsa_signal_store_relaxed : return i_hsa_signal_store_relaxed;
		case HSA_API_ID_hsa_signal_subtract_scacquire : return i_hsa_signal_subtract_scacquire;
		case HSA_API_ID_hsa_signal_and_release : return i_hsa_signal_and_release;
		case HSA_API_ID_hsa_signal_cas_acquire : return i_hsa_signal_cas_acquire;
		case HSA_API_ID_hsa_isa_get_exception_policies : return i_hsa_isa_get_exception_policies;
		case HSA_API_ID_hsa_signal_add_release : return i_hsa_signal_add_release;
		case HSA_API_ID_hsa_code_object_reader_create_from_file : return i_hsa_code_object_reader_create_from_file;
		case HSA_API_ID_hsa_executable_symbol_get_info : return i_hsa_executable_symbol_get_info;
		case HSA_API_ID_hsa_amd_ipc_memory_create : return i_hsa_amd_ipc_memory_create;
		case HSA_API_ID_hsa_queue_store_write_index_release : return i_hsa_queue_store_write_index_release;
		case HSA_API_ID_hsa_amd_memory_fill : return i_hsa_amd_memory_fill;
		case HSA_API_ID_hsa_signal_add_acq_rel : return i_hsa_signal_add_acq_rel;
		case HSA_API_ID_hsa_signal_cas_scacq_screl : return i_hsa_signal_cas_scacq_screl;
		case HSA_API_ID_hsa_queue_cas_write_index_scacq_screl : return i_hsa_queue_cas_write_index_scacq_screl;
		case HSA_API_ID_hsa_amd_ipc_signal_create : return i_hsa_amd_ipc_signal_create;
		case HSA_API_ID_hsa_amd_spm_acquire : return i_hsa_amd_spm_acquire;
		case HSA_API_ID_hsa_amd_coherency_get_type : return i_hsa_amd_coherency_get_type;
		case HSA_API_ID_hsa_system_get_extension_table : return i_hsa_system_get_extension_table;
		case HSA_API_ID_hsa_amd_ipc_memory_detach : return i_hsa_amd_ipc_memory_detach;
		case HSA_API_ID_hsa_ext_image_get_capability_with_layout : return i_hsa_ext_image_get_capability_with_layout;
		case HSA_API_ID_hsa_amd_vmem_address_free : return i_hsa_amd_vmem_address_free;
		case HSA_API_ID_hsa_amd_svm_attributes_set : return i_hsa_amd_svm_attributes_set;
		case HSA_API_ID_hsa_ext_image_import : return i_hsa_ext_image_import;
		case HSA_API_ID_hsa_signal_or_scacquire : return i_hsa_signal_or_scacquire;
		case HSA_API_ID_hsa_amd_svm_prefetch_async : return i_hsa_amd_svm_prefetch_async;
		case HSA_API_ID_hsa_signal_add_scacq_screl : return i_hsa_signal_add_scacq_screl;
		case HSA_API_ID_hsa_signal_silent_store_relaxed : return i_hsa_signal_silent_store_relaxed;
		case HSA_API_ID_hsa_queue_store_read_index_relaxed : return i_hsa_queue_store_read_index_relaxed;
		case HSA_API_ID_hsa_agent_get_exception_policies : return i_hsa_agent_get_exception_policies;
		case HSA_API_ID_hsa_signal_add_scacquire : return i_hsa_signal_add_scacquire;
		case HSA_API_ID_hsa_signal_exchange_release : return i_hsa_signal_exchange_release;
		case HSA_API_ID_hsa_signal_add_relaxed : return i_hsa_signal_add_relaxed;
		case HSA_API_ID_hsa_signal_and_acquire : return i_hsa_signal_and_acquire;
		case HSA_API_ID_hsa_queue_add_write_index_relaxed : return i_hsa_queue_add_write_index_relaxed;
		case HSA_API_ID_hsa_amd_agent_memory_pool_get_info : return i_hsa_amd_agent_memory_pool_get_info;
		case HSA_API_ID_hsa_amd_profiling_set_profiler_enabled : return i_hsa_amd_profiling_set_profiler_enabled;
		case HSA_API_ID_hsa_signal_exchange_acq_rel : return i_hsa_signal_exchange_acq_rel;
		case HSA_API_ID_hsa_queue_cas_write_index_screlease : return i_hsa_queue_cas_write_index_screlease;
		case HSA_API_ID_hsa_amd_async_function : return i_hsa_amd_async_function;
		case HSA_API_ID_hsa_queue_store_read_index_screlease : return i_hsa_queue_store_read_index_screlease;
		case HSA_API_ID_hsa_memory_allocate : return i_hsa_memory_allocate;
		case HSA_API_ID_hsa_amd_signal_value_pointer : return i_hsa_amd_signal_value_pointer;
		case HSA_API_ID_hsa_amd_signal_wait_any : return i_hsa_amd_signal_wait_any;
		case HSA_API_ID_hsa_amd_vmem_retain_alloc_handle : return i_hsa_amd_vmem_retain_alloc_handle;
		case HSA_API_ID_hsa_amd_vmem_set_access : return i_hsa_amd_vmem_set_access;
		case HSA_API_ID_hsa_memory_assign_agent : return i_hsa_memory_assign_agent;
		case HSA_API_ID_hsa_amd_vmem_address_reserve : return i_hsa_amd_vmem_address_reserve;
		case HSA_API_ID_hsa_amd_memory_async_copy_on_engine : return i_hsa_amd_memory_async_copy_on_engine;
		case HSA_API_ID_hsa_signal_xor_scacquire : return i_hsa_signal_xor_scacquire;
		case HSA_API_ID_hsa_amd_coherency_set_type : return i_hsa_amd_coherency_set_type;
		case HSA_API_ID_hsa_queue_inactivate : return i_hsa_queue_inactivate;
		case HSA_API_ID_hsa_amd_vmem_import_shareable_handle : return i_hsa_amd_vmem_import_shareable_handle;
		case HSA_API_ID_hsa_signal_cas_relaxed : return i_hsa_signal_cas_relaxed;
		case HSA_API_ID_hsa_queue_add_write_index_scacquire : return i_hsa_queue_add_write_index_scacquire;
		case HSA_API_ID_hsa_signal_exchange_screlease : return i_hsa_signal_exchange_screlease;
		case HSA_API_ID_hsa_amd_vmem_handle_create : return i_hsa_amd_vmem_handle_create;
		case HSA_API_ID_hsa_agent_get_info : return i_hsa_agent_get_info;
		case HSA_API_ID_hsa_signal_or_acq_rel : return i_hsa_signal_or_acq_rel;
		case HSA_API_ID_hsa_signal_xor_acq_rel : return i_hsa_signal_xor_acq_rel;
		case HSA_API_ID_hsa_system_extension_supported : return i_hsa_system_extension_supported;
		case HSA_API_ID_hsa_signal_group_wait_any_relaxed : return i_hsa_signal_group_wait_any_relaxed;
		case HSA_API_ID_hsa_signal_or_scacq_screl : return i_hsa_signal_or_scacq_screl;
		case HSA_API_ID_hsa_executable_iterate_agent_symbols : return i_hsa_executable_iterate_agent_symbols;
		case HSA_API_ID_hsa_amd_deregister_deallocation_callback : return i_hsa_amd_deregister_deallocation_callback;
		case HSA_API_ID_hsa_signal_create : return i_hsa_signal_create;
		case HSA_API_ID_hsa_amd_memory_copy_engine_status : return i_hsa_amd_memory_copy_engine_status;
		case HSA_API_ID_hsa_ext_image_get_capability : return i_hsa_ext_image_get_capability;
		case HSA_API_ID_hsa_ext_image_destroy : return i_hsa_ext_image_destroy;
		case HSA_API_ID_hsa_queue_store_read_index_release : return i_hsa_queue_store_read_index_release;
		case HSA_API_ID_hsa_signal_wait_scacquire : return i_hsa_signal_wait_scacquire;
		case HSA_API_ID_hsa_executable_get_info : return i_hsa_executable_get_info;
		case HSA_API_ID_hsa_signal_silent_store_screlease : return i_hsa_signal_silent_store_screlease;
		case HSA_API_ID_hsa_executable_validate_alt : return i_hsa_executable_validate_alt;
		case HSA_API_ID_hsa_executable_create : return i_hsa_executable_create;
		case HSA_API_ID_hsa_executable_iterate_program_symbols : return i_hsa_executable_iterate_program_symbols;
		case HSA_API_ID_hsa_memory_free : return i_hsa_memory_free;
		case HSA_API_ID_hsa_amd_memory_pool_free : return i_hsa_amd_memory_pool_free;
		case HSA_API_ID_hsa_amd_memory_migrate : return i_hsa_amd_memory_migrate;
		case HSA_API_ID_hsa_amd_interop_unmap_buffer : return i_hsa_amd_interop_unmap_buffer;
		case HSA_API_ID_hsa_amd_signal_create : return i_hsa_amd_signal_create;
		case HSA_API_ID_hsa_executable_load_program_code_object : return i_hsa_executable_load_program_code_object;
		case HSA_API_ID_hsa_signal_load_relaxed : return i_hsa_signal_load_relaxed;
		case HSA_API_ID_hsa_signal_group_wait_any_scacquire : return i_hsa_signal_group_wait_any_scacquire;
		case HSA_API_ID_hsa_queue_cas_write_index_relaxed : return i_hsa_queue_cas_write_index_relaxed;
		case HSA_API_ID_hsa_signal_subtract_relaxed : return i_hsa_signal_subtract_relaxed;
		case HSA_API_ID_hsa_executable_freeze : return i_hsa_executable_freeze;
		case HSA_API_ID_hsa_queue_load_write_index_acquire : return i_hsa_queue_load_write_index_acquire;
		case HSA_API_ID_hsa_amd_vmem_map : return i_hsa_amd_vmem_map;
		case HSA_API_ID_hsa_amd_ipc_signal_attach : return i_hsa_amd_ipc_signal_attach;
		case HSA_API_ID_hsa_amd_register_system_event_handler : return i_hsa_amd_register_system_event_handler;
		case HSA_API_ID_hsa_amd_queue_cu_set_mask : return i_hsa_amd_queue_cu_set_mask;
		case HSA_API_ID_hsa_ext_image_export : return i_hsa_ext_image_export;
		case HSA_API_ID_hsa_status_string : return i_hsa_status_string;
		case HSA_API_ID_hsa_signal_xor_screlease : return i_hsa_signal_xor_screlease;
		case HSA_API_ID_hsa_queue_add_write_index_acquire : return i_hsa_queue_add_write_index_acquire;
		case HSA_API_ID_hsa_queue_add_write_index_screlease : return i_hsa_queue_add_write_index_screlease;
		case HSA_API_ID_hsa_code_object_get_symbol_from_name : return i_hsa_code_object_get_symbol_from_name;
		case HSA_API_ID_hsa_isa_from_name : return i_hsa_isa_from_name;
		case HSA_API_ID_hsa_amd_agent_iterate_memory_pools : return i_hsa_amd_agent_iterate_memory_pools;
		case HSA_API_ID_hsa_amd_agent_set_async_scratch_limit : return i_hsa_amd_agent_set_async_scratch_limit;
		case HSA_API_ID_hsa_ext_image_create : return i_hsa_ext_image_create;
		case HSA_API_ID_hsa_signal_exchange_scacq_screl : return i_hsa_signal_exchange_scacq_screl;
		case HSA_API_ID_hsa_init : return i_hsa_init;
		case HSA_API_ID_hsa_executable_load_agent_code_object : return i_hsa_executable_load_agent_code_object;
		case HSA_API_ID_hsa_signal_cas_screlease : return i_hsa_signal_cas_screlease;
		case HSA_API_ID_hsa_amd_profiling_get_dispatch_time : return i_hsa_amd_profiling_get_dispatch_time;
		case HSA_API_ID_hsa_amd_portable_close_dmabuf : return i_hsa_amd_portable_close_dmabuf;
		case HSA_API_ID_hsa_memory_copy : return i_hsa_memory_copy;
		case HSA_API_ID_hsa_signal_exchange_scacquire : return i_hsa_signal_exchange_scacquire;
		case HSA_API_ID_hsa_ext_image_data_get_info_with_layout : return i_hsa_ext_image_data_get_info_with_layout;
		case HSA_API_ID_hsa_executable_global_variable_define : return i_hsa_executable_global_variable_define;
		case HSA_API_ID_hsa_signal_add_acquire : return i_hsa_signal_add_acquire;
		case HSA_API_ID_hsa_ext_image_data_get_info : return i_hsa_ext_image_data_get_info;
		case HSA_API_ID_hsa_signal_cas_scacquire : return i_hsa_signal_cas_scacquire;
		case HSA_API_ID_hsa_amd_interop_map_buffer : return i_hsa_amd_interop_map_buffer;
		case HSA_API_ID_hsa_ext_sampler_create : return i_hsa_ext_sampler_create;
		case HSA_API_ID_hsa_code_object_deserialize : return i_hsa_code_object_deserialize;
		case HSA_API_ID_hsa_amd_agents_allow_access : return i_hsa_amd_agents_allow_access; 
        default : return NULL;
    }
    return NULL;
}
 

     
// HSA API Args Data
// uint64_t
// hsa_queue_load_read_index_scacquire (
// 		const hsa_queue_t * queue (struct)
// )
struct args_hsa_queue_load_read_index_scacquire_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
};

#define GET_ARGS_VALUE_hsa_queue_load_read_index_scacquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_load_read_index_scacquire.queue = (const hsa_queue_t *)queue; \
};

#define GET_PTRS_VALUE_hsa_queue_load_read_index_scacquire(args) { \
	if (args->hsa_queue_load_read_index_scacquire.queue != NULL) { \
		args->hsa_queue_load_read_index_scacquire.queue__ref.val = *args->hsa_queue_load_read_index_scacquire.queue; \
	} \
};


// hsa_status_t
// hsa_ext_image_clear (
// 		hsa_agent_t agent (struct)
// 		hsa_ext_image_t image (struct)
// 		const void * data (void)
// 		const hsa_ext_image_region_t * image_region (struct)
// )
struct args_hsa_ext_image_clear_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_ext_image_t image;
	const void * data;
	const hsa_ext_image_region_t * image_region;
	struct { // const hsa_ext_image_region_t *
		 hsa_ext_image_region_t val;
	} image_region__ref;
};

#define GET_ARGS_VALUE_hsa_ext_image_clear(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_ext_image_clear.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_ext_image_clear.image = (hsa_ext_image_t)image; \
	__hsa_activity->hsa_args.hsa_ext_image_clear.data = (const void *)data; \
	__hsa_activity->hsa_args.hsa_ext_image_clear.image_region = (const hsa_ext_image_region_t *)image_region; \
};

#define GET_PTRS_VALUE_hsa_ext_image_clear(args) { \
	if (args->hsa_ext_image_clear.image_region != NULL) { \
		args->hsa_ext_image_clear.image_region__ref.val = *args->hsa_ext_image_clear.image_region; \
	} \
};


// uint64_t
// hsa_queue_cas_write_index_acquire (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t expected (unsigned long)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_cas_write_index_acquire_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t expected;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_cas_write_index_acquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_acquire.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_acquire.expected = (uint64_t)expected; \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_acquire.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_cas_write_index_acquire(args) { \
	if (args->hsa_queue_cas_write_index_acquire.queue != NULL) { \
		args->hsa_queue_cas_write_index_acquire.queue__ref.val = *args->hsa_queue_cas_write_index_acquire.queue; \
	} \
};


// hsa_status_t
// hsa_executable_get_symbol (
// 		hsa_executable_t executable (struct)
// 		const char * module_name (string)
// 		const char * symbol_name (string)
// 		hsa_agent_t agent (struct)
// 		int32_t call_convention (int)
// 		hsa_executable_symbol_t * symbol (struct)
// )
struct args_hsa_executable_get_symbol_t {
	hsa_status_t retval;
	hsa_executable_t executable;
	const char * module_name;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} module_name__ref;
	const char * symbol_name;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} symbol_name__ref;
	hsa_agent_t agent;
	int32_t call_convention;
	hsa_executable_symbol_t * symbol;
	struct { // hsa_executable_symbol_t *
		hsa_executable_symbol_t val;
	} symbol__ref;
};

#define GET_ARGS_VALUE_hsa_executable_get_symbol(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_get_symbol.executable = (hsa_executable_t)executable; \
	__hsa_activity->hsa_args.hsa_executable_get_symbol.module_name = (const char *)module_name; \
	__hsa_activity->hsa_args.hsa_executable_get_symbol.symbol_name = (const char *)symbol_name; \
	__hsa_activity->hsa_args.hsa_executable_get_symbol.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_executable_get_symbol.call_convention = (int32_t)call_convention; \
	__hsa_activity->hsa_args.hsa_executable_get_symbol.symbol = (hsa_executable_symbol_t *)symbol; \
};

#define GET_PTRS_VALUE_hsa_executable_get_symbol(args) { \
	if (args->hsa_executable_get_symbol.module_name != NULL) { \
		strncpy(args->hsa_executable_get_symbol.module_name__ref.val, args->hsa_executable_get_symbol.module_name, HSA_STRING_SIZE_MAX-1); \
	} \
	if (args->hsa_executable_get_symbol.symbol_name != NULL) { \
		strncpy(args->hsa_executable_get_symbol.symbol_name__ref.val, args->hsa_executable_get_symbol.symbol_name, HSA_STRING_SIZE_MAX-1); \
	} \
	if (args->hsa_executable_get_symbol.symbol != NULL) { \
		args->hsa_executable_get_symbol.symbol__ref.val = *args->hsa_executable_get_symbol.symbol; \
	} \
};


// void
// hsa_signal_and_scacq_screl (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_and_scacq_screl_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_and_scacq_screl(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_and_scacq_screl.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_and_scacq_screl.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_and_scacq_screl(args)

// hsa_status_t
// hsa_executable_agent_global_variable_define (
// 		hsa_executable_t executable (struct)
// 		hsa_agent_t agent (struct)
// 		const char * variable_name (string)
// 		void * address (void)
// )
struct args_hsa_executable_agent_global_variable_define_t {
	hsa_status_t retval;
	hsa_executable_t executable;
	hsa_agent_t agent;
	const char * variable_name;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} variable_name__ref;
	void * address;
};

#define GET_ARGS_VALUE_hsa_executable_agent_global_variable_define(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_agent_global_variable_define.executable = (hsa_executable_t)executable; \
	__hsa_activity->hsa_args.hsa_executable_agent_global_variable_define.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_executable_agent_global_variable_define.variable_name = (const char *)variable_name; \
	__hsa_activity->hsa_args.hsa_executable_agent_global_variable_define.address = (void *)address; \
};

#define GET_PTRS_VALUE_hsa_executable_agent_global_variable_define(args) { \
	if (args->hsa_executable_agent_global_variable_define.variable_name != NULL) { \
		strncpy(args->hsa_executable_agent_global_variable_define.variable_name__ref.val, args->hsa_executable_agent_global_variable_define.variable_name, HSA_STRING_SIZE_MAX-1); \
	} \
};


// hsa_status_t
// hsa_code_object_get_symbol (
// 		hsa_code_object_t code_object (struct)
// 		const char * symbol_name (string)
// 		hsa_code_symbol_t * symbol (struct)
// )
struct args_hsa_code_object_get_symbol_t {
	hsa_status_t retval;
	hsa_code_object_t code_object;
	const char * symbol_name;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} symbol_name__ref;
	hsa_code_symbol_t * symbol;
	struct { // hsa_code_symbol_t *
		hsa_code_symbol_t val;
	} symbol__ref;
};

#define GET_ARGS_VALUE_hsa_code_object_get_symbol(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_code_object_get_symbol.code_object = (hsa_code_object_t)code_object; \
	__hsa_activity->hsa_args.hsa_code_object_get_symbol.symbol_name = (const char *)symbol_name; \
	__hsa_activity->hsa_args.hsa_code_object_get_symbol.symbol = (hsa_code_symbol_t *)symbol; \
};

#define GET_PTRS_VALUE_hsa_code_object_get_symbol(args) { \
	if (args->hsa_code_object_get_symbol.symbol_name != NULL) { \
		strncpy(args->hsa_code_object_get_symbol.symbol_name__ref.val, args->hsa_code_object_get_symbol.symbol_name, HSA_STRING_SIZE_MAX-1); \
	} \
	if (args->hsa_code_object_get_symbol.symbol != NULL) { \
		args->hsa_code_object_get_symbol.symbol__ref.val = *args->hsa_code_object_get_symbol.symbol; \
	} \
};


// hsa_status_t
// hsa_amd_signal_async_handler (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_condition_t cond (enum)
// 		hsa_signal_value_t value (long)
// 		hsa_amd_signal_handler handler (function)
// 		void * arg (void)
// )
struct args_hsa_amd_signal_async_handler_t {
	hsa_status_t retval;
	hsa_signal_t signal;
	hsa_signal_condition_t cond;
	hsa_signal_value_t value;
	hsa_amd_signal_handler handler;
	void * arg;
};

#define GET_ARGS_VALUE_hsa_amd_signal_async_handler(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_signal_async_handler.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_amd_signal_async_handler.cond = (hsa_signal_condition_t)cond; \
	__hsa_activity->hsa_args.hsa_amd_signal_async_handler.value = (hsa_signal_value_t)value; \
	__hsa_activity->hsa_args.hsa_amd_signal_async_handler.handler = (hsa_amd_signal_handler)handler; \
	__hsa_activity->hsa_args.hsa_amd_signal_async_handler.arg = (void *)arg; \
};

#define GET_PTRS_VALUE_hsa_amd_signal_async_handler(args)

// void
// hsa_signal_xor_scacq_screl (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_xor_scacq_screl_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_xor_scacq_screl(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_xor_scacq_screl.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_xor_scacq_screl.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_xor_scacq_screl(args)

// void
// hsa_signal_and_relaxed (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_and_relaxed_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_and_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_and_relaxed.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_and_relaxed.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_and_relaxed(args)

// hsa_status_t
// hsa_wavefront_get_info (
// 		hsa_wavefront_t wavefront (struct)
// 		hsa_wavefront_info_t attribute (enum)
// 		void * value (void)
// )
struct args_hsa_wavefront_get_info_t {
	hsa_status_t retval;
	hsa_wavefront_t wavefront;
	hsa_wavefront_info_t attribute;
	void * value;
};

#define GET_ARGS_VALUE_hsa_wavefront_get_info(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_wavefront_get_info.wavefront = (hsa_wavefront_t)wavefront; \
	__hsa_activity->hsa_args.hsa_wavefront_get_info.attribute = (hsa_wavefront_info_t)attribute; \
	__hsa_activity->hsa_args.hsa_wavefront_get_info.value = (void *)value; \
};

#define GET_PTRS_VALUE_hsa_wavefront_get_info(args)

// hsa_status_t
// hsa_amd_memory_lock (
// 		void * host_ptr (void)
// 		size_t size (unsigned long)
// 		hsa_agent_t * agents (struct)
// 		int num_agent (int)
// 		void ** agent_ptr (void)
// )
struct args_hsa_amd_memory_lock_t {
	hsa_status_t retval;
	void * host_ptr;
	size_t size;
	hsa_agent_t * agents;
	struct { // hsa_agent_t *
		hsa_agent_t val;
	} agents__ref;
	int num_agent;
	void ** agent_ptr;
	struct { // void **
		void* ptr1;
	} agent_ptr__ref;
};

#define GET_ARGS_VALUE_hsa_amd_memory_lock(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_memory_lock.host_ptr = (void *)host_ptr; \
	__hsa_activity->hsa_args.hsa_amd_memory_lock.size = (size_t)size; \
	__hsa_activity->hsa_args.hsa_amd_memory_lock.agents = (hsa_agent_t *)agents; \
	__hsa_activity->hsa_args.hsa_amd_memory_lock.num_agent = (int)num_agent; \
	__hsa_activity->hsa_args.hsa_amd_memory_lock.agent_ptr = (void **)agent_ptr; \
};

#define GET_PTRS_VALUE_hsa_amd_memory_lock(args) { \
	if (args->hsa_amd_memory_lock.agents != NULL) { \
		args->hsa_amd_memory_lock.agents__ref.val = *args->hsa_amd_memory_lock.agents; \
	} \
	if (args->hsa_amd_memory_lock.agent_ptr != NULL) { \
		args->hsa_amd_memory_lock.agent_ptr__ref.ptr1 = (void*)*args->hsa_amd_memory_lock.agent_ptr; \
	} \
};


// void
// hsa_signal_or_screlease (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_or_screlease_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_or_screlease(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_or_screlease.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_or_screlease.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_or_screlease(args)

// hsa_status_t
// hsa_executable_validate (
// 		hsa_executable_t executable (struct)
// 		uint32_t * result (unsigned int)
// )
struct args_hsa_executable_validate_t {
	hsa_status_t retval;
	hsa_executable_t executable;
	uint32_t * result;
	struct { // uint32_t *
		uint32_t val;
	} result__ref;
};

#define GET_ARGS_VALUE_hsa_executable_validate(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_validate.executable = (hsa_executable_t)executable; \
	__hsa_activity->hsa_args.hsa_executable_validate.result = (uint32_t *)result; \
};

#define GET_PTRS_VALUE_hsa_executable_validate(args) { \
	if (args->hsa_executable_validate.result != NULL) { \
		args->hsa_executable_validate.result__ref.val = *args->hsa_executable_validate.result; \
	} \
};


// hsa_status_t
// hsa_queue_destroy (
// 		hsa_queue_t * queue (struct)
// )
struct args_hsa_queue_destroy_t {
	hsa_status_t retval;
	hsa_queue_t * queue;
	struct { // hsa_queue_t *
		hsa_queue_t val;
	} queue__ref;
};

#define GET_ARGS_VALUE_hsa_queue_destroy(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_destroy.queue = (hsa_queue_t *)queue; \
};

#define GET_PTRS_VALUE_hsa_queue_destroy(args) { \
	if (args->hsa_queue_destroy.queue != NULL) { \
		args->hsa_queue_destroy.queue__ref.val = *args->hsa_queue_destroy.queue; \
	} \
};


// hsa_status_t
// hsa_isa_get_info (
// 		hsa_isa_t isa (struct)
// 		hsa_isa_info_t attribute (enum)
// 		uint32_t index (unsigned int)
// 		void * value (void)
// )
struct args_hsa_isa_get_info_t {
	hsa_status_t retval;
	hsa_isa_t isa;
	hsa_isa_info_t attribute;
	uint32_t index;
	void * value;
};

#define GET_ARGS_VALUE_hsa_isa_get_info(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_isa_get_info.isa = (hsa_isa_t)isa; \
	__hsa_activity->hsa_args.hsa_isa_get_info.attribute = (hsa_isa_info_t)attribute; \
	__hsa_activity->hsa_args.hsa_isa_get_info.index = (uint32_t)index; \
	__hsa_activity->hsa_args.hsa_isa_get_info.value = (void *)value; \
};

#define GET_PTRS_VALUE_hsa_isa_get_info(args)

// hsa_status_t
// hsa_amd_memory_lock_to_pool (
// 		void * host_ptr (void)
// 		size_t size (unsigned long)
// 		hsa_agent_t * agents (struct)
// 		int num_agent (int)
// 		hsa_amd_memory_pool_t pool (struct)
// 		uint32_t flags (unsigned int)
// 		void ** agent_ptr (void)
// )
struct args_hsa_amd_memory_lock_to_pool_t {
	hsa_status_t retval;
	void * host_ptr;
	size_t size;
	hsa_agent_t * agents;
	struct { // hsa_agent_t *
		hsa_agent_t val;
	} agents__ref;
	int num_agent;
	hsa_amd_memory_pool_t pool;
	uint32_t flags;
	void ** agent_ptr;
	struct { // void **
		void* ptr1;
	} agent_ptr__ref;
};

#define GET_ARGS_VALUE_hsa_amd_memory_lock_to_pool(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_memory_lock_to_pool.host_ptr = (void *)host_ptr; \
	__hsa_activity->hsa_args.hsa_amd_memory_lock_to_pool.size = (size_t)size; \
	__hsa_activity->hsa_args.hsa_amd_memory_lock_to_pool.agents = (hsa_agent_t *)agents; \
	__hsa_activity->hsa_args.hsa_amd_memory_lock_to_pool.num_agent = (int)num_agent; \
	__hsa_activity->hsa_args.hsa_amd_memory_lock_to_pool.pool = (hsa_amd_memory_pool_t)pool; \
	__hsa_activity->hsa_args.hsa_amd_memory_lock_to_pool.flags = (uint32_t)flags; \
	__hsa_activity->hsa_args.hsa_amd_memory_lock_to_pool.agent_ptr = (void **)agent_ptr; \
};

#define GET_PTRS_VALUE_hsa_amd_memory_lock_to_pool(args) { \
	if (args->hsa_amd_memory_lock_to_pool.agents != NULL) { \
		args->hsa_amd_memory_lock_to_pool.agents__ref.val = *args->hsa_amd_memory_lock_to_pool.agents; \
	} \
	if (args->hsa_amd_memory_lock_to_pool.agent_ptr != NULL) { \
		args->hsa_amd_memory_lock_to_pool.agent_ptr__ref.ptr1 = (void*)*args->hsa_amd_memory_lock_to_pool.agent_ptr; \
	} \
};


// hsa_status_t
// hsa_system_major_extension_supported (
// 		uint16_t extension (unsigned short)
// 		uint16_t version_major (unsigned short)
// 		uint16_t * version_minor (unsigned short)
// 		_Bool * result (N/A)
// )
struct args_hsa_system_major_extension_supported_t {
	hsa_status_t retval;
	uint16_t extension;
	uint16_t version_major;
	uint16_t * version_minor;
	struct { // uint16_t *
		uint16_t val;
	} version_minor__ref;
	_Bool * result;
	struct { // _Bool *
		_Bool val;
	} result__ref;
};

#define GET_ARGS_VALUE_hsa_system_major_extension_supported(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_system_major_extension_supported.extension = (uint16_t)extension; \
	__hsa_activity->hsa_args.hsa_system_major_extension_supported.version_major = (uint16_t)version_major; \
	__hsa_activity->hsa_args.hsa_system_major_extension_supported.version_minor = (uint16_t *)version_minor; \
	__hsa_activity->hsa_args.hsa_system_major_extension_supported.result = (_Bool *)result; \
};

#define GET_PTRS_VALUE_hsa_system_major_extension_supported(args) { \
	if (args->hsa_system_major_extension_supported.version_minor != NULL) { \
		args->hsa_system_major_extension_supported.version_minor__ref.val = *args->hsa_system_major_extension_supported.version_minor; \
	} \
	if (args->hsa_system_major_extension_supported.result != NULL) { \
		args->hsa_system_major_extension_supported.result__ref.val = *args->hsa_system_major_extension_supported.result; \
	} \
};


// hsa_status_t
// hsa_amd_profiling_get_async_copy_time (
// 		hsa_signal_t signal (struct)
// 		hsa_amd_profiling_async_copy_time_t * time (struct)
// )
struct args_hsa_amd_profiling_get_async_copy_time_t {
	hsa_status_t retval;
	hsa_signal_t signal;
	hsa_amd_profiling_async_copy_time_t * time;
	struct { // hsa_amd_profiling_async_copy_time_t *
		hsa_amd_profiling_async_copy_time_t val;
	} time__ref;
};

#define GET_ARGS_VALUE_hsa_amd_profiling_get_async_copy_time(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_profiling_get_async_copy_time.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_amd_profiling_get_async_copy_time.time = (hsa_amd_profiling_async_copy_time_t *)time; \
};

#define GET_PTRS_VALUE_hsa_amd_profiling_get_async_copy_time(args) { \
	if (args->hsa_amd_profiling_get_async_copy_time.time != NULL) { \
		args->hsa_amd_profiling_get_async_copy_time.time__ref.val = *args->hsa_amd_profiling_get_async_copy_time.time; \
	} \
};


// void
// hsa_signal_or_relaxed (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_or_relaxed_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_or_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_or_relaxed.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_or_relaxed.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_or_relaxed(args)

// uint64_t
// hsa_queue_cas_write_index_scacquire (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t expected (unsigned long)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_cas_write_index_scacquire_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t expected;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_cas_write_index_scacquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_scacquire.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_scacquire.expected = (uint64_t)expected; \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_scacquire.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_cas_write_index_scacquire(args) { \
	if (args->hsa_queue_cas_write_index_scacquire.queue != NULL) { \
		args->hsa_queue_cas_write_index_scacquire.queue__ref.val = *args->hsa_queue_cas_write_index_scacquire.queue; \
	} \
};


// hsa_status_t
// hsa_code_object_get_info (
// 		hsa_code_object_t code_object (struct)
// 		hsa_code_object_info_t attribute (enum)
// 		void * value (void)
// )
struct args_hsa_code_object_get_info_t {
	hsa_status_t retval;
	hsa_code_object_t code_object;
	hsa_code_object_info_t attribute;
	void * value;
};

#define GET_ARGS_VALUE_hsa_code_object_get_info(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_code_object_get_info.code_object = (hsa_code_object_t)code_object; \
	__hsa_activity->hsa_args.hsa_code_object_get_info.attribute = (hsa_code_object_info_t)attribute; \
	__hsa_activity->hsa_args.hsa_code_object_get_info.value = (void *)value; \
};

#define GET_PTRS_VALUE_hsa_code_object_get_info(args)

// void
// hsa_signal_and_screlease (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_and_screlease_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_and_screlease(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_and_screlease.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_and_screlease.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_and_screlease(args)

// hsa_status_t
// hsa_amd_pointer_info (
// 		const void * ptr (void)
// 		hsa_amd_pointer_info_t * info (struct)
// 		void *(*)(size_t) alloc (function)
// 		uint32_t * num_agents_accessible (unsigned int)
// 		hsa_agent_t ** accessible (struct)
// )
struct args_hsa_amd_pointer_info_t {
	hsa_status_t retval;
	const void * ptr;
	hsa_amd_pointer_info_t * info;
	struct { // hsa_amd_pointer_info_t *
		hsa_amd_pointer_info_t val;
	} info__ref;
	void *(*alloc)(size_t);
	uint32_t * num_agents_accessible;
	struct { // uint32_t *
		uint32_t val;
	} num_agents_accessible__ref;
	hsa_agent_t ** accessible;
	struct { // hsa_agent_t **
		hsa_agent_t* ptr1;
		hsa_agent_t val;
	} accessible__ref;
};

#define GET_ARGS_VALUE_hsa_amd_pointer_info(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_pointer_info.ptr = (const void *)ptr; \
	__hsa_activity->hsa_args.hsa_amd_pointer_info.info = (hsa_amd_pointer_info_t *)info; \
	__hsa_activity->hsa_args.hsa_amd_pointer_info.alloc = (void *(*)(size_t))alloc; \
	__hsa_activity->hsa_args.hsa_amd_pointer_info.num_agents_accessible = (uint32_t *)num_agents_accessible; \
	__hsa_activity->hsa_args.hsa_amd_pointer_info.accessible = (hsa_agent_t **)accessible; \
};

#define GET_PTRS_VALUE_hsa_amd_pointer_info(args) { \
	if (args->hsa_amd_pointer_info.info != NULL) { \
		args->hsa_amd_pointer_info.info__ref.val = *args->hsa_amd_pointer_info.info; \
	} \
	if (args->hsa_amd_pointer_info.num_agents_accessible != NULL) { \
		args->hsa_amd_pointer_info.num_agents_accessible__ref.val = *args->hsa_amd_pointer_info.num_agents_accessible; \
	} \
	if (args->hsa_amd_pointer_info.accessible != NULL) { \
		args->hsa_amd_pointer_info.accessible__ref.ptr1 = (void*)*args->hsa_amd_pointer_info.accessible; \
		if (args->hsa_amd_pointer_info.accessible__ref.ptr1 != NULL) { \
			args->hsa_amd_pointer_info.accessible__ref.val = *args->hsa_amd_pointer_info.accessible__ref.ptr1; \
		} \
	} \
};


// hsa_status_t
// hsa_cache_get_info (
// 		hsa_cache_t cache (struct)
// 		hsa_cache_info_t attribute (enum)
// 		void * value (void)
// )
struct args_hsa_cache_get_info_t {
	hsa_status_t retval;
	hsa_cache_t cache;
	hsa_cache_info_t attribute;
	void * value;
};

#define GET_ARGS_VALUE_hsa_cache_get_info(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_cache_get_info.cache = (hsa_cache_t)cache; \
	__hsa_activity->hsa_args.hsa_cache_get_info.attribute = (hsa_cache_info_t)attribute; \
	__hsa_activity->hsa_args.hsa_cache_get_info.value = (void *)value; \
};

#define GET_PTRS_VALUE_hsa_cache_get_info(args)

// hsa_status_t
// hsa_amd_svm_attributes_get (
// 		void * ptr (void)
// 		size_t size (unsigned long)
// 		hsa_amd_svm_attribute_pair_t * attribute_list (struct)
// 		size_t attribute_count (unsigned long)
// )
struct args_hsa_amd_svm_attributes_get_t {
	hsa_status_t retval;
	void * ptr;
	size_t size;
	hsa_amd_svm_attribute_pair_t * attribute_list;
	struct { // hsa_amd_svm_attribute_pair_t *
		hsa_amd_svm_attribute_pair_t val;
	} attribute_list__ref;
	size_t attribute_count;
};

#define GET_ARGS_VALUE_hsa_amd_svm_attributes_get(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_svm_attributes_get.ptr = (void *)ptr; \
	__hsa_activity->hsa_args.hsa_amd_svm_attributes_get.size = (size_t)size; \
	__hsa_activity->hsa_args.hsa_amd_svm_attributes_get.attribute_list = (hsa_amd_svm_attribute_pair_t *)attribute_list; \
	__hsa_activity->hsa_args.hsa_amd_svm_attributes_get.attribute_count = (size_t)attribute_count; \
};

#define GET_PTRS_VALUE_hsa_amd_svm_attributes_get(args) { \
	if (args->hsa_amd_svm_attributes_get.attribute_list != NULL) { \
		args->hsa_amd_svm_attributes_get.attribute_list__ref.val = *args->hsa_amd_svm_attributes_get.attribute_list; \
	} \
};


// hsa_status_t
// hsa_signal_group_create (
// 		uint32_t num_signals (unsigned int)
// 		const hsa_signal_t * signals (struct)
// 		uint32_t num_consumers (unsigned int)
// 		const hsa_agent_t * consumers (struct)
// 		hsa_signal_group_t * signal_group (struct)
// )
struct args_hsa_signal_group_create_t {
	hsa_status_t retval;
	uint32_t num_signals;
	const hsa_signal_t * signals;
	struct { // const hsa_signal_t *
		 hsa_signal_t val;
	} signals__ref;
	uint32_t num_consumers;
	const hsa_agent_t * consumers;
	struct { // const hsa_agent_t *
		 hsa_agent_t val;
	} consumers__ref;
	hsa_signal_group_t * signal_group;
	struct { // hsa_signal_group_t *
		hsa_signal_group_t val;
	} signal_group__ref;
};

#define GET_ARGS_VALUE_hsa_signal_group_create(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_group_create.num_signals = (uint32_t)num_signals; \
	__hsa_activity->hsa_args.hsa_signal_group_create.signals = (const hsa_signal_t *)signals; \
	__hsa_activity->hsa_args.hsa_signal_group_create.num_consumers = (uint32_t)num_consumers; \
	__hsa_activity->hsa_args.hsa_signal_group_create.consumers = (const hsa_agent_t *)consumers; \
	__hsa_activity->hsa_args.hsa_signal_group_create.signal_group = (hsa_signal_group_t *)signal_group; \
};

#define GET_PTRS_VALUE_hsa_signal_group_create(args) { \
	if (args->hsa_signal_group_create.signals != NULL) { \
		args->hsa_signal_group_create.signals__ref.val = *args->hsa_signal_group_create.signals; \
	} \
	if (args->hsa_signal_group_create.consumers != NULL) { \
		args->hsa_signal_group_create.consumers__ref.val = *args->hsa_signal_group_create.consumers; \
	} \
	if (args->hsa_signal_group_create.signal_group != NULL) { \
		args->hsa_signal_group_create.signal_group__ref.val = *args->hsa_signal_group_create.signal_group; \
	} \
};


// hsa_status_t
// hsa_memory_deregister (
// 		void * ptr (void)
// 		size_t size (unsigned long)
// )
struct args_hsa_memory_deregister_t {
	hsa_status_t retval;
	void * ptr;
	size_t size;
};

#define GET_ARGS_VALUE_hsa_memory_deregister(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_memory_deregister.ptr = (void *)ptr; \
	__hsa_activity->hsa_args.hsa_memory_deregister.size = (size_t)size; \
};

#define GET_PTRS_VALUE_hsa_memory_deregister(args)

// hsa_status_t
// hsa_amd_ipc_memory_attach (
// 		const hsa_amd_ipc_memory_t * handle (struct)
// 		size_t len (unsigned long)
// 		uint32_t num_agents (unsigned int)
// 		const hsa_agent_t * mapping_agents (struct)
// 		void ** mapped_ptr (void)
// )
struct args_hsa_amd_ipc_memory_attach_t {
	hsa_status_t retval;
	const hsa_amd_ipc_memory_t * handle;
	struct { // const hsa_amd_ipc_memory_t *
		 hsa_amd_ipc_memory_t val;
	} handle__ref;
	size_t len;
	uint32_t num_agents;
	const hsa_agent_t * mapping_agents;
	struct { // const hsa_agent_t *
		 hsa_agent_t val;
	} mapping_agents__ref;
	void ** mapped_ptr;
	struct { // void **
		void* ptr1;
	} mapped_ptr__ref;
};

#define GET_ARGS_VALUE_hsa_amd_ipc_memory_attach(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_ipc_memory_attach.handle = (const hsa_amd_ipc_memory_t *)handle; \
	__hsa_activity->hsa_args.hsa_amd_ipc_memory_attach.len = (size_t)len; \
	__hsa_activity->hsa_args.hsa_amd_ipc_memory_attach.num_agents = (uint32_t)num_agents; \
	__hsa_activity->hsa_args.hsa_amd_ipc_memory_attach.mapping_agents = (const hsa_agent_t *)mapping_agents; \
	__hsa_activity->hsa_args.hsa_amd_ipc_memory_attach.mapped_ptr = (void **)mapped_ptr; \
};

#define GET_PTRS_VALUE_hsa_amd_ipc_memory_attach(args) { \
	if (args->hsa_amd_ipc_memory_attach.handle != NULL) { \
		args->hsa_amd_ipc_memory_attach.handle__ref.val = *args->hsa_amd_ipc_memory_attach.handle; \
	} \
	if (args->hsa_amd_ipc_memory_attach.mapping_agents != NULL) { \
		args->hsa_amd_ipc_memory_attach.mapping_agents__ref.val = *args->hsa_amd_ipc_memory_attach.mapping_agents; \
	} \
	if (args->hsa_amd_ipc_memory_attach.mapped_ptr != NULL) { \
		args->hsa_amd_ipc_memory_attach.mapped_ptr__ref.ptr1 = (void*)*args->hsa_amd_ipc_memory_attach.mapped_ptr; \
	} \
};


// hsa_status_t
// hsa_amd_queue_cu_get_mask (
// 		const hsa_queue_t * queue (struct)
// 		uint32_t num_cu_mask_count (unsigned int)
// 		uint32_t * cu_mask (unsigned int)
// )
struct args_hsa_amd_queue_cu_get_mask_t {
	hsa_status_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint32_t num_cu_mask_count;
	uint32_t * cu_mask;
	struct { // uint32_t *
		uint32_t val;
	} cu_mask__ref;
};

#define GET_ARGS_VALUE_hsa_amd_queue_cu_get_mask(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_queue_cu_get_mask.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_amd_queue_cu_get_mask.num_cu_mask_count = (uint32_t)num_cu_mask_count; \
	__hsa_activity->hsa_args.hsa_amd_queue_cu_get_mask.cu_mask = (uint32_t *)cu_mask; \
};

#define GET_PTRS_VALUE_hsa_amd_queue_cu_get_mask(args) { \
	if (args->hsa_amd_queue_cu_get_mask.queue != NULL) { \
		args->hsa_amd_queue_cu_get_mask.queue__ref.val = *args->hsa_amd_queue_cu_get_mask.queue; \
	} \
	if (args->hsa_amd_queue_cu_get_mask.cu_mask != NULL) { \
		args->hsa_amd_queue_cu_get_mask.cu_mask__ref.val = *args->hsa_amd_queue_cu_get_mask.cu_mask; \
	} \
};


// uint64_t
// hsa_queue_add_write_index_scacq_screl (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_add_write_index_scacq_screl_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_add_write_index_scacq_screl(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_add_write_index_scacq_screl.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_add_write_index_scacq_screl.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_add_write_index_scacq_screl(args) { \
	if (args->hsa_queue_add_write_index_scacq_screl.queue != NULL) { \
		args->hsa_queue_add_write_index_scacq_screl.queue__ref.val = *args->hsa_queue_add_write_index_scacq_screl.queue; \
	} \
};


// hsa_status_t
// hsa_agent_major_extension_supported (
// 		uint16_t extension (unsigned short)
// 		hsa_agent_t agent (struct)
// 		uint16_t version_major (unsigned short)
// 		uint16_t * version_minor (unsigned short)
// 		_Bool * result (N/A)
// )
struct args_hsa_agent_major_extension_supported_t {
	hsa_status_t retval;
	uint16_t extension;
	hsa_agent_t agent;
	uint16_t version_major;
	uint16_t * version_minor;
	struct { // uint16_t *
		uint16_t val;
	} version_minor__ref;
	_Bool * result;
	struct { // _Bool *
		_Bool val;
	} result__ref;
};

#define GET_ARGS_VALUE_hsa_agent_major_extension_supported(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_agent_major_extension_supported.extension = (uint16_t)extension; \
	__hsa_activity->hsa_args.hsa_agent_major_extension_supported.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_agent_major_extension_supported.version_major = (uint16_t)version_major; \
	__hsa_activity->hsa_args.hsa_agent_major_extension_supported.version_minor = (uint16_t *)version_minor; \
	__hsa_activity->hsa_args.hsa_agent_major_extension_supported.result = (_Bool *)result; \
};

#define GET_PTRS_VALUE_hsa_agent_major_extension_supported(args) { \
	if (args->hsa_agent_major_extension_supported.version_minor != NULL) { \
		args->hsa_agent_major_extension_supported.version_minor__ref.val = *args->hsa_agent_major_extension_supported.version_minor; \
	} \
	if (args->hsa_agent_major_extension_supported.result != NULL) { \
		args->hsa_agent_major_extension_supported.result__ref.val = *args->hsa_agent_major_extension_supported.result; \
	} \
};


// hsa_status_t
// hsa_ext_sampler_destroy (
// 		hsa_agent_t agent (struct)
// 		hsa_ext_sampler_t sampler (struct)
// )
struct args_hsa_ext_sampler_destroy_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_ext_sampler_t sampler;
};

#define GET_ARGS_VALUE_hsa_ext_sampler_destroy(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_ext_sampler_destroy.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_ext_sampler_destroy.sampler = (hsa_ext_sampler_t)sampler; \
};

#define GET_PTRS_VALUE_hsa_ext_sampler_destroy(args)

// hsa_status_t
// hsa_queue_create (
// 		hsa_agent_t agent (struct)
// 		uint32_t size (unsigned int)
// 		hsa_queue_type32_t type (unsigned int)
// 		void (*)(hsa_status_t, hsa_queue_t *, void *) callback (function)
// 		void * data (void)
// 		uint32_t private_segment_size (unsigned int)
// 		uint32_t group_segment_size (unsigned int)
// 		hsa_queue_t ** queue (struct)
// )
struct args_hsa_queue_create_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	uint32_t size;
	hsa_queue_type32_t type;
	void (*callback)(hsa_status_t, hsa_queue_t *, void *);
	void * data;
	uint32_t private_segment_size;
	uint32_t group_segment_size;
	hsa_queue_t ** queue;
	struct { // hsa_queue_t **
		hsa_queue_t* ptr1;
		hsa_queue_t val;
	} queue__ref;
};

#define GET_ARGS_VALUE_hsa_queue_create(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_create.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_queue_create.size = (uint32_t)size; \
	__hsa_activity->hsa_args.hsa_queue_create.type = (hsa_queue_type32_t)type; \
	__hsa_activity->hsa_args.hsa_queue_create.callback = (void (*)(hsa_status_t, hsa_queue_t *, void *))callback; \
	__hsa_activity->hsa_args.hsa_queue_create.data = (void *)data; \
	__hsa_activity->hsa_args.hsa_queue_create.private_segment_size = (uint32_t)private_segment_size; \
	__hsa_activity->hsa_args.hsa_queue_create.group_segment_size = (uint32_t)group_segment_size; \
	__hsa_activity->hsa_args.hsa_queue_create.queue = (hsa_queue_t **)queue; \
};

#define GET_PTRS_VALUE_hsa_queue_create(args) { \
	if (args->hsa_queue_create.queue != NULL) { \
		args->hsa_queue_create.queue__ref.ptr1 = (void*)*args->hsa_queue_create.queue; \
		if (args->hsa_queue_create.queue__ref.ptr1 != NULL) { \
			args->hsa_queue_create.queue__ref.val = *args->hsa_queue_create.queue__ref.ptr1; \
		} \
	} \
};


// hsa_status_t
// hsa_amd_vmem_get_alloc_properties_from_handle (
// 		hsa_amd_vmem_alloc_handle_t memory_handle (struct)
// 		hsa_amd_memory_pool_t * pool (struct)
// 		hsa_amd_memory_type_t * type (enum)
// )
struct args_hsa_amd_vmem_get_alloc_properties_from_handle_t {
	hsa_status_t retval;
	hsa_amd_vmem_alloc_handle_t memory_handle;
	hsa_amd_memory_pool_t * pool;
	struct { // hsa_amd_memory_pool_t *
		hsa_amd_memory_pool_t val;
	} pool__ref;
	hsa_amd_memory_type_t * type;
	struct { // hsa_amd_memory_type_t *
		hsa_amd_memory_type_t val;
	} type__ref;
};

#define GET_ARGS_VALUE_hsa_amd_vmem_get_alloc_properties_from_handle(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_vmem_get_alloc_properties_from_handle.memory_handle = (hsa_amd_vmem_alloc_handle_t)memory_handle; \
	__hsa_activity->hsa_args.hsa_amd_vmem_get_alloc_properties_from_handle.pool = (hsa_amd_memory_pool_t *)pool; \
	__hsa_activity->hsa_args.hsa_amd_vmem_get_alloc_properties_from_handle.type = (hsa_amd_memory_type_t *)type; \
};

#define GET_PTRS_VALUE_hsa_amd_vmem_get_alloc_properties_from_handle(args) { \
	if (args->hsa_amd_vmem_get_alloc_properties_from_handle.pool != NULL) { \
		args->hsa_amd_vmem_get_alloc_properties_from_handle.pool__ref.val = *args->hsa_amd_vmem_get_alloc_properties_from_handle.pool; \
	} \
	if (args->hsa_amd_vmem_get_alloc_properties_from_handle.type != NULL) { \
		args->hsa_amd_vmem_get_alloc_properties_from_handle.type__ref.val = *args->hsa_amd_vmem_get_alloc_properties_from_handle.type; \
	} \
};


// hsa_status_t
// hsa_amd_memory_pool_allocate (
// 		hsa_amd_memory_pool_t memory_pool (struct)
// 		size_t size (unsigned long)
// 		uint32_t flags (unsigned int)
// 		void ** ptr (void)
// )
struct args_hsa_amd_memory_pool_allocate_t {
	hsa_status_t retval;
	hsa_amd_memory_pool_t memory_pool;
	size_t size;
	uint32_t flags;
	void ** ptr;
	struct { // void **
		void* ptr1;
	} ptr__ref;
};

#define GET_ARGS_VALUE_hsa_amd_memory_pool_allocate(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_memory_pool_allocate.memory_pool = (hsa_amd_memory_pool_t)memory_pool; \
	__hsa_activity->hsa_args.hsa_amd_memory_pool_allocate.size = (size_t)size; \
	__hsa_activity->hsa_args.hsa_amd_memory_pool_allocate.flags = (uint32_t)flags; \
	__hsa_activity->hsa_args.hsa_amd_memory_pool_allocate.ptr = (void **)ptr; \
};

#define GET_PTRS_VALUE_hsa_amd_memory_pool_allocate(args) { \
	if (args->hsa_amd_memory_pool_allocate.ptr != NULL) { \
		args->hsa_amd_memory_pool_allocate.ptr__ref.ptr1 = (void*)*args->hsa_amd_memory_pool_allocate.ptr; \
	} \
};


// void
// hsa_signal_or_release (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_or_release_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_or_release(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_or_release.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_or_release.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_or_release(args)

// uint64_t
// hsa_queue_cas_write_index_acq_rel (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t expected (unsigned long)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_cas_write_index_acq_rel_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t expected;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_cas_write_index_acq_rel(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_acq_rel.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_acq_rel.expected = (uint64_t)expected; \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_acq_rel.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_cas_write_index_acq_rel(args) { \
	if (args->hsa_queue_cas_write_index_acq_rel.queue != NULL) { \
		args->hsa_queue_cas_write_index_acq_rel.queue__ref.val = *args->hsa_queue_cas_write_index_acq_rel.queue; \
	} \
};


// hsa_status_t
// hsa_code_object_serialize (
// 		hsa_code_object_t code_object (struct)
// 		hsa_status_t (*)(size_t, hsa_callback_data_t, void **) alloc_callback (function)
// 		hsa_callback_data_t callback_data (struct)
// 		const char * options (string)
// 		void ** serialized_code_object (void)
// 		size_t * serialized_code_object_size (unsigned long)
// )
struct args_hsa_code_object_serialize_t {
	hsa_status_t retval;
	hsa_code_object_t code_object;
	hsa_status_t (*alloc_callback)(size_t, hsa_callback_data_t, void **);
	hsa_callback_data_t callback_data;
	const char * options;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} options__ref;
	void ** serialized_code_object;
	struct { // void **
		void* ptr1;
	} serialized_code_object__ref;
	size_t * serialized_code_object_size;
	struct { // size_t *
		size_t val;
	} serialized_code_object_size__ref;
};

#define GET_ARGS_VALUE_hsa_code_object_serialize(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_code_object_serialize.code_object = (hsa_code_object_t)code_object; \
	__hsa_activity->hsa_args.hsa_code_object_serialize.alloc_callback = (hsa_status_t (*)(size_t, hsa_callback_data_t, void **))alloc_callback; \
	__hsa_activity->hsa_args.hsa_code_object_serialize.callback_data = (hsa_callback_data_t)callback_data; \
	__hsa_activity->hsa_args.hsa_code_object_serialize.options = (const char *)options; \
	__hsa_activity->hsa_args.hsa_code_object_serialize.serialized_code_object = (void **)serialized_code_object; \
	__hsa_activity->hsa_args.hsa_code_object_serialize.serialized_code_object_size = (size_t *)serialized_code_object_size; \
};

#define GET_PTRS_VALUE_hsa_code_object_serialize(args) { \
	if (args->hsa_code_object_serialize.options != NULL) { \
		strncpy(args->hsa_code_object_serialize.options__ref.val, args->hsa_code_object_serialize.options, HSA_STRING_SIZE_MAX-1); \
	} \
	if (args->hsa_code_object_serialize.serialized_code_object != NULL) { \
		args->hsa_code_object_serialize.serialized_code_object__ref.ptr1 = (void*)*args->hsa_code_object_serialize.serialized_code_object; \
	} \
	if (args->hsa_code_object_serialize.serialized_code_object_size != NULL) { \
		args->hsa_code_object_serialize.serialized_code_object_size__ref.val = *args->hsa_code_object_serialize.serialized_code_object_size; \
	} \
};


// void
// hsa_signal_xor_acquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_xor_acquire_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_xor_acquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_xor_acquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_xor_acquire.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_xor_acquire(args)

// hsa_status_t
// hsa_amd_profiling_convert_tick_to_system_domain (
// 		hsa_agent_t agent (struct)
// 		uint64_t agent_tick (unsigned long)
// 		uint64_t * system_tick (unsigned long)
// )
struct args_hsa_amd_profiling_convert_tick_to_system_domain_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	uint64_t agent_tick;
	uint64_t * system_tick;
	struct { // uint64_t *
		uint64_t val;
	} system_tick__ref;
};

#define GET_ARGS_VALUE_hsa_amd_profiling_convert_tick_to_system_domain(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_profiling_convert_tick_to_system_domain.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_amd_profiling_convert_tick_to_system_domain.agent_tick = (uint64_t)agent_tick; \
	__hsa_activity->hsa_args.hsa_amd_profiling_convert_tick_to_system_domain.system_tick = (uint64_t *)system_tick; \
};

#define GET_PTRS_VALUE_hsa_amd_profiling_convert_tick_to_system_domain(args) { \
	if (args->hsa_amd_profiling_convert_tick_to_system_domain.system_tick != NULL) { \
		args->hsa_amd_profiling_convert_tick_to_system_domain.system_tick__ref.val = *args->hsa_amd_profiling_convert_tick_to_system_domain.system_tick; \
	} \
};


// hsa_status_t
// hsa_executable_destroy (
// 		hsa_executable_t executable (struct)
// )
struct args_hsa_executable_destroy_t {
	hsa_status_t retval;
	hsa_executable_t executable;
};

#define GET_ARGS_VALUE_hsa_executable_destroy(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_destroy.executable = (hsa_executable_t)executable; \
};

#define GET_PTRS_VALUE_hsa_executable_destroy(args)

// void
// hsa_signal_add_screlease (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_add_screlease_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_add_screlease(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_add_screlease.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_add_screlease.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_add_screlease(args)

// hsa_signal_value_t
// hsa_signal_load_acquire (
// 		hsa_signal_t signal (struct)
// )
struct args_hsa_signal_load_acquire_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
};

#define GET_ARGS_VALUE_hsa_signal_load_acquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_load_acquire.signal = (hsa_signal_t)signal; \
};

#define GET_PTRS_VALUE_hsa_signal_load_acquire(args)

// hsa_status_t
// hsa_executable_create_alt (
// 		hsa_profile_t profile (enum)
// 		hsa_default_float_rounding_mode_t default_float_rounding_mode (enum)
// 		const char * options (string)
// 		hsa_executable_t * executable (struct)
// )
struct args_hsa_executable_create_alt_t {
	hsa_status_t retval;
	hsa_profile_t profile;
	hsa_default_float_rounding_mode_t default_float_rounding_mode;
	const char * options;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} options__ref;
	hsa_executable_t * executable;
	struct { // hsa_executable_t *
		hsa_executable_t val;
	} executable__ref;
};

#define GET_ARGS_VALUE_hsa_executable_create_alt(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_create_alt.profile = (hsa_profile_t)profile; \
	__hsa_activity->hsa_args.hsa_executable_create_alt.default_float_rounding_mode = (hsa_default_float_rounding_mode_t)default_float_rounding_mode; \
	__hsa_activity->hsa_args.hsa_executable_create_alt.options = (const char *)options; \
	__hsa_activity->hsa_args.hsa_executable_create_alt.executable = (hsa_executable_t *)executable; \
};

#define GET_PTRS_VALUE_hsa_executable_create_alt(args) { \
	if (args->hsa_executable_create_alt.options != NULL) { \
		strncpy(args->hsa_executable_create_alt.options__ref.val, args->hsa_executable_create_alt.options, HSA_STRING_SIZE_MAX-1); \
	} \
	if (args->hsa_executable_create_alt.executable != NULL) { \
		args->hsa_executable_create_alt.executable__ref.val = *args->hsa_executable_create_alt.executable; \
	} \
};


// hsa_status_t
// hsa_amd_memory_async_copy_rect (
// 		const hsa_pitched_ptr_t * dst (struct)
// 		const hsa_dim3_t * dst_offset (struct)
// 		const hsa_pitched_ptr_t * src (struct)
// 		const hsa_dim3_t * src_offset (struct)
// 		const hsa_dim3_t * range (struct)
// 		hsa_agent_t copy_agent (struct)
// 		hsa_amd_copy_direction_t dir (enum)
// 		uint32_t num_dep_signals (unsigned int)
// 		const hsa_signal_t * dep_signals (struct)
// 		hsa_signal_t completion_signal (struct)
// )
struct args_hsa_amd_memory_async_copy_rect_t {
	hsa_status_t retval;
	const hsa_pitched_ptr_t * dst;
	struct { // const hsa_pitched_ptr_t *
		 hsa_pitched_ptr_t val;
	} dst__ref;
	const hsa_dim3_t * dst_offset;
	struct { // const hsa_dim3_t *
		 hsa_dim3_t val;
	} dst_offset__ref;
	const hsa_pitched_ptr_t * src;
	struct { // const hsa_pitched_ptr_t *
		 hsa_pitched_ptr_t val;
	} src__ref;
	const hsa_dim3_t * src_offset;
	struct { // const hsa_dim3_t *
		 hsa_dim3_t val;
	} src_offset__ref;
	const hsa_dim3_t * range;
	struct { // const hsa_dim3_t *
		 hsa_dim3_t val;
	} range__ref;
	hsa_agent_t copy_agent;
	hsa_amd_copy_direction_t dir;
	uint32_t num_dep_signals;
	const hsa_signal_t * dep_signals;
	struct { // const hsa_signal_t *
		 hsa_signal_t val;
	} dep_signals__ref;
	hsa_signal_t completion_signal;
};

#define GET_ARGS_VALUE_hsa_amd_memory_async_copy_rect(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_rect.dst = (const hsa_pitched_ptr_t *)dst; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_rect.dst_offset = (const hsa_dim3_t *)dst_offset; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_rect.src = (const hsa_pitched_ptr_t *)src; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_rect.src_offset = (const hsa_dim3_t *)src_offset; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_rect.range = (const hsa_dim3_t *)range; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_rect.copy_agent = (hsa_agent_t)copy_agent; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_rect.dir = (hsa_amd_copy_direction_t)dir; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_rect.num_dep_signals = (uint32_t)num_dep_signals; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_rect.dep_signals = (const hsa_signal_t *)dep_signals; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_rect.completion_signal = (hsa_signal_t)completion_signal; \
};

#define GET_PTRS_VALUE_hsa_amd_memory_async_copy_rect(args) { \
	if (args->hsa_amd_memory_async_copy_rect.dst != NULL) { \
		args->hsa_amd_memory_async_copy_rect.dst__ref.val = *args->hsa_amd_memory_async_copy_rect.dst; \
	} \
	if (args->hsa_amd_memory_async_copy_rect.dst_offset != NULL) { \
		args->hsa_amd_memory_async_copy_rect.dst_offset__ref.val = *args->hsa_amd_memory_async_copy_rect.dst_offset; \
	} \
	if (args->hsa_amd_memory_async_copy_rect.src != NULL) { \
		args->hsa_amd_memory_async_copy_rect.src__ref.val = *args->hsa_amd_memory_async_copy_rect.src; \
	} \
	if (args->hsa_amd_memory_async_copy_rect.src_offset != NULL) { \
		args->hsa_amd_memory_async_copy_rect.src_offset__ref.val = *args->hsa_amd_memory_async_copy_rect.src_offset; \
	} \
	if (args->hsa_amd_memory_async_copy_rect.range != NULL) { \
		args->hsa_amd_memory_async_copy_rect.range__ref.val = *args->hsa_amd_memory_async_copy_rect.range; \
	} \
	if (args->hsa_amd_memory_async_copy_rect.dep_signals != NULL) { \
		args->hsa_amd_memory_async_copy_rect.dep_signals__ref.val = *args->hsa_amd_memory_async_copy_rect.dep_signals; \
	} \
};


// hsa_signal_value_t
// hsa_signal_load_scacquire (
// 		hsa_signal_t signal (struct)
// )
struct args_hsa_signal_load_scacquire_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
};

#define GET_ARGS_VALUE_hsa_signal_load_scacquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_load_scacquire.signal = (hsa_signal_t)signal; \
};

#define GET_PTRS_VALUE_hsa_signal_load_scacquire(args)

// hsa_status_t
// hsa_agent_iterate_caches (
// 		hsa_agent_t agent (struct)
// 		hsa_status_t (*)(hsa_cache_t, void *) callback (function)
// 		void * data (void)
// )
struct args_hsa_agent_iterate_caches_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_status_t (*callback)(hsa_cache_t, void *);
	void * data;
};

#define GET_ARGS_VALUE_hsa_agent_iterate_caches(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_agent_iterate_caches.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_agent_iterate_caches.callback = (hsa_status_t (*)(hsa_cache_t, void *))callback; \
	__hsa_activity->hsa_args.hsa_agent_iterate_caches.data = (void *)data; \
};

#define GET_PTRS_VALUE_hsa_agent_iterate_caches(args)

// hsa_signal_value_t
// hsa_signal_cas_acq_rel (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t expected (long)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_cas_acq_rel_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_value_t expected;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_cas_acq_rel(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_cas_acq_rel.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_cas_acq_rel.expected = (hsa_signal_value_t)expected; \
	__hsa_activity->hsa_args.hsa_signal_cas_acq_rel.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_cas_acq_rel(args)

// uint64_t
// hsa_queue_load_read_index_relaxed (
// 		const hsa_queue_t * queue (struct)
// )
struct args_hsa_queue_load_read_index_relaxed_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
};

#define GET_ARGS_VALUE_hsa_queue_load_read_index_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_load_read_index_relaxed.queue = (const hsa_queue_t *)queue; \
};

#define GET_PTRS_VALUE_hsa_queue_load_read_index_relaxed(args) { \
	if (args->hsa_queue_load_read_index_relaxed.queue != NULL) { \
		args->hsa_queue_load_read_index_relaxed.queue__ref.val = *args->hsa_queue_load_read_index_relaxed.queue; \
	} \
};


// hsa_status_t
// hsa_agent_iterate_isas (
// 		hsa_agent_t agent (struct)
// 		hsa_status_t (*)(hsa_isa_t, void *) callback (function)
// 		void * data (void)
// )
struct args_hsa_agent_iterate_isas_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_status_t (*callback)(hsa_isa_t, void *);
	void * data;
};

#define GET_ARGS_VALUE_hsa_agent_iterate_isas(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_agent_iterate_isas.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_agent_iterate_isas.callback = (hsa_status_t (*)(hsa_isa_t, void *))callback; \
	__hsa_activity->hsa_args.hsa_agent_iterate_isas.data = (void *)data; \
};

#define GET_PTRS_VALUE_hsa_agent_iterate_isas(args)

// void
// hsa_signal_xor_relaxed (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_xor_relaxed_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_xor_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_xor_relaxed.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_xor_relaxed.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_xor_relaxed(args)

// void
// hsa_signal_subtract_acq_rel (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_subtract_acq_rel_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_subtract_acq_rel(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_subtract_acq_rel.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_subtract_acq_rel.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_subtract_acq_rel(args)

// uint64_t
// hsa_queue_load_read_index_acquire (
// 		const hsa_queue_t * queue (struct)
// )
struct args_hsa_queue_load_read_index_acquire_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
};

#define GET_ARGS_VALUE_hsa_queue_load_read_index_acquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_load_read_index_acquire.queue = (const hsa_queue_t *)queue; \
};

#define GET_PTRS_VALUE_hsa_queue_load_read_index_acquire(args) { \
	if (args->hsa_queue_load_read_index_acquire.queue != NULL) { \
		args->hsa_queue_load_read_index_acquire.queue__ref.val = *args->hsa_queue_load_read_index_acquire.queue; \
	} \
};


// hsa_status_t
// hsa_isa_get_info_alt (
// 		hsa_isa_t isa (struct)
// 		hsa_isa_info_t attribute (enum)
// 		void * value (void)
// )
struct args_hsa_isa_get_info_alt_t {
	hsa_status_t retval;
	hsa_isa_t isa;
	hsa_isa_info_t attribute;
	void * value;
};

#define GET_ARGS_VALUE_hsa_isa_get_info_alt(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_isa_get_info_alt.isa = (hsa_isa_t)isa; \
	__hsa_activity->hsa_args.hsa_isa_get_info_alt.attribute = (hsa_isa_info_t)attribute; \
	__hsa_activity->hsa_args.hsa_isa_get_info_alt.value = (void *)value; \
};

#define GET_PTRS_VALUE_hsa_isa_get_info_alt(args)

// void
// hsa_signal_and_scacquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_and_scacquire_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_and_scacquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_and_scacquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_and_scacquire.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_and_scacquire(args)

// uint64_t
// hsa_queue_add_write_index_release (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_add_write_index_release_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_add_write_index_release(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_add_write_index_release.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_add_write_index_release.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_add_write_index_release(args) { \
	if (args->hsa_queue_add_write_index_release.queue != NULL) { \
		args->hsa_queue_add_write_index_release.queue__ref.val = *args->hsa_queue_add_write_index_release.queue; \
	} \
};


// hsa_status_t
// hsa_system_get_info (
// 		hsa_system_info_t attribute (enum)
// 		void * value (void)
// )
struct args_hsa_system_get_info_t {
	hsa_status_t retval;
	hsa_system_info_t attribute;
	void * value;
};

#define GET_ARGS_VALUE_hsa_system_get_info(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_system_get_info.attribute = (hsa_system_info_t)attribute; \
	__hsa_activity->hsa_args.hsa_system_get_info.value = (void *)value; \
};

#define GET_PTRS_VALUE_hsa_system_get_info(args)

// hsa_status_t
// hsa_memory_register (
// 		void * ptr (void)
// 		size_t size (unsigned long)
// )
struct args_hsa_memory_register_t {
	hsa_status_t retval;
	void * ptr;
	size_t size;
};

#define GET_ARGS_VALUE_hsa_memory_register(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_memory_register.ptr = (void *)ptr; \
	__hsa_activity->hsa_args.hsa_memory_register.size = (size_t)size; \
};

#define GET_PTRS_VALUE_hsa_memory_register(args)

// hsa_status_t
// hsa_isa_get_round_method (
// 		hsa_isa_t isa (struct)
// 		hsa_fp_type_t fp_type (enum)
// 		hsa_flush_mode_t flush_mode (enum)
// 		hsa_round_method_t * round_method (enum)
// )
struct args_hsa_isa_get_round_method_t {
	hsa_status_t retval;
	hsa_isa_t isa;
	hsa_fp_type_t fp_type;
	hsa_flush_mode_t flush_mode;
	hsa_round_method_t * round_method;
	struct { // hsa_round_method_t *
		hsa_round_method_t val;
	} round_method__ref;
};

#define GET_ARGS_VALUE_hsa_isa_get_round_method(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_isa_get_round_method.isa = (hsa_isa_t)isa; \
	__hsa_activity->hsa_args.hsa_isa_get_round_method.fp_type = (hsa_fp_type_t)fp_type; \
	__hsa_activity->hsa_args.hsa_isa_get_round_method.flush_mode = (hsa_flush_mode_t)flush_mode; \
	__hsa_activity->hsa_args.hsa_isa_get_round_method.round_method = (hsa_round_method_t *)round_method; \
};

#define GET_PTRS_VALUE_hsa_isa_get_round_method(args) { \
	if (args->hsa_isa_get_round_method.round_method != NULL) { \
		args->hsa_isa_get_round_method.round_method__ref.val = *args->hsa_isa_get_round_method.round_method; \
	} \
};


// hsa_status_t
// hsa_isa_iterate_wavefronts (
// 		hsa_isa_t isa (struct)
// 		hsa_status_t (*)(hsa_wavefront_t, void *) callback (function)
// 		void * data (void)
// )
struct args_hsa_isa_iterate_wavefronts_t {
	hsa_status_t retval;
	hsa_isa_t isa;
	hsa_status_t (*callback)(hsa_wavefront_t, void *);
	void * data;
};

#define GET_ARGS_VALUE_hsa_isa_iterate_wavefronts(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_isa_iterate_wavefronts.isa = (hsa_isa_t)isa; \
	__hsa_activity->hsa_args.hsa_isa_iterate_wavefronts.callback = (hsa_status_t (*)(hsa_wavefront_t, void *))callback; \
	__hsa_activity->hsa_args.hsa_isa_iterate_wavefronts.data = (void *)data; \
};

#define GET_PTRS_VALUE_hsa_isa_iterate_wavefronts(args)

// hsa_status_t
// hsa_amd_vmem_unmap (
// 		void * va (void)
// 		size_t size (unsigned long)
// )
struct args_hsa_amd_vmem_unmap_t {
	hsa_status_t retval;
	void * va;
	size_t size;
};

#define GET_ARGS_VALUE_hsa_amd_vmem_unmap(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_vmem_unmap.va = (void *)va; \
	__hsa_activity->hsa_args.hsa_amd_vmem_unmap.size = (size_t)size; \
};

#define GET_PTRS_VALUE_hsa_amd_vmem_unmap(args)

// void
// hsa_signal_subtract_acquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_subtract_acquire_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_subtract_acquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_subtract_acquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_subtract_acquire.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_subtract_acquire(args)

// hsa_status_t
// hsa_system_get_major_extension_table (
// 		uint16_t extension (unsigned short)
// 		uint16_t version_major (unsigned short)
// 		size_t table_length (unsigned long)
// 		void * table (void)
// )
struct args_hsa_system_get_major_extension_table_t {
	hsa_status_t retval;
	uint16_t extension;
	uint16_t version_major;
	size_t table_length;
	void * table;
};

#define GET_ARGS_VALUE_hsa_system_get_major_extension_table(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_system_get_major_extension_table.extension = (uint16_t)extension; \
	__hsa_activity->hsa_args.hsa_system_get_major_extension_table.version_major = (uint16_t)version_major; \
	__hsa_activity->hsa_args.hsa_system_get_major_extension_table.table_length = (size_t)table_length; \
	__hsa_activity->hsa_args.hsa_system_get_major_extension_table.table = (void *)table; \
};

#define GET_PTRS_VALUE_hsa_system_get_major_extension_table(args)

// void
// hsa_signal_store_release (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_store_release_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_store_release(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_store_release.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_store_release.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_store_release(args)

// uint64_t
// hsa_queue_load_write_index_scacquire (
// 		const hsa_queue_t * queue (struct)
// )
struct args_hsa_queue_load_write_index_scacquire_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
};

#define GET_ARGS_VALUE_hsa_queue_load_write_index_scacquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_load_write_index_scacquire.queue = (const hsa_queue_t *)queue; \
};

#define GET_PTRS_VALUE_hsa_queue_load_write_index_scacquire(args) { \
	if (args->hsa_queue_load_write_index_scacquire.queue != NULL) { \
		args->hsa_queue_load_write_index_scacquire.queue__ref.val = *args->hsa_queue_load_write_index_scacquire.queue; \
	} \
};


// hsa_status_t
// hsa_amd_spm_release (
// 		hsa_agent_t preferred_agent (struct)
// )
struct args_hsa_amd_spm_release_t {
	hsa_status_t retval;
	hsa_agent_t preferred_agent;
};

#define GET_ARGS_VALUE_hsa_amd_spm_release(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_spm_release.preferred_agent = (hsa_agent_t)preferred_agent; \
};

#define GET_PTRS_VALUE_hsa_amd_spm_release(args)

// hsa_status_t
// hsa_code_symbol_get_info (
// 		hsa_code_symbol_t code_symbol (struct)
// 		hsa_code_symbol_info_t attribute (enum)
// 		void * value (void)
// )
struct args_hsa_code_symbol_get_info_t {
	hsa_status_t retval;
	hsa_code_symbol_t code_symbol;
	hsa_code_symbol_info_t attribute;
	void * value;
};

#define GET_ARGS_VALUE_hsa_code_symbol_get_info(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_code_symbol_get_info.code_symbol = (hsa_code_symbol_t)code_symbol; \
	__hsa_activity->hsa_args.hsa_code_symbol_get_info.attribute = (hsa_code_symbol_info_t)attribute; \
	__hsa_activity->hsa_args.hsa_code_symbol_get_info.value = (void *)value; \
};

#define GET_PTRS_VALUE_hsa_code_symbol_get_info(args)

// hsa_signal_value_t
// hsa_signal_wait_acquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_condition_t condition (enum)
// 		hsa_signal_value_t compare_value (long)
// 		uint64_t timeout_hint (unsigned long)
// 		hsa_wait_state_t wait_state_hint (enum)
// )
struct args_hsa_signal_wait_acquire_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_condition_t condition;
	hsa_signal_value_t compare_value;
	uint64_t timeout_hint;
	hsa_wait_state_t wait_state_hint;
};

#define GET_ARGS_VALUE_hsa_signal_wait_acquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_wait_acquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_wait_acquire.condition = (hsa_signal_condition_t)condition; \
	__hsa_activity->hsa_args.hsa_signal_wait_acquire.compare_value = (hsa_signal_value_t)compare_value; \
	__hsa_activity->hsa_args.hsa_signal_wait_acquire.timeout_hint = (uint64_t)timeout_hint; \
	__hsa_activity->hsa_args.hsa_signal_wait_acquire.wait_state_hint = (hsa_wait_state_t)wait_state_hint; \
};

#define GET_PTRS_VALUE_hsa_signal_wait_acquire(args)

// hsa_signal_value_t
// hsa_signal_cas_release (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t expected (long)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_cas_release_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_value_t expected;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_cas_release(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_cas_release.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_cas_release.expected = (hsa_signal_value_t)expected; \
	__hsa_activity->hsa_args.hsa_signal_cas_release.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_cas_release(args)

// hsa_status_t
// hsa_amd_memory_pool_get_info (
// 		hsa_amd_memory_pool_t memory_pool (struct)
// 		hsa_amd_memory_pool_info_t attribute (enum)
// 		void * value (void)
// )
struct args_hsa_amd_memory_pool_get_info_t {
	hsa_status_t retval;
	hsa_amd_memory_pool_t memory_pool;
	hsa_amd_memory_pool_info_t attribute;
	void * value;
};

#define GET_ARGS_VALUE_hsa_amd_memory_pool_get_info(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_memory_pool_get_info.memory_pool = (hsa_amd_memory_pool_t)memory_pool; \
	__hsa_activity->hsa_args.hsa_amd_memory_pool_get_info.attribute = (hsa_amd_memory_pool_info_t)attribute; \
	__hsa_activity->hsa_args.hsa_amd_memory_pool_get_info.value = (void *)value; \
};

#define GET_PTRS_VALUE_hsa_amd_memory_pool_get_info(args)

// hsa_status_t
// hsa_amd_pointer_info_set_userdata (
// 		const void * ptr (void)
// 		void * userdata (void)
// )
struct args_hsa_amd_pointer_info_set_userdata_t {
	hsa_status_t retval;
	const void * ptr;
	void * userdata;
};

#define GET_ARGS_VALUE_hsa_amd_pointer_info_set_userdata(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_pointer_info_set_userdata.ptr = (const void *)ptr; \
	__hsa_activity->hsa_args.hsa_amd_pointer_info_set_userdata.userdata = (void *)userdata; \
};

#define GET_PTRS_VALUE_hsa_amd_pointer_info_set_userdata(args)

// hsa_status_t
// hsa_amd_vmem_get_access (
// 		void * va (void)
// 		hsa_access_permission_t * perms (enum)
// 		hsa_agent_t agent_handle (struct)
// )
struct args_hsa_amd_vmem_get_access_t {
	hsa_status_t retval;
	void * va;
	hsa_access_permission_t * perms;
	struct { // hsa_access_permission_t *
		hsa_access_permission_t val;
	} perms__ref;
	hsa_agent_t agent_handle;
};

#define GET_ARGS_VALUE_hsa_amd_vmem_get_access(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_vmem_get_access.va = (void *)va; \
	__hsa_activity->hsa_args.hsa_amd_vmem_get_access.perms = (hsa_access_permission_t *)perms; \
	__hsa_activity->hsa_args.hsa_amd_vmem_get_access.agent_handle = (hsa_agent_t)agent_handle; \
};

#define GET_PTRS_VALUE_hsa_amd_vmem_get_access(args) { \
	if (args->hsa_amd_vmem_get_access.perms != NULL) { \
		args->hsa_amd_vmem_get_access.perms__ref.val = *args->hsa_amd_vmem_get_access.perms; \
	} \
};


// hsa_status_t
// hsa_iterate_agents (
// 		hsa_status_t (*)(hsa_agent_t, void *) callback (function)
// 		void * data (void)
// )
struct args_hsa_iterate_agents_t {
	hsa_status_t retval;
	hsa_status_t (*callback)(hsa_agent_t, void *);
	void * data;
};

#define GET_ARGS_VALUE_hsa_iterate_agents(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_iterate_agents.callback = (hsa_status_t (*)(hsa_agent_t, void *))callback; \
	__hsa_activity->hsa_args.hsa_iterate_agents.data = (void *)data; \
};

#define GET_PTRS_VALUE_hsa_iterate_agents(args)

// void
// hsa_queue_store_write_index_screlease (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_store_write_index_screlease_t {
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_store_write_index_screlease(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_store_write_index_screlease.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_store_write_index_screlease.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_store_write_index_screlease(args) { \
	if (args->hsa_queue_store_write_index_screlease.queue != NULL) { \
		args->hsa_queue_store_write_index_screlease.queue__ref.val = *args->hsa_queue_store_write_index_screlease.queue; \
	} \
};


// hsa_status_t
// hsa_agent_iterate_regions (
// 		hsa_agent_t agent (struct)
// 		hsa_status_t (*)(hsa_region_t, void *) callback (function)
// 		void * data (void)
// )
struct args_hsa_agent_iterate_regions_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_status_t (*callback)(hsa_region_t, void *);
	void * data;
};

#define GET_ARGS_VALUE_hsa_agent_iterate_regions(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_agent_iterate_regions.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_agent_iterate_regions.callback = (hsa_status_t (*)(hsa_region_t, void *))callback; \
	__hsa_activity->hsa_args.hsa_agent_iterate_regions.data = (void *)data; \
};

#define GET_PTRS_VALUE_hsa_agent_iterate_regions(args)

// hsa_status_t
// hsa_executable_readonly_variable_define (
// 		hsa_executable_t executable (struct)
// 		hsa_agent_t agent (struct)
// 		const char * variable_name (string)
// 		void * address (void)
// )
struct args_hsa_executable_readonly_variable_define_t {
	hsa_status_t retval;
	hsa_executable_t executable;
	hsa_agent_t agent;
	const char * variable_name;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} variable_name__ref;
	void * address;
};

#define GET_ARGS_VALUE_hsa_executable_readonly_variable_define(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_readonly_variable_define.executable = (hsa_executable_t)executable; \
	__hsa_activity->hsa_args.hsa_executable_readonly_variable_define.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_executable_readonly_variable_define.variable_name = (const char *)variable_name; \
	__hsa_activity->hsa_args.hsa_executable_readonly_variable_define.address = (void *)address; \
};

#define GET_PTRS_VALUE_hsa_executable_readonly_variable_define(args) { \
	if (args->hsa_executable_readonly_variable_define.variable_name != NULL) { \
		strncpy(args->hsa_executable_readonly_variable_define.variable_name__ref.val, args->hsa_executable_readonly_variable_define.variable_name, HSA_STRING_SIZE_MAX-1); \
	} \
};


// hsa_status_t
// hsa_executable_iterate_symbols (
// 		hsa_executable_t executable (struct)
// 		hsa_status_t (*)(hsa_executable_t, hsa_executable_symbol_t, void *) callback (function)
// 		void * data (void)
// )
struct args_hsa_executable_iterate_symbols_t {
	hsa_status_t retval;
	hsa_executable_t executable;
	hsa_status_t (*callback)(hsa_executable_t, hsa_executable_symbol_t, void *);
	void * data;
};

#define GET_ARGS_VALUE_hsa_executable_iterate_symbols(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_iterate_symbols.executable = (hsa_executable_t)executable; \
	__hsa_activity->hsa_args.hsa_executable_iterate_symbols.callback = (hsa_status_t (*)(hsa_executable_t, hsa_executable_symbol_t, void *))callback; \
	__hsa_activity->hsa_args.hsa_executable_iterate_symbols.data = (void *)data; \
};

#define GET_PTRS_VALUE_hsa_executable_iterate_symbols(args)

// hsa_status_t
// hsa_amd_vmem_handle_release (
// 		hsa_amd_vmem_alloc_handle_t memory_handle (struct)
// )
struct args_hsa_amd_vmem_handle_release_t {
	hsa_status_t retval;
	hsa_amd_vmem_alloc_handle_t memory_handle;
};

#define GET_ARGS_VALUE_hsa_amd_vmem_handle_release(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_vmem_handle_release.memory_handle = (hsa_amd_vmem_alloc_handle_t)memory_handle; \
};

#define GET_PTRS_VALUE_hsa_amd_vmem_handle_release(args)

// hsa_status_t
// hsa_amd_memory_pool_can_migrate (
// 		hsa_amd_memory_pool_t src_memory_pool (struct)
// 		hsa_amd_memory_pool_t dst_memory_pool (struct)
// 		_Bool * result (N/A)
// )
struct args_hsa_amd_memory_pool_can_migrate_t {
	hsa_status_t retval;
	hsa_amd_memory_pool_t src_memory_pool;
	hsa_amd_memory_pool_t dst_memory_pool;
	_Bool * result;
	struct { // _Bool *
		_Bool val;
	} result__ref;
};

#define GET_ARGS_VALUE_hsa_amd_memory_pool_can_migrate(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_memory_pool_can_migrate.src_memory_pool = (hsa_amd_memory_pool_t)src_memory_pool; \
	__hsa_activity->hsa_args.hsa_amd_memory_pool_can_migrate.dst_memory_pool = (hsa_amd_memory_pool_t)dst_memory_pool; \
	__hsa_activity->hsa_args.hsa_amd_memory_pool_can_migrate.result = (_Bool *)result; \
};

#define GET_PTRS_VALUE_hsa_amd_memory_pool_can_migrate(args) { \
	if (args->hsa_amd_memory_pool_can_migrate.result != NULL) { \
		args->hsa_amd_memory_pool_can_migrate.result__ref.val = *args->hsa_amd_memory_pool_can_migrate.result; \
	} \
};


// void
// hsa_queue_store_write_index_relaxed (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_store_write_index_relaxed_t {
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_store_write_index_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_store_write_index_relaxed.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_store_write_index_relaxed.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_store_write_index_relaxed(args) { \
	if (args->hsa_queue_store_write_index_relaxed.queue != NULL) { \
		args->hsa_queue_store_write_index_relaxed.queue__ref.val = *args->hsa_queue_store_write_index_relaxed.queue; \
	} \
};


// hsa_status_t
// hsa_amd_image_get_info_max_dim (
// 		hsa_agent_t agent (struct)
// 		hsa_agent_info_t attribute (enum)
// 		void * value (void)
// )
struct args_hsa_amd_image_get_info_max_dim_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_agent_info_t attribute;
	void * value;
};

#define GET_ARGS_VALUE_hsa_amd_image_get_info_max_dim(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_image_get_info_max_dim.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_amd_image_get_info_max_dim.attribute = (hsa_agent_info_t)attribute; \
	__hsa_activity->hsa_args.hsa_amd_image_get_info_max_dim.value = (void *)value; \
};

#define GET_PTRS_VALUE_hsa_amd_image_get_info_max_dim(args)

// hsa_status_t
// hsa_amd_spm_set_dest_buffer (
// 		hsa_agent_t preferred_agent (struct)
// 		size_t size_in_bytes (unsigned long)
// 		uint32_t * timeout (unsigned int)
// 		uint32_t * size_copied (unsigned int)
// 		void * dest (void)
// 		_Bool * is_data_loss (N/A)
// )
struct args_hsa_amd_spm_set_dest_buffer_t {
	hsa_status_t retval;
	hsa_agent_t preferred_agent;
	size_t size_in_bytes;
	uint32_t * timeout;
	struct { // uint32_t *
		uint32_t val;
	} timeout__ref;
	uint32_t * size_copied;
	struct { // uint32_t *
		uint32_t val;
	} size_copied__ref;
	void * dest;
	_Bool * is_data_loss;
	struct { // _Bool *
		_Bool val;
	} is_data_loss__ref;
};

#define GET_ARGS_VALUE_hsa_amd_spm_set_dest_buffer(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_spm_set_dest_buffer.preferred_agent = (hsa_agent_t)preferred_agent; \
	__hsa_activity->hsa_args.hsa_amd_spm_set_dest_buffer.size_in_bytes = (size_t)size_in_bytes; \
	__hsa_activity->hsa_args.hsa_amd_spm_set_dest_buffer.timeout = (uint32_t *)timeout; \
	__hsa_activity->hsa_args.hsa_amd_spm_set_dest_buffer.size_copied = (uint32_t *)size_copied; \
	__hsa_activity->hsa_args.hsa_amd_spm_set_dest_buffer.dest = (void *)dest; \
	__hsa_activity->hsa_args.hsa_amd_spm_set_dest_buffer.is_data_loss = (_Bool *)is_data_loss; \
};

#define GET_PTRS_VALUE_hsa_amd_spm_set_dest_buffer(args) { \
	if (args->hsa_amd_spm_set_dest_buffer.timeout != NULL) { \
		args->hsa_amd_spm_set_dest_buffer.timeout__ref.val = *args->hsa_amd_spm_set_dest_buffer.timeout; \
	} \
	if (args->hsa_amd_spm_set_dest_buffer.size_copied != NULL) { \
		args->hsa_amd_spm_set_dest_buffer.size_copied__ref.val = *args->hsa_amd_spm_set_dest_buffer.size_copied; \
	} \
	if (args->hsa_amd_spm_set_dest_buffer.is_data_loss != NULL) { \
		args->hsa_amd_spm_set_dest_buffer.is_data_loss__ref.val = *args->hsa_amd_spm_set_dest_buffer.is_data_loss; \
	} \
};


// hsa_signal_value_t
// hsa_signal_wait_relaxed (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_condition_t condition (enum)
// 		hsa_signal_value_t compare_value (long)
// 		uint64_t timeout_hint (unsigned long)
// 		hsa_wait_state_t wait_state_hint (enum)
// )
struct args_hsa_signal_wait_relaxed_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_condition_t condition;
	hsa_signal_value_t compare_value;
	uint64_t timeout_hint;
	hsa_wait_state_t wait_state_hint;
};

#define GET_ARGS_VALUE_hsa_signal_wait_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_wait_relaxed.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_wait_relaxed.condition = (hsa_signal_condition_t)condition; \
	__hsa_activity->hsa_args.hsa_signal_wait_relaxed.compare_value = (hsa_signal_value_t)compare_value; \
	__hsa_activity->hsa_args.hsa_signal_wait_relaxed.timeout_hint = (uint64_t)timeout_hint; \
	__hsa_activity->hsa_args.hsa_signal_wait_relaxed.wait_state_hint = (hsa_wait_state_t)wait_state_hint; \
};

#define GET_PTRS_VALUE_hsa_signal_wait_relaxed(args)

// hsa_status_t
// hsa_amd_portable_export_dmabuf (
// 		const void * ptr (void)
// 		size_t size (unsigned long)
// 		int * dmabuf (int)
// 		uint64_t * offset (unsigned long)
// )
struct args_hsa_amd_portable_export_dmabuf_t {
	hsa_status_t retval;
	const void * ptr;
	size_t size;
	int * dmabuf;
	struct { // int *
		int val;
	} dmabuf__ref;
	uint64_t * offset;
	struct { // uint64_t *
		uint64_t val;
	} offset__ref;
};

#define GET_ARGS_VALUE_hsa_amd_portable_export_dmabuf(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_portable_export_dmabuf.ptr = (const void *)ptr; \
	__hsa_activity->hsa_args.hsa_amd_portable_export_dmabuf.size = (size_t)size; \
	__hsa_activity->hsa_args.hsa_amd_portable_export_dmabuf.dmabuf = (int *)dmabuf; \
	__hsa_activity->hsa_args.hsa_amd_portable_export_dmabuf.offset = (uint64_t *)offset; \
};

#define GET_PTRS_VALUE_hsa_amd_portable_export_dmabuf(args) { \
	if (args->hsa_amd_portable_export_dmabuf.dmabuf != NULL) { \
		args->hsa_amd_portable_export_dmabuf.dmabuf__ref.val = *args->hsa_amd_portable_export_dmabuf.dmabuf; \
	} \
	if (args->hsa_amd_portable_export_dmabuf.offset != NULL) { \
		args->hsa_amd_portable_export_dmabuf.offset__ref.val = *args->hsa_amd_portable_export_dmabuf.offset; \
	} \
};


// void
// hsa_signal_store_screlease (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_store_screlease_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_store_screlease(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_store_screlease.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_store_screlease.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_store_screlease(args)

// void
// hsa_signal_or_acquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_or_acquire_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_or_acquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_or_acquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_or_acquire.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_or_acquire(args)

// hsa_status_t
// hsa_code_object_destroy (
// 		hsa_code_object_t code_object (struct)
// )
struct args_hsa_code_object_destroy_t {
	hsa_status_t retval;
	hsa_code_object_t code_object;
};

#define GET_ARGS_VALUE_hsa_code_object_destroy(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_code_object_destroy.code_object = (hsa_code_object_t)code_object; \
};

#define GET_PTRS_VALUE_hsa_code_object_destroy(args)

// void
// hsa_signal_subtract_screlease (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_subtract_screlease_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_subtract_screlease(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_subtract_screlease.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_subtract_screlease.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_subtract_screlease(args)

// hsa_status_t
// hsa_region_get_info (
// 		hsa_region_t region (struct)
// 		hsa_region_info_t attribute (enum)
// 		void * value (void)
// )
struct args_hsa_region_get_info_t {
	hsa_status_t retval;
	hsa_region_t region;
	hsa_region_info_t attribute;
	void * value;
};

#define GET_ARGS_VALUE_hsa_region_get_info(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_region_get_info.region = (hsa_region_t)region; \
	__hsa_activity->hsa_args.hsa_region_get_info.attribute = (hsa_region_info_t)attribute; \
	__hsa_activity->hsa_args.hsa_region_get_info.value = (void *)value; \
};

#define GET_PTRS_VALUE_hsa_region_get_info(args)

// void
// hsa_signal_subtract_scacq_screl (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_subtract_scacq_screl_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_subtract_scacq_screl(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_subtract_scacq_screl.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_subtract_scacq_screl.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_subtract_scacq_screl(args)

// hsa_status_t
// hsa_amd_profiling_async_copy_enable (
// 		_Bool enable (N/A)
// )
struct args_hsa_amd_profiling_async_copy_enable_t {
	hsa_status_t retval;
	_Bool enable;
};

#define GET_ARGS_VALUE_hsa_amd_profiling_async_copy_enable(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_profiling_async_copy_enable.enable = (_Bool)enable; \
};

#define GET_PTRS_VALUE_hsa_amd_profiling_async_copy_enable(args)

// hsa_status_t
// hsa_soft_queue_create (
// 		hsa_region_t region (struct)
// 		uint32_t size (unsigned int)
// 		hsa_queue_type32_t type (unsigned int)
// 		uint32_t features (unsigned int)
// 		hsa_signal_t doorbell_signal (struct)
// 		hsa_queue_t ** queue (struct)
// )
struct args_hsa_soft_queue_create_t {
	hsa_status_t retval;
	hsa_region_t region;
	uint32_t size;
	hsa_queue_type32_t type;
	uint32_t features;
	hsa_signal_t doorbell_signal;
	hsa_queue_t ** queue;
	struct { // hsa_queue_t **
		hsa_queue_t* ptr1;
		hsa_queue_t val;
	} queue__ref;
};

#define GET_ARGS_VALUE_hsa_soft_queue_create(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_soft_queue_create.region = (hsa_region_t)region; \
	__hsa_activity->hsa_args.hsa_soft_queue_create.size = (uint32_t)size; \
	__hsa_activity->hsa_args.hsa_soft_queue_create.type = (hsa_queue_type32_t)type; \
	__hsa_activity->hsa_args.hsa_soft_queue_create.features = (uint32_t)features; \
	__hsa_activity->hsa_args.hsa_soft_queue_create.doorbell_signal = (hsa_signal_t)doorbell_signal; \
	__hsa_activity->hsa_args.hsa_soft_queue_create.queue = (hsa_queue_t **)queue; \
};

#define GET_PTRS_VALUE_hsa_soft_queue_create(args) { \
	if (args->hsa_soft_queue_create.queue != NULL) { \
		args->hsa_soft_queue_create.queue__ref.ptr1 = (void*)*args->hsa_soft_queue_create.queue; \
		if (args->hsa_soft_queue_create.queue__ref.ptr1 != NULL) { \
			args->hsa_soft_queue_create.queue__ref.val = *args->hsa_soft_queue_create.queue__ref.ptr1; \
		} \
	} \
};


// hsa_status_t
// hsa_shut_down (
// )
struct args_hsa_shut_down_t {
	hsa_status_t retval;

};

#define GET_ARGS_VALUE_hsa_shut_down(__hsa_activity)
#define GET_PTRS_VALUE_hsa_shut_down(args)

// hsa_status_t
// hsa_code_object_iterate_symbols (
// 		hsa_code_object_t code_object (struct)
// 		hsa_status_t (*)(hsa_code_object_t, hsa_code_symbol_t, void *) callback (function)
// 		void * data (void)
// )
struct args_hsa_code_object_iterate_symbols_t {
	hsa_status_t retval;
	hsa_code_object_t code_object;
	hsa_status_t (*callback)(hsa_code_object_t, hsa_code_symbol_t, void *);
	void * data;
};

#define GET_ARGS_VALUE_hsa_code_object_iterate_symbols(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_code_object_iterate_symbols.code_object = (hsa_code_object_t)code_object; \
	__hsa_activity->hsa_args.hsa_code_object_iterate_symbols.callback = (hsa_status_t (*)(hsa_code_object_t, hsa_code_symbol_t, void *))callback; \
	__hsa_activity->hsa_args.hsa_code_object_iterate_symbols.data = (void *)data; \
};

#define GET_PTRS_VALUE_hsa_code_object_iterate_symbols(args)

// hsa_status_t
// hsa_extension_get_name (
// 		uint16_t extension (unsigned short)
// 		const char ** name (string)
// )
struct args_hsa_extension_get_name_t {
	hsa_status_t retval;
	uint16_t extension;
	const char ** name;
	struct { // const char **
		 char* ptr1;
		char val[HSA_STRING_SIZE_MAX];
	} name__ref;
};

#define GET_ARGS_VALUE_hsa_extension_get_name(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_extension_get_name.extension = (uint16_t)extension; \
	__hsa_activity->hsa_args.hsa_extension_get_name.name = (const char **)name; \
};

#define GET_PTRS_VALUE_hsa_extension_get_name(args) { \
	if (args->hsa_extension_get_name.name != NULL) { \
		args->hsa_extension_get_name.name__ref.ptr1 = (void*)*args->hsa_extension_get_name.name; \
		if (args->hsa_extension_get_name.name__ref.ptr1 != NULL) { \
			strncpy(args->hsa_extension_get_name.name__ref.val, args->hsa_extension_get_name.name__ref.ptr1, HSA_STRING_SIZE_MAX-1); \
		} \
	} \
};


// hsa_signal_value_t
// hsa_signal_exchange_relaxed (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_exchange_relaxed_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_exchange_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_exchange_relaxed.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_exchange_relaxed.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_exchange_relaxed(args)

// hsa_status_t
// hsa_executable_get_symbol_by_name (
// 		hsa_executable_t executable (struct)
// 		const char * symbol_name (string)
// 		const hsa_agent_t * agent (struct)
// 		hsa_executable_symbol_t * symbol (struct)
// )
struct args_hsa_executable_get_symbol_by_name_t {
	hsa_status_t retval;
	hsa_executable_t executable;
	const char * symbol_name;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} symbol_name__ref;
	const hsa_agent_t * agent;
	struct { // const hsa_agent_t *
		 hsa_agent_t val;
	} agent__ref;
	hsa_executable_symbol_t * symbol;
	struct { // hsa_executable_symbol_t *
		hsa_executable_symbol_t val;
	} symbol__ref;
};

#define GET_ARGS_VALUE_hsa_executable_get_symbol_by_name(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_get_symbol_by_name.executable = (hsa_executable_t)executable; \
	__hsa_activity->hsa_args.hsa_executable_get_symbol_by_name.symbol_name = (const char *)symbol_name; \
	__hsa_activity->hsa_args.hsa_executable_get_symbol_by_name.agent = (const hsa_agent_t *)agent; \
	__hsa_activity->hsa_args.hsa_executable_get_symbol_by_name.symbol = (hsa_executable_symbol_t *)symbol; \
};

#define GET_PTRS_VALUE_hsa_executable_get_symbol_by_name(args) { \
	if (args->hsa_executable_get_symbol_by_name.symbol_name != NULL) { \
		strncpy(args->hsa_executable_get_symbol_by_name.symbol_name__ref.val, args->hsa_executable_get_symbol_by_name.symbol_name, HSA_STRING_SIZE_MAX-1); \
	} \
	if (args->hsa_executable_get_symbol_by_name.agent != NULL) { \
		args->hsa_executable_get_symbol_by_name.agent__ref.val = *args->hsa_executable_get_symbol_by_name.agent; \
	} \
	if (args->hsa_executable_get_symbol_by_name.symbol != NULL) { \
		args->hsa_executable_get_symbol_by_name.symbol__ref.val = *args->hsa_executable_get_symbol_by_name.symbol; \
	} \
};


// hsa_status_t
// hsa_ext_image_create_with_layout (
// 		hsa_agent_t agent (struct)
// 		const hsa_ext_image_descriptor_t * image_descriptor (struct)
// 		const void * image_data (void)
// 		hsa_access_permission_t access_permission (enum)
// 		hsa_ext_image_data_layout_t image_data_layout (enum)
// 		size_t image_data_row_pitch (unsigned long)
// 		size_t image_data_slice_pitch (unsigned long)
// 		hsa_ext_image_t * image (struct)
// )
struct args_hsa_ext_image_create_with_layout_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	const hsa_ext_image_descriptor_t * image_descriptor;
	struct { // const hsa_ext_image_descriptor_t *
		 hsa_ext_image_descriptor_t val;
	} image_descriptor__ref;
	const void * image_data;
	hsa_access_permission_t access_permission;
	hsa_ext_image_data_layout_t image_data_layout;
	size_t image_data_row_pitch;
	size_t image_data_slice_pitch;
	hsa_ext_image_t * image;
	struct { // hsa_ext_image_t *
		hsa_ext_image_t val;
	} image__ref;
};

#define GET_ARGS_VALUE_hsa_ext_image_create_with_layout(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_ext_image_create_with_layout.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_ext_image_create_with_layout.image_descriptor = (const hsa_ext_image_descriptor_t *)image_descriptor; \
	__hsa_activity->hsa_args.hsa_ext_image_create_with_layout.image_data = (const void *)image_data; \
	__hsa_activity->hsa_args.hsa_ext_image_create_with_layout.access_permission = (hsa_access_permission_t)access_permission; \
	__hsa_activity->hsa_args.hsa_ext_image_create_with_layout.image_data_layout = (hsa_ext_image_data_layout_t)image_data_layout; \
	__hsa_activity->hsa_args.hsa_ext_image_create_with_layout.image_data_row_pitch = (size_t)image_data_row_pitch; \
	__hsa_activity->hsa_args.hsa_ext_image_create_with_layout.image_data_slice_pitch = (size_t)image_data_slice_pitch; \
	__hsa_activity->hsa_args.hsa_ext_image_create_with_layout.image = (hsa_ext_image_t *)image; \
};

#define GET_PTRS_VALUE_hsa_ext_image_create_with_layout(args) { \
	if (args->hsa_ext_image_create_with_layout.image_descriptor != NULL) { \
		args->hsa_ext_image_create_with_layout.image_descriptor__ref.val = *args->hsa_ext_image_create_with_layout.image_descriptor; \
	} \
	if (args->hsa_ext_image_create_with_layout.image != NULL) { \
		args->hsa_ext_image_create_with_layout.image__ref.val = *args->hsa_ext_image_create_with_layout.image; \
	} \
};


// uint64_t
// hsa_queue_load_write_index_relaxed (
// 		const hsa_queue_t * queue (struct)
// )
struct args_hsa_queue_load_write_index_relaxed_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
};

#define GET_ARGS_VALUE_hsa_queue_load_write_index_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_load_write_index_relaxed.queue = (const hsa_queue_t *)queue; \
};

#define GET_PTRS_VALUE_hsa_queue_load_write_index_relaxed(args) { \
	if (args->hsa_queue_load_write_index_relaxed.queue != NULL) { \
		args->hsa_queue_load_write_index_relaxed.queue__ref.val = *args->hsa_queue_load_write_index_relaxed.queue; \
	} \
};


// hsa_status_t
// hsa_ext_image_copy (
// 		hsa_agent_t agent (struct)
// 		hsa_ext_image_t src_image (struct)
// 		const hsa_dim3_t * src_offset (struct)
// 		hsa_ext_image_t dst_image (struct)
// 		const hsa_dim3_t * dst_offset (struct)
// 		const hsa_dim3_t * range (struct)
// )
struct args_hsa_ext_image_copy_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_ext_image_t src_image;
	const hsa_dim3_t * src_offset;
	struct { // const hsa_dim3_t *
		 hsa_dim3_t val;
	} src_offset__ref;
	hsa_ext_image_t dst_image;
	const hsa_dim3_t * dst_offset;
	struct { // const hsa_dim3_t *
		 hsa_dim3_t val;
	} dst_offset__ref;
	const hsa_dim3_t * range;
	struct { // const hsa_dim3_t *
		 hsa_dim3_t val;
	} range__ref;
};

#define GET_ARGS_VALUE_hsa_ext_image_copy(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_ext_image_copy.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_ext_image_copy.src_image = (hsa_ext_image_t)src_image; \
	__hsa_activity->hsa_args.hsa_ext_image_copy.src_offset = (const hsa_dim3_t *)src_offset; \
	__hsa_activity->hsa_args.hsa_ext_image_copy.dst_image = (hsa_ext_image_t)dst_image; \
	__hsa_activity->hsa_args.hsa_ext_image_copy.dst_offset = (const hsa_dim3_t *)dst_offset; \
	__hsa_activity->hsa_args.hsa_ext_image_copy.range = (const hsa_dim3_t *)range; \
};

#define GET_PTRS_VALUE_hsa_ext_image_copy(args) { \
	if (args->hsa_ext_image_copy.src_offset != NULL) { \
		args->hsa_ext_image_copy.src_offset__ref.val = *args->hsa_ext_image_copy.src_offset; \
	} \
	if (args->hsa_ext_image_copy.dst_offset != NULL) { \
		args->hsa_ext_image_copy.dst_offset__ref.val = *args->hsa_ext_image_copy.dst_offset; \
	} \
	if (args->hsa_ext_image_copy.range != NULL) { \
		args->hsa_ext_image_copy.range__ref.val = *args->hsa_ext_image_copy.range; \
	} \
};


// hsa_status_t
// hsa_code_object_reader_create_from_memory (
// 		const void * code_object (void)
// 		size_t size (unsigned long)
// 		hsa_code_object_reader_t * code_object_reader (struct)
// )
struct args_hsa_code_object_reader_create_from_memory_t {
	hsa_status_t retval;
	const void * code_object;
	size_t size;
	hsa_code_object_reader_t * code_object_reader;
	struct { // hsa_code_object_reader_t *
		hsa_code_object_reader_t val;
	} code_object_reader__ref;
};

#define GET_ARGS_VALUE_hsa_code_object_reader_create_from_memory(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_code_object_reader_create_from_memory.code_object = (const void *)code_object; \
	__hsa_activity->hsa_args.hsa_code_object_reader_create_from_memory.size = (size_t)size; \
	__hsa_activity->hsa_args.hsa_code_object_reader_create_from_memory.code_object_reader = (hsa_code_object_reader_t *)code_object_reader; \
};

#define GET_PTRS_VALUE_hsa_code_object_reader_create_from_memory(args) { \
	if (args->hsa_code_object_reader_create_from_memory.code_object_reader != NULL) { \
		args->hsa_code_object_reader_create_from_memory.code_object_reader__ref.val = *args->hsa_code_object_reader_create_from_memory.code_object_reader; \
	} \
};


// void
// hsa_signal_xor_release (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_xor_release_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_xor_release(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_xor_release.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_xor_release.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_xor_release(args)

// hsa_status_t
// hsa_amd_register_deallocation_callback (
// 		void * ptr (void)
// 		hsa_amd_deallocation_callback_t callback (function)
// 		void * user_data (void)
// )
struct args_hsa_amd_register_deallocation_callback_t {
	hsa_status_t retval;
	void * ptr;
	hsa_amd_deallocation_callback_t callback;
	void * user_data;
};

#define GET_ARGS_VALUE_hsa_amd_register_deallocation_callback(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_register_deallocation_callback.ptr = (void *)ptr; \
	__hsa_activity->hsa_args.hsa_amd_register_deallocation_callback.callback = (hsa_amd_deallocation_callback_t)callback; \
	__hsa_activity->hsa_args.hsa_amd_register_deallocation_callback.user_data = (void *)user_data; \
};

#define GET_PTRS_VALUE_hsa_amd_register_deallocation_callback(args)

// hsa_status_t
// hsa_signal_group_destroy (
// 		hsa_signal_group_t signal_group (struct)
// )
struct args_hsa_signal_group_destroy_t {
	hsa_status_t retval;
	hsa_signal_group_t signal_group;
};

#define GET_ARGS_VALUE_hsa_signal_group_destroy(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_group_destroy.signal_group = (hsa_signal_group_t)signal_group; \
};

#define GET_PTRS_VALUE_hsa_signal_group_destroy(args)

// hsa_status_t
// hsa_amd_vmem_export_shareable_handle (
// 		int * dmabuf_fd (int)
// 		hsa_amd_vmem_alloc_handle_t handle (struct)
// 		uint64_t flags (unsigned long)
// )
struct args_hsa_amd_vmem_export_shareable_handle_t {
	hsa_status_t retval;
	int * dmabuf_fd;
	struct { // int *
		int val;
	} dmabuf_fd__ref;
	hsa_amd_vmem_alloc_handle_t handle;
	uint64_t flags;
};

#define GET_ARGS_VALUE_hsa_amd_vmem_export_shareable_handle(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_vmem_export_shareable_handle.dmabuf_fd = (int *)dmabuf_fd; \
	__hsa_activity->hsa_args.hsa_amd_vmem_export_shareable_handle.handle = (hsa_amd_vmem_alloc_handle_t)handle; \
	__hsa_activity->hsa_args.hsa_amd_vmem_export_shareable_handle.flags = (uint64_t)flags; \
};

#define GET_PTRS_VALUE_hsa_amd_vmem_export_shareable_handle(args) { \
	if (args->hsa_amd_vmem_export_shareable_handle.dmabuf_fd != NULL) { \
		args->hsa_amd_vmem_export_shareable_handle.dmabuf_fd__ref.val = *args->hsa_amd_vmem_export_shareable_handle.dmabuf_fd; \
	} \
};


// uint64_t
// hsa_queue_add_write_index_acq_rel (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_add_write_index_acq_rel_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_add_write_index_acq_rel(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_add_write_index_acq_rel.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_add_write_index_acq_rel.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_add_write_index_acq_rel(args) { \
	if (args->hsa_queue_add_write_index_acq_rel.queue != NULL) { \
		args->hsa_queue_add_write_index_acq_rel.queue__ref.val = *args->hsa_queue_add_write_index_acq_rel.queue; \
	} \
};


// hsa_status_t
// hsa_signal_destroy (
// 		hsa_signal_t signal (struct)
// )
struct args_hsa_signal_destroy_t {
	hsa_status_t retval;
	hsa_signal_t signal;
};

#define GET_ARGS_VALUE_hsa_signal_destroy(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_destroy.signal = (hsa_signal_t)signal; \
};

#define GET_PTRS_VALUE_hsa_signal_destroy(args)

// hsa_status_t
// hsa_agent_extension_supported (
// 		uint16_t extension (unsigned short)
// 		hsa_agent_t agent (struct)
// 		uint16_t version_major (unsigned short)
// 		uint16_t version_minor (unsigned short)
// 		_Bool * result (N/A)
// )
struct args_hsa_agent_extension_supported_t {
	hsa_status_t retval;
	uint16_t extension;
	hsa_agent_t agent;
	uint16_t version_major;
	uint16_t version_minor;
	_Bool * result;
	struct { // _Bool *
		_Bool val;
	} result__ref;
};

#define GET_ARGS_VALUE_hsa_agent_extension_supported(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_agent_extension_supported.extension = (uint16_t)extension; \
	__hsa_activity->hsa_args.hsa_agent_extension_supported.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_agent_extension_supported.version_major = (uint16_t)version_major; \
	__hsa_activity->hsa_args.hsa_agent_extension_supported.version_minor = (uint16_t)version_minor; \
	__hsa_activity->hsa_args.hsa_agent_extension_supported.result = (_Bool *)result; \
};

#define GET_PTRS_VALUE_hsa_agent_extension_supported(args) { \
	if (args->hsa_agent_extension_supported.result != NULL) { \
		args->hsa_agent_extension_supported.result__ref.val = *args->hsa_agent_extension_supported.result; \
	} \
};


// hsa_status_t
// hsa_amd_image_create (
// 		hsa_agent_t agent (struct)
// 		const hsa_ext_image_descriptor_t * image_descriptor (struct)
// 		const hsa_amd_image_descriptor_t * image_layout (struct)
// 		const void * image_data (void)
// 		hsa_access_permission_t access_permission (enum)
// 		hsa_ext_image_t * image (struct)
// )
struct args_hsa_amd_image_create_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	const hsa_ext_image_descriptor_t * image_descriptor;
	struct { // const hsa_ext_image_descriptor_t *
		 hsa_ext_image_descriptor_t val;
	} image_descriptor__ref;
	const hsa_amd_image_descriptor_t * image_layout;
	struct { // const hsa_amd_image_descriptor_t *
		 hsa_amd_image_descriptor_t val;
	} image_layout__ref;
	const void * image_data;
	hsa_access_permission_t access_permission;
	hsa_ext_image_t * image;
	struct { // hsa_ext_image_t *
		hsa_ext_image_t val;
	} image__ref;
};

#define GET_ARGS_VALUE_hsa_amd_image_create(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_image_create.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_amd_image_create.image_descriptor = (const hsa_ext_image_descriptor_t *)image_descriptor; \
	__hsa_activity->hsa_args.hsa_amd_image_create.image_layout = (const hsa_amd_image_descriptor_t *)image_layout; \
	__hsa_activity->hsa_args.hsa_amd_image_create.image_data = (const void *)image_data; \
	__hsa_activity->hsa_args.hsa_amd_image_create.access_permission = (hsa_access_permission_t)access_permission; \
	__hsa_activity->hsa_args.hsa_amd_image_create.image = (hsa_ext_image_t *)image; \
};

#define GET_PTRS_VALUE_hsa_amd_image_create(args) { \
	if (args->hsa_amd_image_create.image_descriptor != NULL) { \
		args->hsa_amd_image_create.image_descriptor__ref.val = *args->hsa_amd_image_create.image_descriptor; \
	} \
	if (args->hsa_amd_image_create.image_layout != NULL) { \
		args->hsa_amd_image_create.image_layout__ref.val = *args->hsa_amd_image_create.image_layout; \
	} \
	if (args->hsa_amd_image_create.image != NULL) { \
		args->hsa_amd_image_create.image__ref.val = *args->hsa_amd_image_create.image; \
	} \
};


// hsa_status_t
// hsa_isa_compatible (
// 		hsa_isa_t code_object_isa (struct)
// 		hsa_isa_t agent_isa (struct)
// 		_Bool * result (N/A)
// )
struct args_hsa_isa_compatible_t {
	hsa_status_t retval;
	hsa_isa_t code_object_isa;
	hsa_isa_t agent_isa;
	_Bool * result;
	struct { // _Bool *
		_Bool val;
	} result__ref;
};

#define GET_ARGS_VALUE_hsa_isa_compatible(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_isa_compatible.code_object_isa = (hsa_isa_t)code_object_isa; \
	__hsa_activity->hsa_args.hsa_isa_compatible.agent_isa = (hsa_isa_t)agent_isa; \
	__hsa_activity->hsa_args.hsa_isa_compatible.result = (_Bool *)result; \
};

#define GET_PTRS_VALUE_hsa_isa_compatible(args) { \
	if (args->hsa_isa_compatible.result != NULL) { \
		args->hsa_isa_compatible.result__ref.val = *args->hsa_isa_compatible.result; \
	} \
};


// hsa_status_t
// hsa_amd_queue_set_priority (
// 		hsa_queue_t * queue (struct)
// 		hsa_amd_queue_priority_t priority (enum)
// )
struct args_hsa_amd_queue_set_priority_t {
	hsa_status_t retval;
	hsa_queue_t * queue;
	struct { // hsa_queue_t *
		hsa_queue_t val;
	} queue__ref;
	hsa_amd_queue_priority_t priority;
};

#define GET_ARGS_VALUE_hsa_amd_queue_set_priority(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_queue_set_priority.queue = (hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_amd_queue_set_priority.priority = (hsa_amd_queue_priority_t)priority; \
};

#define GET_PTRS_VALUE_hsa_amd_queue_set_priority(args) { \
	if (args->hsa_amd_queue_set_priority.queue != NULL) { \
		args->hsa_amd_queue_set_priority.queue__ref.val = *args->hsa_amd_queue_set_priority.queue; \
	} \
};


// void
// hsa_signal_and_acq_rel (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_and_acq_rel_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_and_acq_rel(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_and_acq_rel.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_and_acq_rel.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_and_acq_rel(args)

// hsa_status_t
// hsa_code_object_reader_destroy (
// 		hsa_code_object_reader_t code_object_reader (struct)
// )
struct args_hsa_code_object_reader_destroy_t {
	hsa_status_t retval;
	hsa_code_object_reader_t code_object_reader;
};

#define GET_ARGS_VALUE_hsa_code_object_reader_destroy(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_code_object_reader_destroy.code_object_reader = (hsa_code_object_reader_t)code_object_reader; \
};

#define GET_PTRS_VALUE_hsa_code_object_reader_destroy(args)

// uint64_t
// hsa_queue_cas_write_index_release (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t expected (unsigned long)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_cas_write_index_release_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t expected;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_cas_write_index_release(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_release.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_release.expected = (uint64_t)expected; \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_release.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_cas_write_index_release(args) { \
	if (args->hsa_queue_cas_write_index_release.queue != NULL) { \
		args->hsa_queue_cas_write_index_release.queue__ref.val = *args->hsa_queue_cas_write_index_release.queue; \
	} \
};


// hsa_status_t
// hsa_executable_load_code_object (
// 		hsa_executable_t executable (struct)
// 		hsa_agent_t agent (struct)
// 		hsa_code_object_t code_object (struct)
// 		const char * options (string)
// )
struct args_hsa_executable_load_code_object_t {
	hsa_status_t retval;
	hsa_executable_t executable;
	hsa_agent_t agent;
	hsa_code_object_t code_object;
	const char * options;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} options__ref;
};

#define GET_ARGS_VALUE_hsa_executable_load_code_object(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_load_code_object.executable = (hsa_executable_t)executable; \
	__hsa_activity->hsa_args.hsa_executable_load_code_object.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_executable_load_code_object.code_object = (hsa_code_object_t)code_object; \
	__hsa_activity->hsa_args.hsa_executable_load_code_object.options = (const char *)options; \
};

#define GET_PTRS_VALUE_hsa_executable_load_code_object(args) { \
	if (args->hsa_executable_load_code_object.options != NULL) { \
		strncpy(args->hsa_executable_load_code_object.options__ref.val, args->hsa_executable_load_code_object.options, HSA_STRING_SIZE_MAX-1); \
	} \
};


// hsa_status_t
// hsa_amd_memory_async_copy (
// 		void * dst (void)
// 		hsa_agent_t dst_agent (struct)
// 		const void * src (void)
// 		hsa_agent_t src_agent (struct)
// 		size_t size (unsigned long)
// 		uint32_t num_dep_signals (unsigned int)
// 		const hsa_signal_t * dep_signals (struct)
// 		hsa_signal_t completion_signal (struct)
// )
struct args_hsa_amd_memory_async_copy_t {
	hsa_status_t retval;
	void * dst;
	hsa_agent_t dst_agent;
	const void * src;
	hsa_agent_t src_agent;
	size_t size;
	uint32_t num_dep_signals;
	const hsa_signal_t * dep_signals;
	struct { // const hsa_signal_t *
		 hsa_signal_t val;
	} dep_signals__ref;
	hsa_signal_t completion_signal;
};

#define GET_ARGS_VALUE_hsa_amd_memory_async_copy(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy.dst = (void *)dst; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy.dst_agent = (hsa_agent_t)dst_agent; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy.src = (const void *)src; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy.src_agent = (hsa_agent_t)src_agent; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy.size = (size_t)size; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy.num_dep_signals = (uint32_t)num_dep_signals; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy.dep_signals = (const hsa_signal_t *)dep_signals; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy.completion_signal = (hsa_signal_t)completion_signal; \
};

#define GET_PTRS_VALUE_hsa_amd_memory_async_copy(args) { \
	if (args->hsa_amd_memory_async_copy.dep_signals != NULL) { \
		args->hsa_amd_memory_async_copy.dep_signals__ref.val = *args->hsa_amd_memory_async_copy.dep_signals; \
	} \
};


// hsa_status_t
// hsa_amd_memory_unlock (
// 		void * host_ptr (void)
// )
struct args_hsa_amd_memory_unlock_t {
	hsa_status_t retval;
	void * host_ptr;
};

#define GET_ARGS_VALUE_hsa_amd_memory_unlock(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_memory_unlock.host_ptr = (void *)host_ptr; \
};

#define GET_PTRS_VALUE_hsa_amd_memory_unlock(args)

// void
// hsa_signal_subtract_release (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_subtract_release_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_subtract_release(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_subtract_release.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_subtract_release.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_subtract_release(args)

// hsa_signal_value_t
// hsa_signal_exchange_acquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_exchange_acquire_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_exchange_acquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_exchange_acquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_exchange_acquire.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_exchange_acquire(args)

// void
// hsa_signal_store_relaxed (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_store_relaxed_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_store_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_store_relaxed.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_store_relaxed.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_store_relaxed(args)

// void
// hsa_signal_subtract_scacquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_subtract_scacquire_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_subtract_scacquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_subtract_scacquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_subtract_scacquire.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_subtract_scacquire(args)

// void
// hsa_signal_and_release (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_and_release_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_and_release(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_and_release.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_and_release.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_and_release(args)

// hsa_signal_value_t
// hsa_signal_cas_acquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t expected (long)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_cas_acquire_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_value_t expected;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_cas_acquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_cas_acquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_cas_acquire.expected = (hsa_signal_value_t)expected; \
	__hsa_activity->hsa_args.hsa_signal_cas_acquire.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_cas_acquire(args)

// hsa_status_t
// hsa_isa_get_exception_policies (
// 		hsa_isa_t isa (struct)
// 		hsa_profile_t profile (enum)
// 		uint16_t * mask (unsigned short)
// )
struct args_hsa_isa_get_exception_policies_t {
	hsa_status_t retval;
	hsa_isa_t isa;
	hsa_profile_t profile;
	uint16_t * mask;
	struct { // uint16_t *
		uint16_t val;
	} mask__ref;
};

#define GET_ARGS_VALUE_hsa_isa_get_exception_policies(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_isa_get_exception_policies.isa = (hsa_isa_t)isa; \
	__hsa_activity->hsa_args.hsa_isa_get_exception_policies.profile = (hsa_profile_t)profile; \
	__hsa_activity->hsa_args.hsa_isa_get_exception_policies.mask = (uint16_t *)mask; \
};

#define GET_PTRS_VALUE_hsa_isa_get_exception_policies(args) { \
	if (args->hsa_isa_get_exception_policies.mask != NULL) { \
		args->hsa_isa_get_exception_policies.mask__ref.val = *args->hsa_isa_get_exception_policies.mask; \
	} \
};


// void
// hsa_signal_add_release (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_add_release_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_add_release(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_add_release.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_add_release.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_add_release(args)

// hsa_status_t
// hsa_code_object_reader_create_from_file (
// 		hsa_file_t file (int)
// 		hsa_code_object_reader_t * code_object_reader (struct)
// )
struct args_hsa_code_object_reader_create_from_file_t {
	hsa_status_t retval;
	hsa_file_t file;
	hsa_code_object_reader_t * code_object_reader;
	struct { // hsa_code_object_reader_t *
		hsa_code_object_reader_t val;
	} code_object_reader__ref;
};

#define GET_ARGS_VALUE_hsa_code_object_reader_create_from_file(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_code_object_reader_create_from_file.file = (hsa_file_t)file; \
	__hsa_activity->hsa_args.hsa_code_object_reader_create_from_file.code_object_reader = (hsa_code_object_reader_t *)code_object_reader; \
};

#define GET_PTRS_VALUE_hsa_code_object_reader_create_from_file(args) { \
	if (args->hsa_code_object_reader_create_from_file.code_object_reader != NULL) { \
		args->hsa_code_object_reader_create_from_file.code_object_reader__ref.val = *args->hsa_code_object_reader_create_from_file.code_object_reader; \
	} \
};


// hsa_status_t
// hsa_executable_symbol_get_info (
// 		hsa_executable_symbol_t executable_symbol (struct)
// 		hsa_executable_symbol_info_t attribute (enum)
// 		void * value (void)
// )
struct args_hsa_executable_symbol_get_info_t {
	hsa_status_t retval;
	hsa_executable_symbol_t executable_symbol;
	hsa_executable_symbol_info_t attribute;
	void * value;
};

#define GET_ARGS_VALUE_hsa_executable_symbol_get_info(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_symbol_get_info.executable_symbol = (hsa_executable_symbol_t)executable_symbol; \
	__hsa_activity->hsa_args.hsa_executable_symbol_get_info.attribute = (hsa_executable_symbol_info_t)attribute; \
	__hsa_activity->hsa_args.hsa_executable_symbol_get_info.value = (void *)value; \
};

#define GET_PTRS_VALUE_hsa_executable_symbol_get_info(args)

// hsa_status_t
// hsa_amd_ipc_memory_create (
// 		void * ptr (void)
// 		size_t len (unsigned long)
// 		hsa_amd_ipc_memory_t * handle (struct)
// )
struct args_hsa_amd_ipc_memory_create_t {
	hsa_status_t retval;
	void * ptr;
	size_t len;
	hsa_amd_ipc_memory_t * handle;
	struct { // hsa_amd_ipc_memory_t *
		hsa_amd_ipc_memory_t val;
	} handle__ref;
};

#define GET_ARGS_VALUE_hsa_amd_ipc_memory_create(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_ipc_memory_create.ptr = (void *)ptr; \
	__hsa_activity->hsa_args.hsa_amd_ipc_memory_create.len = (size_t)len; \
	__hsa_activity->hsa_args.hsa_amd_ipc_memory_create.handle = (hsa_amd_ipc_memory_t *)handle; \
};

#define GET_PTRS_VALUE_hsa_amd_ipc_memory_create(args) { \
	if (args->hsa_amd_ipc_memory_create.handle != NULL) { \
		args->hsa_amd_ipc_memory_create.handle__ref.val = *args->hsa_amd_ipc_memory_create.handle; \
	} \
};


// void
// hsa_queue_store_write_index_release (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_store_write_index_release_t {
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_store_write_index_release(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_store_write_index_release.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_store_write_index_release.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_store_write_index_release(args) { \
	if (args->hsa_queue_store_write_index_release.queue != NULL) { \
		args->hsa_queue_store_write_index_release.queue__ref.val = *args->hsa_queue_store_write_index_release.queue; \
	} \
};


// hsa_status_t
// hsa_amd_memory_fill (
// 		void * ptr (void)
// 		uint32_t value (unsigned int)
// 		size_t count (unsigned long)
// )
struct args_hsa_amd_memory_fill_t {
	hsa_status_t retval;
	void * ptr;
	uint32_t value;
	size_t count;
};

#define GET_ARGS_VALUE_hsa_amd_memory_fill(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_memory_fill.ptr = (void *)ptr; \
	__hsa_activity->hsa_args.hsa_amd_memory_fill.value = (uint32_t)value; \
	__hsa_activity->hsa_args.hsa_amd_memory_fill.count = (size_t)count; \
};

#define GET_PTRS_VALUE_hsa_amd_memory_fill(args)

// void
// hsa_signal_add_acq_rel (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_add_acq_rel_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_add_acq_rel(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_add_acq_rel.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_add_acq_rel.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_add_acq_rel(args)

// hsa_signal_value_t
// hsa_signal_cas_scacq_screl (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t expected (long)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_cas_scacq_screl_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_value_t expected;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_cas_scacq_screl(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_cas_scacq_screl.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_cas_scacq_screl.expected = (hsa_signal_value_t)expected; \
	__hsa_activity->hsa_args.hsa_signal_cas_scacq_screl.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_cas_scacq_screl(args)

// uint64_t
// hsa_queue_cas_write_index_scacq_screl (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t expected (unsigned long)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_cas_write_index_scacq_screl_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t expected;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_cas_write_index_scacq_screl(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_scacq_screl.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_scacq_screl.expected = (uint64_t)expected; \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_scacq_screl.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_cas_write_index_scacq_screl(args) { \
	if (args->hsa_queue_cas_write_index_scacq_screl.queue != NULL) { \
		args->hsa_queue_cas_write_index_scacq_screl.queue__ref.val = *args->hsa_queue_cas_write_index_scacq_screl.queue; \
	} \
};


// hsa_status_t
// hsa_amd_ipc_signal_create (
// 		hsa_signal_t signal (struct)
// 		hsa_amd_ipc_signal_t * handle (struct)
// )
struct args_hsa_amd_ipc_signal_create_t {
	hsa_status_t retval;
	hsa_signal_t signal;
	hsa_amd_ipc_signal_t * handle;
	struct { // hsa_amd_ipc_signal_t *
		hsa_amd_ipc_signal_t val;
	} handle__ref;
};

#define GET_ARGS_VALUE_hsa_amd_ipc_signal_create(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_ipc_signal_create.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_amd_ipc_signal_create.handle = (hsa_amd_ipc_signal_t *)handle; \
};

#define GET_PTRS_VALUE_hsa_amd_ipc_signal_create(args) { \
	if (args->hsa_amd_ipc_signal_create.handle != NULL) { \
		args->hsa_amd_ipc_signal_create.handle__ref.val = *args->hsa_amd_ipc_signal_create.handle; \
	} \
};


// hsa_status_t
// hsa_amd_spm_acquire (
// 		hsa_agent_t preferred_agent (struct)
// )
struct args_hsa_amd_spm_acquire_t {
	hsa_status_t retval;
	hsa_agent_t preferred_agent;
};

#define GET_ARGS_VALUE_hsa_amd_spm_acquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_spm_acquire.preferred_agent = (hsa_agent_t)preferred_agent; \
};

#define GET_PTRS_VALUE_hsa_amd_spm_acquire(args)

// hsa_status_t
// hsa_amd_coherency_get_type (
// 		hsa_agent_t agent (struct)
// 		hsa_amd_coherency_type_t * type (enum)
// )
struct args_hsa_amd_coherency_get_type_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_amd_coherency_type_t * type;
	struct { // hsa_amd_coherency_type_t *
		hsa_amd_coherency_type_t val;
	} type__ref;
};

#define GET_ARGS_VALUE_hsa_amd_coherency_get_type(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_coherency_get_type.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_amd_coherency_get_type.type = (hsa_amd_coherency_type_t *)type; \
};

#define GET_PTRS_VALUE_hsa_amd_coherency_get_type(args) { \
	if (args->hsa_amd_coherency_get_type.type != NULL) { \
		args->hsa_amd_coherency_get_type.type__ref.val = *args->hsa_amd_coherency_get_type.type; \
	} \
};


// hsa_status_t
// hsa_system_get_extension_table (
// 		uint16_t extension (unsigned short)
// 		uint16_t version_major (unsigned short)
// 		uint16_t version_minor (unsigned short)
// 		void * table (void)
// )
struct args_hsa_system_get_extension_table_t {
	hsa_status_t retval;
	uint16_t extension;
	uint16_t version_major;
	uint16_t version_minor;
	void * table;
};

#define GET_ARGS_VALUE_hsa_system_get_extension_table(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_system_get_extension_table.extension = (uint16_t)extension; \
	__hsa_activity->hsa_args.hsa_system_get_extension_table.version_major = (uint16_t)version_major; \
	__hsa_activity->hsa_args.hsa_system_get_extension_table.version_minor = (uint16_t)version_minor; \
	__hsa_activity->hsa_args.hsa_system_get_extension_table.table = (void *)table; \
};

#define GET_PTRS_VALUE_hsa_system_get_extension_table(args)

// hsa_status_t
// hsa_amd_ipc_memory_detach (
// 		void * mapped_ptr (void)
// )
struct args_hsa_amd_ipc_memory_detach_t {
	hsa_status_t retval;
	void * mapped_ptr;
};

#define GET_ARGS_VALUE_hsa_amd_ipc_memory_detach(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_ipc_memory_detach.mapped_ptr = (void *)mapped_ptr; \
};

#define GET_PTRS_VALUE_hsa_amd_ipc_memory_detach(args)

// hsa_status_t
// hsa_ext_image_get_capability_with_layout (
// 		hsa_agent_t agent (struct)
// 		hsa_ext_image_geometry_t geometry (enum)
// 		const hsa_ext_image_format_t * image_format (struct)
// 		hsa_ext_image_data_layout_t image_data_layout (enum)
// 		uint32_t * capability_mask (unsigned int)
// )
struct args_hsa_ext_image_get_capability_with_layout_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_ext_image_geometry_t geometry;
	const hsa_ext_image_format_t * image_format;
	struct { // const hsa_ext_image_format_t *
		 hsa_ext_image_format_t val;
	} image_format__ref;
	hsa_ext_image_data_layout_t image_data_layout;
	uint32_t * capability_mask;
	struct { // uint32_t *
		uint32_t val;
	} capability_mask__ref;
};

#define GET_ARGS_VALUE_hsa_ext_image_get_capability_with_layout(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_ext_image_get_capability_with_layout.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_ext_image_get_capability_with_layout.geometry = (hsa_ext_image_geometry_t)geometry; \
	__hsa_activity->hsa_args.hsa_ext_image_get_capability_with_layout.image_format = (const hsa_ext_image_format_t *)image_format; \
	__hsa_activity->hsa_args.hsa_ext_image_get_capability_with_layout.image_data_layout = (hsa_ext_image_data_layout_t)image_data_layout; \
	__hsa_activity->hsa_args.hsa_ext_image_get_capability_with_layout.capability_mask = (uint32_t *)capability_mask; \
};

#define GET_PTRS_VALUE_hsa_ext_image_get_capability_with_layout(args) { \
	if (args->hsa_ext_image_get_capability_with_layout.image_format != NULL) { \
		args->hsa_ext_image_get_capability_with_layout.image_format__ref.val = *args->hsa_ext_image_get_capability_with_layout.image_format; \
	} \
	if (args->hsa_ext_image_get_capability_with_layout.capability_mask != NULL) { \
		args->hsa_ext_image_get_capability_with_layout.capability_mask__ref.val = *args->hsa_ext_image_get_capability_with_layout.capability_mask; \
	} \
};


// hsa_status_t
// hsa_amd_vmem_address_free (
// 		void * va (void)
// 		size_t size (unsigned long)
// )
struct args_hsa_amd_vmem_address_free_t {
	hsa_status_t retval;
	void * va;
	size_t size;
};

#define GET_ARGS_VALUE_hsa_amd_vmem_address_free(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_vmem_address_free.va = (void *)va; \
	__hsa_activity->hsa_args.hsa_amd_vmem_address_free.size = (size_t)size; \
};

#define GET_PTRS_VALUE_hsa_amd_vmem_address_free(args)

// hsa_status_t
// hsa_amd_svm_attributes_set (
// 		void * ptr (void)
// 		size_t size (unsigned long)
// 		hsa_amd_svm_attribute_pair_t * attribute_list (struct)
// 		size_t attribute_count (unsigned long)
// )
struct args_hsa_amd_svm_attributes_set_t {
	hsa_status_t retval;
	void * ptr;
	size_t size;
	hsa_amd_svm_attribute_pair_t * attribute_list;
	struct { // hsa_amd_svm_attribute_pair_t *
		hsa_amd_svm_attribute_pair_t val;
	} attribute_list__ref;
	size_t attribute_count;
};

#define GET_ARGS_VALUE_hsa_amd_svm_attributes_set(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_svm_attributes_set.ptr = (void *)ptr; \
	__hsa_activity->hsa_args.hsa_amd_svm_attributes_set.size = (size_t)size; \
	__hsa_activity->hsa_args.hsa_amd_svm_attributes_set.attribute_list = (hsa_amd_svm_attribute_pair_t *)attribute_list; \
	__hsa_activity->hsa_args.hsa_amd_svm_attributes_set.attribute_count = (size_t)attribute_count; \
};

#define GET_PTRS_VALUE_hsa_amd_svm_attributes_set(args) { \
	if (args->hsa_amd_svm_attributes_set.attribute_list != NULL) { \
		args->hsa_amd_svm_attributes_set.attribute_list__ref.val = *args->hsa_amd_svm_attributes_set.attribute_list; \
	} \
};


// hsa_status_t
// hsa_ext_image_import (
// 		hsa_agent_t agent (struct)
// 		const void * src_memory (void)
// 		size_t src_row_pitch (unsigned long)
// 		size_t src_slice_pitch (unsigned long)
// 		hsa_ext_image_t dst_image (struct)
// 		const hsa_ext_image_region_t * image_region (struct)
// )
struct args_hsa_ext_image_import_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	const void * src_memory;
	size_t src_row_pitch;
	size_t src_slice_pitch;
	hsa_ext_image_t dst_image;
	const hsa_ext_image_region_t * image_region;
	struct { // const hsa_ext_image_region_t *
		 hsa_ext_image_region_t val;
	} image_region__ref;
};

#define GET_ARGS_VALUE_hsa_ext_image_import(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_ext_image_import.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_ext_image_import.src_memory = (const void *)src_memory; \
	__hsa_activity->hsa_args.hsa_ext_image_import.src_row_pitch = (size_t)src_row_pitch; \
	__hsa_activity->hsa_args.hsa_ext_image_import.src_slice_pitch = (size_t)src_slice_pitch; \
	__hsa_activity->hsa_args.hsa_ext_image_import.dst_image = (hsa_ext_image_t)dst_image; \
	__hsa_activity->hsa_args.hsa_ext_image_import.image_region = (const hsa_ext_image_region_t *)image_region; \
};

#define GET_PTRS_VALUE_hsa_ext_image_import(args) { \
	if (args->hsa_ext_image_import.image_region != NULL) { \
		args->hsa_ext_image_import.image_region__ref.val = *args->hsa_ext_image_import.image_region; \
	} \
};


// void
// hsa_signal_or_scacquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_or_scacquire_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_or_scacquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_or_scacquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_or_scacquire.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_or_scacquire(args)

// hsa_status_t
// hsa_amd_svm_prefetch_async (
// 		void * ptr (void)
// 		size_t size (unsigned long)
// 		hsa_agent_t agent (struct)
// 		uint32_t num_dep_signals (unsigned int)
// 		const hsa_signal_t * dep_signals (struct)
// 		hsa_signal_t completion_signal (struct)
// )
struct args_hsa_amd_svm_prefetch_async_t {
	hsa_status_t retval;
	void * ptr;
	size_t size;
	hsa_agent_t agent;
	uint32_t num_dep_signals;
	const hsa_signal_t * dep_signals;
	struct { // const hsa_signal_t *
		 hsa_signal_t val;
	} dep_signals__ref;
	hsa_signal_t completion_signal;
};

#define GET_ARGS_VALUE_hsa_amd_svm_prefetch_async(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_svm_prefetch_async.ptr = (void *)ptr; \
	__hsa_activity->hsa_args.hsa_amd_svm_prefetch_async.size = (size_t)size; \
	__hsa_activity->hsa_args.hsa_amd_svm_prefetch_async.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_amd_svm_prefetch_async.num_dep_signals = (uint32_t)num_dep_signals; \
	__hsa_activity->hsa_args.hsa_amd_svm_prefetch_async.dep_signals = (const hsa_signal_t *)dep_signals; \
	__hsa_activity->hsa_args.hsa_amd_svm_prefetch_async.completion_signal = (hsa_signal_t)completion_signal; \
};

#define GET_PTRS_VALUE_hsa_amd_svm_prefetch_async(args) { \
	if (args->hsa_amd_svm_prefetch_async.dep_signals != NULL) { \
		args->hsa_amd_svm_prefetch_async.dep_signals__ref.val = *args->hsa_amd_svm_prefetch_async.dep_signals; \
	} \
};


// void
// hsa_signal_add_scacq_screl (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_add_scacq_screl_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_add_scacq_screl(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_add_scacq_screl.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_add_scacq_screl.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_add_scacq_screl(args)

// void
// hsa_signal_silent_store_relaxed (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_silent_store_relaxed_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_silent_store_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_silent_store_relaxed.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_silent_store_relaxed.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_silent_store_relaxed(args)

// void
// hsa_queue_store_read_index_relaxed (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_store_read_index_relaxed_t {
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_store_read_index_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_store_read_index_relaxed.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_store_read_index_relaxed.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_store_read_index_relaxed(args) { \
	if (args->hsa_queue_store_read_index_relaxed.queue != NULL) { \
		args->hsa_queue_store_read_index_relaxed.queue__ref.val = *args->hsa_queue_store_read_index_relaxed.queue; \
	} \
};


// hsa_status_t
// hsa_agent_get_exception_policies (
// 		hsa_agent_t agent (struct)
// 		hsa_profile_t profile (enum)
// 		uint16_t * mask (unsigned short)
// )
struct args_hsa_agent_get_exception_policies_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_profile_t profile;
	uint16_t * mask;
	struct { // uint16_t *
		uint16_t val;
	} mask__ref;
};

#define GET_ARGS_VALUE_hsa_agent_get_exception_policies(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_agent_get_exception_policies.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_agent_get_exception_policies.profile = (hsa_profile_t)profile; \
	__hsa_activity->hsa_args.hsa_agent_get_exception_policies.mask = (uint16_t *)mask; \
};

#define GET_PTRS_VALUE_hsa_agent_get_exception_policies(args) { \
	if (args->hsa_agent_get_exception_policies.mask != NULL) { \
		args->hsa_agent_get_exception_policies.mask__ref.val = *args->hsa_agent_get_exception_policies.mask; \
	} \
};


// void
// hsa_signal_add_scacquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_add_scacquire_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_add_scacquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_add_scacquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_add_scacquire.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_add_scacquire(args)

// hsa_signal_value_t
// hsa_signal_exchange_release (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_exchange_release_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_exchange_release(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_exchange_release.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_exchange_release.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_exchange_release(args)

// void
// hsa_signal_add_relaxed (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_add_relaxed_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_add_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_add_relaxed.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_add_relaxed.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_add_relaxed(args)

// void
// hsa_signal_and_acquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_and_acquire_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_and_acquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_and_acquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_and_acquire.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_and_acquire(args)

// uint64_t
// hsa_queue_add_write_index_relaxed (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_add_write_index_relaxed_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_add_write_index_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_add_write_index_relaxed.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_add_write_index_relaxed.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_add_write_index_relaxed(args) { \
	if (args->hsa_queue_add_write_index_relaxed.queue != NULL) { \
		args->hsa_queue_add_write_index_relaxed.queue__ref.val = *args->hsa_queue_add_write_index_relaxed.queue; \
	} \
};


// hsa_status_t
// hsa_amd_agent_memory_pool_get_info (
// 		hsa_agent_t agent (struct)
// 		hsa_amd_memory_pool_t memory_pool (struct)
// 		hsa_amd_agent_memory_pool_info_t attribute (enum)
// 		void * value (void)
// )
struct args_hsa_amd_agent_memory_pool_get_info_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_amd_memory_pool_t memory_pool;
	hsa_amd_agent_memory_pool_info_t attribute;
	void * value;
};

#define GET_ARGS_VALUE_hsa_amd_agent_memory_pool_get_info(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_agent_memory_pool_get_info.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_amd_agent_memory_pool_get_info.memory_pool = (hsa_amd_memory_pool_t)memory_pool; \
	__hsa_activity->hsa_args.hsa_amd_agent_memory_pool_get_info.attribute = (hsa_amd_agent_memory_pool_info_t)attribute; \
	__hsa_activity->hsa_args.hsa_amd_agent_memory_pool_get_info.value = (void *)value; \
};

#define GET_PTRS_VALUE_hsa_amd_agent_memory_pool_get_info(args)

// hsa_status_t
// hsa_amd_profiling_set_profiler_enabled (
// 		hsa_queue_t * queue (struct)
// 		int enable (int)
// )
struct args_hsa_amd_profiling_set_profiler_enabled_t {
	hsa_status_t retval;
	hsa_queue_t * queue;
	struct { // hsa_queue_t *
		hsa_queue_t val;
	} queue__ref;
	int enable;
};

#define GET_ARGS_VALUE_hsa_amd_profiling_set_profiler_enabled(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_profiling_set_profiler_enabled.queue = (hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_amd_profiling_set_profiler_enabled.enable = (int)enable; \
};

#define GET_PTRS_VALUE_hsa_amd_profiling_set_profiler_enabled(args) { \
	if (args->hsa_amd_profiling_set_profiler_enabled.queue != NULL) { \
		args->hsa_amd_profiling_set_profiler_enabled.queue__ref.val = *args->hsa_amd_profiling_set_profiler_enabled.queue; \
	} \
};


// hsa_signal_value_t
// hsa_signal_exchange_acq_rel (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_exchange_acq_rel_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_exchange_acq_rel(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_exchange_acq_rel.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_exchange_acq_rel.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_exchange_acq_rel(args)

// uint64_t
// hsa_queue_cas_write_index_screlease (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t expected (unsigned long)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_cas_write_index_screlease_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t expected;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_cas_write_index_screlease(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_screlease.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_screlease.expected = (uint64_t)expected; \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_screlease.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_cas_write_index_screlease(args) { \
	if (args->hsa_queue_cas_write_index_screlease.queue != NULL) { \
		args->hsa_queue_cas_write_index_screlease.queue__ref.val = *args->hsa_queue_cas_write_index_screlease.queue; \
	} \
};


// hsa_status_t
// hsa_amd_async_function (
// 		void (*)(void *) callback (function)
// 		void * arg (void)
// )
struct args_hsa_amd_async_function_t {
	hsa_status_t retval;
	void (*callback)(void *);
	void * arg;
};

#define GET_ARGS_VALUE_hsa_amd_async_function(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_async_function.callback = (void (*)(void *))callback; \
	__hsa_activity->hsa_args.hsa_amd_async_function.arg = (void *)arg; \
};

#define GET_PTRS_VALUE_hsa_amd_async_function(args)

// void
// hsa_queue_store_read_index_screlease (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_store_read_index_screlease_t {
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_store_read_index_screlease(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_store_read_index_screlease.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_store_read_index_screlease.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_store_read_index_screlease(args) { \
	if (args->hsa_queue_store_read_index_screlease.queue != NULL) { \
		args->hsa_queue_store_read_index_screlease.queue__ref.val = *args->hsa_queue_store_read_index_screlease.queue; \
	} \
};


// hsa_status_t
// hsa_memory_allocate (
// 		hsa_region_t region (struct)
// 		size_t size (unsigned long)
// 		void ** ptr (void)
// )
struct args_hsa_memory_allocate_t {
	hsa_status_t retval;
	hsa_region_t region;
	size_t size;
	void ** ptr;
	struct { // void **
		void* ptr1;
	} ptr__ref;
};

#define GET_ARGS_VALUE_hsa_memory_allocate(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_memory_allocate.region = (hsa_region_t)region; \
	__hsa_activity->hsa_args.hsa_memory_allocate.size = (size_t)size; \
	__hsa_activity->hsa_args.hsa_memory_allocate.ptr = (void **)ptr; \
};

#define GET_PTRS_VALUE_hsa_memory_allocate(args) { \
	if (args->hsa_memory_allocate.ptr != NULL) { \
		args->hsa_memory_allocate.ptr__ref.ptr1 = (void*)*args->hsa_memory_allocate.ptr; \
	} \
};


// hsa_status_t
// hsa_amd_signal_value_pointer (
// 		hsa_signal_t signal (struct)
// 		volatile hsa_signal_value_t ** value_ptr (long)
// )
struct args_hsa_amd_signal_value_pointer_t {
	hsa_status_t retval;
	hsa_signal_t signal;
	volatile hsa_signal_value_t ** value_ptr;
	struct { // volatile hsa_signal_value_t **
		volatile hsa_signal_value_t* ptr1;
		volatile hsa_signal_value_t val;
	} value_ptr__ref;
};

#define GET_ARGS_VALUE_hsa_amd_signal_value_pointer(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_signal_value_pointer.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_amd_signal_value_pointer.value_ptr = (volatile hsa_signal_value_t **)value_ptr; \
};

#define GET_PTRS_VALUE_hsa_amd_signal_value_pointer(args) { \
	if (args->hsa_amd_signal_value_pointer.value_ptr != NULL) { \
		args->hsa_amd_signal_value_pointer.value_ptr__ref.ptr1 = (void*)*args->hsa_amd_signal_value_pointer.value_ptr; \
		if (args->hsa_amd_signal_value_pointer.value_ptr__ref.ptr1 != NULL) { \
			args->hsa_amd_signal_value_pointer.value_ptr__ref.val = *args->hsa_amd_signal_value_pointer.value_ptr__ref.ptr1; \
		} \
	} \
};


// uint32_t
// hsa_amd_signal_wait_any (
// 		uint32_t signal_count (unsigned int)
// 		hsa_signal_t * signals (struct)
// 		hsa_signal_condition_t * conds (enum)
// 		hsa_signal_value_t * values (long)
// 		uint64_t timeout_hint (unsigned long)
// 		hsa_wait_state_t wait_hint (enum)
// 		hsa_signal_value_t * satisfying_value (long)
// )
struct args_hsa_amd_signal_wait_any_t {
	uint32_t retval;
	uint32_t signal_count;
	hsa_signal_t * signals;
	struct { // hsa_signal_t *
		hsa_signal_t val;
	} signals__ref;
	hsa_signal_condition_t * conds;
	struct { // hsa_signal_condition_t *
		hsa_signal_condition_t val;
	} conds__ref;
	hsa_signal_value_t * values;
	struct { // hsa_signal_value_t *
		hsa_signal_value_t val;
	} values__ref;
	uint64_t timeout_hint;
	hsa_wait_state_t wait_hint;
	hsa_signal_value_t * satisfying_value;
	struct { // hsa_signal_value_t *
		hsa_signal_value_t val;
	} satisfying_value__ref;
};

#define GET_ARGS_VALUE_hsa_amd_signal_wait_any(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_signal_wait_any.signal_count = (uint32_t)signal_count; \
	__hsa_activity->hsa_args.hsa_amd_signal_wait_any.signals = (hsa_signal_t *)signals; \
	__hsa_activity->hsa_args.hsa_amd_signal_wait_any.conds = (hsa_signal_condition_t *)conds; \
	__hsa_activity->hsa_args.hsa_amd_signal_wait_any.values = (hsa_signal_value_t *)values; \
	__hsa_activity->hsa_args.hsa_amd_signal_wait_any.timeout_hint = (uint64_t)timeout_hint; \
	__hsa_activity->hsa_args.hsa_amd_signal_wait_any.wait_hint = (hsa_wait_state_t)wait_hint; \
	__hsa_activity->hsa_args.hsa_amd_signal_wait_any.satisfying_value = (hsa_signal_value_t *)satisfying_value; \
};

#define GET_PTRS_VALUE_hsa_amd_signal_wait_any(args) { \
	if (args->hsa_amd_signal_wait_any.signals != NULL) { \
		args->hsa_amd_signal_wait_any.signals__ref.val = *args->hsa_amd_signal_wait_any.signals; \
	} \
	if (args->hsa_amd_signal_wait_any.conds != NULL) { \
		args->hsa_amd_signal_wait_any.conds__ref.val = *args->hsa_amd_signal_wait_any.conds; \
	} \
	if (args->hsa_amd_signal_wait_any.values != NULL) { \
		args->hsa_amd_signal_wait_any.values__ref.val = *args->hsa_amd_signal_wait_any.values; \
	} \
	if (args->hsa_amd_signal_wait_any.satisfying_value != NULL) { \
		args->hsa_amd_signal_wait_any.satisfying_value__ref.val = *args->hsa_amd_signal_wait_any.satisfying_value; \
	} \
};


// hsa_status_t
// hsa_amd_vmem_retain_alloc_handle (
// 		hsa_amd_vmem_alloc_handle_t * memory_handle (struct)
// 		void * addr (void)
// )
struct args_hsa_amd_vmem_retain_alloc_handle_t {
	hsa_status_t retval;
	hsa_amd_vmem_alloc_handle_t * memory_handle;
	struct { // hsa_amd_vmem_alloc_handle_t *
		hsa_amd_vmem_alloc_handle_t val;
	} memory_handle__ref;
	void * addr;
};

#define GET_ARGS_VALUE_hsa_amd_vmem_retain_alloc_handle(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_vmem_retain_alloc_handle.memory_handle = (hsa_amd_vmem_alloc_handle_t *)memory_handle; \
	__hsa_activity->hsa_args.hsa_amd_vmem_retain_alloc_handle.addr = (void *)addr; \
};

#define GET_PTRS_VALUE_hsa_amd_vmem_retain_alloc_handle(args) { \
	if (args->hsa_amd_vmem_retain_alloc_handle.memory_handle != NULL) { \
		args->hsa_amd_vmem_retain_alloc_handle.memory_handle__ref.val = *args->hsa_amd_vmem_retain_alloc_handle.memory_handle; \
	} \
};


// hsa_status_t
// hsa_amd_vmem_set_access (
// 		void * va (void)
// 		size_t size (unsigned long)
// 		const hsa_amd_memory_access_desc_t * desc (struct)
// 		size_t desc_cnt (unsigned long)
// )
struct args_hsa_amd_vmem_set_access_t {
	hsa_status_t retval;
	void * va;
	size_t size;
	const hsa_amd_memory_access_desc_t * desc;
	struct { // const hsa_amd_memory_access_desc_t *
		 hsa_amd_memory_access_desc_t val;
	} desc__ref;
	size_t desc_cnt;
};

#define GET_ARGS_VALUE_hsa_amd_vmem_set_access(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_vmem_set_access.va = (void *)va; \
	__hsa_activity->hsa_args.hsa_amd_vmem_set_access.size = (size_t)size; \
	__hsa_activity->hsa_args.hsa_amd_vmem_set_access.desc = (const hsa_amd_memory_access_desc_t *)desc; \
	__hsa_activity->hsa_args.hsa_amd_vmem_set_access.desc_cnt = (size_t)desc_cnt; \
};

#define GET_PTRS_VALUE_hsa_amd_vmem_set_access(args) { \
	if (args->hsa_amd_vmem_set_access.desc != NULL) { \
		args->hsa_amd_vmem_set_access.desc__ref.val = *args->hsa_amd_vmem_set_access.desc; \
	} \
};


// hsa_status_t
// hsa_memory_assign_agent (
// 		void * ptr (void)
// 		hsa_agent_t agent (struct)
// 		hsa_access_permission_t access (enum)
// )
struct args_hsa_memory_assign_agent_t {
	hsa_status_t retval;
	void * ptr;
	hsa_agent_t agent;
	hsa_access_permission_t access;
};

#define GET_ARGS_VALUE_hsa_memory_assign_agent(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_memory_assign_agent.ptr = (void *)ptr; \
	__hsa_activity->hsa_args.hsa_memory_assign_agent.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_memory_assign_agent.access = (hsa_access_permission_t)access; \
};

#define GET_PTRS_VALUE_hsa_memory_assign_agent(args)

// hsa_status_t
// hsa_amd_vmem_address_reserve (
// 		void ** va (void)
// 		size_t size (unsigned long)
// 		uint64_t address (unsigned long)
// 		uint64_t flags (unsigned long)
// )
struct args_hsa_amd_vmem_address_reserve_t {
	hsa_status_t retval;
	void ** va;
	struct { // void **
		void* ptr1;
	} va__ref;
	size_t size;
	uint64_t address;
	uint64_t flags;
};

#define GET_ARGS_VALUE_hsa_amd_vmem_address_reserve(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_vmem_address_reserve.va = (void **)va; \
	__hsa_activity->hsa_args.hsa_amd_vmem_address_reserve.size = (size_t)size; \
	__hsa_activity->hsa_args.hsa_amd_vmem_address_reserve.address = (uint64_t)address; \
	__hsa_activity->hsa_args.hsa_amd_vmem_address_reserve.flags = (uint64_t)flags; \
};

#define GET_PTRS_VALUE_hsa_amd_vmem_address_reserve(args) { \
	if (args->hsa_amd_vmem_address_reserve.va != NULL) { \
		args->hsa_amd_vmem_address_reserve.va__ref.ptr1 = (void*)*args->hsa_amd_vmem_address_reserve.va; \
	} \
};


// hsa_status_t
// hsa_amd_memory_async_copy_on_engine (
// 		void * dst (void)
// 		hsa_agent_t dst_agent (struct)
// 		const void * src (void)
// 		hsa_agent_t src_agent (struct)
// 		size_t size (unsigned long)
// 		uint32_t num_dep_signals (unsigned int)
// 		const hsa_signal_t * dep_signals (struct)
// 		hsa_signal_t completion_signal (struct)
// 		hsa_amd_sdma_engine_id_t engine_id (enum)
// 		_Bool force_copy_on_sdma (N/A)
// )
struct args_hsa_amd_memory_async_copy_on_engine_t {
	hsa_status_t retval;
	void * dst;
	hsa_agent_t dst_agent;
	const void * src;
	hsa_agent_t src_agent;
	size_t size;
	uint32_t num_dep_signals;
	const hsa_signal_t * dep_signals;
	struct { // const hsa_signal_t *
		 hsa_signal_t val;
	} dep_signals__ref;
	hsa_signal_t completion_signal;
	hsa_amd_sdma_engine_id_t engine_id;
	_Bool force_copy_on_sdma;
};

#define GET_ARGS_VALUE_hsa_amd_memory_async_copy_on_engine(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_on_engine.dst = (void *)dst; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_on_engine.dst_agent = (hsa_agent_t)dst_agent; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_on_engine.src = (const void *)src; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_on_engine.src_agent = (hsa_agent_t)src_agent; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_on_engine.size = (size_t)size; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_on_engine.num_dep_signals = (uint32_t)num_dep_signals; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_on_engine.dep_signals = (const hsa_signal_t *)dep_signals; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_on_engine.completion_signal = (hsa_signal_t)completion_signal; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_on_engine.engine_id = (hsa_amd_sdma_engine_id_t)engine_id; \
	__hsa_activity->hsa_args.hsa_amd_memory_async_copy_on_engine.force_copy_on_sdma = (_Bool)force_copy_on_sdma; \
};

#define GET_PTRS_VALUE_hsa_amd_memory_async_copy_on_engine(args) { \
	if (args->hsa_amd_memory_async_copy_on_engine.dep_signals != NULL) { \
		args->hsa_amd_memory_async_copy_on_engine.dep_signals__ref.val = *args->hsa_amd_memory_async_copy_on_engine.dep_signals; \
	} \
};


// void
// hsa_signal_xor_scacquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_xor_scacquire_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_xor_scacquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_xor_scacquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_xor_scacquire.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_xor_scacquire(args)

// hsa_status_t
// hsa_amd_coherency_set_type (
// 		hsa_agent_t agent (struct)
// 		hsa_amd_coherency_type_t type (enum)
// )
struct args_hsa_amd_coherency_set_type_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_amd_coherency_type_t type;
};

#define GET_ARGS_VALUE_hsa_amd_coherency_set_type(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_coherency_set_type.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_amd_coherency_set_type.type = (hsa_amd_coherency_type_t)type; \
};

#define GET_PTRS_VALUE_hsa_amd_coherency_set_type(args)

// hsa_status_t
// hsa_queue_inactivate (
// 		hsa_queue_t * queue (struct)
// )
struct args_hsa_queue_inactivate_t {
	hsa_status_t retval;
	hsa_queue_t * queue;
	struct { // hsa_queue_t *
		hsa_queue_t val;
	} queue__ref;
};

#define GET_ARGS_VALUE_hsa_queue_inactivate(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_inactivate.queue = (hsa_queue_t *)queue; \
};

#define GET_PTRS_VALUE_hsa_queue_inactivate(args) { \
	if (args->hsa_queue_inactivate.queue != NULL) { \
		args->hsa_queue_inactivate.queue__ref.val = *args->hsa_queue_inactivate.queue; \
	} \
};


// hsa_status_t
// hsa_amd_vmem_import_shareable_handle (
// 		int dmabuf_fd (int)
// 		hsa_amd_vmem_alloc_handle_t * handle (struct)
// )
struct args_hsa_amd_vmem_import_shareable_handle_t {
	hsa_status_t retval;
	int dmabuf_fd;
	hsa_amd_vmem_alloc_handle_t * handle;
	struct { // hsa_amd_vmem_alloc_handle_t *
		hsa_amd_vmem_alloc_handle_t val;
	} handle__ref;
};

#define GET_ARGS_VALUE_hsa_amd_vmem_import_shareable_handle(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_vmem_import_shareable_handle.dmabuf_fd = (int)dmabuf_fd; \
	__hsa_activity->hsa_args.hsa_amd_vmem_import_shareable_handle.handle = (hsa_amd_vmem_alloc_handle_t *)handle; \
};

#define GET_PTRS_VALUE_hsa_amd_vmem_import_shareable_handle(args) { \
	if (args->hsa_amd_vmem_import_shareable_handle.handle != NULL) { \
		args->hsa_amd_vmem_import_shareable_handle.handle__ref.val = *args->hsa_amd_vmem_import_shareable_handle.handle; \
	} \
};


// hsa_signal_value_t
// hsa_signal_cas_relaxed (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t expected (long)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_cas_relaxed_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_value_t expected;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_cas_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_cas_relaxed.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_cas_relaxed.expected = (hsa_signal_value_t)expected; \
	__hsa_activity->hsa_args.hsa_signal_cas_relaxed.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_cas_relaxed(args)

// uint64_t
// hsa_queue_add_write_index_scacquire (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_add_write_index_scacquire_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_add_write_index_scacquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_add_write_index_scacquire.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_add_write_index_scacquire.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_add_write_index_scacquire(args) { \
	if (args->hsa_queue_add_write_index_scacquire.queue != NULL) { \
		args->hsa_queue_add_write_index_scacquire.queue__ref.val = *args->hsa_queue_add_write_index_scacquire.queue; \
	} \
};


// hsa_signal_value_t
// hsa_signal_exchange_screlease (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_exchange_screlease_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_exchange_screlease(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_exchange_screlease.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_exchange_screlease.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_exchange_screlease(args)

// hsa_status_t
// hsa_amd_vmem_handle_create (
// 		hsa_amd_memory_pool_t pool (struct)
// 		size_t size (unsigned long)
// 		hsa_amd_memory_type_t type (enum)
// 		uint64_t flags (unsigned long)
// 		hsa_amd_vmem_alloc_handle_t * memory_handle (struct)
// )
struct args_hsa_amd_vmem_handle_create_t {
	hsa_status_t retval;
	hsa_amd_memory_pool_t pool;
	size_t size;
	hsa_amd_memory_type_t type;
	uint64_t flags;
	hsa_amd_vmem_alloc_handle_t * memory_handle;
	struct { // hsa_amd_vmem_alloc_handle_t *
		hsa_amd_vmem_alloc_handle_t val;
	} memory_handle__ref;
};

#define GET_ARGS_VALUE_hsa_amd_vmem_handle_create(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_vmem_handle_create.pool = (hsa_amd_memory_pool_t)pool; \
	__hsa_activity->hsa_args.hsa_amd_vmem_handle_create.size = (size_t)size; \
	__hsa_activity->hsa_args.hsa_amd_vmem_handle_create.type = (hsa_amd_memory_type_t)type; \
	__hsa_activity->hsa_args.hsa_amd_vmem_handle_create.flags = (uint64_t)flags; \
	__hsa_activity->hsa_args.hsa_amd_vmem_handle_create.memory_handle = (hsa_amd_vmem_alloc_handle_t *)memory_handle; \
};

#define GET_PTRS_VALUE_hsa_amd_vmem_handle_create(args) { \
	if (args->hsa_amd_vmem_handle_create.memory_handle != NULL) { \
		args->hsa_amd_vmem_handle_create.memory_handle__ref.val = *args->hsa_amd_vmem_handle_create.memory_handle; \
	} \
};


// hsa_status_t
// hsa_agent_get_info (
// 		hsa_agent_t agent (struct)
// 		hsa_agent_info_t attribute (enum)
// 		void * value (void)
// )
struct args_hsa_agent_get_info_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_agent_info_t attribute;
	void * value;
};

#define GET_ARGS_VALUE_hsa_agent_get_info(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_agent_get_info.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_agent_get_info.attribute = (hsa_agent_info_t)attribute; \
	__hsa_activity->hsa_args.hsa_agent_get_info.value = (void *)value; \
};

#define GET_PTRS_VALUE_hsa_agent_get_info(args)

// void
// hsa_signal_or_acq_rel (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_or_acq_rel_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_or_acq_rel(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_or_acq_rel.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_or_acq_rel.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_or_acq_rel(args)

// void
// hsa_signal_xor_acq_rel (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_xor_acq_rel_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_xor_acq_rel(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_xor_acq_rel.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_xor_acq_rel.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_xor_acq_rel(args)

// hsa_status_t
// hsa_system_extension_supported (
// 		uint16_t extension (unsigned short)
// 		uint16_t version_major (unsigned short)
// 		uint16_t version_minor (unsigned short)
// 		_Bool * result (N/A)
// )
struct args_hsa_system_extension_supported_t {
	hsa_status_t retval;
	uint16_t extension;
	uint16_t version_major;
	uint16_t version_minor;
	_Bool * result;
	struct { // _Bool *
		_Bool val;
	} result__ref;
};

#define GET_ARGS_VALUE_hsa_system_extension_supported(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_system_extension_supported.extension = (uint16_t)extension; \
	__hsa_activity->hsa_args.hsa_system_extension_supported.version_major = (uint16_t)version_major; \
	__hsa_activity->hsa_args.hsa_system_extension_supported.version_minor = (uint16_t)version_minor; \
	__hsa_activity->hsa_args.hsa_system_extension_supported.result = (_Bool *)result; \
};

#define GET_PTRS_VALUE_hsa_system_extension_supported(args) { \
	if (args->hsa_system_extension_supported.result != NULL) { \
		args->hsa_system_extension_supported.result__ref.val = *args->hsa_system_extension_supported.result; \
	} \
};


// hsa_status_t
// hsa_signal_group_wait_any_relaxed (
// 		hsa_signal_group_t signal_group (struct)
// 		const hsa_signal_condition_t * conditions (enum)
// 		const hsa_signal_value_t * compare_values (long)
// 		hsa_wait_state_t wait_state_hint (enum)
// 		hsa_signal_t * signal (struct)
// 		hsa_signal_value_t * value (long)
// )
struct args_hsa_signal_group_wait_any_relaxed_t {
	hsa_status_t retval;
	hsa_signal_group_t signal_group;
	const hsa_signal_condition_t * conditions;
	struct { // const hsa_signal_condition_t *
		 hsa_signal_condition_t val;
	} conditions__ref;
	const hsa_signal_value_t * compare_values;
	struct { // const hsa_signal_value_t *
		 hsa_signal_value_t val;
	} compare_values__ref;
	hsa_wait_state_t wait_state_hint;
	hsa_signal_t * signal;
	struct { // hsa_signal_t *
		hsa_signal_t val;
	} signal__ref;
	hsa_signal_value_t * value;
	struct { // hsa_signal_value_t *
		hsa_signal_value_t val;
	} value__ref;
};

#define GET_ARGS_VALUE_hsa_signal_group_wait_any_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_group_wait_any_relaxed.signal_group = (hsa_signal_group_t)signal_group; \
	__hsa_activity->hsa_args.hsa_signal_group_wait_any_relaxed.conditions = (const hsa_signal_condition_t *)conditions; \
	__hsa_activity->hsa_args.hsa_signal_group_wait_any_relaxed.compare_values = (const hsa_signal_value_t *)compare_values; \
	__hsa_activity->hsa_args.hsa_signal_group_wait_any_relaxed.wait_state_hint = (hsa_wait_state_t)wait_state_hint; \
	__hsa_activity->hsa_args.hsa_signal_group_wait_any_relaxed.signal = (hsa_signal_t *)signal; \
	__hsa_activity->hsa_args.hsa_signal_group_wait_any_relaxed.value = (hsa_signal_value_t *)value; \
};

#define GET_PTRS_VALUE_hsa_signal_group_wait_any_relaxed(args) { \
	if (args->hsa_signal_group_wait_any_relaxed.conditions != NULL) { \
		args->hsa_signal_group_wait_any_relaxed.conditions__ref.val = *args->hsa_signal_group_wait_any_relaxed.conditions; \
	} \
	if (args->hsa_signal_group_wait_any_relaxed.compare_values != NULL) { \
		args->hsa_signal_group_wait_any_relaxed.compare_values__ref.val = *args->hsa_signal_group_wait_any_relaxed.compare_values; \
	} \
	if (args->hsa_signal_group_wait_any_relaxed.signal != NULL) { \
		args->hsa_signal_group_wait_any_relaxed.signal__ref.val = *args->hsa_signal_group_wait_any_relaxed.signal; \
	} \
	if (args->hsa_signal_group_wait_any_relaxed.value != NULL) { \
		args->hsa_signal_group_wait_any_relaxed.value__ref.val = *args->hsa_signal_group_wait_any_relaxed.value; \
	} \
};


// void
// hsa_signal_or_scacq_screl (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_or_scacq_screl_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_or_scacq_screl(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_or_scacq_screl.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_or_scacq_screl.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_or_scacq_screl(args)

// hsa_status_t
// hsa_executable_iterate_agent_symbols (
// 		hsa_executable_t executable (struct)
// 		hsa_agent_t agent (struct)
// 		hsa_status_t (*)(hsa_executable_t, hsa_agent_t, hsa_executable_symbol_t, void *) callback (function)
// 		void * data (void)
// )
struct args_hsa_executable_iterate_agent_symbols_t {
	hsa_status_t retval;
	hsa_executable_t executable;
	hsa_agent_t agent;
	hsa_status_t (*callback)(hsa_executable_t, hsa_agent_t, hsa_executable_symbol_t, void *);
	void * data;
};

#define GET_ARGS_VALUE_hsa_executable_iterate_agent_symbols(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_iterate_agent_symbols.executable = (hsa_executable_t)executable; \
	__hsa_activity->hsa_args.hsa_executable_iterate_agent_symbols.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_executable_iterate_agent_symbols.callback = (hsa_status_t (*)(hsa_executable_t, hsa_agent_t, hsa_executable_symbol_t, void *))callback; \
	__hsa_activity->hsa_args.hsa_executable_iterate_agent_symbols.data = (void *)data; \
};

#define GET_PTRS_VALUE_hsa_executable_iterate_agent_symbols(args)

// hsa_status_t
// hsa_amd_deregister_deallocation_callback (
// 		void * ptr (void)
// 		hsa_amd_deallocation_callback_t callback (function)
// )
struct args_hsa_amd_deregister_deallocation_callback_t {
	hsa_status_t retval;
	void * ptr;
	hsa_amd_deallocation_callback_t callback;
};

#define GET_ARGS_VALUE_hsa_amd_deregister_deallocation_callback(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_deregister_deallocation_callback.ptr = (void *)ptr; \
	__hsa_activity->hsa_args.hsa_amd_deregister_deallocation_callback.callback = (hsa_amd_deallocation_callback_t)callback; \
};

#define GET_PTRS_VALUE_hsa_amd_deregister_deallocation_callback(args)

// hsa_status_t
// hsa_signal_create (
// 		hsa_signal_value_t initial_value (long)
// 		uint32_t num_consumers (unsigned int)
// 		const hsa_agent_t * consumers (struct)
// 		hsa_signal_t * signal (struct)
// )
struct args_hsa_signal_create_t {
	hsa_status_t retval;
	hsa_signal_value_t initial_value;
	uint32_t num_consumers;
	const hsa_agent_t * consumers;
	struct { // const hsa_agent_t *
		 hsa_agent_t val;
	} consumers__ref;
	hsa_signal_t * signal;
	struct { // hsa_signal_t *
		hsa_signal_t val;
	} signal__ref;
};

#define GET_ARGS_VALUE_hsa_signal_create(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_create.initial_value = (hsa_signal_value_t)initial_value; \
	__hsa_activity->hsa_args.hsa_signal_create.num_consumers = (uint32_t)num_consumers; \
	__hsa_activity->hsa_args.hsa_signal_create.consumers = (const hsa_agent_t *)consumers; \
	__hsa_activity->hsa_args.hsa_signal_create.signal = (hsa_signal_t *)signal; \
};

#define GET_PTRS_VALUE_hsa_signal_create(args) { \
	if (args->hsa_signal_create.consumers != NULL) { \
		args->hsa_signal_create.consumers__ref.val = *args->hsa_signal_create.consumers; \
	} \
	if (args->hsa_signal_create.signal != NULL) { \
		args->hsa_signal_create.signal__ref.val = *args->hsa_signal_create.signal; \
	} \
};


// hsa_status_t
// hsa_amd_memory_copy_engine_status (
// 		hsa_agent_t dst_agent (struct)
// 		hsa_agent_t src_agent (struct)
// 		uint32_t * engine_ids_mask (unsigned int)
// )
struct args_hsa_amd_memory_copy_engine_status_t {
	hsa_status_t retval;
	hsa_agent_t dst_agent;
	hsa_agent_t src_agent;
	uint32_t * engine_ids_mask;
	struct { // uint32_t *
		uint32_t val;
	} engine_ids_mask__ref;
};

#define GET_ARGS_VALUE_hsa_amd_memory_copy_engine_status(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_memory_copy_engine_status.dst_agent = (hsa_agent_t)dst_agent; \
	__hsa_activity->hsa_args.hsa_amd_memory_copy_engine_status.src_agent = (hsa_agent_t)src_agent; \
	__hsa_activity->hsa_args.hsa_amd_memory_copy_engine_status.engine_ids_mask = (uint32_t *)engine_ids_mask; \
};

#define GET_PTRS_VALUE_hsa_amd_memory_copy_engine_status(args) { \
	if (args->hsa_amd_memory_copy_engine_status.engine_ids_mask != NULL) { \
		args->hsa_amd_memory_copy_engine_status.engine_ids_mask__ref.val = *args->hsa_amd_memory_copy_engine_status.engine_ids_mask; \
	} \
};


// hsa_status_t
// hsa_ext_image_get_capability (
// 		hsa_agent_t agent (struct)
// 		hsa_ext_image_geometry_t geometry (enum)
// 		const hsa_ext_image_format_t * image_format (struct)
// 		uint32_t * capability_mask (unsigned int)
// )
struct args_hsa_ext_image_get_capability_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_ext_image_geometry_t geometry;
	const hsa_ext_image_format_t * image_format;
	struct { // const hsa_ext_image_format_t *
		 hsa_ext_image_format_t val;
	} image_format__ref;
	uint32_t * capability_mask;
	struct { // uint32_t *
		uint32_t val;
	} capability_mask__ref;
};

#define GET_ARGS_VALUE_hsa_ext_image_get_capability(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_ext_image_get_capability.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_ext_image_get_capability.geometry = (hsa_ext_image_geometry_t)geometry; \
	__hsa_activity->hsa_args.hsa_ext_image_get_capability.image_format = (const hsa_ext_image_format_t *)image_format; \
	__hsa_activity->hsa_args.hsa_ext_image_get_capability.capability_mask = (uint32_t *)capability_mask; \
};

#define GET_PTRS_VALUE_hsa_ext_image_get_capability(args) { \
	if (args->hsa_ext_image_get_capability.image_format != NULL) { \
		args->hsa_ext_image_get_capability.image_format__ref.val = *args->hsa_ext_image_get_capability.image_format; \
	} \
	if (args->hsa_ext_image_get_capability.capability_mask != NULL) { \
		args->hsa_ext_image_get_capability.capability_mask__ref.val = *args->hsa_ext_image_get_capability.capability_mask; \
	} \
};


// hsa_status_t
// hsa_ext_image_destroy (
// 		hsa_agent_t agent (struct)
// 		hsa_ext_image_t image (struct)
// )
struct args_hsa_ext_image_destroy_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_ext_image_t image;
};

#define GET_ARGS_VALUE_hsa_ext_image_destroy(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_ext_image_destroy.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_ext_image_destroy.image = (hsa_ext_image_t)image; \
};

#define GET_PTRS_VALUE_hsa_ext_image_destroy(args)

// void
// hsa_queue_store_read_index_release (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_store_read_index_release_t {
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_store_read_index_release(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_store_read_index_release.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_store_read_index_release.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_store_read_index_release(args) { \
	if (args->hsa_queue_store_read_index_release.queue != NULL) { \
		args->hsa_queue_store_read_index_release.queue__ref.val = *args->hsa_queue_store_read_index_release.queue; \
	} \
};


// hsa_signal_value_t
// hsa_signal_wait_scacquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_condition_t condition (enum)
// 		hsa_signal_value_t compare_value (long)
// 		uint64_t timeout_hint (unsigned long)
// 		hsa_wait_state_t wait_state_hint (enum)
// )
struct args_hsa_signal_wait_scacquire_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_condition_t condition;
	hsa_signal_value_t compare_value;
	uint64_t timeout_hint;
	hsa_wait_state_t wait_state_hint;
};

#define GET_ARGS_VALUE_hsa_signal_wait_scacquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_wait_scacquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_wait_scacquire.condition = (hsa_signal_condition_t)condition; \
	__hsa_activity->hsa_args.hsa_signal_wait_scacquire.compare_value = (hsa_signal_value_t)compare_value; \
	__hsa_activity->hsa_args.hsa_signal_wait_scacquire.timeout_hint = (uint64_t)timeout_hint; \
	__hsa_activity->hsa_args.hsa_signal_wait_scacquire.wait_state_hint = (hsa_wait_state_t)wait_state_hint; \
};

#define GET_PTRS_VALUE_hsa_signal_wait_scacquire(args)

// hsa_status_t
// hsa_executable_get_info (
// 		hsa_executable_t executable (struct)
// 		hsa_executable_info_t attribute (enum)
// 		void * value (void)
// )
struct args_hsa_executable_get_info_t {
	hsa_status_t retval;
	hsa_executable_t executable;
	hsa_executable_info_t attribute;
	void * value;
};

#define GET_ARGS_VALUE_hsa_executable_get_info(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_get_info.executable = (hsa_executable_t)executable; \
	__hsa_activity->hsa_args.hsa_executable_get_info.attribute = (hsa_executable_info_t)attribute; \
	__hsa_activity->hsa_args.hsa_executable_get_info.value = (void *)value; \
};

#define GET_PTRS_VALUE_hsa_executable_get_info(args)

// void
// hsa_signal_silent_store_screlease (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_silent_store_screlease_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_silent_store_screlease(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_silent_store_screlease.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_silent_store_screlease.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_silent_store_screlease(args)

// hsa_status_t
// hsa_executable_validate_alt (
// 		hsa_executable_t executable (struct)
// 		const char * options (string)
// 		uint32_t * result (unsigned int)
// )
struct args_hsa_executable_validate_alt_t {
	hsa_status_t retval;
	hsa_executable_t executable;
	const char * options;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} options__ref;
	uint32_t * result;
	struct { // uint32_t *
		uint32_t val;
	} result__ref;
};

#define GET_ARGS_VALUE_hsa_executable_validate_alt(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_validate_alt.executable = (hsa_executable_t)executable; \
	__hsa_activity->hsa_args.hsa_executable_validate_alt.options = (const char *)options; \
	__hsa_activity->hsa_args.hsa_executable_validate_alt.result = (uint32_t *)result; \
};

#define GET_PTRS_VALUE_hsa_executable_validate_alt(args) { \
	if (args->hsa_executable_validate_alt.options != NULL) { \
		strncpy(args->hsa_executable_validate_alt.options__ref.val, args->hsa_executable_validate_alt.options, HSA_STRING_SIZE_MAX-1); \
	} \
	if (args->hsa_executable_validate_alt.result != NULL) { \
		args->hsa_executable_validate_alt.result__ref.val = *args->hsa_executable_validate_alt.result; \
	} \
};


// hsa_status_t
// hsa_executable_create (
// 		hsa_profile_t profile (enum)
// 		hsa_executable_state_t executable_state (enum)
// 		const char * options (string)
// 		hsa_executable_t * executable (struct)
// )
struct args_hsa_executable_create_t {
	hsa_status_t retval;
	hsa_profile_t profile;
	hsa_executable_state_t executable_state;
	const char * options;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} options__ref;
	hsa_executable_t * executable;
	struct { // hsa_executable_t *
		hsa_executable_t val;
	} executable__ref;
};

#define GET_ARGS_VALUE_hsa_executable_create(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_create.profile = (hsa_profile_t)profile; \
	__hsa_activity->hsa_args.hsa_executable_create.executable_state = (hsa_executable_state_t)executable_state; \
	__hsa_activity->hsa_args.hsa_executable_create.options = (const char *)options; \
	__hsa_activity->hsa_args.hsa_executable_create.executable = (hsa_executable_t *)executable; \
};

#define GET_PTRS_VALUE_hsa_executable_create(args) { \
	if (args->hsa_executable_create.options != NULL) { \
		strncpy(args->hsa_executable_create.options__ref.val, args->hsa_executable_create.options, HSA_STRING_SIZE_MAX-1); \
	} \
	if (args->hsa_executable_create.executable != NULL) { \
		args->hsa_executable_create.executable__ref.val = *args->hsa_executable_create.executable; \
	} \
};


// hsa_status_t
// hsa_executable_iterate_program_symbols (
// 		hsa_executable_t executable (struct)
// 		hsa_status_t (*)(hsa_executable_t, hsa_executable_symbol_t, void *) callback (function)
// 		void * data (void)
// )
struct args_hsa_executable_iterate_program_symbols_t {
	hsa_status_t retval;
	hsa_executable_t executable;
	hsa_status_t (*callback)(hsa_executable_t, hsa_executable_symbol_t, void *);
	void * data;
};

#define GET_ARGS_VALUE_hsa_executable_iterate_program_symbols(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_iterate_program_symbols.executable = (hsa_executable_t)executable; \
	__hsa_activity->hsa_args.hsa_executable_iterate_program_symbols.callback = (hsa_status_t (*)(hsa_executable_t, hsa_executable_symbol_t, void *))callback; \
	__hsa_activity->hsa_args.hsa_executable_iterate_program_symbols.data = (void *)data; \
};

#define GET_PTRS_VALUE_hsa_executable_iterate_program_symbols(args)

// hsa_status_t
// hsa_memory_free (
// 		void * ptr (void)
// )
struct args_hsa_memory_free_t {
	hsa_status_t retval;
	void * ptr;
};

#define GET_ARGS_VALUE_hsa_memory_free(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_memory_free.ptr = (void *)ptr; \
};

#define GET_PTRS_VALUE_hsa_memory_free(args)

// hsa_status_t
// hsa_amd_memory_pool_free (
// 		void * ptr (void)
// )
struct args_hsa_amd_memory_pool_free_t {
	hsa_status_t retval;
	void * ptr;
};

#define GET_ARGS_VALUE_hsa_amd_memory_pool_free(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_memory_pool_free.ptr = (void *)ptr; \
};

#define GET_PTRS_VALUE_hsa_amd_memory_pool_free(args)

// hsa_status_t
// hsa_amd_memory_migrate (
// 		const void * ptr (void)
// 		hsa_amd_memory_pool_t memory_pool (struct)
// 		uint32_t flags (unsigned int)
// )
struct args_hsa_amd_memory_migrate_t {
	hsa_status_t retval;
	const void * ptr;
	hsa_amd_memory_pool_t memory_pool;
	uint32_t flags;
};

#define GET_ARGS_VALUE_hsa_amd_memory_migrate(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_memory_migrate.ptr = (const void *)ptr; \
	__hsa_activity->hsa_args.hsa_amd_memory_migrate.memory_pool = (hsa_amd_memory_pool_t)memory_pool; \
	__hsa_activity->hsa_args.hsa_amd_memory_migrate.flags = (uint32_t)flags; \
};

#define GET_PTRS_VALUE_hsa_amd_memory_migrate(args)

// hsa_status_t
// hsa_amd_interop_unmap_buffer (
// 		void * ptr (void)
// )
struct args_hsa_amd_interop_unmap_buffer_t {
	hsa_status_t retval;
	void * ptr;
};

#define GET_ARGS_VALUE_hsa_amd_interop_unmap_buffer(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_interop_unmap_buffer.ptr = (void *)ptr; \
};

#define GET_PTRS_VALUE_hsa_amd_interop_unmap_buffer(args)

// hsa_status_t
// hsa_amd_signal_create (
// 		hsa_signal_value_t initial_value (long)
// 		uint32_t num_consumers (unsigned int)
// 		const hsa_agent_t * consumers (struct)
// 		uint64_t attributes (unsigned long)
// 		hsa_signal_t * signal (struct)
// )
struct args_hsa_amd_signal_create_t {
	hsa_status_t retval;
	hsa_signal_value_t initial_value;
	uint32_t num_consumers;
	const hsa_agent_t * consumers;
	struct { // const hsa_agent_t *
		 hsa_agent_t val;
	} consumers__ref;
	uint64_t attributes;
	hsa_signal_t * signal;
	struct { // hsa_signal_t *
		hsa_signal_t val;
	} signal__ref;
};

#define GET_ARGS_VALUE_hsa_amd_signal_create(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_signal_create.initial_value = (hsa_signal_value_t)initial_value; \
	__hsa_activity->hsa_args.hsa_amd_signal_create.num_consumers = (uint32_t)num_consumers; \
	__hsa_activity->hsa_args.hsa_amd_signal_create.consumers = (const hsa_agent_t *)consumers; \
	__hsa_activity->hsa_args.hsa_amd_signal_create.attributes = (uint64_t)attributes; \
	__hsa_activity->hsa_args.hsa_amd_signal_create.signal = (hsa_signal_t *)signal; \
};

#define GET_PTRS_VALUE_hsa_amd_signal_create(args) { \
	if (args->hsa_amd_signal_create.consumers != NULL) { \
		args->hsa_amd_signal_create.consumers__ref.val = *args->hsa_amd_signal_create.consumers; \
	} \
	if (args->hsa_amd_signal_create.signal != NULL) { \
		args->hsa_amd_signal_create.signal__ref.val = *args->hsa_amd_signal_create.signal; \
	} \
};


// hsa_status_t
// hsa_executable_load_program_code_object (
// 		hsa_executable_t executable (struct)
// 		hsa_code_object_reader_t code_object_reader (struct)
// 		const char * options (string)
// 		hsa_loaded_code_object_t * loaded_code_object (struct)
// )
struct args_hsa_executable_load_program_code_object_t {
	hsa_status_t retval;
	hsa_executable_t executable;
	hsa_code_object_reader_t code_object_reader;
	const char * options;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} options__ref;
	hsa_loaded_code_object_t * loaded_code_object;
	struct { // hsa_loaded_code_object_t *
		hsa_loaded_code_object_t val;
	} loaded_code_object__ref;
};

#define GET_ARGS_VALUE_hsa_executable_load_program_code_object(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_load_program_code_object.executable = (hsa_executable_t)executable; \
	__hsa_activity->hsa_args.hsa_executable_load_program_code_object.code_object_reader = (hsa_code_object_reader_t)code_object_reader; \
	__hsa_activity->hsa_args.hsa_executable_load_program_code_object.options = (const char *)options; \
	__hsa_activity->hsa_args.hsa_executable_load_program_code_object.loaded_code_object = (hsa_loaded_code_object_t *)loaded_code_object; \
};

#define GET_PTRS_VALUE_hsa_executable_load_program_code_object(args) { \
	if (args->hsa_executable_load_program_code_object.options != NULL) { \
		strncpy(args->hsa_executable_load_program_code_object.options__ref.val, args->hsa_executable_load_program_code_object.options, HSA_STRING_SIZE_MAX-1); \
	} \
	if (args->hsa_executable_load_program_code_object.loaded_code_object != NULL) { \
		args->hsa_executable_load_program_code_object.loaded_code_object__ref.val = *args->hsa_executable_load_program_code_object.loaded_code_object; \
	} \
};


// hsa_signal_value_t
// hsa_signal_load_relaxed (
// 		hsa_signal_t signal (struct)
// )
struct args_hsa_signal_load_relaxed_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
};

#define GET_ARGS_VALUE_hsa_signal_load_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_load_relaxed.signal = (hsa_signal_t)signal; \
};

#define GET_PTRS_VALUE_hsa_signal_load_relaxed(args)

// hsa_status_t
// hsa_signal_group_wait_any_scacquire (
// 		hsa_signal_group_t signal_group (struct)
// 		const hsa_signal_condition_t * conditions (enum)
// 		const hsa_signal_value_t * compare_values (long)
// 		hsa_wait_state_t wait_state_hint (enum)
// 		hsa_signal_t * signal (struct)
// 		hsa_signal_value_t * value (long)
// )
struct args_hsa_signal_group_wait_any_scacquire_t {
	hsa_status_t retval;
	hsa_signal_group_t signal_group;
	const hsa_signal_condition_t * conditions;
	struct { // const hsa_signal_condition_t *
		 hsa_signal_condition_t val;
	} conditions__ref;
	const hsa_signal_value_t * compare_values;
	struct { // const hsa_signal_value_t *
		 hsa_signal_value_t val;
	} compare_values__ref;
	hsa_wait_state_t wait_state_hint;
	hsa_signal_t * signal;
	struct { // hsa_signal_t *
		hsa_signal_t val;
	} signal__ref;
	hsa_signal_value_t * value;
	struct { // hsa_signal_value_t *
		hsa_signal_value_t val;
	} value__ref;
};

#define GET_ARGS_VALUE_hsa_signal_group_wait_any_scacquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_group_wait_any_scacquire.signal_group = (hsa_signal_group_t)signal_group; \
	__hsa_activity->hsa_args.hsa_signal_group_wait_any_scacquire.conditions = (const hsa_signal_condition_t *)conditions; \
	__hsa_activity->hsa_args.hsa_signal_group_wait_any_scacquire.compare_values = (const hsa_signal_value_t *)compare_values; \
	__hsa_activity->hsa_args.hsa_signal_group_wait_any_scacquire.wait_state_hint = (hsa_wait_state_t)wait_state_hint; \
	__hsa_activity->hsa_args.hsa_signal_group_wait_any_scacquire.signal = (hsa_signal_t *)signal; \
	__hsa_activity->hsa_args.hsa_signal_group_wait_any_scacquire.value = (hsa_signal_value_t *)value; \
};

#define GET_PTRS_VALUE_hsa_signal_group_wait_any_scacquire(args) { \
	if (args->hsa_signal_group_wait_any_scacquire.conditions != NULL) { \
		args->hsa_signal_group_wait_any_scacquire.conditions__ref.val = *args->hsa_signal_group_wait_any_scacquire.conditions; \
	} \
	if (args->hsa_signal_group_wait_any_scacquire.compare_values != NULL) { \
		args->hsa_signal_group_wait_any_scacquire.compare_values__ref.val = *args->hsa_signal_group_wait_any_scacquire.compare_values; \
	} \
	if (args->hsa_signal_group_wait_any_scacquire.signal != NULL) { \
		args->hsa_signal_group_wait_any_scacquire.signal__ref.val = *args->hsa_signal_group_wait_any_scacquire.signal; \
	} \
	if (args->hsa_signal_group_wait_any_scacquire.value != NULL) { \
		args->hsa_signal_group_wait_any_scacquire.value__ref.val = *args->hsa_signal_group_wait_any_scacquire.value; \
	} \
};


// uint64_t
// hsa_queue_cas_write_index_relaxed (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t expected (unsigned long)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_cas_write_index_relaxed_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t expected;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_cas_write_index_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_relaxed.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_relaxed.expected = (uint64_t)expected; \
	__hsa_activity->hsa_args.hsa_queue_cas_write_index_relaxed.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_cas_write_index_relaxed(args) { \
	if (args->hsa_queue_cas_write_index_relaxed.queue != NULL) { \
		args->hsa_queue_cas_write_index_relaxed.queue__ref.val = *args->hsa_queue_cas_write_index_relaxed.queue; \
	} \
};


// void
// hsa_signal_subtract_relaxed (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_subtract_relaxed_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_subtract_relaxed(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_subtract_relaxed.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_subtract_relaxed.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_subtract_relaxed(args)

// hsa_status_t
// hsa_executable_freeze (
// 		hsa_executable_t executable (struct)
// 		const char * options (string)
// )
struct args_hsa_executable_freeze_t {
	hsa_status_t retval;
	hsa_executable_t executable;
	const char * options;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} options__ref;
};

#define GET_ARGS_VALUE_hsa_executable_freeze(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_freeze.executable = (hsa_executable_t)executable; \
	__hsa_activity->hsa_args.hsa_executable_freeze.options = (const char *)options; \
};

#define GET_PTRS_VALUE_hsa_executable_freeze(args) { \
	if (args->hsa_executable_freeze.options != NULL) { \
		strncpy(args->hsa_executable_freeze.options__ref.val, args->hsa_executable_freeze.options, HSA_STRING_SIZE_MAX-1); \
	} \
};


// uint64_t
// hsa_queue_load_write_index_acquire (
// 		const hsa_queue_t * queue (struct)
// )
struct args_hsa_queue_load_write_index_acquire_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
};

#define GET_ARGS_VALUE_hsa_queue_load_write_index_acquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_load_write_index_acquire.queue = (const hsa_queue_t *)queue; \
};

#define GET_PTRS_VALUE_hsa_queue_load_write_index_acquire(args) { \
	if (args->hsa_queue_load_write_index_acquire.queue != NULL) { \
		args->hsa_queue_load_write_index_acquire.queue__ref.val = *args->hsa_queue_load_write_index_acquire.queue; \
	} \
};


// hsa_status_t
// hsa_amd_vmem_map (
// 		void * va (void)
// 		size_t size (unsigned long)
// 		size_t in_offset (unsigned long)
// 		hsa_amd_vmem_alloc_handle_t memory_handle (struct)
// 		uint64_t flags (unsigned long)
// )
struct args_hsa_amd_vmem_map_t {
	hsa_status_t retval;
	void * va;
	size_t size;
	size_t in_offset;
	hsa_amd_vmem_alloc_handle_t memory_handle;
	uint64_t flags;
};

#define GET_ARGS_VALUE_hsa_amd_vmem_map(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_vmem_map.va = (void *)va; \
	__hsa_activity->hsa_args.hsa_amd_vmem_map.size = (size_t)size; \
	__hsa_activity->hsa_args.hsa_amd_vmem_map.in_offset = (size_t)in_offset; \
	__hsa_activity->hsa_args.hsa_amd_vmem_map.memory_handle = (hsa_amd_vmem_alloc_handle_t)memory_handle; \
	__hsa_activity->hsa_args.hsa_amd_vmem_map.flags = (uint64_t)flags; \
};

#define GET_PTRS_VALUE_hsa_amd_vmem_map(args)

// hsa_status_t
// hsa_amd_ipc_signal_attach (
// 		const hsa_amd_ipc_signal_t * handle (struct)
// 		hsa_signal_t * signal (struct)
// )
struct args_hsa_amd_ipc_signal_attach_t {
	hsa_status_t retval;
	const hsa_amd_ipc_signal_t * handle;
	struct { // const hsa_amd_ipc_signal_t *
		 hsa_amd_ipc_signal_t val;
	} handle__ref;
	hsa_signal_t * signal;
	struct { // hsa_signal_t *
		hsa_signal_t val;
	} signal__ref;
};

#define GET_ARGS_VALUE_hsa_amd_ipc_signal_attach(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_ipc_signal_attach.handle = (const hsa_amd_ipc_signal_t *)handle; \
	__hsa_activity->hsa_args.hsa_amd_ipc_signal_attach.signal = (hsa_signal_t *)signal; \
};

#define GET_PTRS_VALUE_hsa_amd_ipc_signal_attach(args) { \
	if (args->hsa_amd_ipc_signal_attach.handle != NULL) { \
		args->hsa_amd_ipc_signal_attach.handle__ref.val = *args->hsa_amd_ipc_signal_attach.handle; \
	} \
	if (args->hsa_amd_ipc_signal_attach.signal != NULL) { \
		args->hsa_amd_ipc_signal_attach.signal__ref.val = *args->hsa_amd_ipc_signal_attach.signal; \
	} \
};


// hsa_status_t
// hsa_amd_register_system_event_handler (
// 		hsa_amd_system_event_callback_t callback (function)
// 		void * data (void)
// )
struct args_hsa_amd_register_system_event_handler_t {
	hsa_status_t retval;
	hsa_amd_system_event_callback_t callback;
	void * data;
};

#define GET_ARGS_VALUE_hsa_amd_register_system_event_handler(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_register_system_event_handler.callback = (hsa_amd_system_event_callback_t)callback; \
	__hsa_activity->hsa_args.hsa_amd_register_system_event_handler.data = (void *)data; \
};

#define GET_PTRS_VALUE_hsa_amd_register_system_event_handler(args)

// hsa_status_t
// hsa_amd_queue_cu_set_mask (
// 		const hsa_queue_t * queue (struct)
// 		uint32_t num_cu_mask_count (unsigned int)
// 		const uint32_t * cu_mask (unsigned int)
// )
struct args_hsa_amd_queue_cu_set_mask_t {
	hsa_status_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint32_t num_cu_mask_count;
	const uint32_t * cu_mask;
	struct { // const uint32_t *
		 uint32_t val;
	} cu_mask__ref;
};

#define GET_ARGS_VALUE_hsa_amd_queue_cu_set_mask(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_queue_cu_set_mask.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_amd_queue_cu_set_mask.num_cu_mask_count = (uint32_t)num_cu_mask_count; \
	__hsa_activity->hsa_args.hsa_amd_queue_cu_set_mask.cu_mask = (const uint32_t *)cu_mask; \
};

#define GET_PTRS_VALUE_hsa_amd_queue_cu_set_mask(args) { \
	if (args->hsa_amd_queue_cu_set_mask.queue != NULL) { \
		args->hsa_amd_queue_cu_set_mask.queue__ref.val = *args->hsa_amd_queue_cu_set_mask.queue; \
	} \
	if (args->hsa_amd_queue_cu_set_mask.cu_mask != NULL) { \
		args->hsa_amd_queue_cu_set_mask.cu_mask__ref.val = *args->hsa_amd_queue_cu_set_mask.cu_mask; \
	} \
};


// hsa_status_t
// hsa_ext_image_export (
// 		hsa_agent_t agent (struct)
// 		hsa_ext_image_t src_image (struct)
// 		void * dst_memory (void)
// 		size_t dst_row_pitch (unsigned long)
// 		size_t dst_slice_pitch (unsigned long)
// 		const hsa_ext_image_region_t * image_region (struct)
// )
struct args_hsa_ext_image_export_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_ext_image_t src_image;
	void * dst_memory;
	size_t dst_row_pitch;
	size_t dst_slice_pitch;
	const hsa_ext_image_region_t * image_region;
	struct { // const hsa_ext_image_region_t *
		 hsa_ext_image_region_t val;
	} image_region__ref;
};

#define GET_ARGS_VALUE_hsa_ext_image_export(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_ext_image_export.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_ext_image_export.src_image = (hsa_ext_image_t)src_image; \
	__hsa_activity->hsa_args.hsa_ext_image_export.dst_memory = (void *)dst_memory; \
	__hsa_activity->hsa_args.hsa_ext_image_export.dst_row_pitch = (size_t)dst_row_pitch; \
	__hsa_activity->hsa_args.hsa_ext_image_export.dst_slice_pitch = (size_t)dst_slice_pitch; \
	__hsa_activity->hsa_args.hsa_ext_image_export.image_region = (const hsa_ext_image_region_t *)image_region; \
};

#define GET_PTRS_VALUE_hsa_ext_image_export(args) { \
	if (args->hsa_ext_image_export.image_region != NULL) { \
		args->hsa_ext_image_export.image_region__ref.val = *args->hsa_ext_image_export.image_region; \
	} \
};


// hsa_status_t
// hsa_status_string (
// 		hsa_status_t status (enum)
// 		const char ** status_string (string)
// )
struct args_hsa_status_string_t {
	hsa_status_t retval;
	hsa_status_t status;
	const char ** status_string;
	struct { // const char **
		 char* ptr1;
		char val[HSA_STRING_SIZE_MAX];
	} status_string__ref;
};

#define GET_ARGS_VALUE_hsa_status_string(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_status_string.status = (hsa_status_t)status; \
	__hsa_activity->hsa_args.hsa_status_string.status_string = (const char **)status_string; \
};

#define GET_PTRS_VALUE_hsa_status_string(args) { \
	if (args->hsa_status_string.status_string != NULL) { \
		args->hsa_status_string.status_string__ref.ptr1 = (void*)*args->hsa_status_string.status_string; \
		if (args->hsa_status_string.status_string__ref.ptr1 != NULL) { \
			strncpy(args->hsa_status_string.status_string__ref.val, args->hsa_status_string.status_string__ref.ptr1, HSA_STRING_SIZE_MAX-1); \
		} \
	} \
};


// void
// hsa_signal_xor_screlease (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_xor_screlease_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_xor_screlease(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_xor_screlease.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_xor_screlease.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_xor_screlease(args)

// uint64_t
// hsa_queue_add_write_index_acquire (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_add_write_index_acquire_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_add_write_index_acquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_add_write_index_acquire.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_add_write_index_acquire.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_add_write_index_acquire(args) { \
	if (args->hsa_queue_add_write_index_acquire.queue != NULL) { \
		args->hsa_queue_add_write_index_acquire.queue__ref.val = *args->hsa_queue_add_write_index_acquire.queue; \
	} \
};


// uint64_t
// hsa_queue_add_write_index_screlease (
// 		const hsa_queue_t * queue (struct)
// 		uint64_t value (unsigned long)
// )
struct args_hsa_queue_add_write_index_screlease_t {
	uint64_t retval;
	const hsa_queue_t * queue;
	struct { // const hsa_queue_t *
		 hsa_queue_t val;
	} queue__ref;
	uint64_t value;
};

#define GET_ARGS_VALUE_hsa_queue_add_write_index_screlease(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_queue_add_write_index_screlease.queue = (const hsa_queue_t *)queue; \
	__hsa_activity->hsa_args.hsa_queue_add_write_index_screlease.value = (uint64_t)value; \
};

#define GET_PTRS_VALUE_hsa_queue_add_write_index_screlease(args) { \
	if (args->hsa_queue_add_write_index_screlease.queue != NULL) { \
		args->hsa_queue_add_write_index_screlease.queue__ref.val = *args->hsa_queue_add_write_index_screlease.queue; \
	} \
};


// hsa_status_t
// hsa_code_object_get_symbol_from_name (
// 		hsa_code_object_t code_object (struct)
// 		const char * module_name (string)
// 		const char * symbol_name (string)
// 		hsa_code_symbol_t * symbol (struct)
// )
struct args_hsa_code_object_get_symbol_from_name_t {
	hsa_status_t retval;
	hsa_code_object_t code_object;
	const char * module_name;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} module_name__ref;
	const char * symbol_name;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} symbol_name__ref;
	hsa_code_symbol_t * symbol;
	struct { // hsa_code_symbol_t *
		hsa_code_symbol_t val;
	} symbol__ref;
};

#define GET_ARGS_VALUE_hsa_code_object_get_symbol_from_name(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_code_object_get_symbol_from_name.code_object = (hsa_code_object_t)code_object; \
	__hsa_activity->hsa_args.hsa_code_object_get_symbol_from_name.module_name = (const char *)module_name; \
	__hsa_activity->hsa_args.hsa_code_object_get_symbol_from_name.symbol_name = (const char *)symbol_name; \
	__hsa_activity->hsa_args.hsa_code_object_get_symbol_from_name.symbol = (hsa_code_symbol_t *)symbol; \
};

#define GET_PTRS_VALUE_hsa_code_object_get_symbol_from_name(args) { \
	if (args->hsa_code_object_get_symbol_from_name.module_name != NULL) { \
		strncpy(args->hsa_code_object_get_symbol_from_name.module_name__ref.val, args->hsa_code_object_get_symbol_from_name.module_name, HSA_STRING_SIZE_MAX-1); \
	} \
	if (args->hsa_code_object_get_symbol_from_name.symbol_name != NULL) { \
		strncpy(args->hsa_code_object_get_symbol_from_name.symbol_name__ref.val, args->hsa_code_object_get_symbol_from_name.symbol_name, HSA_STRING_SIZE_MAX-1); \
	} \
	if (args->hsa_code_object_get_symbol_from_name.symbol != NULL) { \
		args->hsa_code_object_get_symbol_from_name.symbol__ref.val = *args->hsa_code_object_get_symbol_from_name.symbol; \
	} \
};


// hsa_status_t
// hsa_isa_from_name (
// 		const char * name (string)
// 		hsa_isa_t * isa (struct)
// )
struct args_hsa_isa_from_name_t {
	hsa_status_t retval;
	const char * name;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} name__ref;
	hsa_isa_t * isa;
	struct { // hsa_isa_t *
		hsa_isa_t val;
	} isa__ref;
};

#define GET_ARGS_VALUE_hsa_isa_from_name(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_isa_from_name.name = (const char *)name; \
	__hsa_activity->hsa_args.hsa_isa_from_name.isa = (hsa_isa_t *)isa; \
};

#define GET_PTRS_VALUE_hsa_isa_from_name(args) { \
	if (args->hsa_isa_from_name.name != NULL) { \
		strncpy(args->hsa_isa_from_name.name__ref.val, args->hsa_isa_from_name.name, HSA_STRING_SIZE_MAX-1); \
	} \
	if (args->hsa_isa_from_name.isa != NULL) { \
		args->hsa_isa_from_name.isa__ref.val = *args->hsa_isa_from_name.isa; \
	} \
};


// hsa_status_t
// hsa_amd_agent_iterate_memory_pools (
// 		hsa_agent_t agent (struct)
// 		hsa_status_t (*)(hsa_amd_memory_pool_t, void *) callback (function)
// 		void * data (void)
// )
struct args_hsa_amd_agent_iterate_memory_pools_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_status_t (*callback)(hsa_amd_memory_pool_t, void *);
	void * data;
};

#define GET_ARGS_VALUE_hsa_amd_agent_iterate_memory_pools(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_agent_iterate_memory_pools.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_amd_agent_iterate_memory_pools.callback = (hsa_status_t (*)(hsa_amd_memory_pool_t, void *))callback; \
	__hsa_activity->hsa_args.hsa_amd_agent_iterate_memory_pools.data = (void *)data; \
};

#define GET_PTRS_VALUE_hsa_amd_agent_iterate_memory_pools(args)

// hsa_status_t
// hsa_amd_agent_set_async_scratch_limit (
// 		hsa_agent_t agent (struct)
// 		size_t threshold (unsigned long)
// )
struct args_hsa_amd_agent_set_async_scratch_limit_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	size_t threshold;
};

#define GET_ARGS_VALUE_hsa_amd_agent_set_async_scratch_limit(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_agent_set_async_scratch_limit.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_amd_agent_set_async_scratch_limit.threshold = (size_t)threshold; \
};

#define GET_PTRS_VALUE_hsa_amd_agent_set_async_scratch_limit(args)

// hsa_status_t
// hsa_ext_image_create (
// 		hsa_agent_t agent (struct)
// 		const hsa_ext_image_descriptor_t * image_descriptor (struct)
// 		const void * image_data (void)
// 		hsa_access_permission_t access_permission (enum)
// 		hsa_ext_image_t * image (struct)
// )
struct args_hsa_ext_image_create_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	const hsa_ext_image_descriptor_t * image_descriptor;
	struct { // const hsa_ext_image_descriptor_t *
		 hsa_ext_image_descriptor_t val;
	} image_descriptor__ref;
	const void * image_data;
	hsa_access_permission_t access_permission;
	hsa_ext_image_t * image;
	struct { // hsa_ext_image_t *
		hsa_ext_image_t val;
	} image__ref;
};

#define GET_ARGS_VALUE_hsa_ext_image_create(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_ext_image_create.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_ext_image_create.image_descriptor = (const hsa_ext_image_descriptor_t *)image_descriptor; \
	__hsa_activity->hsa_args.hsa_ext_image_create.image_data = (const void *)image_data; \
	__hsa_activity->hsa_args.hsa_ext_image_create.access_permission = (hsa_access_permission_t)access_permission; \
	__hsa_activity->hsa_args.hsa_ext_image_create.image = (hsa_ext_image_t *)image; \
};

#define GET_PTRS_VALUE_hsa_ext_image_create(args) { \
	if (args->hsa_ext_image_create.image_descriptor != NULL) { \
		args->hsa_ext_image_create.image_descriptor__ref.val = *args->hsa_ext_image_create.image_descriptor; \
	} \
	if (args->hsa_ext_image_create.image != NULL) { \
		args->hsa_ext_image_create.image__ref.val = *args->hsa_ext_image_create.image; \
	} \
};


// hsa_signal_value_t
// hsa_signal_exchange_scacq_screl (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_exchange_scacq_screl_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_exchange_scacq_screl(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_exchange_scacq_screl.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_exchange_scacq_screl.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_exchange_scacq_screl(args)

// hsa_status_t
// hsa_init (
// )
struct args_hsa_init_t {
	hsa_status_t retval;

};

#define GET_ARGS_VALUE_hsa_init(__hsa_activity)
#define GET_PTRS_VALUE_hsa_init(args)

// hsa_status_t
// hsa_executable_load_agent_code_object (
// 		hsa_executable_t executable (struct)
// 		hsa_agent_t agent (struct)
// 		hsa_code_object_reader_t code_object_reader (struct)
// 		const char * options (string)
// 		hsa_loaded_code_object_t * loaded_code_object (struct)
// )
struct args_hsa_executable_load_agent_code_object_t {
	hsa_status_t retval;
	hsa_executable_t executable;
	hsa_agent_t agent;
	hsa_code_object_reader_t code_object_reader;
	const char * options;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} options__ref;
	hsa_loaded_code_object_t * loaded_code_object;
	struct { // hsa_loaded_code_object_t *
		hsa_loaded_code_object_t val;
	} loaded_code_object__ref;
};

#define GET_ARGS_VALUE_hsa_executable_load_agent_code_object(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_load_agent_code_object.executable = (hsa_executable_t)executable; \
	__hsa_activity->hsa_args.hsa_executable_load_agent_code_object.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_executable_load_agent_code_object.code_object_reader = (hsa_code_object_reader_t)code_object_reader; \
	__hsa_activity->hsa_args.hsa_executable_load_agent_code_object.options = (const char *)options; \
	__hsa_activity->hsa_args.hsa_executable_load_agent_code_object.loaded_code_object = (hsa_loaded_code_object_t *)loaded_code_object; \
};

#define GET_PTRS_VALUE_hsa_executable_load_agent_code_object(args) { \
	if (args->hsa_executable_load_agent_code_object.options != NULL) { \
		strncpy(args->hsa_executable_load_agent_code_object.options__ref.val, args->hsa_executable_load_agent_code_object.options, HSA_STRING_SIZE_MAX-1); \
	} \
	if (args->hsa_executable_load_agent_code_object.loaded_code_object != NULL) { \
		args->hsa_executable_load_agent_code_object.loaded_code_object__ref.val = *args->hsa_executable_load_agent_code_object.loaded_code_object; \
	} \
};


// hsa_signal_value_t
// hsa_signal_cas_screlease (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t expected (long)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_cas_screlease_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_value_t expected;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_cas_screlease(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_cas_screlease.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_cas_screlease.expected = (hsa_signal_value_t)expected; \
	__hsa_activity->hsa_args.hsa_signal_cas_screlease.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_cas_screlease(args)

// hsa_status_t
// hsa_amd_profiling_get_dispatch_time (
// 		hsa_agent_t agent (struct)
// 		hsa_signal_t signal (struct)
// 		hsa_amd_profiling_dispatch_time_t * time (struct)
// )
struct args_hsa_amd_profiling_get_dispatch_time_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	hsa_signal_t signal;
	hsa_amd_profiling_dispatch_time_t * time;
	struct { // hsa_amd_profiling_dispatch_time_t *
		hsa_amd_profiling_dispatch_time_t val;
	} time__ref;
};

#define GET_ARGS_VALUE_hsa_amd_profiling_get_dispatch_time(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_profiling_get_dispatch_time.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_amd_profiling_get_dispatch_time.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_amd_profiling_get_dispatch_time.time = (hsa_amd_profiling_dispatch_time_t *)time; \
};

#define GET_PTRS_VALUE_hsa_amd_profiling_get_dispatch_time(args) { \
	if (args->hsa_amd_profiling_get_dispatch_time.time != NULL) { \
		args->hsa_amd_profiling_get_dispatch_time.time__ref.val = *args->hsa_amd_profiling_get_dispatch_time.time; \
	} \
};


// hsa_status_t
// hsa_amd_portable_close_dmabuf (
// 		int dmabuf (int)
// )
struct args_hsa_amd_portable_close_dmabuf_t {
	hsa_status_t retval;
	int dmabuf;
};

#define GET_ARGS_VALUE_hsa_amd_portable_close_dmabuf(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_portable_close_dmabuf.dmabuf = (int)dmabuf; \
};

#define GET_PTRS_VALUE_hsa_amd_portable_close_dmabuf(args)

// hsa_status_t
// hsa_memory_copy (
// 		void * dst (void)
// 		const void * src (void)
// 		size_t size (unsigned long)
// )
struct args_hsa_memory_copy_t {
	hsa_status_t retval;
	void * dst;
	const void * src;
	size_t size;
};

#define GET_ARGS_VALUE_hsa_memory_copy(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_memory_copy.dst = (void *)dst; \
	__hsa_activity->hsa_args.hsa_memory_copy.src = (const void *)src; \
	__hsa_activity->hsa_args.hsa_memory_copy.size = (size_t)size; \
};

#define GET_PTRS_VALUE_hsa_memory_copy(args)

// hsa_signal_value_t
// hsa_signal_exchange_scacquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_exchange_scacquire_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_exchange_scacquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_exchange_scacquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_exchange_scacquire.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_exchange_scacquire(args)

// hsa_status_t
// hsa_ext_image_data_get_info_with_layout (
// 		hsa_agent_t agent (struct)
// 		const hsa_ext_image_descriptor_t * image_descriptor (struct)
// 		hsa_access_permission_t access_permission (enum)
// 		hsa_ext_image_data_layout_t image_data_layout (enum)
// 		size_t image_data_row_pitch (unsigned long)
// 		size_t image_data_slice_pitch (unsigned long)
// 		hsa_ext_image_data_info_t * image_data_info (struct)
// )
struct args_hsa_ext_image_data_get_info_with_layout_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	const hsa_ext_image_descriptor_t * image_descriptor;
	struct { // const hsa_ext_image_descriptor_t *
		 hsa_ext_image_descriptor_t val;
	} image_descriptor__ref;
	hsa_access_permission_t access_permission;
	hsa_ext_image_data_layout_t image_data_layout;
	size_t image_data_row_pitch;
	size_t image_data_slice_pitch;
	hsa_ext_image_data_info_t * image_data_info;
	struct { // hsa_ext_image_data_info_t *
		hsa_ext_image_data_info_t val;
	} image_data_info__ref;
};

#define GET_ARGS_VALUE_hsa_ext_image_data_get_info_with_layout(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_ext_image_data_get_info_with_layout.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_ext_image_data_get_info_with_layout.image_descriptor = (const hsa_ext_image_descriptor_t *)image_descriptor; \
	__hsa_activity->hsa_args.hsa_ext_image_data_get_info_with_layout.access_permission = (hsa_access_permission_t)access_permission; \
	__hsa_activity->hsa_args.hsa_ext_image_data_get_info_with_layout.image_data_layout = (hsa_ext_image_data_layout_t)image_data_layout; \
	__hsa_activity->hsa_args.hsa_ext_image_data_get_info_with_layout.image_data_row_pitch = (size_t)image_data_row_pitch; \
	__hsa_activity->hsa_args.hsa_ext_image_data_get_info_with_layout.image_data_slice_pitch = (size_t)image_data_slice_pitch; \
	__hsa_activity->hsa_args.hsa_ext_image_data_get_info_with_layout.image_data_info = (hsa_ext_image_data_info_t *)image_data_info; \
};

#define GET_PTRS_VALUE_hsa_ext_image_data_get_info_with_layout(args) { \
	if (args->hsa_ext_image_data_get_info_with_layout.image_descriptor != NULL) { \
		args->hsa_ext_image_data_get_info_with_layout.image_descriptor__ref.val = *args->hsa_ext_image_data_get_info_with_layout.image_descriptor; \
	} \
	if (args->hsa_ext_image_data_get_info_with_layout.image_data_info != NULL) { \
		args->hsa_ext_image_data_get_info_with_layout.image_data_info__ref.val = *args->hsa_ext_image_data_get_info_with_layout.image_data_info; \
	} \
};


// hsa_status_t
// hsa_executable_global_variable_define (
// 		hsa_executable_t executable (struct)
// 		const char * variable_name (string)
// 		void * address (void)
// )
struct args_hsa_executable_global_variable_define_t {
	hsa_status_t retval;
	hsa_executable_t executable;
	const char * variable_name;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} variable_name__ref;
	void * address;
};

#define GET_ARGS_VALUE_hsa_executable_global_variable_define(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_executable_global_variable_define.executable = (hsa_executable_t)executable; \
	__hsa_activity->hsa_args.hsa_executable_global_variable_define.variable_name = (const char *)variable_name; \
	__hsa_activity->hsa_args.hsa_executable_global_variable_define.address = (void *)address; \
};

#define GET_PTRS_VALUE_hsa_executable_global_variable_define(args) { \
	if (args->hsa_executable_global_variable_define.variable_name != NULL) { \
		strncpy(args->hsa_executable_global_variable_define.variable_name__ref.val, args->hsa_executable_global_variable_define.variable_name, HSA_STRING_SIZE_MAX-1); \
	} \
};


// void
// hsa_signal_add_acquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_add_acquire_t {
	hsa_signal_t signal;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_add_acquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_add_acquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_add_acquire.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_add_acquire(args)

// hsa_status_t
// hsa_ext_image_data_get_info (
// 		hsa_agent_t agent (struct)
// 		const hsa_ext_image_descriptor_t * image_descriptor (struct)
// 		hsa_access_permission_t access_permission (enum)
// 		hsa_ext_image_data_info_t * image_data_info (struct)
// )
struct args_hsa_ext_image_data_get_info_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	const hsa_ext_image_descriptor_t * image_descriptor;
	struct { // const hsa_ext_image_descriptor_t *
		 hsa_ext_image_descriptor_t val;
	} image_descriptor__ref;
	hsa_access_permission_t access_permission;
	hsa_ext_image_data_info_t * image_data_info;
	struct { // hsa_ext_image_data_info_t *
		hsa_ext_image_data_info_t val;
	} image_data_info__ref;
};

#define GET_ARGS_VALUE_hsa_ext_image_data_get_info(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_ext_image_data_get_info.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_ext_image_data_get_info.image_descriptor = (const hsa_ext_image_descriptor_t *)image_descriptor; \
	__hsa_activity->hsa_args.hsa_ext_image_data_get_info.access_permission = (hsa_access_permission_t)access_permission; \
	__hsa_activity->hsa_args.hsa_ext_image_data_get_info.image_data_info = (hsa_ext_image_data_info_t *)image_data_info; \
};

#define GET_PTRS_VALUE_hsa_ext_image_data_get_info(args) { \
	if (args->hsa_ext_image_data_get_info.image_descriptor != NULL) { \
		args->hsa_ext_image_data_get_info.image_descriptor__ref.val = *args->hsa_ext_image_data_get_info.image_descriptor; \
	} \
	if (args->hsa_ext_image_data_get_info.image_data_info != NULL) { \
		args->hsa_ext_image_data_get_info.image_data_info__ref.val = *args->hsa_ext_image_data_get_info.image_data_info; \
	} \
};


// hsa_signal_value_t
// hsa_signal_cas_scacquire (
// 		hsa_signal_t signal (struct)
// 		hsa_signal_value_t expected (long)
// 		hsa_signal_value_t value (long)
// )
struct args_hsa_signal_cas_scacquire_t {
	hsa_signal_value_t retval;
	hsa_signal_t signal;
	hsa_signal_value_t expected;
	hsa_signal_value_t value;
};

#define GET_ARGS_VALUE_hsa_signal_cas_scacquire(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_signal_cas_scacquire.signal = (hsa_signal_t)signal; \
	__hsa_activity->hsa_args.hsa_signal_cas_scacquire.expected = (hsa_signal_value_t)expected; \
	__hsa_activity->hsa_args.hsa_signal_cas_scacquire.value = (hsa_signal_value_t)value; \
};

#define GET_PTRS_VALUE_hsa_signal_cas_scacquire(args)

// hsa_status_t
// hsa_amd_interop_map_buffer (
// 		uint32_t num_agents (unsigned int)
// 		hsa_agent_t * agents (struct)
// 		int interop_handle (int)
// 		uint32_t flags (unsigned int)
// 		size_t * size (unsigned long)
// 		void ** ptr (void)
// 		size_t * metadata_size (unsigned long)
// 		const void ** metadata (void)
// )
struct args_hsa_amd_interop_map_buffer_t {
	hsa_status_t retval;
	uint32_t num_agents;
	hsa_agent_t * agents;
	struct { // hsa_agent_t *
		hsa_agent_t val;
	} agents__ref;
	int interop_handle;
	uint32_t flags;
	size_t * size;
	struct { // size_t *
		size_t val;
	} size__ref;
	void ** ptr;
	struct { // void **
		void* ptr1;
	} ptr__ref;
	size_t * metadata_size;
	struct { // size_t *
		size_t val;
	} metadata_size__ref;
	const void ** metadata;
	struct { // const void **
		 void* ptr1;
	} metadata__ref;
};

#define GET_ARGS_VALUE_hsa_amd_interop_map_buffer(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_interop_map_buffer.num_agents = (uint32_t)num_agents; \
	__hsa_activity->hsa_args.hsa_amd_interop_map_buffer.agents = (hsa_agent_t *)agents; \
	__hsa_activity->hsa_args.hsa_amd_interop_map_buffer.interop_handle = (int)interop_handle; \
	__hsa_activity->hsa_args.hsa_amd_interop_map_buffer.flags = (uint32_t)flags; \
	__hsa_activity->hsa_args.hsa_amd_interop_map_buffer.size = (size_t *)size; \
	__hsa_activity->hsa_args.hsa_amd_interop_map_buffer.ptr = (void **)ptr; \
	__hsa_activity->hsa_args.hsa_amd_interop_map_buffer.metadata_size = (size_t *)metadata_size; \
	__hsa_activity->hsa_args.hsa_amd_interop_map_buffer.metadata = (const void **)metadata; \
};

#define GET_PTRS_VALUE_hsa_amd_interop_map_buffer(args) { \
	if (args->hsa_amd_interop_map_buffer.agents != NULL) { \
		args->hsa_amd_interop_map_buffer.agents__ref.val = *args->hsa_amd_interop_map_buffer.agents; \
	} \
	if (args->hsa_amd_interop_map_buffer.size != NULL) { \
		args->hsa_amd_interop_map_buffer.size__ref.val = *args->hsa_amd_interop_map_buffer.size; \
	} \
	if (args->hsa_amd_interop_map_buffer.ptr != NULL) { \
		args->hsa_amd_interop_map_buffer.ptr__ref.ptr1 = (void*)*args->hsa_amd_interop_map_buffer.ptr; \
	} \
	if (args->hsa_amd_interop_map_buffer.metadata_size != NULL) { \
		args->hsa_amd_interop_map_buffer.metadata_size__ref.val = *args->hsa_amd_interop_map_buffer.metadata_size; \
	} \
	if (args->hsa_amd_interop_map_buffer.metadata != NULL) { \
		args->hsa_amd_interop_map_buffer.metadata__ref.ptr1 = (void*)*args->hsa_amd_interop_map_buffer.metadata; \
	} \
};


// hsa_status_t
// hsa_ext_sampler_create (
// 		hsa_agent_t agent (struct)
// 		const hsa_ext_sampler_descriptor_t * sampler_descriptor (struct)
// 		hsa_ext_sampler_t * sampler (struct)
// )
struct args_hsa_ext_sampler_create_t {
	hsa_status_t retval;
	hsa_agent_t agent;
	const hsa_ext_sampler_descriptor_t * sampler_descriptor;
	struct { // const hsa_ext_sampler_descriptor_t *
		 hsa_ext_sampler_descriptor_t val;
	} sampler_descriptor__ref;
	hsa_ext_sampler_t * sampler;
	struct { // hsa_ext_sampler_t *
		hsa_ext_sampler_t val;
	} sampler__ref;
};

#define GET_ARGS_VALUE_hsa_ext_sampler_create(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_ext_sampler_create.agent = (hsa_agent_t)agent; \
	__hsa_activity->hsa_args.hsa_ext_sampler_create.sampler_descriptor = (const hsa_ext_sampler_descriptor_t *)sampler_descriptor; \
	__hsa_activity->hsa_args.hsa_ext_sampler_create.sampler = (hsa_ext_sampler_t *)sampler; \
};

#define GET_PTRS_VALUE_hsa_ext_sampler_create(args) { \
	if (args->hsa_ext_sampler_create.sampler_descriptor != NULL) { \
		args->hsa_ext_sampler_create.sampler_descriptor__ref.val = *args->hsa_ext_sampler_create.sampler_descriptor; \
	} \
	if (args->hsa_ext_sampler_create.sampler != NULL) { \
		args->hsa_ext_sampler_create.sampler__ref.val = *args->hsa_ext_sampler_create.sampler; \
	} \
};


// hsa_status_t
// hsa_code_object_deserialize (
// 		void * serialized_code_object (void)
// 		size_t serialized_code_object_size (unsigned long)
// 		const char * options (string)
// 		hsa_code_object_t * code_object (struct)
// )
struct args_hsa_code_object_deserialize_t {
	hsa_status_t retval;
	void * serialized_code_object;
	size_t serialized_code_object_size;
	const char * options;
	struct { // const char *
		char val[HSA_STRING_SIZE_MAX];
	} options__ref;
	hsa_code_object_t * code_object;
	struct { // hsa_code_object_t *
		hsa_code_object_t val;
	} code_object__ref;
};

#define GET_ARGS_VALUE_hsa_code_object_deserialize(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_code_object_deserialize.serialized_code_object = (void *)serialized_code_object; \
	__hsa_activity->hsa_args.hsa_code_object_deserialize.serialized_code_object_size = (size_t)serialized_code_object_size; \
	__hsa_activity->hsa_args.hsa_code_object_deserialize.options = (const char *)options; \
	__hsa_activity->hsa_args.hsa_code_object_deserialize.code_object = (hsa_code_object_t *)code_object; \
};

#define GET_PTRS_VALUE_hsa_code_object_deserialize(args) { \
	if (args->hsa_code_object_deserialize.options != NULL) { \
		strncpy(args->hsa_code_object_deserialize.options__ref.val, args->hsa_code_object_deserialize.options, HSA_STRING_SIZE_MAX-1); \
	} \
	if (args->hsa_code_object_deserialize.code_object != NULL) { \
		args->hsa_code_object_deserialize.code_object__ref.val = *args->hsa_code_object_deserialize.code_object; \
	} \
};


// hsa_status_t
// hsa_amd_agents_allow_access (
// 		uint32_t num_agents (unsigned int)
// 		const hsa_agent_t * agents (struct)
// 		const uint32_t * flags (unsigned int)
// 		const void * ptr (void)
// )
struct args_hsa_amd_agents_allow_access_t {
	hsa_status_t retval;
	uint32_t num_agents;
	const hsa_agent_t * agents;
	struct { // const hsa_agent_t *
		 hsa_agent_t val;
	} agents__ref;
	const uint32_t * flags;
	struct { // const uint32_t *
		 uint32_t val;
	} flags__ref;
	const void * ptr;
};

#define GET_ARGS_VALUE_hsa_amd_agents_allow_access(__hsa_activity) { \
	__hsa_activity->hsa_args.hsa_amd_agents_allow_access.num_agents = (uint32_t)num_agents; \
	__hsa_activity->hsa_args.hsa_amd_agents_allow_access.agents = (const hsa_agent_t *)agents; \
	__hsa_activity->hsa_args.hsa_amd_agents_allow_access.flags = (const uint32_t *)flags; \
	__hsa_activity->hsa_args.hsa_amd_agents_allow_access.ptr = (const void *)ptr; \
};

#define GET_PTRS_VALUE_hsa_amd_agents_allow_access(args) { \
	if (args->hsa_amd_agents_allow_access.agents != NULL) { \
		args->hsa_amd_agents_allow_access.agents__ref.val = *args->hsa_amd_agents_allow_access.agents; \
	} \
	if (args->hsa_amd_agents_allow_access.flags != NULL) { \
		args->hsa_amd_agents_allow_access.flags__ref.val = *args->hsa_amd_agents_allow_access.flags; \
	} \
};

 

     
// HSA API args
typedef union hsa_api_args_s {
	struct args_hsa_queue_load_read_index_scacquire_t hsa_queue_load_read_index_scacquire;
	struct args_hsa_ext_image_clear_t hsa_ext_image_clear;
	struct args_hsa_queue_cas_write_index_acquire_t hsa_queue_cas_write_index_acquire;
	struct args_hsa_executable_get_symbol_t hsa_executable_get_symbol;
	struct args_hsa_signal_and_scacq_screl_t hsa_signal_and_scacq_screl;
	struct args_hsa_executable_agent_global_variable_define_t hsa_executable_agent_global_variable_define;
	struct args_hsa_code_object_get_symbol_t hsa_code_object_get_symbol;
	struct args_hsa_amd_signal_async_handler_t hsa_amd_signal_async_handler;
	struct args_hsa_signal_xor_scacq_screl_t hsa_signal_xor_scacq_screl;
	struct args_hsa_signal_and_relaxed_t hsa_signal_and_relaxed;
	struct args_hsa_wavefront_get_info_t hsa_wavefront_get_info;
	struct args_hsa_amd_memory_lock_t hsa_amd_memory_lock;
	struct args_hsa_signal_or_screlease_t hsa_signal_or_screlease;
	struct args_hsa_executable_validate_t hsa_executable_validate;
	struct args_hsa_queue_destroy_t hsa_queue_destroy;
	struct args_hsa_isa_get_info_t hsa_isa_get_info;
	struct args_hsa_amd_memory_lock_to_pool_t hsa_amd_memory_lock_to_pool;
	struct args_hsa_system_major_extension_supported_t hsa_system_major_extension_supported;
	struct args_hsa_amd_profiling_get_async_copy_time_t hsa_amd_profiling_get_async_copy_time;
	struct args_hsa_signal_or_relaxed_t hsa_signal_or_relaxed;
	struct args_hsa_queue_cas_write_index_scacquire_t hsa_queue_cas_write_index_scacquire;
	struct args_hsa_code_object_get_info_t hsa_code_object_get_info;
	struct args_hsa_signal_and_screlease_t hsa_signal_and_screlease;
	struct args_hsa_amd_pointer_info_t hsa_amd_pointer_info;
	struct args_hsa_cache_get_info_t hsa_cache_get_info;
	struct args_hsa_amd_svm_attributes_get_t hsa_amd_svm_attributes_get;
	struct args_hsa_signal_group_create_t hsa_signal_group_create;
	struct args_hsa_memory_deregister_t hsa_memory_deregister;
	struct args_hsa_amd_ipc_memory_attach_t hsa_amd_ipc_memory_attach;
	struct args_hsa_amd_queue_cu_get_mask_t hsa_amd_queue_cu_get_mask;
	struct args_hsa_queue_add_write_index_scacq_screl_t hsa_queue_add_write_index_scacq_screl;
	struct args_hsa_agent_major_extension_supported_t hsa_agent_major_extension_supported;
	struct args_hsa_ext_sampler_destroy_t hsa_ext_sampler_destroy;
	struct args_hsa_queue_create_t hsa_queue_create;
	struct args_hsa_amd_vmem_get_alloc_properties_from_handle_t hsa_amd_vmem_get_alloc_properties_from_handle;
	struct args_hsa_amd_memory_pool_allocate_t hsa_amd_memory_pool_allocate;
	struct args_hsa_signal_or_release_t hsa_signal_or_release;
	struct args_hsa_queue_cas_write_index_acq_rel_t hsa_queue_cas_write_index_acq_rel;
	struct args_hsa_code_object_serialize_t hsa_code_object_serialize;
	struct args_hsa_signal_xor_acquire_t hsa_signal_xor_acquire;
	struct args_hsa_amd_profiling_convert_tick_to_system_domain_t hsa_amd_profiling_convert_tick_to_system_domain;
	struct args_hsa_executable_destroy_t hsa_executable_destroy;
	struct args_hsa_signal_add_screlease_t hsa_signal_add_screlease;
	struct args_hsa_signal_load_acquire_t hsa_signal_load_acquire;
	struct args_hsa_executable_create_alt_t hsa_executable_create_alt;
	struct args_hsa_amd_memory_async_copy_rect_t hsa_amd_memory_async_copy_rect;
	struct args_hsa_signal_load_scacquire_t hsa_signal_load_scacquire;
	struct args_hsa_agent_iterate_caches_t hsa_agent_iterate_caches;
	struct args_hsa_signal_cas_acq_rel_t hsa_signal_cas_acq_rel;
	struct args_hsa_queue_load_read_index_relaxed_t hsa_queue_load_read_index_relaxed;
	struct args_hsa_agent_iterate_isas_t hsa_agent_iterate_isas;
	struct args_hsa_signal_xor_relaxed_t hsa_signal_xor_relaxed;
	struct args_hsa_signal_subtract_acq_rel_t hsa_signal_subtract_acq_rel;
	struct args_hsa_queue_load_read_index_acquire_t hsa_queue_load_read_index_acquire;
	struct args_hsa_isa_get_info_alt_t hsa_isa_get_info_alt;
	struct args_hsa_signal_and_scacquire_t hsa_signal_and_scacquire;
	struct args_hsa_queue_add_write_index_release_t hsa_queue_add_write_index_release;
	struct args_hsa_system_get_info_t hsa_system_get_info;
	struct args_hsa_memory_register_t hsa_memory_register;
	struct args_hsa_isa_get_round_method_t hsa_isa_get_round_method;
	struct args_hsa_isa_iterate_wavefronts_t hsa_isa_iterate_wavefronts;
	struct args_hsa_amd_vmem_unmap_t hsa_amd_vmem_unmap;
	struct args_hsa_signal_subtract_acquire_t hsa_signal_subtract_acquire;
	struct args_hsa_system_get_major_extension_table_t hsa_system_get_major_extension_table;
	struct args_hsa_signal_store_release_t hsa_signal_store_release;
	struct args_hsa_queue_load_write_index_scacquire_t hsa_queue_load_write_index_scacquire;
	struct args_hsa_amd_spm_release_t hsa_amd_spm_release;
	struct args_hsa_code_symbol_get_info_t hsa_code_symbol_get_info;
	struct args_hsa_signal_wait_acquire_t hsa_signal_wait_acquire;
	struct args_hsa_signal_cas_release_t hsa_signal_cas_release;
	struct args_hsa_amd_memory_pool_get_info_t hsa_amd_memory_pool_get_info;
	struct args_hsa_amd_pointer_info_set_userdata_t hsa_amd_pointer_info_set_userdata;
	struct args_hsa_amd_vmem_get_access_t hsa_amd_vmem_get_access;
	struct args_hsa_iterate_agents_t hsa_iterate_agents;
	struct args_hsa_queue_store_write_index_screlease_t hsa_queue_store_write_index_screlease;
	struct args_hsa_agent_iterate_regions_t hsa_agent_iterate_regions;
	struct args_hsa_executable_readonly_variable_define_t hsa_executable_readonly_variable_define;
	struct args_hsa_executable_iterate_symbols_t hsa_executable_iterate_symbols;
	struct args_hsa_amd_vmem_handle_release_t hsa_amd_vmem_handle_release;
	struct args_hsa_amd_memory_pool_can_migrate_t hsa_amd_memory_pool_can_migrate;
	struct args_hsa_queue_store_write_index_relaxed_t hsa_queue_store_write_index_relaxed;
	struct args_hsa_amd_image_get_info_max_dim_t hsa_amd_image_get_info_max_dim;
	struct args_hsa_amd_spm_set_dest_buffer_t hsa_amd_spm_set_dest_buffer;
	struct args_hsa_signal_wait_relaxed_t hsa_signal_wait_relaxed;
	struct args_hsa_amd_portable_export_dmabuf_t hsa_amd_portable_export_dmabuf;
	struct args_hsa_signal_store_screlease_t hsa_signal_store_screlease;
	struct args_hsa_signal_or_acquire_t hsa_signal_or_acquire;
	struct args_hsa_code_object_destroy_t hsa_code_object_destroy;
	struct args_hsa_signal_subtract_screlease_t hsa_signal_subtract_screlease;
	struct args_hsa_region_get_info_t hsa_region_get_info;
	struct args_hsa_signal_subtract_scacq_screl_t hsa_signal_subtract_scacq_screl;
	struct args_hsa_amd_profiling_async_copy_enable_t hsa_amd_profiling_async_copy_enable;
	struct args_hsa_soft_queue_create_t hsa_soft_queue_create;
	struct args_hsa_shut_down_t hsa_shut_down;
	struct args_hsa_code_object_iterate_symbols_t hsa_code_object_iterate_symbols;
	struct args_hsa_extension_get_name_t hsa_extension_get_name;
	struct args_hsa_signal_exchange_relaxed_t hsa_signal_exchange_relaxed;
	struct args_hsa_executable_get_symbol_by_name_t hsa_executable_get_symbol_by_name;
	struct args_hsa_ext_image_create_with_layout_t hsa_ext_image_create_with_layout;
	struct args_hsa_queue_load_write_index_relaxed_t hsa_queue_load_write_index_relaxed;
	struct args_hsa_ext_image_copy_t hsa_ext_image_copy;
	struct args_hsa_code_object_reader_create_from_memory_t hsa_code_object_reader_create_from_memory;
	struct args_hsa_signal_xor_release_t hsa_signal_xor_release;
	struct args_hsa_amd_register_deallocation_callback_t hsa_amd_register_deallocation_callback;
	struct args_hsa_signal_group_destroy_t hsa_signal_group_destroy;
	struct args_hsa_amd_vmem_export_shareable_handle_t hsa_amd_vmem_export_shareable_handle;
	struct args_hsa_queue_add_write_index_acq_rel_t hsa_queue_add_write_index_acq_rel;
	struct args_hsa_signal_destroy_t hsa_signal_destroy;
	struct args_hsa_agent_extension_supported_t hsa_agent_extension_supported;
	struct args_hsa_amd_image_create_t hsa_amd_image_create;
	struct args_hsa_isa_compatible_t hsa_isa_compatible;
	struct args_hsa_amd_queue_set_priority_t hsa_amd_queue_set_priority;
	struct args_hsa_signal_and_acq_rel_t hsa_signal_and_acq_rel;
	struct args_hsa_code_object_reader_destroy_t hsa_code_object_reader_destroy;
	struct args_hsa_queue_cas_write_index_release_t hsa_queue_cas_write_index_release;
	struct args_hsa_executable_load_code_object_t hsa_executable_load_code_object;
	struct args_hsa_amd_memory_async_copy_t hsa_amd_memory_async_copy;
	struct args_hsa_amd_memory_unlock_t hsa_amd_memory_unlock;
	struct args_hsa_signal_subtract_release_t hsa_signal_subtract_release;
	struct args_hsa_signal_exchange_acquire_t hsa_signal_exchange_acquire;
	struct args_hsa_signal_store_relaxed_t hsa_signal_store_relaxed;
	struct args_hsa_signal_subtract_scacquire_t hsa_signal_subtract_scacquire;
	struct args_hsa_signal_and_release_t hsa_signal_and_release;
	struct args_hsa_signal_cas_acquire_t hsa_signal_cas_acquire;
	struct args_hsa_isa_get_exception_policies_t hsa_isa_get_exception_policies;
	struct args_hsa_signal_add_release_t hsa_signal_add_release;
	struct args_hsa_code_object_reader_create_from_file_t hsa_code_object_reader_create_from_file;
	struct args_hsa_executable_symbol_get_info_t hsa_executable_symbol_get_info;
	struct args_hsa_amd_ipc_memory_create_t hsa_amd_ipc_memory_create;
	struct args_hsa_queue_store_write_index_release_t hsa_queue_store_write_index_release;
	struct args_hsa_amd_memory_fill_t hsa_amd_memory_fill;
	struct args_hsa_signal_add_acq_rel_t hsa_signal_add_acq_rel;
	struct args_hsa_signal_cas_scacq_screl_t hsa_signal_cas_scacq_screl;
	struct args_hsa_queue_cas_write_index_scacq_screl_t hsa_queue_cas_write_index_scacq_screl;
	struct args_hsa_amd_ipc_signal_create_t hsa_amd_ipc_signal_create;
	struct args_hsa_amd_spm_acquire_t hsa_amd_spm_acquire;
	struct args_hsa_amd_coherency_get_type_t hsa_amd_coherency_get_type;
	struct args_hsa_system_get_extension_table_t hsa_system_get_extension_table;
	struct args_hsa_amd_ipc_memory_detach_t hsa_amd_ipc_memory_detach;
	struct args_hsa_ext_image_get_capability_with_layout_t hsa_ext_image_get_capability_with_layout;
	struct args_hsa_amd_vmem_address_free_t hsa_amd_vmem_address_free;
	struct args_hsa_amd_svm_attributes_set_t hsa_amd_svm_attributes_set;
	struct args_hsa_ext_image_import_t hsa_ext_image_import;
	struct args_hsa_signal_or_scacquire_t hsa_signal_or_scacquire;
	struct args_hsa_amd_svm_prefetch_async_t hsa_amd_svm_prefetch_async;
	struct args_hsa_signal_add_scacq_screl_t hsa_signal_add_scacq_screl;
	struct args_hsa_signal_silent_store_relaxed_t hsa_signal_silent_store_relaxed;
	struct args_hsa_queue_store_read_index_relaxed_t hsa_queue_store_read_index_relaxed;
	struct args_hsa_agent_get_exception_policies_t hsa_agent_get_exception_policies;
	struct args_hsa_signal_add_scacquire_t hsa_signal_add_scacquire;
	struct args_hsa_signal_exchange_release_t hsa_signal_exchange_release;
	struct args_hsa_signal_add_relaxed_t hsa_signal_add_relaxed;
	struct args_hsa_signal_and_acquire_t hsa_signal_and_acquire;
	struct args_hsa_queue_add_write_index_relaxed_t hsa_queue_add_write_index_relaxed;
	struct args_hsa_amd_agent_memory_pool_get_info_t hsa_amd_agent_memory_pool_get_info;
	struct args_hsa_amd_profiling_set_profiler_enabled_t hsa_amd_profiling_set_profiler_enabled;
	struct args_hsa_signal_exchange_acq_rel_t hsa_signal_exchange_acq_rel;
	struct args_hsa_queue_cas_write_index_screlease_t hsa_queue_cas_write_index_screlease;
	struct args_hsa_amd_async_function_t hsa_amd_async_function;
	struct args_hsa_queue_store_read_index_screlease_t hsa_queue_store_read_index_screlease;
	struct args_hsa_memory_allocate_t hsa_memory_allocate;
	struct args_hsa_amd_signal_value_pointer_t hsa_amd_signal_value_pointer;
	struct args_hsa_amd_signal_wait_any_t hsa_amd_signal_wait_any;
	struct args_hsa_amd_vmem_retain_alloc_handle_t hsa_amd_vmem_retain_alloc_handle;
	struct args_hsa_amd_vmem_set_access_t hsa_amd_vmem_set_access;
	struct args_hsa_memory_assign_agent_t hsa_memory_assign_agent;
	struct args_hsa_amd_vmem_address_reserve_t hsa_amd_vmem_address_reserve;
	struct args_hsa_amd_memory_async_copy_on_engine_t hsa_amd_memory_async_copy_on_engine;
	struct args_hsa_signal_xor_scacquire_t hsa_signal_xor_scacquire;
	struct args_hsa_amd_coherency_set_type_t hsa_amd_coherency_set_type;
	struct args_hsa_queue_inactivate_t hsa_queue_inactivate;
	struct args_hsa_amd_vmem_import_shareable_handle_t hsa_amd_vmem_import_shareable_handle;
	struct args_hsa_signal_cas_relaxed_t hsa_signal_cas_relaxed;
	struct args_hsa_queue_add_write_index_scacquire_t hsa_queue_add_write_index_scacquire;
	struct args_hsa_signal_exchange_screlease_t hsa_signal_exchange_screlease;
	struct args_hsa_amd_vmem_handle_create_t hsa_amd_vmem_handle_create;
	struct args_hsa_agent_get_info_t hsa_agent_get_info;
	struct args_hsa_signal_or_acq_rel_t hsa_signal_or_acq_rel;
	struct args_hsa_signal_xor_acq_rel_t hsa_signal_xor_acq_rel;
	struct args_hsa_system_extension_supported_t hsa_system_extension_supported;
	struct args_hsa_signal_group_wait_any_relaxed_t hsa_signal_group_wait_any_relaxed;
	struct args_hsa_signal_or_scacq_screl_t hsa_signal_or_scacq_screl;
	struct args_hsa_executable_iterate_agent_symbols_t hsa_executable_iterate_agent_symbols;
	struct args_hsa_amd_deregister_deallocation_callback_t hsa_amd_deregister_deallocation_callback;
	struct args_hsa_signal_create_t hsa_signal_create;
	struct args_hsa_amd_memory_copy_engine_status_t hsa_amd_memory_copy_engine_status;
	struct args_hsa_ext_image_get_capability_t hsa_ext_image_get_capability;
	struct args_hsa_ext_image_destroy_t hsa_ext_image_destroy;
	struct args_hsa_queue_store_read_index_release_t hsa_queue_store_read_index_release;
	struct args_hsa_signal_wait_scacquire_t hsa_signal_wait_scacquire;
	struct args_hsa_executable_get_info_t hsa_executable_get_info;
	struct args_hsa_signal_silent_store_screlease_t hsa_signal_silent_store_screlease;
	struct args_hsa_executable_validate_alt_t hsa_executable_validate_alt;
	struct args_hsa_executable_create_t hsa_executable_create;
	struct args_hsa_executable_iterate_program_symbols_t hsa_executable_iterate_program_symbols;
	struct args_hsa_memory_free_t hsa_memory_free;
	struct args_hsa_amd_memory_pool_free_t hsa_amd_memory_pool_free;
	struct args_hsa_amd_memory_migrate_t hsa_amd_memory_migrate;
	struct args_hsa_amd_interop_unmap_buffer_t hsa_amd_interop_unmap_buffer;
	struct args_hsa_amd_signal_create_t hsa_amd_signal_create;
	struct args_hsa_executable_load_program_code_object_t hsa_executable_load_program_code_object;
	struct args_hsa_signal_load_relaxed_t hsa_signal_load_relaxed;
	struct args_hsa_signal_group_wait_any_scacquire_t hsa_signal_group_wait_any_scacquire;
	struct args_hsa_queue_cas_write_index_relaxed_t hsa_queue_cas_write_index_relaxed;
	struct args_hsa_signal_subtract_relaxed_t hsa_signal_subtract_relaxed;
	struct args_hsa_executable_freeze_t hsa_executable_freeze;
	struct args_hsa_queue_load_write_index_acquire_t hsa_queue_load_write_index_acquire;
	struct args_hsa_amd_vmem_map_t hsa_amd_vmem_map;
	struct args_hsa_amd_ipc_signal_attach_t hsa_amd_ipc_signal_attach;
	struct args_hsa_amd_register_system_event_handler_t hsa_amd_register_system_event_handler;
	struct args_hsa_amd_queue_cu_set_mask_t hsa_amd_queue_cu_set_mask;
	struct args_hsa_ext_image_export_t hsa_ext_image_export;
	struct args_hsa_status_string_t hsa_status_string;
	struct args_hsa_signal_xor_screlease_t hsa_signal_xor_screlease;
	struct args_hsa_queue_add_write_index_acquire_t hsa_queue_add_write_index_acquire;
	struct args_hsa_queue_add_write_index_screlease_t hsa_queue_add_write_index_screlease;
	struct args_hsa_code_object_get_symbol_from_name_t hsa_code_object_get_symbol_from_name;
	struct args_hsa_isa_from_name_t hsa_isa_from_name;
	struct args_hsa_amd_agent_iterate_memory_pools_t hsa_amd_agent_iterate_memory_pools;
	struct args_hsa_amd_agent_set_async_scratch_limit_t hsa_amd_agent_set_async_scratch_limit;
	struct args_hsa_ext_image_create_t hsa_ext_image_create;
	struct args_hsa_signal_exchange_scacq_screl_t hsa_signal_exchange_scacq_screl;
	struct args_hsa_init_t hsa_init;
	struct args_hsa_executable_load_agent_code_object_t hsa_executable_load_agent_code_object;
	struct args_hsa_signal_cas_screlease_t hsa_signal_cas_screlease;
	struct args_hsa_amd_profiling_get_dispatch_time_t hsa_amd_profiling_get_dispatch_time;
	struct args_hsa_amd_portable_close_dmabuf_t hsa_amd_portable_close_dmabuf;
	struct args_hsa_memory_copy_t hsa_memory_copy;
	struct args_hsa_signal_exchange_scacquire_t hsa_signal_exchange_scacquire;
	struct args_hsa_ext_image_data_get_info_with_layout_t hsa_ext_image_data_get_info_with_layout;
	struct args_hsa_executable_global_variable_define_t hsa_executable_global_variable_define;
	struct args_hsa_signal_add_acquire_t hsa_signal_add_acquire;
	struct args_hsa_ext_image_data_get_info_t hsa_ext_image_data_get_info;
	struct args_hsa_signal_cas_scacquire_t hsa_signal_cas_scacquire;
	struct args_hsa_amd_interop_map_buffer_t hsa_amd_interop_map_buffer;
	struct args_hsa_ext_sampler_create_t hsa_ext_sampler_create;
	struct args_hsa_code_object_deserialize_t hsa_code_object_deserialize;
	struct args_hsa_amd_agents_allow_access_t hsa_amd_agents_allow_access; 
} hsa_api_args_t;
 

     
// Get HSA API function pointed arguments values
static inline void get_hsa_pointed_args_for(hsa_api_id_t id, hsa_api_args_t* args, bool is_enter) 
{
    if (!is_enter) {
        switch(id) {
			case HSA_API_ID_hsa_queue_load_read_index_scacquire : GET_PTRS_VALUE_hsa_queue_load_read_index_scacquire(args); return;
			case HSA_API_ID_hsa_ext_image_clear : GET_PTRS_VALUE_hsa_ext_image_clear(args); return;
			case HSA_API_ID_hsa_queue_cas_write_index_acquire : GET_PTRS_VALUE_hsa_queue_cas_write_index_acquire(args); return;
			case HSA_API_ID_hsa_executable_get_symbol : GET_PTRS_VALUE_hsa_executable_get_symbol(args); return;
			case HSA_API_ID_hsa_signal_and_scacq_screl : GET_PTRS_VALUE_hsa_signal_and_scacq_screl(args); return;
			case HSA_API_ID_hsa_executable_agent_global_variable_define : GET_PTRS_VALUE_hsa_executable_agent_global_variable_define(args); return;
			case HSA_API_ID_hsa_code_object_get_symbol : GET_PTRS_VALUE_hsa_code_object_get_symbol(args); return;
			case HSA_API_ID_hsa_amd_signal_async_handler : GET_PTRS_VALUE_hsa_amd_signal_async_handler(args); return;
			case HSA_API_ID_hsa_signal_xor_scacq_screl : GET_PTRS_VALUE_hsa_signal_xor_scacq_screl(args); return;
			case HSA_API_ID_hsa_signal_and_relaxed : GET_PTRS_VALUE_hsa_signal_and_relaxed(args); return;
			case HSA_API_ID_hsa_wavefront_get_info : GET_PTRS_VALUE_hsa_wavefront_get_info(args); return;
			case HSA_API_ID_hsa_amd_memory_lock : GET_PTRS_VALUE_hsa_amd_memory_lock(args); return;
			case HSA_API_ID_hsa_signal_or_screlease : GET_PTRS_VALUE_hsa_signal_or_screlease(args); return;
			case HSA_API_ID_hsa_executable_validate : GET_PTRS_VALUE_hsa_executable_validate(args); return;
			case HSA_API_ID_hsa_isa_get_info : GET_PTRS_VALUE_hsa_isa_get_info(args); return;
			case HSA_API_ID_hsa_amd_memory_lock_to_pool : GET_PTRS_VALUE_hsa_amd_memory_lock_to_pool(args); return;
			case HSA_API_ID_hsa_system_major_extension_supported : GET_PTRS_VALUE_hsa_system_major_extension_supported(args); return;
			case HSA_API_ID_hsa_amd_profiling_get_async_copy_time : GET_PTRS_VALUE_hsa_amd_profiling_get_async_copy_time(args); return;
			case HSA_API_ID_hsa_signal_or_relaxed : GET_PTRS_VALUE_hsa_signal_or_relaxed(args); return;
			case HSA_API_ID_hsa_queue_cas_write_index_scacquire : GET_PTRS_VALUE_hsa_queue_cas_write_index_scacquire(args); return;
			case HSA_API_ID_hsa_code_object_get_info : GET_PTRS_VALUE_hsa_code_object_get_info(args); return;
			case HSA_API_ID_hsa_signal_and_screlease : GET_PTRS_VALUE_hsa_signal_and_screlease(args); return;
			case HSA_API_ID_hsa_amd_pointer_info : GET_PTRS_VALUE_hsa_amd_pointer_info(args); return;
			case HSA_API_ID_hsa_cache_get_info : GET_PTRS_VALUE_hsa_cache_get_info(args); return;
			case HSA_API_ID_hsa_amd_svm_attributes_get : GET_PTRS_VALUE_hsa_amd_svm_attributes_get(args); return;
			case HSA_API_ID_hsa_signal_group_create : GET_PTRS_VALUE_hsa_signal_group_create(args); return;
			case HSA_API_ID_hsa_memory_deregister : GET_PTRS_VALUE_hsa_memory_deregister(args); return;
			case HSA_API_ID_hsa_amd_ipc_memory_attach : GET_PTRS_VALUE_hsa_amd_ipc_memory_attach(args); return;
			case HSA_API_ID_hsa_amd_queue_cu_get_mask : GET_PTRS_VALUE_hsa_amd_queue_cu_get_mask(args); return;
			case HSA_API_ID_hsa_queue_add_write_index_scacq_screl : GET_PTRS_VALUE_hsa_queue_add_write_index_scacq_screl(args); return;
			case HSA_API_ID_hsa_agent_major_extension_supported : GET_PTRS_VALUE_hsa_agent_major_extension_supported(args); return;
			case HSA_API_ID_hsa_ext_sampler_destroy : GET_PTRS_VALUE_hsa_ext_sampler_destroy(args); return;
			case HSA_API_ID_hsa_queue_create : GET_PTRS_VALUE_hsa_queue_create(args); return;
			case HSA_API_ID_hsa_amd_vmem_get_alloc_properties_from_handle : GET_PTRS_VALUE_hsa_amd_vmem_get_alloc_properties_from_handle(args); return;
			case HSA_API_ID_hsa_amd_memory_pool_allocate : GET_PTRS_VALUE_hsa_amd_memory_pool_allocate(args); return;
			case HSA_API_ID_hsa_signal_or_release : GET_PTRS_VALUE_hsa_signal_or_release(args); return;
			case HSA_API_ID_hsa_queue_cas_write_index_acq_rel : GET_PTRS_VALUE_hsa_queue_cas_write_index_acq_rel(args); return;
			case HSA_API_ID_hsa_code_object_serialize : GET_PTRS_VALUE_hsa_code_object_serialize(args); return;
			case HSA_API_ID_hsa_signal_xor_acquire : GET_PTRS_VALUE_hsa_signal_xor_acquire(args); return;
			case HSA_API_ID_hsa_amd_profiling_convert_tick_to_system_domain : GET_PTRS_VALUE_hsa_amd_profiling_convert_tick_to_system_domain(args); return;
			case HSA_API_ID_hsa_executable_destroy : GET_PTRS_VALUE_hsa_executable_destroy(args); return;
			case HSA_API_ID_hsa_signal_add_screlease : GET_PTRS_VALUE_hsa_signal_add_screlease(args); return;
			case HSA_API_ID_hsa_signal_load_acquire : GET_PTRS_VALUE_hsa_signal_load_acquire(args); return;
			case HSA_API_ID_hsa_executable_create_alt : GET_PTRS_VALUE_hsa_executable_create_alt(args); return;
			case HSA_API_ID_hsa_amd_memory_async_copy_rect : GET_PTRS_VALUE_hsa_amd_memory_async_copy_rect(args); return;
			case HSA_API_ID_hsa_signal_load_scacquire : GET_PTRS_VALUE_hsa_signal_load_scacquire(args); return;
			case HSA_API_ID_hsa_agent_iterate_caches : GET_PTRS_VALUE_hsa_agent_iterate_caches(args); return;
			case HSA_API_ID_hsa_signal_cas_acq_rel : GET_PTRS_VALUE_hsa_signal_cas_acq_rel(args); return;
			case HSA_API_ID_hsa_queue_load_read_index_relaxed : GET_PTRS_VALUE_hsa_queue_load_read_index_relaxed(args); return;
			case HSA_API_ID_hsa_agent_iterate_isas : GET_PTRS_VALUE_hsa_agent_iterate_isas(args); return;
			case HSA_API_ID_hsa_signal_xor_relaxed : GET_PTRS_VALUE_hsa_signal_xor_relaxed(args); return;
			case HSA_API_ID_hsa_signal_subtract_acq_rel : GET_PTRS_VALUE_hsa_signal_subtract_acq_rel(args); return;
			case HSA_API_ID_hsa_queue_load_read_index_acquire : GET_PTRS_VALUE_hsa_queue_load_read_index_acquire(args); return;
			case HSA_API_ID_hsa_isa_get_info_alt : GET_PTRS_VALUE_hsa_isa_get_info_alt(args); return;
			case HSA_API_ID_hsa_signal_and_scacquire : GET_PTRS_VALUE_hsa_signal_and_scacquire(args); return;
			case HSA_API_ID_hsa_queue_add_write_index_release : GET_PTRS_VALUE_hsa_queue_add_write_index_release(args); return;
			case HSA_API_ID_hsa_system_get_info : GET_PTRS_VALUE_hsa_system_get_info(args); return;
			case HSA_API_ID_hsa_memory_register : GET_PTRS_VALUE_hsa_memory_register(args); return;
			case HSA_API_ID_hsa_isa_get_round_method : GET_PTRS_VALUE_hsa_isa_get_round_method(args); return;
			case HSA_API_ID_hsa_isa_iterate_wavefronts : GET_PTRS_VALUE_hsa_isa_iterate_wavefronts(args); return;
			case HSA_API_ID_hsa_amd_vmem_unmap : GET_PTRS_VALUE_hsa_amd_vmem_unmap(args); return;
			case HSA_API_ID_hsa_signal_subtract_acquire : GET_PTRS_VALUE_hsa_signal_subtract_acquire(args); return;
			case HSA_API_ID_hsa_system_get_major_extension_table : GET_PTRS_VALUE_hsa_system_get_major_extension_table(args); return;
			case HSA_API_ID_hsa_signal_store_release : GET_PTRS_VALUE_hsa_signal_store_release(args); return;
			case HSA_API_ID_hsa_queue_load_write_index_scacquire : GET_PTRS_VALUE_hsa_queue_load_write_index_scacquire(args); return;
			case HSA_API_ID_hsa_amd_spm_release : GET_PTRS_VALUE_hsa_amd_spm_release(args); return;
			case HSA_API_ID_hsa_code_symbol_get_info : GET_PTRS_VALUE_hsa_code_symbol_get_info(args); return;
			case HSA_API_ID_hsa_signal_wait_acquire : GET_PTRS_VALUE_hsa_signal_wait_acquire(args); return;
			case HSA_API_ID_hsa_signal_cas_release : GET_PTRS_VALUE_hsa_signal_cas_release(args); return;
			case HSA_API_ID_hsa_amd_memory_pool_get_info : GET_PTRS_VALUE_hsa_amd_memory_pool_get_info(args); return;
			case HSA_API_ID_hsa_amd_pointer_info_set_userdata : GET_PTRS_VALUE_hsa_amd_pointer_info_set_userdata(args); return;
			case HSA_API_ID_hsa_amd_vmem_get_access : GET_PTRS_VALUE_hsa_amd_vmem_get_access(args); return;
			case HSA_API_ID_hsa_iterate_agents : GET_PTRS_VALUE_hsa_iterate_agents(args); return;
			case HSA_API_ID_hsa_queue_store_write_index_screlease : GET_PTRS_VALUE_hsa_queue_store_write_index_screlease(args); return;
			case HSA_API_ID_hsa_agent_iterate_regions : GET_PTRS_VALUE_hsa_agent_iterate_regions(args); return;
			case HSA_API_ID_hsa_executable_readonly_variable_define : GET_PTRS_VALUE_hsa_executable_readonly_variable_define(args); return;
			case HSA_API_ID_hsa_executable_iterate_symbols : GET_PTRS_VALUE_hsa_executable_iterate_symbols(args); return;
			case HSA_API_ID_hsa_amd_vmem_handle_release : GET_PTRS_VALUE_hsa_amd_vmem_handle_release(args); return;
			case HSA_API_ID_hsa_amd_memory_pool_can_migrate : GET_PTRS_VALUE_hsa_amd_memory_pool_can_migrate(args); return;
			case HSA_API_ID_hsa_queue_store_write_index_relaxed : GET_PTRS_VALUE_hsa_queue_store_write_index_relaxed(args); return;
			case HSA_API_ID_hsa_amd_image_get_info_max_dim : GET_PTRS_VALUE_hsa_amd_image_get_info_max_dim(args); return;
			case HSA_API_ID_hsa_amd_spm_set_dest_buffer : GET_PTRS_VALUE_hsa_amd_spm_set_dest_buffer(args); return;
			case HSA_API_ID_hsa_signal_wait_relaxed : GET_PTRS_VALUE_hsa_signal_wait_relaxed(args); return;
			case HSA_API_ID_hsa_amd_portable_export_dmabuf : GET_PTRS_VALUE_hsa_amd_portable_export_dmabuf(args); return;
			case HSA_API_ID_hsa_signal_store_screlease : GET_PTRS_VALUE_hsa_signal_store_screlease(args); return;
			case HSA_API_ID_hsa_signal_or_acquire : GET_PTRS_VALUE_hsa_signal_or_acquire(args); return;
			case HSA_API_ID_hsa_code_object_destroy : GET_PTRS_VALUE_hsa_code_object_destroy(args); return;
			case HSA_API_ID_hsa_signal_subtract_screlease : GET_PTRS_VALUE_hsa_signal_subtract_screlease(args); return;
			case HSA_API_ID_hsa_region_get_info : GET_PTRS_VALUE_hsa_region_get_info(args); return;
			case HSA_API_ID_hsa_signal_subtract_scacq_screl : GET_PTRS_VALUE_hsa_signal_subtract_scacq_screl(args); return;
			case HSA_API_ID_hsa_amd_profiling_async_copy_enable : GET_PTRS_VALUE_hsa_amd_profiling_async_copy_enable(args); return;
			case HSA_API_ID_hsa_soft_queue_create : GET_PTRS_VALUE_hsa_soft_queue_create(args); return;
			case HSA_API_ID_hsa_shut_down : GET_PTRS_VALUE_hsa_shut_down(args); return;
			case HSA_API_ID_hsa_code_object_iterate_symbols : GET_PTRS_VALUE_hsa_code_object_iterate_symbols(args); return;
			case HSA_API_ID_hsa_extension_get_name : GET_PTRS_VALUE_hsa_extension_get_name(args); return;
			case HSA_API_ID_hsa_signal_exchange_relaxed : GET_PTRS_VALUE_hsa_signal_exchange_relaxed(args); return;
			case HSA_API_ID_hsa_executable_get_symbol_by_name : GET_PTRS_VALUE_hsa_executable_get_symbol_by_name(args); return;
			case HSA_API_ID_hsa_ext_image_create_with_layout : GET_PTRS_VALUE_hsa_ext_image_create_with_layout(args); return;
			case HSA_API_ID_hsa_queue_load_write_index_relaxed : GET_PTRS_VALUE_hsa_queue_load_write_index_relaxed(args); return;
			case HSA_API_ID_hsa_ext_image_copy : GET_PTRS_VALUE_hsa_ext_image_copy(args); return;
			case HSA_API_ID_hsa_code_object_reader_create_from_memory : GET_PTRS_VALUE_hsa_code_object_reader_create_from_memory(args); return;
			case HSA_API_ID_hsa_signal_xor_release : GET_PTRS_VALUE_hsa_signal_xor_release(args); return;
			case HSA_API_ID_hsa_amd_register_deallocation_callback : GET_PTRS_VALUE_hsa_amd_register_deallocation_callback(args); return;
			case HSA_API_ID_hsa_signal_group_destroy : GET_PTRS_VALUE_hsa_signal_group_destroy(args); return;
			case HSA_API_ID_hsa_amd_vmem_export_shareable_handle : GET_PTRS_VALUE_hsa_amd_vmem_export_shareable_handle(args); return;
			case HSA_API_ID_hsa_queue_add_write_index_acq_rel : GET_PTRS_VALUE_hsa_queue_add_write_index_acq_rel(args); return;
			case HSA_API_ID_hsa_signal_destroy : GET_PTRS_VALUE_hsa_signal_destroy(args); return;
			case HSA_API_ID_hsa_agent_extension_supported : GET_PTRS_VALUE_hsa_agent_extension_supported(args); return;
			case HSA_API_ID_hsa_amd_image_create : GET_PTRS_VALUE_hsa_amd_image_create(args); return;
			case HSA_API_ID_hsa_isa_compatible : GET_PTRS_VALUE_hsa_isa_compatible(args); return;
			case HSA_API_ID_hsa_amd_queue_set_priority : GET_PTRS_VALUE_hsa_amd_queue_set_priority(args); return;
			case HSA_API_ID_hsa_signal_and_acq_rel : GET_PTRS_VALUE_hsa_signal_and_acq_rel(args); return;
			case HSA_API_ID_hsa_code_object_reader_destroy : GET_PTRS_VALUE_hsa_code_object_reader_destroy(args); return;
			case HSA_API_ID_hsa_queue_cas_write_index_release : GET_PTRS_VALUE_hsa_queue_cas_write_index_release(args); return;
			case HSA_API_ID_hsa_executable_load_code_object : GET_PTRS_VALUE_hsa_executable_load_code_object(args); return;
			case HSA_API_ID_hsa_amd_memory_async_copy : GET_PTRS_VALUE_hsa_amd_memory_async_copy(args); return;
			case HSA_API_ID_hsa_amd_memory_unlock : GET_PTRS_VALUE_hsa_amd_memory_unlock(args); return;
			case HSA_API_ID_hsa_signal_subtract_release : GET_PTRS_VALUE_hsa_signal_subtract_release(args); return;
			case HSA_API_ID_hsa_signal_exchange_acquire : GET_PTRS_VALUE_hsa_signal_exchange_acquire(args); return;
			case HSA_API_ID_hsa_signal_store_relaxed : GET_PTRS_VALUE_hsa_signal_store_relaxed(args); return;
			case HSA_API_ID_hsa_signal_subtract_scacquire : GET_PTRS_VALUE_hsa_signal_subtract_scacquire(args); return;
			case HSA_API_ID_hsa_signal_and_release : GET_PTRS_VALUE_hsa_signal_and_release(args); return;
			case HSA_API_ID_hsa_signal_cas_acquire : GET_PTRS_VALUE_hsa_signal_cas_acquire(args); return;
			case HSA_API_ID_hsa_isa_get_exception_policies : GET_PTRS_VALUE_hsa_isa_get_exception_policies(args); return;
			case HSA_API_ID_hsa_signal_add_release : GET_PTRS_VALUE_hsa_signal_add_release(args); return;
			case HSA_API_ID_hsa_code_object_reader_create_from_file : GET_PTRS_VALUE_hsa_code_object_reader_create_from_file(args); return;
			case HSA_API_ID_hsa_executable_symbol_get_info : GET_PTRS_VALUE_hsa_executable_symbol_get_info(args); return;
			case HSA_API_ID_hsa_amd_ipc_memory_create : GET_PTRS_VALUE_hsa_amd_ipc_memory_create(args); return;
			case HSA_API_ID_hsa_queue_store_write_index_release : GET_PTRS_VALUE_hsa_queue_store_write_index_release(args); return;
			case HSA_API_ID_hsa_amd_memory_fill : GET_PTRS_VALUE_hsa_amd_memory_fill(args); return;
			case HSA_API_ID_hsa_signal_add_acq_rel : GET_PTRS_VALUE_hsa_signal_add_acq_rel(args); return;
			case HSA_API_ID_hsa_signal_cas_scacq_screl : GET_PTRS_VALUE_hsa_signal_cas_scacq_screl(args); return;
			case HSA_API_ID_hsa_queue_cas_write_index_scacq_screl : GET_PTRS_VALUE_hsa_queue_cas_write_index_scacq_screl(args); return;
			case HSA_API_ID_hsa_amd_ipc_signal_create : GET_PTRS_VALUE_hsa_amd_ipc_signal_create(args); return;
			case HSA_API_ID_hsa_amd_spm_acquire : GET_PTRS_VALUE_hsa_amd_spm_acquire(args); return;
			case HSA_API_ID_hsa_amd_coherency_get_type : GET_PTRS_VALUE_hsa_amd_coherency_get_type(args); return;
			case HSA_API_ID_hsa_system_get_extension_table : GET_PTRS_VALUE_hsa_system_get_extension_table(args); return;
			case HSA_API_ID_hsa_amd_ipc_memory_detach : GET_PTRS_VALUE_hsa_amd_ipc_memory_detach(args); return;
			case HSA_API_ID_hsa_ext_image_get_capability_with_layout : GET_PTRS_VALUE_hsa_ext_image_get_capability_with_layout(args); return;
			case HSA_API_ID_hsa_amd_svm_attributes_set : GET_PTRS_VALUE_hsa_amd_svm_attributes_set(args); return;
			case HSA_API_ID_hsa_ext_image_import : GET_PTRS_VALUE_hsa_ext_image_import(args); return;
			case HSA_API_ID_hsa_signal_or_scacquire : GET_PTRS_VALUE_hsa_signal_or_scacquire(args); return;
			case HSA_API_ID_hsa_amd_svm_prefetch_async : GET_PTRS_VALUE_hsa_amd_svm_prefetch_async(args); return;
			case HSA_API_ID_hsa_signal_add_scacq_screl : GET_PTRS_VALUE_hsa_signal_add_scacq_screl(args); return;
			case HSA_API_ID_hsa_signal_silent_store_relaxed : GET_PTRS_VALUE_hsa_signal_silent_store_relaxed(args); return;
			case HSA_API_ID_hsa_queue_store_read_index_relaxed : GET_PTRS_VALUE_hsa_queue_store_read_index_relaxed(args); return;
			case HSA_API_ID_hsa_agent_get_exception_policies : GET_PTRS_VALUE_hsa_agent_get_exception_policies(args); return;
			case HSA_API_ID_hsa_signal_add_scacquire : GET_PTRS_VALUE_hsa_signal_add_scacquire(args); return;
			case HSA_API_ID_hsa_signal_exchange_release : GET_PTRS_VALUE_hsa_signal_exchange_release(args); return;
			case HSA_API_ID_hsa_signal_add_relaxed : GET_PTRS_VALUE_hsa_signal_add_relaxed(args); return;
			case HSA_API_ID_hsa_signal_and_acquire : GET_PTRS_VALUE_hsa_signal_and_acquire(args); return;
			case HSA_API_ID_hsa_queue_add_write_index_relaxed : GET_PTRS_VALUE_hsa_queue_add_write_index_relaxed(args); return;
			case HSA_API_ID_hsa_amd_agent_memory_pool_get_info : GET_PTRS_VALUE_hsa_amd_agent_memory_pool_get_info(args); return;
			case HSA_API_ID_hsa_amd_profiling_set_profiler_enabled : GET_PTRS_VALUE_hsa_amd_profiling_set_profiler_enabled(args); return;
			case HSA_API_ID_hsa_signal_exchange_acq_rel : GET_PTRS_VALUE_hsa_signal_exchange_acq_rel(args); return;
			case HSA_API_ID_hsa_queue_cas_write_index_screlease : GET_PTRS_VALUE_hsa_queue_cas_write_index_screlease(args); return;
			case HSA_API_ID_hsa_amd_async_function : GET_PTRS_VALUE_hsa_amd_async_function(args); return;
			case HSA_API_ID_hsa_queue_store_read_index_screlease : GET_PTRS_VALUE_hsa_queue_store_read_index_screlease(args); return;
			case HSA_API_ID_hsa_memory_allocate : GET_PTRS_VALUE_hsa_memory_allocate(args); return;
			case HSA_API_ID_hsa_amd_signal_value_pointer : GET_PTRS_VALUE_hsa_amd_signal_value_pointer(args); return;
			case HSA_API_ID_hsa_amd_signal_wait_any : GET_PTRS_VALUE_hsa_amd_signal_wait_any(args); return;
			case HSA_API_ID_hsa_amd_vmem_retain_alloc_handle : GET_PTRS_VALUE_hsa_amd_vmem_retain_alloc_handle(args); return;
			case HSA_API_ID_hsa_amd_vmem_set_access : GET_PTRS_VALUE_hsa_amd_vmem_set_access(args); return;
			case HSA_API_ID_hsa_memory_assign_agent : GET_PTRS_VALUE_hsa_memory_assign_agent(args); return;
			case HSA_API_ID_hsa_amd_vmem_address_reserve : GET_PTRS_VALUE_hsa_amd_vmem_address_reserve(args); return;
			case HSA_API_ID_hsa_amd_memory_async_copy_on_engine : GET_PTRS_VALUE_hsa_amd_memory_async_copy_on_engine(args); return;
			case HSA_API_ID_hsa_signal_xor_scacquire : GET_PTRS_VALUE_hsa_signal_xor_scacquire(args); return;
			case HSA_API_ID_hsa_amd_coherency_set_type : GET_PTRS_VALUE_hsa_amd_coherency_set_type(args); return;
			case HSA_API_ID_hsa_queue_inactivate : GET_PTRS_VALUE_hsa_queue_inactivate(args); return;
			case HSA_API_ID_hsa_amd_vmem_import_shareable_handle : GET_PTRS_VALUE_hsa_amd_vmem_import_shareable_handle(args); return;
			case HSA_API_ID_hsa_signal_cas_relaxed : GET_PTRS_VALUE_hsa_signal_cas_relaxed(args); return;
			case HSA_API_ID_hsa_queue_add_write_index_scacquire : GET_PTRS_VALUE_hsa_queue_add_write_index_scacquire(args); return;
			case HSA_API_ID_hsa_signal_exchange_screlease : GET_PTRS_VALUE_hsa_signal_exchange_screlease(args); return;
			case HSA_API_ID_hsa_amd_vmem_handle_create : GET_PTRS_VALUE_hsa_amd_vmem_handle_create(args); return;
			case HSA_API_ID_hsa_agent_get_info : GET_PTRS_VALUE_hsa_agent_get_info(args); return;
			case HSA_API_ID_hsa_signal_or_acq_rel : GET_PTRS_VALUE_hsa_signal_or_acq_rel(args); return;
			case HSA_API_ID_hsa_signal_xor_acq_rel : GET_PTRS_VALUE_hsa_signal_xor_acq_rel(args); return;
			case HSA_API_ID_hsa_system_extension_supported : GET_PTRS_VALUE_hsa_system_extension_supported(args); return;
			case HSA_API_ID_hsa_signal_group_wait_any_relaxed : GET_PTRS_VALUE_hsa_signal_group_wait_any_relaxed(args); return;
			case HSA_API_ID_hsa_signal_or_scacq_screl : GET_PTRS_VALUE_hsa_signal_or_scacq_screl(args); return;
			case HSA_API_ID_hsa_executable_iterate_agent_symbols : GET_PTRS_VALUE_hsa_executable_iterate_agent_symbols(args); return;
			case HSA_API_ID_hsa_amd_deregister_deallocation_callback : GET_PTRS_VALUE_hsa_amd_deregister_deallocation_callback(args); return;
			case HSA_API_ID_hsa_signal_create : GET_PTRS_VALUE_hsa_signal_create(args); return;
			case HSA_API_ID_hsa_amd_memory_copy_engine_status : GET_PTRS_VALUE_hsa_amd_memory_copy_engine_status(args); return;
			case HSA_API_ID_hsa_ext_image_get_capability : GET_PTRS_VALUE_hsa_ext_image_get_capability(args); return;
			case HSA_API_ID_hsa_ext_image_destroy : GET_PTRS_VALUE_hsa_ext_image_destroy(args); return;
			case HSA_API_ID_hsa_queue_store_read_index_release : GET_PTRS_VALUE_hsa_queue_store_read_index_release(args); return;
			case HSA_API_ID_hsa_signal_wait_scacquire : GET_PTRS_VALUE_hsa_signal_wait_scacquire(args); return;
			case HSA_API_ID_hsa_executable_get_info : GET_PTRS_VALUE_hsa_executable_get_info(args); return;
			case HSA_API_ID_hsa_signal_silent_store_screlease : GET_PTRS_VALUE_hsa_signal_silent_store_screlease(args); return;
			case HSA_API_ID_hsa_executable_validate_alt : GET_PTRS_VALUE_hsa_executable_validate_alt(args); return;
			case HSA_API_ID_hsa_executable_create : GET_PTRS_VALUE_hsa_executable_create(args); return;
			case HSA_API_ID_hsa_executable_iterate_program_symbols : GET_PTRS_VALUE_hsa_executable_iterate_program_symbols(args); return;
			case HSA_API_ID_hsa_amd_memory_migrate : GET_PTRS_VALUE_hsa_amd_memory_migrate(args); return;
			case HSA_API_ID_hsa_amd_interop_unmap_buffer : GET_PTRS_VALUE_hsa_amd_interop_unmap_buffer(args); return;
			case HSA_API_ID_hsa_amd_signal_create : GET_PTRS_VALUE_hsa_amd_signal_create(args); return;
			case HSA_API_ID_hsa_executable_load_program_code_object : GET_PTRS_VALUE_hsa_executable_load_program_code_object(args); return;
			case HSA_API_ID_hsa_signal_load_relaxed : GET_PTRS_VALUE_hsa_signal_load_relaxed(args); return;
			case HSA_API_ID_hsa_signal_group_wait_any_scacquire : GET_PTRS_VALUE_hsa_signal_group_wait_any_scacquire(args); return;
			case HSA_API_ID_hsa_queue_cas_write_index_relaxed : GET_PTRS_VALUE_hsa_queue_cas_write_index_relaxed(args); return;
			case HSA_API_ID_hsa_signal_subtract_relaxed : GET_PTRS_VALUE_hsa_signal_subtract_relaxed(args); return;
			case HSA_API_ID_hsa_executable_freeze : GET_PTRS_VALUE_hsa_executable_freeze(args); return;
			case HSA_API_ID_hsa_queue_load_write_index_acquire : GET_PTRS_VALUE_hsa_queue_load_write_index_acquire(args); return;
			case HSA_API_ID_hsa_amd_vmem_map : GET_PTRS_VALUE_hsa_amd_vmem_map(args); return;
			case HSA_API_ID_hsa_amd_ipc_signal_attach : GET_PTRS_VALUE_hsa_amd_ipc_signal_attach(args); return;
			case HSA_API_ID_hsa_amd_register_system_event_handler : GET_PTRS_VALUE_hsa_amd_register_system_event_handler(args); return;
			case HSA_API_ID_hsa_amd_queue_cu_set_mask : GET_PTRS_VALUE_hsa_amd_queue_cu_set_mask(args); return;
			case HSA_API_ID_hsa_ext_image_export : GET_PTRS_VALUE_hsa_ext_image_export(args); return;
			case HSA_API_ID_hsa_status_string : GET_PTRS_VALUE_hsa_status_string(args); return;
			case HSA_API_ID_hsa_signal_xor_screlease : GET_PTRS_VALUE_hsa_signal_xor_screlease(args); return;
			case HSA_API_ID_hsa_queue_add_write_index_acquire : GET_PTRS_VALUE_hsa_queue_add_write_index_acquire(args); return;
			case HSA_API_ID_hsa_queue_add_write_index_screlease : GET_PTRS_VALUE_hsa_queue_add_write_index_screlease(args); return;
			case HSA_API_ID_hsa_code_object_get_symbol_from_name : GET_PTRS_VALUE_hsa_code_object_get_symbol_from_name(args); return;
			case HSA_API_ID_hsa_isa_from_name : GET_PTRS_VALUE_hsa_isa_from_name(args); return;
			case HSA_API_ID_hsa_amd_agent_iterate_memory_pools : GET_PTRS_VALUE_hsa_amd_agent_iterate_memory_pools(args); return;
			case HSA_API_ID_hsa_amd_agent_set_async_scratch_limit : GET_PTRS_VALUE_hsa_amd_agent_set_async_scratch_limit(args); return;
			case HSA_API_ID_hsa_ext_image_create : GET_PTRS_VALUE_hsa_ext_image_create(args); return;
			case HSA_API_ID_hsa_signal_exchange_scacq_screl : GET_PTRS_VALUE_hsa_signal_exchange_scacq_screl(args); return;
			case HSA_API_ID_hsa_init : GET_PTRS_VALUE_hsa_init(args); return;
			case HSA_API_ID_hsa_executable_load_agent_code_object : GET_PTRS_VALUE_hsa_executable_load_agent_code_object(args); return;
			case HSA_API_ID_hsa_signal_cas_screlease : GET_PTRS_VALUE_hsa_signal_cas_screlease(args); return;
			case HSA_API_ID_hsa_amd_profiling_get_dispatch_time : GET_PTRS_VALUE_hsa_amd_profiling_get_dispatch_time(args); return;
			case HSA_API_ID_hsa_amd_portable_close_dmabuf : GET_PTRS_VALUE_hsa_amd_portable_close_dmabuf(args); return;
			case HSA_API_ID_hsa_memory_copy : GET_PTRS_VALUE_hsa_memory_copy(args); return;
			case HSA_API_ID_hsa_signal_exchange_scacquire : GET_PTRS_VALUE_hsa_signal_exchange_scacquire(args); return;
			case HSA_API_ID_hsa_ext_image_data_get_info_with_layout : GET_PTRS_VALUE_hsa_ext_image_data_get_info_with_layout(args); return;
			case HSA_API_ID_hsa_executable_global_variable_define : GET_PTRS_VALUE_hsa_executable_global_variable_define(args); return;
			case HSA_API_ID_hsa_signal_add_acquire : GET_PTRS_VALUE_hsa_signal_add_acquire(args); return;
			case HSA_API_ID_hsa_ext_image_data_get_info : GET_PTRS_VALUE_hsa_ext_image_data_get_info(args); return;
			case HSA_API_ID_hsa_signal_cas_scacquire : GET_PTRS_VALUE_hsa_signal_cas_scacquire(args); return;
			case HSA_API_ID_hsa_amd_interop_map_buffer : GET_PTRS_VALUE_hsa_amd_interop_map_buffer(args); return;
			case HSA_API_ID_hsa_ext_sampler_create : GET_PTRS_VALUE_hsa_ext_sampler_create(args); return;
			case HSA_API_ID_hsa_code_object_deserialize : GET_PTRS_VALUE_hsa_code_object_deserialize(args); return;
			case HSA_API_ID_hsa_amd_agents_allow_access : GET_PTRS_VALUE_hsa_amd_agents_allow_access(args); return; 
            default : break;
        }
    } else {
        switch(id) {
			case HSA_API_ID_hsa_queue_destroy : GET_PTRS_VALUE_hsa_queue_destroy(args); return;
			case HSA_API_ID_hsa_amd_vmem_address_free : GET_PTRS_VALUE_hsa_amd_vmem_address_free(args); return;
			case HSA_API_ID_hsa_memory_free : GET_PTRS_VALUE_hsa_memory_free(args); return;
			case HSA_API_ID_hsa_amd_memory_pool_free : GET_PTRS_VALUE_hsa_amd_memory_pool_free(args); return; 
            default : break;
        }
    }
}
 

     
// HSA Functions Types
typedef uint64_t (*__hsa_queue_load_read_index_scacquire_t)(const hsa_queue_t * queue);
typedef hsa_status_t (*__hsa_ext_image_clear_t)(hsa_agent_t agent, hsa_ext_image_t image, const void * data, const hsa_ext_image_region_t * image_region);
typedef uint64_t (*__hsa_queue_cas_write_index_acquire_t)(const hsa_queue_t * queue, uint64_t expected, uint64_t value);
typedef hsa_status_t (*__hsa_executable_get_symbol_t)(hsa_executable_t executable, const char * module_name, const char * symbol_name, hsa_agent_t agent, int32_t call_convention, hsa_executable_symbol_t * symbol);
typedef void (*__hsa_signal_and_scacq_screl_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_executable_agent_global_variable_define_t)(hsa_executable_t executable, hsa_agent_t agent, const char * variable_name, void * address);
typedef hsa_status_t (*__hsa_code_object_get_symbol_t)(hsa_code_object_t code_object, const char * symbol_name, hsa_code_symbol_t * symbol);
typedef hsa_status_t (*__hsa_amd_signal_async_handler_t)(hsa_signal_t signal, hsa_signal_condition_t cond, hsa_signal_value_t value, hsa_amd_signal_handler handler, void * arg);
typedef void (*__hsa_signal_xor_scacq_screl_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef void (*__hsa_signal_and_relaxed_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_wavefront_get_info_t)(hsa_wavefront_t wavefront, hsa_wavefront_info_t attribute, void * value);
typedef hsa_status_t (*__hsa_amd_memory_lock_t)(void * host_ptr, size_t size, hsa_agent_t * agents, int num_agent, void ** agent_ptr);
typedef void (*__hsa_signal_or_screlease_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_executable_validate_t)(hsa_executable_t executable, uint32_t * result);
typedef hsa_status_t (*__hsa_queue_destroy_t)(hsa_queue_t * queue);
typedef hsa_status_t (*__hsa_isa_get_info_t)(hsa_isa_t isa, hsa_isa_info_t attribute, uint32_t index, void * value);
typedef hsa_status_t (*__hsa_amd_memory_lock_to_pool_t)(void * host_ptr, size_t size, hsa_agent_t * agents, int num_agent, hsa_amd_memory_pool_t pool, uint32_t flags, void ** agent_ptr);
typedef hsa_status_t (*__hsa_system_major_extension_supported_t)(uint16_t extension, uint16_t version_major, uint16_t * version_minor, _Bool * result);
typedef hsa_status_t (*__hsa_amd_profiling_get_async_copy_time_t)(hsa_signal_t signal, hsa_amd_profiling_async_copy_time_t * time);
typedef void (*__hsa_signal_or_relaxed_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef uint64_t (*__hsa_queue_cas_write_index_scacquire_t)(const hsa_queue_t * queue, uint64_t expected, uint64_t value);
typedef hsa_status_t (*__hsa_code_object_get_info_t)(hsa_code_object_t code_object, hsa_code_object_info_t attribute, void * value);
typedef void (*__hsa_signal_and_screlease_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_amd_pointer_info_t)(const void * ptr, hsa_amd_pointer_info_t * info, void *(*alloc)(size_t), uint32_t * num_agents_accessible, hsa_agent_t ** accessible);
typedef hsa_status_t (*__hsa_cache_get_info_t)(hsa_cache_t cache, hsa_cache_info_t attribute, void * value);
typedef hsa_status_t (*__hsa_amd_svm_attributes_get_t)(void * ptr, size_t size, hsa_amd_svm_attribute_pair_t * attribute_list, size_t attribute_count);
typedef hsa_status_t (*__hsa_signal_group_create_t)(uint32_t num_signals, const hsa_signal_t * signals, uint32_t num_consumers, const hsa_agent_t * consumers, hsa_signal_group_t * signal_group);
typedef hsa_status_t (*__hsa_memory_deregister_t)(void * ptr, size_t size);
typedef hsa_status_t (*__hsa_amd_ipc_memory_attach_t)(const hsa_amd_ipc_memory_t * handle, size_t len, uint32_t num_agents, const hsa_agent_t * mapping_agents, void ** mapped_ptr);
typedef hsa_status_t (*__hsa_amd_queue_cu_get_mask_t)(const hsa_queue_t * queue, uint32_t num_cu_mask_count, uint32_t * cu_mask);
typedef uint64_t (*__hsa_queue_add_write_index_scacq_screl_t)(const hsa_queue_t * queue, uint64_t value);
typedef hsa_status_t (*__hsa_agent_major_extension_supported_t)(uint16_t extension, hsa_agent_t agent, uint16_t version_major, uint16_t * version_minor, _Bool * result);
typedef hsa_status_t (*__hsa_ext_sampler_destroy_t)(hsa_agent_t agent, hsa_ext_sampler_t sampler);
typedef hsa_status_t (*__hsa_queue_create_t)(hsa_agent_t agent, uint32_t size, hsa_queue_type32_t type, void (*callback)(hsa_status_t, hsa_queue_t *, void *), void * data, uint32_t private_segment_size, uint32_t group_segment_size, hsa_queue_t ** queue);
typedef hsa_status_t (*__hsa_amd_vmem_get_alloc_properties_from_handle_t)(hsa_amd_vmem_alloc_handle_t memory_handle, hsa_amd_memory_pool_t * pool, hsa_amd_memory_type_t * type);
typedef hsa_status_t (*__hsa_amd_memory_pool_allocate_t)(hsa_amd_memory_pool_t memory_pool, size_t size, uint32_t flags, void ** ptr);
typedef void (*__hsa_signal_or_release_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef uint64_t (*__hsa_queue_cas_write_index_acq_rel_t)(const hsa_queue_t * queue, uint64_t expected, uint64_t value);
typedef hsa_status_t (*__hsa_code_object_serialize_t)(hsa_code_object_t code_object, hsa_status_t (*alloc_callback)(size_t, hsa_callback_data_t, void **), hsa_callback_data_t callback_data, const char * options, void ** serialized_code_object, size_t * serialized_code_object_size);
typedef void (*__hsa_signal_xor_acquire_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_amd_profiling_convert_tick_to_system_domain_t)(hsa_agent_t agent, uint64_t agent_tick, uint64_t * system_tick);
typedef hsa_status_t (*__hsa_executable_destroy_t)(hsa_executable_t executable);
typedef void (*__hsa_signal_add_screlease_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_signal_value_t (*__hsa_signal_load_acquire_t)(hsa_signal_t signal);
typedef hsa_status_t (*__hsa_executable_create_alt_t)(hsa_profile_t profile, hsa_default_float_rounding_mode_t default_float_rounding_mode, const char * options, hsa_executable_t * executable);
typedef hsa_status_t (*__hsa_amd_memory_async_copy_rect_t)(const hsa_pitched_ptr_t * dst, const hsa_dim3_t * dst_offset, const hsa_pitched_ptr_t * src, const hsa_dim3_t * src_offset, const hsa_dim3_t * range, hsa_agent_t copy_agent, hsa_amd_copy_direction_t dir, uint32_t num_dep_signals, const hsa_signal_t * dep_signals, hsa_signal_t completion_signal);
typedef hsa_signal_value_t (*__hsa_signal_load_scacquire_t)(hsa_signal_t signal);
typedef hsa_status_t (*__hsa_agent_iterate_caches_t)(hsa_agent_t agent, hsa_status_t (*callback)(hsa_cache_t, void *), void * data);
typedef hsa_signal_value_t (*__hsa_signal_cas_acq_rel_t)(hsa_signal_t signal, hsa_signal_value_t expected, hsa_signal_value_t value);
typedef uint64_t (*__hsa_queue_load_read_index_relaxed_t)(const hsa_queue_t * queue);
typedef hsa_status_t (*__hsa_agent_iterate_isas_t)(hsa_agent_t agent, hsa_status_t (*callback)(hsa_isa_t, void *), void * data);
typedef void (*__hsa_signal_xor_relaxed_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef void (*__hsa_signal_subtract_acq_rel_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef uint64_t (*__hsa_queue_load_read_index_acquire_t)(const hsa_queue_t * queue);
typedef hsa_status_t (*__hsa_isa_get_info_alt_t)(hsa_isa_t isa, hsa_isa_info_t attribute, void * value);
typedef void (*__hsa_signal_and_scacquire_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef uint64_t (*__hsa_queue_add_write_index_release_t)(const hsa_queue_t * queue, uint64_t value);
typedef hsa_status_t (*__hsa_system_get_info_t)(hsa_system_info_t attribute, void * value);
typedef hsa_status_t (*__hsa_memory_register_t)(void * ptr, size_t size);
typedef hsa_status_t (*__hsa_isa_get_round_method_t)(hsa_isa_t isa, hsa_fp_type_t fp_type, hsa_flush_mode_t flush_mode, hsa_round_method_t * round_method);
typedef hsa_status_t (*__hsa_isa_iterate_wavefronts_t)(hsa_isa_t isa, hsa_status_t (*callback)(hsa_wavefront_t, void *), void * data);
typedef hsa_status_t (*__hsa_amd_vmem_unmap_t)(void * va, size_t size);
typedef void (*__hsa_signal_subtract_acquire_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_system_get_major_extension_table_t)(uint16_t extension, uint16_t version_major, size_t table_length, void * table);
typedef void (*__hsa_signal_store_release_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef uint64_t (*__hsa_queue_load_write_index_scacquire_t)(const hsa_queue_t * queue);
typedef hsa_status_t (*__hsa_amd_spm_release_t)(hsa_agent_t preferred_agent);
typedef hsa_status_t (*__hsa_code_symbol_get_info_t)(hsa_code_symbol_t code_symbol, hsa_code_symbol_info_t attribute, void * value);
typedef hsa_signal_value_t (*__hsa_signal_wait_acquire_t)(hsa_signal_t signal, hsa_signal_condition_t condition, hsa_signal_value_t compare_value, uint64_t timeout_hint, hsa_wait_state_t wait_state_hint);
typedef hsa_signal_value_t (*__hsa_signal_cas_release_t)(hsa_signal_t signal, hsa_signal_value_t expected, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_amd_memory_pool_get_info_t)(hsa_amd_memory_pool_t memory_pool, hsa_amd_memory_pool_info_t attribute, void * value);
typedef hsa_status_t (*__hsa_amd_pointer_info_set_userdata_t)(const void * ptr, void * userdata);
typedef hsa_status_t (*__hsa_amd_vmem_get_access_t)(void * va, hsa_access_permission_t * perms, hsa_agent_t agent_handle);
typedef hsa_status_t (*__hsa_iterate_agents_t)(hsa_status_t (*callback)(hsa_agent_t, void *), void * data);
typedef void (*__hsa_queue_store_write_index_screlease_t)(const hsa_queue_t * queue, uint64_t value);
typedef hsa_status_t (*__hsa_agent_iterate_regions_t)(hsa_agent_t agent, hsa_status_t (*callback)(hsa_region_t, void *), void * data);
typedef hsa_status_t (*__hsa_executable_readonly_variable_define_t)(hsa_executable_t executable, hsa_agent_t agent, const char * variable_name, void * address);
typedef hsa_status_t (*__hsa_executable_iterate_symbols_t)(hsa_executable_t executable, hsa_status_t (*callback)(hsa_executable_t, hsa_executable_symbol_t, void *), void * data);
typedef hsa_status_t (*__hsa_amd_vmem_handle_release_t)(hsa_amd_vmem_alloc_handle_t memory_handle);
typedef hsa_status_t (*__hsa_amd_memory_pool_can_migrate_t)(hsa_amd_memory_pool_t src_memory_pool, hsa_amd_memory_pool_t dst_memory_pool, _Bool * result);
typedef void (*__hsa_queue_store_write_index_relaxed_t)(const hsa_queue_t * queue, uint64_t value);
typedef hsa_status_t (*__hsa_amd_image_get_info_max_dim_t)(hsa_agent_t agent, hsa_agent_info_t attribute, void * value);
typedef hsa_status_t (*__hsa_amd_spm_set_dest_buffer_t)(hsa_agent_t preferred_agent, size_t size_in_bytes, uint32_t * timeout, uint32_t * size_copied, void * dest, _Bool * is_data_loss);
typedef hsa_signal_value_t (*__hsa_signal_wait_relaxed_t)(hsa_signal_t signal, hsa_signal_condition_t condition, hsa_signal_value_t compare_value, uint64_t timeout_hint, hsa_wait_state_t wait_state_hint);
typedef hsa_status_t (*__hsa_amd_portable_export_dmabuf_t)(const void * ptr, size_t size, int * dmabuf, uint64_t * offset);
typedef void (*__hsa_signal_store_screlease_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef void (*__hsa_signal_or_acquire_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_code_object_destroy_t)(hsa_code_object_t code_object);
typedef void (*__hsa_signal_subtract_screlease_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_region_get_info_t)(hsa_region_t region, hsa_region_info_t attribute, void * value);
typedef void (*__hsa_signal_subtract_scacq_screl_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_amd_profiling_async_copy_enable_t)(_Bool enable);
typedef hsa_status_t (*__hsa_soft_queue_create_t)(hsa_region_t region, uint32_t size, hsa_queue_type32_t type, uint32_t features, hsa_signal_t doorbell_signal, hsa_queue_t ** queue);
typedef hsa_status_t (*__hsa_shut_down_t)();
typedef hsa_status_t (*__hsa_code_object_iterate_symbols_t)(hsa_code_object_t code_object, hsa_status_t (*callback)(hsa_code_object_t, hsa_code_symbol_t, void *), void * data);
typedef hsa_status_t (*__hsa_extension_get_name_t)(uint16_t extension, const char ** name);
typedef hsa_signal_value_t (*__hsa_signal_exchange_relaxed_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_executable_get_symbol_by_name_t)(hsa_executable_t executable, const char * symbol_name, const hsa_agent_t * agent, hsa_executable_symbol_t * symbol);
typedef hsa_status_t (*__hsa_ext_image_create_with_layout_t)(hsa_agent_t agent, const hsa_ext_image_descriptor_t * image_descriptor, const void * image_data, hsa_access_permission_t access_permission, hsa_ext_image_data_layout_t image_data_layout, size_t image_data_row_pitch, size_t image_data_slice_pitch, hsa_ext_image_t * image);
typedef uint64_t (*__hsa_queue_load_write_index_relaxed_t)(const hsa_queue_t * queue);
typedef hsa_status_t (*__hsa_ext_image_copy_t)(hsa_agent_t agent, hsa_ext_image_t src_image, const hsa_dim3_t * src_offset, hsa_ext_image_t dst_image, const hsa_dim3_t * dst_offset, const hsa_dim3_t * range);
typedef hsa_status_t (*__hsa_code_object_reader_create_from_memory_t)(const void * code_object, size_t size, hsa_code_object_reader_t * code_object_reader);
typedef void (*__hsa_signal_xor_release_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_amd_register_deallocation_callback_t)(void * ptr, hsa_amd_deallocation_callback_t callback, void * user_data);
typedef hsa_status_t (*__hsa_signal_group_destroy_t)(hsa_signal_group_t signal_group);
typedef hsa_status_t (*__hsa_amd_vmem_export_shareable_handle_t)(int * dmabuf_fd, hsa_amd_vmem_alloc_handle_t handle, uint64_t flags);
typedef uint64_t (*__hsa_queue_add_write_index_acq_rel_t)(const hsa_queue_t * queue, uint64_t value);
typedef hsa_status_t (*__hsa_signal_destroy_t)(hsa_signal_t signal);
typedef hsa_status_t (*__hsa_agent_extension_supported_t)(uint16_t extension, hsa_agent_t agent, uint16_t version_major, uint16_t version_minor, _Bool * result);
typedef hsa_status_t (*__hsa_amd_image_create_t)(hsa_agent_t agent, const hsa_ext_image_descriptor_t * image_descriptor, const hsa_amd_image_descriptor_t * image_layout, const void * image_data, hsa_access_permission_t access_permission, hsa_ext_image_t * image);
typedef hsa_status_t (*__hsa_isa_compatible_t)(hsa_isa_t code_object_isa, hsa_isa_t agent_isa, _Bool * result);
typedef hsa_status_t (*__hsa_amd_queue_set_priority_t)(hsa_queue_t * queue, hsa_amd_queue_priority_t priority);
typedef void (*__hsa_signal_and_acq_rel_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_code_object_reader_destroy_t)(hsa_code_object_reader_t code_object_reader);
typedef uint64_t (*__hsa_queue_cas_write_index_release_t)(const hsa_queue_t * queue, uint64_t expected, uint64_t value);
typedef hsa_status_t (*__hsa_executable_load_code_object_t)(hsa_executable_t executable, hsa_agent_t agent, hsa_code_object_t code_object, const char * options);
typedef hsa_status_t (*__hsa_amd_memory_async_copy_t)(void * dst, hsa_agent_t dst_agent, const void * src, hsa_agent_t src_agent, size_t size, uint32_t num_dep_signals, const hsa_signal_t * dep_signals, hsa_signal_t completion_signal);
typedef hsa_status_t (*__hsa_amd_memory_unlock_t)(void * host_ptr);
typedef void (*__hsa_signal_subtract_release_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_signal_value_t (*__hsa_signal_exchange_acquire_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef void (*__hsa_signal_store_relaxed_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef void (*__hsa_signal_subtract_scacquire_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef void (*__hsa_signal_and_release_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_signal_value_t (*__hsa_signal_cas_acquire_t)(hsa_signal_t signal, hsa_signal_value_t expected, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_isa_get_exception_policies_t)(hsa_isa_t isa, hsa_profile_t profile, uint16_t * mask);
typedef void (*__hsa_signal_add_release_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_code_object_reader_create_from_file_t)(hsa_file_t file, hsa_code_object_reader_t * code_object_reader);
typedef hsa_status_t (*__hsa_executable_symbol_get_info_t)(hsa_executable_symbol_t executable_symbol, hsa_executable_symbol_info_t attribute, void * value);
typedef hsa_status_t (*__hsa_amd_ipc_memory_create_t)(void * ptr, size_t len, hsa_amd_ipc_memory_t * handle);
typedef void (*__hsa_queue_store_write_index_release_t)(const hsa_queue_t * queue, uint64_t value);
typedef hsa_status_t (*__hsa_amd_memory_fill_t)(void * ptr, uint32_t value, size_t count);
typedef void (*__hsa_signal_add_acq_rel_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_signal_value_t (*__hsa_signal_cas_scacq_screl_t)(hsa_signal_t signal, hsa_signal_value_t expected, hsa_signal_value_t value);
typedef uint64_t (*__hsa_queue_cas_write_index_scacq_screl_t)(const hsa_queue_t * queue, uint64_t expected, uint64_t value);
typedef hsa_status_t (*__hsa_amd_ipc_signal_create_t)(hsa_signal_t signal, hsa_amd_ipc_signal_t * handle);
typedef hsa_status_t (*__hsa_amd_spm_acquire_t)(hsa_agent_t preferred_agent);
typedef hsa_status_t (*__hsa_amd_coherency_get_type_t)(hsa_agent_t agent, hsa_amd_coherency_type_t * type);
typedef hsa_status_t (*__hsa_system_get_extension_table_t)(uint16_t extension, uint16_t version_major, uint16_t version_minor, void * table);
typedef hsa_status_t (*__hsa_amd_ipc_memory_detach_t)(void * mapped_ptr);
typedef hsa_status_t (*__hsa_ext_image_get_capability_with_layout_t)(hsa_agent_t agent, hsa_ext_image_geometry_t geometry, const hsa_ext_image_format_t * image_format, hsa_ext_image_data_layout_t image_data_layout, uint32_t * capability_mask);
typedef hsa_status_t (*__hsa_amd_vmem_address_free_t)(void * va, size_t size);
typedef hsa_status_t (*__hsa_amd_svm_attributes_set_t)(void * ptr, size_t size, hsa_amd_svm_attribute_pair_t * attribute_list, size_t attribute_count);
typedef hsa_status_t (*__hsa_ext_image_import_t)(hsa_agent_t agent, const void * src_memory, size_t src_row_pitch, size_t src_slice_pitch, hsa_ext_image_t dst_image, const hsa_ext_image_region_t * image_region);
typedef void (*__hsa_signal_or_scacquire_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_amd_svm_prefetch_async_t)(void * ptr, size_t size, hsa_agent_t agent, uint32_t num_dep_signals, const hsa_signal_t * dep_signals, hsa_signal_t completion_signal);
typedef void (*__hsa_signal_add_scacq_screl_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef void (*__hsa_signal_silent_store_relaxed_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef void (*__hsa_queue_store_read_index_relaxed_t)(const hsa_queue_t * queue, uint64_t value);
typedef hsa_status_t (*__hsa_agent_get_exception_policies_t)(hsa_agent_t agent, hsa_profile_t profile, uint16_t * mask);
typedef void (*__hsa_signal_add_scacquire_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_signal_value_t (*__hsa_signal_exchange_release_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef void (*__hsa_signal_add_relaxed_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef void (*__hsa_signal_and_acquire_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef uint64_t (*__hsa_queue_add_write_index_relaxed_t)(const hsa_queue_t * queue, uint64_t value);
typedef hsa_status_t (*__hsa_amd_agent_memory_pool_get_info_t)(hsa_agent_t agent, hsa_amd_memory_pool_t memory_pool, hsa_amd_agent_memory_pool_info_t attribute, void * value);
typedef hsa_status_t (*__hsa_amd_profiling_set_profiler_enabled_t)(hsa_queue_t * queue, int enable);
typedef hsa_signal_value_t (*__hsa_signal_exchange_acq_rel_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef uint64_t (*__hsa_queue_cas_write_index_screlease_t)(const hsa_queue_t * queue, uint64_t expected, uint64_t value);
typedef hsa_status_t (*__hsa_amd_async_function_t)(void (*callback)(void *), void * arg);
typedef void (*__hsa_queue_store_read_index_screlease_t)(const hsa_queue_t * queue, uint64_t value);
typedef hsa_status_t (*__hsa_memory_allocate_t)(hsa_region_t region, size_t size, void ** ptr);
typedef hsa_status_t (*__hsa_amd_signal_value_pointer_t)(hsa_signal_t signal, volatile hsa_signal_value_t ** value_ptr);
typedef uint32_t (*__hsa_amd_signal_wait_any_t)(uint32_t signal_count, hsa_signal_t * signals, hsa_signal_condition_t * conds, hsa_signal_value_t * values, uint64_t timeout_hint, hsa_wait_state_t wait_hint, hsa_signal_value_t * satisfying_value);
typedef hsa_status_t (*__hsa_amd_vmem_retain_alloc_handle_t)(hsa_amd_vmem_alloc_handle_t * memory_handle, void * addr);
typedef hsa_status_t (*__hsa_amd_vmem_set_access_t)(void * va, size_t size, const hsa_amd_memory_access_desc_t * desc, size_t desc_cnt);
typedef hsa_status_t (*__hsa_memory_assign_agent_t)(void * ptr, hsa_agent_t agent, hsa_access_permission_t access);
typedef hsa_status_t (*__hsa_amd_vmem_address_reserve_t)(void ** va, size_t size, uint64_t address, uint64_t flags);
typedef hsa_status_t (*__hsa_amd_memory_async_copy_on_engine_t)(void * dst, hsa_agent_t dst_agent, const void * src, hsa_agent_t src_agent, size_t size, uint32_t num_dep_signals, const hsa_signal_t * dep_signals, hsa_signal_t completion_signal, hsa_amd_sdma_engine_id_t engine_id, _Bool force_copy_on_sdma);
typedef void (*__hsa_signal_xor_scacquire_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_amd_coherency_set_type_t)(hsa_agent_t agent, hsa_amd_coherency_type_t type);
typedef hsa_status_t (*__hsa_queue_inactivate_t)(hsa_queue_t * queue);
typedef hsa_status_t (*__hsa_amd_vmem_import_shareable_handle_t)(int dmabuf_fd, hsa_amd_vmem_alloc_handle_t * handle);
typedef hsa_signal_value_t (*__hsa_signal_cas_relaxed_t)(hsa_signal_t signal, hsa_signal_value_t expected, hsa_signal_value_t value);
typedef uint64_t (*__hsa_queue_add_write_index_scacquire_t)(const hsa_queue_t * queue, uint64_t value);
typedef hsa_signal_value_t (*__hsa_signal_exchange_screlease_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_amd_vmem_handle_create_t)(hsa_amd_memory_pool_t pool, size_t size, hsa_amd_memory_type_t type, uint64_t flags, hsa_amd_vmem_alloc_handle_t * memory_handle);
typedef hsa_status_t (*__hsa_agent_get_info_t)(hsa_agent_t agent, hsa_agent_info_t attribute, void * value);
typedef void (*__hsa_signal_or_acq_rel_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef void (*__hsa_signal_xor_acq_rel_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_system_extension_supported_t)(uint16_t extension, uint16_t version_major, uint16_t version_minor, _Bool * result);
typedef hsa_status_t (*__hsa_signal_group_wait_any_relaxed_t)(hsa_signal_group_t signal_group, const hsa_signal_condition_t * conditions, const hsa_signal_value_t * compare_values, hsa_wait_state_t wait_state_hint, hsa_signal_t * signal, hsa_signal_value_t * value);
typedef void (*__hsa_signal_or_scacq_screl_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_executable_iterate_agent_symbols_t)(hsa_executable_t executable, hsa_agent_t agent, hsa_status_t (*callback)(hsa_executable_t, hsa_agent_t, hsa_executable_symbol_t, void *), void * data);
typedef hsa_status_t (*__hsa_amd_deregister_deallocation_callback_t)(void * ptr, hsa_amd_deallocation_callback_t callback);
typedef hsa_status_t (*__hsa_signal_create_t)(hsa_signal_value_t initial_value, uint32_t num_consumers, const hsa_agent_t * consumers, hsa_signal_t * signal);
typedef hsa_status_t (*__hsa_amd_memory_copy_engine_status_t)(hsa_agent_t dst_agent, hsa_agent_t src_agent, uint32_t * engine_ids_mask);
typedef hsa_status_t (*__hsa_ext_image_get_capability_t)(hsa_agent_t agent, hsa_ext_image_geometry_t geometry, const hsa_ext_image_format_t * image_format, uint32_t * capability_mask);
typedef hsa_status_t (*__hsa_ext_image_destroy_t)(hsa_agent_t agent, hsa_ext_image_t image);
typedef void (*__hsa_queue_store_read_index_release_t)(const hsa_queue_t * queue, uint64_t value);
typedef hsa_signal_value_t (*__hsa_signal_wait_scacquire_t)(hsa_signal_t signal, hsa_signal_condition_t condition, hsa_signal_value_t compare_value, uint64_t timeout_hint, hsa_wait_state_t wait_state_hint);
typedef hsa_status_t (*__hsa_executable_get_info_t)(hsa_executable_t executable, hsa_executable_info_t attribute, void * value);
typedef void (*__hsa_signal_silent_store_screlease_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_executable_validate_alt_t)(hsa_executable_t executable, const char * options, uint32_t * result);
typedef hsa_status_t (*__hsa_executable_create_t)(hsa_profile_t profile, hsa_executable_state_t executable_state, const char * options, hsa_executable_t * executable);
typedef hsa_status_t (*__hsa_executable_iterate_program_symbols_t)(hsa_executable_t executable, hsa_status_t (*callback)(hsa_executable_t, hsa_executable_symbol_t, void *), void * data);
typedef hsa_status_t (*__hsa_memory_free_t)(void * ptr);
typedef hsa_status_t (*__hsa_amd_memory_pool_free_t)(void * ptr);
typedef hsa_status_t (*__hsa_amd_memory_migrate_t)(const void * ptr, hsa_amd_memory_pool_t memory_pool, uint32_t flags);
typedef hsa_status_t (*__hsa_amd_interop_unmap_buffer_t)(void * ptr);
typedef hsa_status_t (*__hsa_amd_signal_create_t)(hsa_signal_value_t initial_value, uint32_t num_consumers, const hsa_agent_t * consumers, uint64_t attributes, hsa_signal_t * signal);
typedef hsa_status_t (*__hsa_executable_load_program_code_object_t)(hsa_executable_t executable, hsa_code_object_reader_t code_object_reader, const char * options, hsa_loaded_code_object_t * loaded_code_object);
typedef hsa_signal_value_t (*__hsa_signal_load_relaxed_t)(hsa_signal_t signal);
typedef hsa_status_t (*__hsa_signal_group_wait_any_scacquire_t)(hsa_signal_group_t signal_group, const hsa_signal_condition_t * conditions, const hsa_signal_value_t * compare_values, hsa_wait_state_t wait_state_hint, hsa_signal_t * signal, hsa_signal_value_t * value);
typedef uint64_t (*__hsa_queue_cas_write_index_relaxed_t)(const hsa_queue_t * queue, uint64_t expected, uint64_t value);
typedef void (*__hsa_signal_subtract_relaxed_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_executable_freeze_t)(hsa_executable_t executable, const char * options);
typedef uint64_t (*__hsa_queue_load_write_index_acquire_t)(const hsa_queue_t * queue);
typedef hsa_status_t (*__hsa_amd_vmem_map_t)(void * va, size_t size, size_t in_offset, hsa_amd_vmem_alloc_handle_t memory_handle, uint64_t flags);
typedef hsa_status_t (*__hsa_amd_ipc_signal_attach_t)(const hsa_amd_ipc_signal_t * handle, hsa_signal_t * signal);
typedef hsa_status_t (*__hsa_amd_register_system_event_handler_t)(hsa_amd_system_event_callback_t callback, void * data);
typedef hsa_status_t (*__hsa_amd_queue_cu_set_mask_t)(const hsa_queue_t * queue, uint32_t num_cu_mask_count, const uint32_t * cu_mask);
typedef hsa_status_t (*__hsa_ext_image_export_t)(hsa_agent_t agent, hsa_ext_image_t src_image, void * dst_memory, size_t dst_row_pitch, size_t dst_slice_pitch, const hsa_ext_image_region_t * image_region);
typedef hsa_status_t (*__hsa_status_string_t)(hsa_status_t status, const char ** status_string);
typedef void (*__hsa_signal_xor_screlease_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef uint64_t (*__hsa_queue_add_write_index_acquire_t)(const hsa_queue_t * queue, uint64_t value);
typedef uint64_t (*__hsa_queue_add_write_index_screlease_t)(const hsa_queue_t * queue, uint64_t value);
typedef hsa_status_t (*__hsa_code_object_get_symbol_from_name_t)(hsa_code_object_t code_object, const char * module_name, const char * symbol_name, hsa_code_symbol_t * symbol);
typedef hsa_status_t (*__hsa_isa_from_name_t)(const char * name, hsa_isa_t * isa);
typedef hsa_status_t (*__hsa_amd_agent_iterate_memory_pools_t)(hsa_agent_t agent, hsa_status_t (*callback)(hsa_amd_memory_pool_t, void *), void * data);
typedef hsa_status_t (*__hsa_amd_agent_set_async_scratch_limit_t)(hsa_agent_t agent, size_t threshold);
typedef hsa_status_t (*__hsa_ext_image_create_t)(hsa_agent_t agent, const hsa_ext_image_descriptor_t * image_descriptor, const void * image_data, hsa_access_permission_t access_permission, hsa_ext_image_t * image);
typedef hsa_signal_value_t (*__hsa_signal_exchange_scacq_screl_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_init_t)();
typedef hsa_status_t (*__hsa_executable_load_agent_code_object_t)(hsa_executable_t executable, hsa_agent_t agent, hsa_code_object_reader_t code_object_reader, const char * options, hsa_loaded_code_object_t * loaded_code_object);
typedef hsa_signal_value_t (*__hsa_signal_cas_screlease_t)(hsa_signal_t signal, hsa_signal_value_t expected, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_amd_profiling_get_dispatch_time_t)(hsa_agent_t agent, hsa_signal_t signal, hsa_amd_profiling_dispatch_time_t * time);
typedef hsa_status_t (*__hsa_amd_portable_close_dmabuf_t)(int dmabuf);
typedef hsa_status_t (*__hsa_memory_copy_t)(void * dst, const void * src, size_t size);
typedef hsa_signal_value_t (*__hsa_signal_exchange_scacquire_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_ext_image_data_get_info_with_layout_t)(hsa_agent_t agent, const hsa_ext_image_descriptor_t * image_descriptor, hsa_access_permission_t access_permission, hsa_ext_image_data_layout_t image_data_layout, size_t image_data_row_pitch, size_t image_data_slice_pitch, hsa_ext_image_data_info_t * image_data_info);
typedef hsa_status_t (*__hsa_executable_global_variable_define_t)(hsa_executable_t executable, const char * variable_name, void * address);
typedef void (*__hsa_signal_add_acquire_t)(hsa_signal_t signal, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_ext_image_data_get_info_t)(hsa_agent_t agent, const hsa_ext_image_descriptor_t * image_descriptor, hsa_access_permission_t access_permission, hsa_ext_image_data_info_t * image_data_info);
typedef hsa_signal_value_t (*__hsa_signal_cas_scacquire_t)(hsa_signal_t signal, hsa_signal_value_t expected, hsa_signal_value_t value);
typedef hsa_status_t (*__hsa_amd_interop_map_buffer_t)(uint32_t num_agents, hsa_agent_t * agents, int interop_handle, uint32_t flags, size_t * size, void ** ptr, size_t * metadata_size, const void ** metadata);
typedef hsa_status_t (*__hsa_ext_sampler_create_t)(hsa_agent_t agent, const hsa_ext_sampler_descriptor_t * sampler_descriptor, hsa_ext_sampler_t * sampler);
typedef hsa_status_t (*__hsa_code_object_deserialize_t)(void * serialized_code_object, size_t serialized_code_object_size, const char * options, hsa_code_object_t * code_object);
typedef hsa_status_t (*__hsa_amd_agents_allow_access_t)(uint32_t num_agents, const hsa_agent_t * agents, const uint32_t * flags, const void * ptr); 

#endif // HSA_PROFILED_FUNCTIONS_H