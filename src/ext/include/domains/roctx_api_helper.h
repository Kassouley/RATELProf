...
typedef enum {
    FOR_EACH_ROCTX_FUNC(GET_FUNC_API_ID)
...
{
    switch(id) {
		FOR_EACH_ROCTX_FUNC(GET_FUNAME_BY_ID_OF)
        default : return NULL;
    }
    return NULL;
}
...
static inline roctx_api_id_t get_roctx_funid_by_name(const char* name) 
{
    ...
    FOR_EACH_ROCTX_FUNC(GET_FUNID_BY_NAME_OF)
...
{
    switch(id) {
		FOR_EACH_ROCTX_FUNC(GET_FUNADDR_BY_ID_OF)
        default : return NULL;
    }
    return NULL;
}
