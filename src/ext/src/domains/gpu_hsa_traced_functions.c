...
hsa_status_t i_gpu_hsa_queue_create(hsa_agent_t agent, uint32_t size, hsa_queue_type32_t type, void (*callback)(hsa_status_t, hsa_queue_t *, void *), void * data, uint32_t private_segment_size, uint32_t group_segment_size, hsa_queue_t ** queue, void* return_address) {
    hsa_status_t __ret = CALL_GPU_FUNC(hsa_queue_create, agent, size, type, callback, data, private_segment_size, group_segment_size, queue, return_address);
	if (__ret == HSA_STATUS_SUCCESS) ratelprof_intercept_queue_object(agent, queue);
	return __ret;
};


void i_gpu_hsa_signal_store_relaxed(hsa_signal_t signal, hsa_signal_value_t value, void* return_address) {
    ratelprof_intercept_dispatch(signal, value);
    CALL_GPU_FUNC(hsa_signal_store_relaxed, signal, value, return_address);
};


void i_gpu_hsa_signal_store_screlease(hsa_signal_t signal, hsa_signal_value_t value, void* return_address) {
    ratelprof_intercept_dispatch(signal, value);
    CALL_GPU_FUNC(hsa_signal_store_screlease, signal, value, return_address);
};


void i_gpu_hsa_signal_store_release(hsa_signal_t signal, hsa_signal_value_t value, void* return_address) {
    ratelprof_intercept_dispatch(signal, value);
    CALL_GPU_FUNC(hsa_signal_store_release, signal, value, return_address);
};
...
hsa_status_t i_gpu_hsa_amd_memory_async_copy_on_engine(void * dst, hsa_agent_t dst_agent, const void * src, hsa_agent_t src_agent, size_t size, uint32_t num_dep_signals, const hsa_signal_t * dep_signals, hsa_signal_t completion_signal, hsa_amd_sdma_engine_id_t engine_id, _Bool force_copy_on_sdma, void* return_address) {
