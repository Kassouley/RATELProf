...
static inline void init_json_content(ratelprof_buffer_t* json_buffer)
{
    int i = 0;
...
static inline void fini_json_content(ratelprof_buffer_t* json_buffer)
{
    size_t i = 0;
    ...
    {
    ratelprof_add_to_buffer(json_buffer, "\t\t\"%" PRIu64 "\": %" PRIu64 ",\n", agents_list[i], i);
    }
    ratelprof_add_to_buffer(json_buffer, "\t\t\"%" PRIu64 "\": %" PRIu64 "\n\t}\n}", agents_list[i], i);
}
...
ratelprof_status_t ratelprof_plugin_initialize(ratelprof_plugin_t** plugin) 
{
    ...
    if (!plugin) return RATELPROF_STATUS_INVALID_PTR;
    if (*plugin) return RATELPROF_STATUS_PLUGIN_ALREADY_INIT;

    p = (ratelprof_plugin_t*)malloc(sizeof(ratelprof_plugin_t));
    if (!p) return RATELPROF_STATUS_MALLOC_FAILED;

    p->omp_tgt_callback_handler.on_enter = on_enter_omp_tgt_callback;
    p->omp_tgt_callback_handler.on_exit = on_exit_omp_tgt_callback;
    ...
    p->json_buffer = (ratelprof_buffer_t*) malloc(sizeof(ratelprof_buffer_t));
    if (!p->json_buffer) return RATELPROF_STATUS_MALLOC_FAILED;
    status = ratelprof_init_buffer(p->json_buffer, get_output_file(), 2048);
...
ratelprof_status_t ratelprof_plugin_finalize(ratelprof_plugin_t** plugin) 
{
    ...
    p = *plugin;
    if (!p) return RATELPROF_STATUS_PLUGIN_IS_NULL;

    fini_json_content(p->json_buffer);

    status = ratelprof_flush_buffer(p->json_buffer);
    if (status != RATELPROF_STATUS_SUCCESS) return status;

    status = ratelprof_close_buffer(p->json_buffer);
    if (status != RATELPROF_STATUS_SUCCESS) return status;
