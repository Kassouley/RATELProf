...
rprofrep_status_t rprofrep_get_section_buffer(
    ...
    rprofrep_header_entry_t* section = &header->sections[section_id];

    if (section->size == 0) return RPROFREP_STATUS_SUCCESS;

    uint8_t* buffer = (uint8_t*) malloc(section->size * sizeof(uint8_t));
    RPROFREP_CHECK_ALLOC(buffer);

    if (fseek(ctx->handle, (long)section->offset, SEEK_SET) != 0) {
        free(buffer);
        return RPROFREP_STATUS_INVALID_REPORT("Failed to seek to %s section from report.\n", get_section_name(section_id));
    }

    size_t readed = fread(buffer, 1, section->size, ctx->handle);
    if (readed != section->size) {
        free(buffer);
    ...
    *out_section_buffer = buffer;
    if (out_section_size)
        *out_section_size = (size_t)section->size;
